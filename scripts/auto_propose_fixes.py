...
def sync_current_state_governance_issues(
    ...
    if q35_audit_active:
        ...
        if q35_redesign_machine.get("execution_blocked_after_floor_cross"):
            q35_action = (
                "維持 q35 scaling audit 的 runtime gap / redesign entry_quality / allowed_layers 已同步到 docs/probe/issues；"
                "本輪 discriminative redesign 只跨過 scoring floor，runtime gate/support 仍讓 allowed_layers=0，下一步只追 exact-support 與 score-only 治理，不得把 floor-cross 當成 deployment closure。"
            )
        else:
            q35_action = (
                "維持 q35 scaling audit 的 overall_verdict / redesign verdict / gap-to-floor 已同步到 docs/probe/issues；"
    ...
    if circuit_breaker_active:
        ...
        release_text = (
            f"最近 {recent_window} 筆目前 {current_recent_wins}/{recent_window} 勝，解除至少需要 {required_recent_wins} 勝，還差 {additional_wins_needed} 勝；"
        ...
        breaker_title = (
            f"熔斷解除條件未達（最近 {recent_window} 筆目前 {current_recent_wins}/{recent_window} 勝，解除至少需要 {required_recent_wins} 勝）"
        ...
        breaker_summary = {
            ...
            "api_trade_allowed_actions": ["wait", "reduce", "sell", "shadow_buy", "paper_buy", "diagnostics", "mode_toggle"],
            "api_trade_guardrail_context": "買入 / 加倉會在 ExecutionService.submit_order 前先檢查即時部署阻塞點；阻塞期間等待 / 觀望不送單，減倉 / 賣出可作風險降低路徑；shadow_buy / paper_buy 必須強制 dry-run 且不得送出 live order",
        }
        upsert_issue(
            tracker,
            "P0",
