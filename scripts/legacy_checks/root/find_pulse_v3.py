"""Find best Pulse replacement using computed features from raw data"""
import sqlite3, numpy as np, pandas as pd
from scipy import stats

conn = sqlite3.connect(r"C:\Users\Kazuha\repo\Poly-Trader\poly_trader.db")

# Get raw data and labels - use same timestamp join that works
# The label timestamps are every 5 minutes; raw are also every ~1 min
# Use approximate join (nearest within 5 minutes)

raw_rows = conn.execute("""
    SELECT timestamp, close_price, volume, funding_rate, volatility, oi_roc
...
raw_df = pd.DataFrame(raw_rows, columns=['ts','close','volume','fr','volatility','oi_roc'])
lbl_df = pd.DataFrame(lbl_rows, columns=['ts','label'])
raw_df['ts'] = pd.to_datetime(raw_df['ts'])
lbl_df['ts'] = pd.to_datetime(lbl_df['ts'])

# Compute candidates on raw_df (sorted by time)
...
raw_df['ret_12'] = raw_df['close'].pct_change(12)
raw_df['vol_24'] = rets.rolling(24).std()
raw_df['vol_ratio_12_24'] = raw_df['vol12'] / (raw_df['vol_24'] + 1e-10)

# Merge with labels (nearest within 5min)
...
candidates = ['vol_ratio', 'vol_mom', 'vol_spike6', 'ret_6', 'ret_12', 'vol_ratio_12_24']
...
for cand in candidates:
    if cand not in merged_recent.columns:
        continue
