...
scripts, heartbeat run logs, runtime log files, generated scan scratch output,
Python/pytest caches, frontend build output, and CatBoost training scratch
output.  It deliberately preserves venvs, databases, model artifacts, and
current-state machine artifacts such as ``data/live_predict_probe.json`` even
when those paths are ignored by ``.gitignore``.
"""
...
GENERATED_SCAN_OUTPUTS = {
    "data/model_strategy_param_scan_latest.json",
}

HEAVY_PROTECTED_ARTIFACTS = (
    "poly_trader.db",
    "poly_trader.db-wal",
    "poly_trader.db-shm",
    "venv",
...
def _is_under_protected_dir(path: Path, root: Path) -> bool:
    ...
    if len(rel_parts) >= 2 and rel_parts[0] == "web" and rel_parts[1] == "node_modules":
        return True
    if len(rel_parts) >= 2 and rel_parts[0] == "data" and rel_parts[1] == "live_trading":
        return True
    return False
...
def _is_current_state_artifact(path: Path, root: Path) -> bool:
    ...
    if len(rel) != 2 or rel[0] not in {"data", "model"} or not path.is_file():
        return False
    if path.suffix.lower() not in {".json", ".pkl"}:
        return False
...
def _is_generated_scan_output(path: Path, root: Path) -> bool:
    return _safe_relative(path, root) in GENERATED_SCAN_OUTPUTS and path.is_file()
...
def _iter_cleanup_candidates(root: Path = PROJECT_ROOT) -> Iterable[CleanupCandidate]:
    ...
    # Local runtime logs can grow quickly (rotated root logs and monitor logs are
    # common). Keep machine-readable current-state JSON, but remove plain logs.
    for path in root.rglob("*"):
        ...
        if _is_runtime_log(path):
            candidate = add(
                path,
                "runtime_log",
                "generated runtime log; current truth lives in JSON contracts and DB state",
            )
            if candidate:
                yield candidate

    # Large one-off model scans are regenerated by Strategy Lab / heartbeat and
    # are not part of the product source surface.
    for rel in sorted(GENERATED_SCAN_OUTPUTS):
        path = root / rel
        if _is_generated_scan_output(path, root):
            candidate = add(path, "generated_scan_output", "generated parameter-scan scratch output")
            if candidate:
                yield candidate
...
def list_heavy_protected_artifacts(root: Path = PROJECT_ROOT) -> list[dict[str, object]]:
    artifacts: list[dict[str, object]] = []
    for rel in HEAVY_PROTECTED_ARTIFACTS:
        path = root / rel
        if not path.exists():
            continue
        artifacts.append(
            {
                ...
                "reason": "protected by cleanroom policy; review/migrate explicitly before deletion",
            }
        )
    def artifact_bytes(item: dict[str, object]) -> int:
        value = item.get("bytes", 0)
        return value if isinstance(value, int) else 0

    return sorted(artifacts, key=artifact_bytes, reverse=True)
...
def build_audit(root: Path = PROJECT_ROOT) -> dict[str, object]:
    ...
    tracked_ignored = [
        path for path in _run_git(["ls-files", "-ci", "--exclude-standard"], root) if (root / path).exists()
    ]
    ...
    return {
        ...
        "heavy_protected_artifacts": list_heavy_protected_artifacts(root),
        "protected_policy": {
            "never_clean_by_default": sorted(PROTECTED_DIR_NAMES),
            "heavy_protected_artifacts": HEAVY_PROTECTED_ARTIFACTS,
...
def render_text(audit: dict[str, object], max_list: int = 20) -> str:
    ...
    heavy = audit.get("heavy_protected_artifacts", [])
    if isinstance(heavy, list) and heavy:
        lines.append("")
        lines.append("## Heavy protected artifacts (not cleaned by default)")
        for item in heavy[:max_list]:
            if isinstance(item, dict):
                lines.append(
                    f"- {item.get('path', '')} ({_human_bytes(_as_int_value(item.get('bytes', 0)))}) — {item.get('reason', '')}"
                )
    lines.append("")
