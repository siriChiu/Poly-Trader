...
def _format_doc_number(value: Any, digits: int = 1) -> str:
    try:
...
def _high_conviction_topk_freshness(generated_at: Any, *, now: datetime | None = None) -> Dict[str, Any]:
    ...
    stale_after_minutes = HIGH_CONVICTION_TOPK_STALE_AFTER_SEC / 60.0
    ...
    generated_dt = _safe_parse_datetime(generated_at)
...
def _compact_high_conviction_topk_matrix_summary(
    ...
    compact = {
        ...
        "artifact_freshness_reason": freshness.get("reason"),
        "artifact_age_minutes": freshness.get("age_minutes"),
        "artifact_stale_after_minutes": freshness.get("stale_after_minutes"),
        "artifact_deployment_blocking": freshness.get("deployment_blocking"),
...
def _verification_lines(issue: Dict[str, Any]) -> list[str]:
    ...
    if issue.get("id") == "P0_high_conviction_topk_roi_gate" and isinstance(verify, list):
        normalized: list[str] = []
        for item in verify:
            text = str(item).strip()
            if not text:
...
def _issue_action_text(issue: Dict[str, Any]) -> str:
    if issue.get("id") == "P0_high_conviction_topk_roi_gate":
        return (
            "把 high-conviction top-k 從 ROI-only 觀測列升級為風控 / 離線驗證 / 部署門檻優先排序；"
            "nearest-deployable 候選優先顯示，但即時分桶 / 支持阻塞未解除前仍維持模擬觀察 / 影子驗證 / 僅觀察。"
        )
        ...
        if latest_matrix:
            ...
            matrix_lines.extend(
                [
                    ...
                    f"`freshness={latest_matrix.get('artifact_freshness_status', '—')}` / "
                    f"`age_min={_format_doc_number(latest_matrix.get('artifact_age_minutes'), 1)}` / "
                    f"`stale_after_min={_format_doc_number(latest_matrix.get('artifact_stale_after_minutes'), 0)}` / "
                    f"`deployment_blocking={latest_matrix.get('artifact_deployment_blocking', '—')}` / "
    ...
    *,
    ...
    if has_release_math:
        ...
        execution_status_fact_line = (
            f"  - {breaker_release_ui_line}；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 {support_scope_operator_label} support / 治理背景；{runtime_copy_docs_clause}"
        )
        execution_status_roadmap_heading = "- **Execution Status / Bot 營運 已顯示熔斷解除條件**"
        execution_status_roadmap_line = (
            f"  - {breaker_release_ui_line}；操作員執行介面先看熔斷解除條件，再看 {support_scope_operator_label} support / 背景治理；{runtime_copy_docs_clause}"
        )
        ...
        execution_status_fact_line = (
            f"  - {current_live_blocker_ui_line}；`/execution/status` 與 `/execution` 會先顯示即時部署阻塞點，再顯示 {support_scope_operator_label} support / 治理背景；{runtime_copy_docs_clause}"
        )
        execution_status_roadmap_heading = "- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**"
        execution_status_roadmap_line = (
            f"  - {current_live_blocker_ui_line}；操作員執行介面先看即時部署阻塞點，再看 {support_scope_operator_label} support / 背景治理；{runtime_copy_docs_clause}"
        )
    ...
    docs_sync_line = f"current-state docs 已 overwrite sync 到 `issues.json / live probe / drilldown / leaderboard_feature_profile_probe / high_conviction_topk` 最新 truth；`/execution` 快捷列已補上 `/api/status` 初次同步 fail-closed：買入 / 啟用自動模式暫停，等待 / 觀望與減碼保留；`/api/execution/overview` / `/api/execution/runs` 已走 20s operator-workspace timeout，避免 8s default 把可用 Bot 營運 payload 誤報成 `API timeout`；`/api/trade` 買入 / 加倉直接入口也會依即時部署阻塞點 409 暫停，且保留等待 / 觀望與減倉 / 賣出風險降低路徑；Dashboard 啟動連續性卡會把 feature deferred / repair_deferred 顯示成警示與心跳維護收斂文案；{execution_status_docs_sync_clause}；{runtime_copy_docs_clause}；metadata smoke venue rows 已帶 per-venue proof_state / blockers / operator_next_action / verify_next，讓 Dashboard / Execution / Lab 直接顯示實單證據缺口，且 venue error 會先轉成操作員繁中 copy（例如不支援的交易場館）"
...
def collect_q35_scaling_audit_diagnostics() -> Dict[str, Any]:
    ...
    return {
        ...
        "current_live": {
            ...
            "allowed_layers": current_live.get("allowed_layers"),
            "allowed_layers_raw_reason": current_live.get("allowed_layers_raw_reason"),
            "allowed_layers_reason": current_live.get("allowed_layers_reason"),
            "execution_guardrail_reason": current_live.get("execution_guardrail_reason"),
            "deployment_blocker": current_live.get("deployment_blocker"),
            "deployment_blocker_source": current_live.get("deployment_blocker_source"),
            "runtime_closure_state": current_live.get("runtime_closure_state"),
            "support_route_verdict": current_live.get("support_route_verdict"),
            "support_route_deployable": current_live.get("support_route_deployable"),
            "current_live_structure_bucket_rows": current_live.get("current_live_structure_bucket_rows"),
            "minimum_support_rows": current_live.get("minimum_support_rows"),
            "current_live_structure_bucket_gap_to_minimum": current_live.get("current_live_structure_bucket_gap_to_minimum"),
        ...
        "deployment_grade_component_experiment": {
            ...
            "runtime_allowed_layers": deployment_experiment.get("runtime_allowed_layers"),
            "runtime_allowed_layers_raw_reason": deployment_experiment.get("runtime_allowed_layers_raw_reason"),
            "runtime_allowed_layers_reason": deployment_experiment.get("runtime_allowed_layers_reason"),
            "runtime_execution_guardrail_reason": deployment_experiment.get("runtime_execution_guardrail_reason"),
            "runtime_deployment_blocker": deployment_experiment.get("runtime_deployment_blocker"),
            "runtime_execution_blocker": deployment_experiment.get("runtime_execution_blocker"),
            "runtime_closure_state": deployment_experiment.get("runtime_closure_state"),
            "support_route_verdict": deployment_experiment.get("support_route_verdict"),
            "support_route_deployable": deployment_experiment.get("support_route_deployable"),
            "current_live_structure_bucket_rows": deployment_experiment.get("current_live_structure_bucket_rows"),
            "minimum_support_rows": deployment_experiment.get("minimum_support_rows"),
            "current_live_structure_bucket_gap_to_minimum": deployment_experiment.get("current_live_structure_bucket_gap_to_minimum"),
        ...
        "base_stack_redesign_experiment": {
            ...
            "current_allowed_layers_raw": redesign_experiment.get("current_allowed_layers_raw"),
            "current_allowed_layers": redesign_experiment.get("current_allowed_layers"),
            "runtime_execution_blocked": redesign_experiment.get("runtime_execution_blocked"),
            "runtime_execution_blocker": redesign_experiment.get("runtime_execution_blocker"),
