...
- Verdict: `classification={fill_verdict.get('classification', '—')}`, current calibration window `{fill_verdict.get('current_calibration_window', fill_identity.get('calibration_window', '—'))}`, current exact bucket rows `{fill_verdict.get('current_exact_bucket_rows', rows)}/{fill_verdict.get('minimum_support_rows', minimum)}`, `gap={fill_verdict.get('gap_to_minimum', gap)}`, `time_to_evidence_bucket={fill_verdict.get('time_to_evidence_bucket', '—')}`, `missing_capability_class={fill_verdict.get('missing_capability_class', '—')}`, `alternative_solution_required={_bool_text(fill_verdict.get('alternative_solution_required'))}`。
...
1. **Exact current support lane**：刷新 live probe / support audit / support-fill feasibility，直接顯示 current rows 是否從 `{rows}/{minimum}` 開始 movement；若 `delta_vs_previous=0` 或 `stagnant_run_count` 持續增加，必須說明缺的是 Map / Tool / Signal / Constraint / Review 哪一類能力。
