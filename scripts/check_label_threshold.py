...
# Check label_sell_win distribution and future_return stats
try:
    stats = c.execute("""
        SELECT 
            COUNT(*) as total,
            SUM(CASE WHEN label_sell_win = 1 THEN 1 ELSE 0 END) as wins,
            SUM(CASE WHEN label_sell_win = 0 THEN 1 ELSE 0 END) as losses,
...
rows = c.execute("SELECT future_return_pct FROM labels WHERE label_sell_win IS NOT NULL").fetchall()
