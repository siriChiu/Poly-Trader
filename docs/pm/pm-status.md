...
- `data/live_predict_probe.json` generated at `2026-05-22T16:11:26.325770Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.295108`；`regime_label=bear` / `regime_gate=CAUTION` / `entry_quality_label=C` / decision quality score `—`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-22T16:11:31.401941Z`；verdict `mixed_horizon_false_positive`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-22T16:11:36.752676+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24914`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-22T16:11:51.245027Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-22T16:11:22.490291+00:00`。
- Full sample rows `24776`。
- Recent canonical window `100`: win_rate `38.0%`, dominant regime `chop(94.0%)`, alerts `regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-22T16:11:48.877581+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q00`。
- Verdict: `classification=true_support_under_minimum`, current calibration window `200`, current exact bucket rows `1/50`, identity rows before bucket filter `48`, non-current-bucket identity rows `47`, `gap=49`, `time_to_evidence_bucket=within_week_if_exact_identity_keeps_accumulating`, `missing_capability_class=Signal/Support`, `alternative_solution_required=true`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-22T16:11:52.102489Z`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q00`、exact support `1/50 gap=49`、breaker `release_ready=true` / `24/50`、Top-K `artifact_freshness_status=fresh` / `samples=24914`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `100` win_rate `38.0%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
