...
- `data/live_predict_probe.json` generated at `2026-05-21T16:02:36.493040Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.656183`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=D` / decision quality score `—`。
...
- Support progress: `support_progress_status=regressed_under_minimum` / `regression_basis=same_identity_same_semantic_signature` / `previous_rows=121` / `delta_vs_previous=-120` / `stagnant_run_count=0` / legacy reference is reference-only because support identity does not close current deployment.
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-21T16:01:47.579751Z`；verdict `mixed_horizon_false_positive`。
- Release context: `release_ready=true`, recent-window wins `29/50`, required wins `15/50`, `additional_recent_window_wins_needed=0`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-21T16:02:27.009680+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24803`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-21T16:02:42.653894Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-21T16:01:38.667770+00:00`。
- Full sample rows `24665`。
- Recent canonical window `100`: win_rate `79.0%`, dominant regime `bear(58.0%)`, alerts `regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-21T16:02:40.324235+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-21T16:02:43.529237Z`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q15`、exact support `1/50 gap=49`、breaker `release_ready=true` / `29/50`、Top-K `artifact_freshness_status=fresh` / `samples=24803`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `100` win_rate `79.0%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
