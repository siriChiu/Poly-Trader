...
- `data/live_predict_probe.json` generated at `2026-05-26T17:20:53.977045Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=CIRCUIT_BREAKER` / `should_trade=false` / confidence `0.500000`；`regime_label=bear` / `regime_gate=CAUTION` / `entry_quality_label=D` / `decision_quality_score=0.2843`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-26T17:20:03.831848Z`；verdict `canonical_breaker_active`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-26T17:20:43.183239+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25321`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-26T17:21:00.615967Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-26T17:19:54.115738+00:00`。
- Full sample rows `25183`。
- Recent canonical window `100`: win_rate `37.0%`, dominant regime `chop(97.0%)`, alerts `regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-26T17:20:58.003715+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q00`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-26T17:21:01.525096Z`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q00`、exact support `0/50 gap=50`、breaker `release_ready=false` / `0/50`、Top-K `artifact_freshness_status=fresh` / `samples=25321`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
