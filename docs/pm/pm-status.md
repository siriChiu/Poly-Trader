...
- `data/live_predict_probe.json` generated at `2026-05-24T00:12:52.396480Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.272211`；`regime_label=bear` / `regime_gate=CAUTION` / `entry_quality_label=D` / decision quality score `—`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-24T00:12:54.338039Z`；verdict `breaker_clear`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-24T00:12:56.721331+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25059`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-24T00:13:10.570743Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-24T00:12:41.578500+00:00`。
- Full sample rows `24921`。
- Recent canonical window `100`: win_rate `21.0%`, dominant regime `bear(55.0%)`, alerts `regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-24T00:13:07.494383+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q35`。
- Verdict: `classification=true_support_under_minimum`, current calibration window `200`, current exact bucket rows `0/50`, identity rows before bucket filter `6`, non-current-bucket identity rows `6`, `gap=50`, `time_to_evidence_bucket=unknown_until_exact_identity_rows_start_accumulating`, `missing_capability_class=Signal/Support`, `alternative_solution_required=true`。
- Reference-only evidence: `best_reference_window=all`, `best_reference_exact_bucket_rows=1`, `best_reference_evidence_role=reference_only_calibration_window_mismatch`；reference rows cannot be counted as deployable support unless support identity is deliberately rebaselined and fully reverified.
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-24T00:13:11.505850Z`。
