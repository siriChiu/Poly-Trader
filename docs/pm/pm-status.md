...
_最後更新：2026-05-18 15:31:02 CST_
...
最新 `data/live_predict_probe.json`（`2026-05-18T07:02:05.967606Z`）顯示：`target_col=simulated_pyramid_win`、`signal=CIRCUIT_BREAKER`、`should_trade=false`、`deployment_blocker=circuit_breaker_active`、`deployment_blocker_source=circuit_breaker`、`runtime_closure_state=circuit_breaker_active`、`release_ready=false`。熔斷 release math 是：`current_streak=27`（需 `<50`，這一項已達）、最近 50 筆 `wins=6/50`、`current_recent_window_win_rate=12.0%`，需至少 `15/50` 與 `>=30%`，因此 `additional_recent_window_wins_needed=9`。
...
- `data/live_predict_probe.json` generated at `2026-05-18T07:02:05.967606Z`.
...
- Release condition: `release_ready=false`; recent-window gate is `6/50` wins, needs `15/50`, so `additional_recent_window_wins_needed=9`; current streak is `27`, below the `<50` streak ceiling.
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-18T07:02:49.393785+00:00`; `artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`.
- Matrix status: `samples=24537`, `row_count=24`, `deployable_rows=0`, `risk_qualified_rows=6`, `runtime_blocked_candidate_rows=6`.
- Runtime overlay is synced to live truth: `deployment_blocker=circuit_breaker_active`, `release_ready=false`, `recent_window_wins=6/50`, `additional_recent_window_wins_needed=9`, `support_route_verdict=exact_bucket_unsupported_block`, bucket rows `0/50`, `gap=50`.
...
- `data/execution_metadata_smoke.json` generated at `2026-05-18T07:02:58.909061Z`.
...
- `data/recent_drift_report.json` generated at `2026-05-18T07:02:01.768384+00:00`.
...
- Quality metrics: `avg_simulated_pnl=-0.0064`, `avg_simulated_quality=-0.0765`, `avg_drawdown_penalty=0.1943`.
