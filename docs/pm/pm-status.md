...
- `data/live_predict_probe.json` generated at `2026-05-22T20:13:52.165479Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.484338`；`regime_label=bear` / `regime_gate=BLOCK` / `entry_quality_label=C` / decision quality score `—`。
...
- Support progress: `support_progress_status=stalled_under_minimum` / `regression_basis=same_identity_same_semantic_signature` / `previous_rows=0` / `delta_vs_previous=0` / `stagnant_run_count=2` / legacy reference is reference-only because support identity does not close current deployment.
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-22T20:13:54.137321Z`；verdict `mixed_horizon_false_positive`。
- Release context: `release_ready=true`, recent-window wins `22/50`, required wins `15/50`, `additional_recent_window_wins_needed=0`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-22T20:13:56.536173+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24936`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-22T20:14:09.498598Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-22T20:13:48.352878+00:00`。
- Full sample rows `24798`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-22T20:14:07.082492+00:00`；scanned current support identity bucket is `BLOCK|bear_bias200_hard_block|q00`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-22T20:14:10.400633Z`。
