...
- `data/live_predict_probe.json` generated at `2026-05-28T05:08:23.162438Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=CIRCUIT_BREAKER` / `should_trade=false` / confidence `0.500000`；`regime_label=bear` / `regime_gate=BLOCK` / `entry_quality_label=C` / `decision_quality_score=-0.2284`。
...
- Current-live support: `current_live_structure_bucket=BLOCK|bear_bias200_hard_block|q00`, `support_route_verdict=exact_bucket_present_but_below_minimum`, `support_governance_route=exact_live_bucket_present_but_below_minimum`, rows `9/50`, `gap=41`。
- Support progress: `support_progress_status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `previous_rows=8` / `delta_vs_previous=1` / `stagnant_run_count=0` / `semantic_signature_delta_vs_previous=1` / `semantic_signature_stagnant_run_count=0` / `semantic_signature_stalled_support_accumulation=false` / legacy reference is reference-only because support identity does not close current deployment.
...
**PM verdict：接受「熔斷仍 active（recent `0/50`，需要 `15/50`，還差 `15` 勝），且 current exact support 仍是 `9/50`、gap `41`，尚未建立同一 support identity 的精準樣本，所以 live buy/add 仍 fail-closed」。不可把 legacy rows、exact-live-lane proxy rows、Top-K OOS pass、或單一 support/governance gate 包裝成 deployable。**

### Circuit breaker

- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-28T05:08:10.566149Z`；verdict `canonical_breaker_active`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-28T05:08:13.488838+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25428`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
- Matrix payload: `deployable_rows=0`, `risk_qualified_rows=6`, `support_route=exact_bucket_present_but_below_minimum`, `deployment_blocker=circuit_breaker_active`, `current_live_structure_bucket=BLOCK|bear_bias200_hard_block|q00`, bucket rows `9/50`, `gap=41`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-28T05:08:29.838747Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-28T05:08:00.344263+00:00`。
- Full sample rows `25290`。
- Recent canonical window `100`: win_rate `0.0%`, dominant regime `chop(54.0%)`, alerts `constant_target`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-28T05:08:27.137722+00:00`；scanned current support identity bucket is `BLOCK|bear_bias200_hard_block|q00`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `9/50`, identity rows before bucket filter `9`, non-current-bucket identity rows `0`, `gap=41`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
- Reference-only evidence: `best_reference_window=600`, `best_reference_exact_bucket_rows=82`, `best_reference_evidence_role=reference_only_calibration_window_mismatch`；reference rows cannot be counted as deployable support unless support identity is deliberately rebaselined and fully reverified.
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-28T05:08:30.765401Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `circuit_breaker_active`，support 邊界是 `BLOCK|bear_bias200_hard_block|q00` `9/50 gap=41`。
