...
- `data/live_predict_probe.json` generated at `2026-05-27T22:11:10.025200Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=CIRCUIT_BREAKER` / `should_trade=false` / confidence `0.500000`；`regime_label=bear` / `regime_gate=BLOCK` / `entry_quality_label=C` / `decision_quality_score=-0.2305`。
...
**PM verdict：接受「熔斷仍 active（recent `0/50`，需要 `15/50`，還差 `15` 勝），且 current exact support 仍是 `5/50`、gap `45`，尚未建立同一 support identity 的精準樣本，所以 live buy/add 仍 fail-closed」。不可把 legacy rows、exact-live-lane proxy rows、Top-K OOS pass、或單一 support/governance gate 包裝成 deployable。**

### Circuit breaker

- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-27T22:10:54.905079Z`；verdict `canonical_breaker_active`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-27T22:10:57.380622+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25418`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
- Matrix payload: `deployable_rows=0`, `risk_qualified_rows=6`, `support_route=exact_bucket_present_but_below_minimum`, `deployment_blocker=circuit_breaker_active`, `current_live_structure_bucket=BLOCK|bear_bias200_hard_block|q00`, bucket rows `5/50`, `gap=45`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-27T22:11:16.735127Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-27T22:10:49.158452+00:00`。
- Full sample rows `25280`。
- Recent canonical window `100`: win_rate `0.0%`, dominant regime `chop(64.0%)`, alerts `constant_target`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-27T22:11:13.858760+00:00`；scanned current support identity bucket is `BLOCK|bear_bias200_hard_block|q00`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `5/50`, identity rows before bucket filter `5`, non-current-bucket identity rows `0`, `gap=45`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
- Reference-only evidence: `best_reference_window=600`, `best_reference_exact_bucket_rows=78`, `best_reference_evidence_role=reference_only_calibration_window_mismatch`；reference rows cannot be counted as deployable support unless support identity is deliberately rebaselined and fully reverified.
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-27T22:11:17.664579Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `circuit_breaker_active`，support 邊界是 `BLOCK|bear_bias200_hard_block|q00` `5/50 gap=45`。
