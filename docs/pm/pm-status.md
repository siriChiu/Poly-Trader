...
_最後更新：2026-05-19 06:33:05 CST_
...
最新 `data/live_predict_probe.json`（`2026-05-18T22:15:46.406978Z`，本 PM run wall-clock age 約 `17.3m`）顯示：`target_col=simulated_pyramid_win`、`signal=CIRCUIT_BREAKER`、`should_trade=false`、`deployment_blocker=circuit_breaker_active`、`runtime_closure_state=circuit_breaker_active`、`release_ready=false`。熔斷 release math 從前一版 `4/50` 推進到 `9/50`：最近 50 筆 `current_recent_window_wins=9/50`、`current_recent_window_win_rate=18.0%`，至少需要 `15/50` 與 `>=30%`，因此 `additional_recent_window_wins_needed=6`；`current_streak=0` 通過 `<50` 的連敗上限，但 recent-win-rate gate 仍未通過。
...
- `data/live_predict_probe.json` generated at `2026-05-18T22:15:46.406978Z`；canonical target `simulated_pyramid_win`。
...
- Release condition: `release_ready=false`; recent-window gate is `9/50` wins, requires `15/50`, so `additional_recent_window_wins_needed=6`; `current_streak=0` passes the `<50` streak ceiling.
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-18T22:15:53.217046+00:00`; artifact reports `artifact_freshness_status=fresh` and PM wall-clock age is about `17.2m`, under `artifact_stale_after_minutes=60.0`.
- Matrix payload: `samples=24578`, `row_count=24`, `deployable_rows=0`, `risk_qualified_rows=6`, `runtime_blocked_candidate_rows=6`.
- Runtime overlay: `deployment_blocker=circuit_breaker_active`, `release_ready=false`, `recent_window_wins=9/50`, `additional_recent_window_wins_needed=6`, `support_route=insufficient_support_everywhere`, `support_governance_route=exact_live_lane_proxy_available`, bucket rows `0/50`, `gap=50`.
...
- `data/execution_metadata_smoke.json` generated at `2026-05-18T22:16:02.776751Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-18T22:15:42.642953+00:00`。
- Target: `simulated_pyramid_win`; full sample `24440` rows, full win rate `61.71%`。
- Blocking window truth: last 100 rows win rate `24.0%`, wins/losses `24/76`, delta vs full sample `-37.71pp`, dominant regime `bear=100.0%`。
- Quality metrics: `avg_simulated_pnl=-0.0057`, `avg_simulated_quality=-0.0741`, `avg_drawdown_penalty=0.24`, weekend share `100.0%`。
...
- PM-relevant P1s: train-CV gap, model stability, regime drift, venue readiness, fin_netflow auth blocker, leaderboard recent-window contract, nest_pred TLS verification, and q15 exact support under-minimum governance.
...
6. **Recent drift lane**：針對 last-100 win_rate `24.0%`、bear `100%`、losses `76/100`、weekend concentration / regime shift / compressed sparse features 的 canonical pathology，下一輪給出一個可測的 blocker root-cause patch 或 falsification test。
