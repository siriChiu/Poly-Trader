...
- `data/live_predict_probe.json` generated at `2026-05-22T12:10:40.235041Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=CIRCUIT_BREAKER` / `should_trade=false` / confidence `0.500000`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=D` / decision quality score `—`。
...
- Support progress: `support_progress_status=regressed_under_minimum` / `regression_basis=same_identity_same_semantic_signature` / `previous_rows=6` / `delta_vs_previous=0` / `stagnant_run_count=2` / legacy reference is reference-only because support identity does not close current deployment.
...
**PM verdict：接受「熔斷仍 active（recent `14/50`，需要 `15/50`，還差 `1` 勝），且 current exact support 仍是 `6/50`、gap `44`，尚未建立同一 support identity 的精準樣本，所以 live buy/add 仍 fail-closed」。不可把 legacy rows、exact-live-lane proxy rows、Top-K OOS pass、或單一 support/governance gate 包裝成 deployable。**

### Circuit breaker

- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-22T12:10:42.318427Z`；verdict `canonical_breaker_active`。
- Release context: `release_ready=false`, recent-window wins `14/50`, required wins `15/50`, `additional_recent_window_wins_needed=1`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-22T12:10:44.633119+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24898`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
- Matrix payload: `deployable_rows=0`, `risk_qualified_rows=6`, `support_route=exact_bucket_present_but_below_minimum`, `deployment_blocker=circuit_breaker_active`, `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15`, bucket rows `6/50`, `gap=44`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-22T12:11:01.074611Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-22T12:10:36.352792+00:00`。
- Full sample rows `24760`。
- Recent canonical window `100`: win_rate `34.0%`, dominant regime `chop(100.0%)`, alerts `regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-22T12:10:58.547836+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-22T12:11:01.998971Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `circuit_breaker_active`，support 邊界是 `CAUTION|base_caution_regime_or_bias|q15` `6/50 gap=44`。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `100` win_rate `34.0%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
