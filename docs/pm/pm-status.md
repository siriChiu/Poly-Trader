...
- `data/live_predict_probe.json` generated at `2026-05-23T09:40:39.474196Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=CIRCUIT_BREAKER` / `should_trade=false` / confidence `0.500000`；`regime_label=bear` / `regime_gate=BLOCK` / `entry_quality_label=B` / decision quality score `—`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-23T09:40:41.610345Z`；verdict `canonical_breaker_active`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-23T09:40:46.251087+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24997`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-23T09:40:59.991270Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-23T09:40:35.668255+00:00`。
- Full sample rows `24859`。
- Recent canonical window `100`: win_rate `11.0%`, dominant regime `chop(89.0%)`, alerts `label_imbalance, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-23T09:40:57.689019+00:00`；scanned current support identity bucket is `BLOCK|bear_bias200_hard_block|q00`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-23T09:41:00.957561Z`。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `100` win_rate `11.0%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
