...
- `data/live_predict_probe.json` generated at `2026-05-26T21:15:26.607960Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=CIRCUIT_BREAKER` / `should_trade=false` / confidence `0.500000`；`regime_label=bear` / `regime_gate=BLOCK` / `entry_quality_label=D` / `decision_quality_score=-0.0306`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-26T21:15:12.069737Z`；verdict `canonical_breaker_active`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-26T21:15:16.885423+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25338`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-26T21:15:33.385394Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-26T21:15:00.584194+00:00`。
- Full sample rows `25200`。
- Recent canonical window `100`: win_rate `20.0%`, dominant regime `chop(98.0%)`, alerts `label_imbalance, regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-26T21:15:30.668191+00:00`；scanned current support identity bucket is `BLOCK|bear_bias200_hard_block|q00`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `0/50`, identity rows before bucket filter `3`, non-current-bucket identity rows `3`, `gap=50`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-26T21:15:34.313164Z`。
