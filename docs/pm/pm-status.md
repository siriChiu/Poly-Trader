...
- `data/live_predict_probe.json` generated at `2026-05-22T06:16:02.837174Z`；canonical target is `simulated_pyramid_win`。
...
- Support progress: `support_progress_status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `previous_rows=30` / `delta_vs_previous=3` / `stagnant_run_count=0` / legacy reference is reference-only because support identity does not close current deployment.
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-22T06:15:45.767669Z`；verdict `canonical_breaker_active`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-22T06:15:50.583574+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24868`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-22T06:16:08.895050Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-22T06:15:36.797163+00:00`。
- Full sample rows `24730`。
- Recent canonical window `100`: win_rate `33.0%`, dominant regime `chop(100.0%)`, alerts `regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-22T06:16:06.595015+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-22T06:16:09.794958Z`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q15`、exact support `33/50 gap=17`、breaker `release_ready=false` / `13/50`、Top-K `artifact_freshness_status=fresh` / `samples=24868`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `100` win_rate `33.0%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
