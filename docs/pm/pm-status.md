...
**State：`ORANGE_framework_capture_risk` governance overlay + `ORANGE_customer_value_gap` freshness overlay；safe lane remains `YELLOW_shadow_or_paper_usable`；`ORANGE_alternative_solution_required` remains active.**

PM 結論：客戶成功仍是北極星，但 live buy/add safety gate 不可被 customer urgency 推翻。承接上一輪 PM handoff：維持 current-live exact-support blocker、交付 paper/shadow / dry-run / falsification / support-fill proof，且不可降低 live gate。fresh runtime truth 顯示 current-live bucket 是 `CAUTION|base_caution_regime_or_bias|q35`；PM 決策不變：current exact support 是 `12/50`、`gap=38`、`support_route_verdict=exact_bucket_present_but_below_minimum`，`support_governance_route=exact_live_bucket_present_but_below_minimum` 只能當 support-governance signal，不是部署閉環。
...
- `data/live_predict_probe.json` generated at `2026-05-25T06:26:48.547089Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.579973`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=D` / `decision_quality_score=0.0488`。
...
- Guardrail truth: `allowed_layers_raw=0` but `allowed_layers=0`；`allowed_layers_reason=under_minimum_exact_live_structure_bucket`；`execution_guardrail_reason=under_minimum_exact_live_structure_bucket`。
- Current-live support: `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35`, `support_route_verdict=exact_bucket_present_but_below_minimum`, `support_governance_route=exact_live_bucket_present_but_below_minimum`, rows `12/50`, `gap=38`。
- Support progress: `support_progress_status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `previous_rows=12` / `delta_vs_previous=0` / `stagnant_run_count=2` / legacy reference is reference-only because support identity does not close current deployment.
...
**PM verdict：接受「熔斷已解除，但 current exact support 仍是 `12/50`、gap `38`，尚未建立同一 support identity 的精準樣本，所以 live buy/add 仍 fail-closed」。不可把 legacy rows、exact-live-lane proxy rows、Top-K OOS pass、或單一 support/governance gate 包裝成 deployable。**

### Circuit breaker

- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-25T06:26:31.034468Z`；verdict `breaker_clear`。
- Release context: `release_ready=true`, recent-window wins `40/50`, required wins `15/50`, `additional_recent_window_wins_needed=0`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-25T06:26:37.712689+00:00`；artifact self-report remains `artifact_freshness_status=fresh` / `artifact_age_minutes=0.4055`, but PM wall-clock audit at `2026-05-25T15:37 CST` shows `PM_wall_clock_freshness=stale_reference_only` / `PM_wall_clock_age≈71m` > `artifact_stale_after_minutes=60.0`；`artifact_deployment_blocking=false`, `samples=25188`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
- Matrix payload: `deployable_rows=0`, `risk_qualified_rows=6`, `support_route=exact_bucket_present_but_below_minimum`, `deployment_blocker=under_minimum_exact_live_structure_bucket`, `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35`, bucket rows `12/50`, `gap=38`。
...
**PM verdict：Top-K is no longer PM-fresh by wall clock; it is reference-only research / paper-shadow evidence until refreshed or explicitly labeled stale in `/api/models/leaderboard` / Strategy Lab. `deployable_rows=0` still means no risk-on live action.**

### Venue readiness

- `data/execution_metadata_smoke.json` generated at `2026-05-25T06:26:58.062819Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-25T06:26:12.529698+00:00`。
- Full sample rows `25050`。
- Recent canonical window `100`: win_rate `90.0%`, dominant regime `bear(64.0%)`, alerts `label_imbalance, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-25T06:26:55.311246+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q35`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `12/50`, identity rows before bucket filter `24`, non-current-bucket identity rows `12`, `gap=38`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
- Reference-only evidence: `best_reference_window=all`, `best_reference_exact_bucket_rows=3291`, `best_reference_evidence_role=reference_only_calibration_window_mismatch`；reference rows cannot be counted as deployable support unless support identity is deliberately rebaselined and fully reverified.
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-25T06:26:58.978647Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `under_minimum_exact_live_structure_bucket`，support 邊界是 `CAUTION|base_caution_regime_or_bias|q35` `12/50 gap=38`。
2. **Strategy Lab**：看 Top-K / leaderboard 研究候選、OOS ROI、win rate、drawdown、profit factor、worst fold 與 runtime-blocked 原因；但 Top-K matrix 目前是 PM wall-clock stale reference-only，必須刷新或在 UI/API 明確標示 stale；`deployable_rows=0` 時只能作 research / paper-shadow evidence。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay**、新增 **`ORANGE_customer_value_gap` freshness overlay**，且 **`ORANGE_alternative_solution_required`** 仍有效；這不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事或自報 fresh 欄位捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q35`、exact support `12/50 gap=38`、breaker `release_ready=true` / `40/50`，並把 Top-K 從 artifact self-report `fresh` / `samples=25188` 改判為 PM wall-clock `stale_reference_only`（約 `71m` > `60m`）；Execution Console / Strategy Lab paper-shadow lane 保留，但 no live exposure。
