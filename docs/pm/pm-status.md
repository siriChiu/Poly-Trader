...
- `data/live_predict_probe.json` generated at `2026-05-25T17:31:54.413185Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.560575`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=D` / `decision_quality_score=0.4130`。
...
- Current-live support: `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35`, `support_route_verdict=exact_bucket_present_but_below_minimum`, `support_governance_route=exact_live_bucket_present_but_below_minimum`, rows `25/50`, `gap=25`。
- Support progress: `support_progress_status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `previous_rows=25` / `delta_vs_previous=0` / `stagnant_run_count=2` / legacy reference is reference-only because support identity does not close current deployment.
...
**PM verdict：接受「熔斷已解除，但 current exact support 仍是 `25/50`、gap `25`，尚未建立同一 support identity 的精準樣本，所以 live buy/add 仍 fail-closed」。不可把 legacy rows、exact-live-lane proxy rows、Top-K OOS pass、或單一 support/governance gate 包裝成 deployable。**

### Circuit breaker

- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-25T17:30:24.264023Z`；verdict `breaker_clear`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-25T17:31:42.067695+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25230`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
- Matrix payload: `deployable_rows=0`, `risk_qualified_rows=6`, `support_route=exact_bucket_present_but_below_minimum`, `deployment_blocker=under_minimum_exact_live_structure_bucket`, `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35`, bucket rows `25/50`, `gap=25`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-25T17:32:02.261354Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-25T17:30:09.975200+00:00`。
- Full sample rows `25092`。
- Recent canonical window `100`: win_rate `90.0%`, dominant regime `chop(74.0%)`, alerts `label_imbalance, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-25T17:31:59.306265+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q35`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `25/50`, identity rows before bucket filter `25`, non-current-bucket identity rows `0`, `gap=25`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
- Reference-only evidence: `best_reference_window=all`, `best_reference_exact_bucket_rows=3304`, `best_reference_evidence_role=reference_only_calibration_window_mismatch`；reference rows cannot be counted as deployable support unless support identity is deliberately rebaselined and fully reverified.
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-25T17:32:03.160026Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `under_minimum_exact_live_structure_bucket`，support 邊界是 `CAUTION|base_caution_regime_or_bias|q35` `25/50 gap=25`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q35`、exact support `25/50 gap=25`、breaker `release_ready=true` / `50/50`、Top-K `artifact_freshness_status=fresh` / `samples=25230`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
