...
_最後更新：2026-05-18 17:33:15 CST_
...
最新 `data/live_predict_probe.json`（`2026-05-18T09:22:23.447807Z`）顯示：`target_col=simulated_pyramid_win`、`signal=CIRCUIT_BREAKER`、`should_trade=false`、`deployment_blocker=circuit_breaker_active`、`deployment_blocker_source=circuit_breaker`、`runtime_closure_state=circuit_breaker_active`、`release_ready=false`。熔斷 release math 是：`current_streak=32`（需 `<50`，這一項已達）、最近 50 筆 `wins=6/50`、`current_recent_window_win_rate=12.0%`，需至少 `15/50` 與 `>=30%`，因此 `additional_recent_window_wins_needed=9`。
...
- `data/live_predict_probe.json` generated at `2026-05-18T09:22:23.447807Z`.
...
- Release condition: `release_ready=false`; recent-window gate is `6/50` wins, needs `15/50`, so `additional_recent_window_wins_needed=9`; current streak is `32`, below the `<50` streak ceiling.
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-18T09:02:45.168477+00:00`; freshness checked at `2026-05-18T09:23:10.191969+00:00`; `artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`.
- Matrix status: `samples=24542`, `row_count=24`, `deployable_rows=0`, `risk_qualified_rows=6`, `runtime_blocked_candidate_rows=6`.
- Runtime overlay is synced to live truth: `deployment_blocker=circuit_breaker_active`, `release_ready=false`, `recent_window_wins=6/50`, `additional_recent_window_wins_needed=9`, `support_governance_route=exact_live_lane_proxy_available`, bucket rows `0/50`, `gap=50`, source live probe `2026-05-18T09:22:23.447807Z`.
...
- `data/execution_metadata_smoke.json` generated at `2026-05-18T09:23:08.284347Z`.
...
- `data/recent_drift_report.json` generated at `2026-05-18T09:22:20.479493+00:00`.
- Target: `simulated_pyramid_win`; blocking window: last 100 rows.
- `win_rate=23.0%`, wins/losses `23/77`, dominant regime `bear=100.0%`.
- Quality metrics: `avg_simulated_pnl=-0.0066`, `avg_simulated_quality=-0.0778`, `avg_drawdown_penalty=0.1941`.
- Alerts: `regime_concentration`, `regime_shift`; adverse/tail target-0 streak remains `30` per current engineering docs.

**PM verdict：近期品質惡化支持 fail-closed；也要求工程把 safe customer lane 做成可操作產品，而不是只說等待。**

### Issue tracker

- `issues.json` has 9 open issues: 3 P0 and 6 P1.
...
6. **Recent drift lane**：針對 last-100 win_rate 23%、bear 100%、losses 77/100、regime concentration / regime shift、target-0 streak 30 的 canonical pathology，下一輪給出一個可測的 blocker root-cause patch 或 falsification test。
