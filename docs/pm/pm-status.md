...
- `data/live_predict_probe.json` generated at `2026-05-22T00:11:34.433076Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.628122`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=C` / decision quality score `—`。
...
- Support progress: `support_progress_status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `previous_rows=24` / `delta_vs_previous=1` / `stagnant_run_count=0` / legacy reference is reference-only because support identity does not close current deployment.
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-22T00:11:22.088223Z`；verdict `breaker_clear`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-22T00:11:24.861015+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24840`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-22T00:11:41.194658Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-22T00:11:13.198454+00:00`。
- Full sample rows `24702`。
- Recent canonical window `500`: win_rate `43.0%`, dominant regime `bear(72.4%)`, alerts `regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-22T00:11:39.013320+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-22T00:11:42.134030Z`。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `500` win_rate `43.0%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
