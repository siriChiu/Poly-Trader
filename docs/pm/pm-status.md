...
- `data/live_predict_probe.json` generated at `2026-05-22T15:11:02.759258Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.324956`；`regime_label=bear` / `regime_gate=CAUTION` / `entry_quality_label=C` / decision quality score `—`。
...
- Current-live support: `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q00`, `support_route_verdict=exact_bucket_present_but_below_minimum`, `support_governance_route=exact_live_bucket_present_but_below_minimum`, rows `1/50`, `gap=49`。
- Support progress: `support_progress_status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `previous_rows=1` / `delta_vs_previous=0` / `stagnant_run_count=2` / legacy reference is reference-only because support identity does not close current deployment.
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-22T15:11:07.739810Z`；verdict `mixed_horizon_false_positive`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-22T15:11:10.121410+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24912`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
- Matrix payload: `deployable_rows=0`, `risk_qualified_rows=6`, `support_route=exact_bucket_present_but_below_minimum`, `deployment_blocker=under_minimum_exact_live_structure_bucket`, `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q00`, bucket rows `1/50`, `gap=49`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-22T15:11:24.790806Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-22T15:10:58.845011+00:00`。
- Full sample rows `24774`。
- Recent canonical window `100`: win_rate `40.0%`, dominant regime `chop(94.0%)`, alerts `regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-22T15:11:22.490427+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q00`。
- Verdict: `classification=true_support_under_minimum`, current calibration window `200`, current exact bucket rows `1/50`, `gap=49`, `time_to_evidence_bucket=within_week_if_exact_identity_keeps_accumulating`, `missing_capability_class=Signal/Support`, `alternative_solution_required=true`。
- Reference-only evidence: `best_reference_window=600`, `best_reference_exact_bucket_rows=40`, `best_reference_evidence_role=reference_only_calibration_window_mismatch`；reference rows cannot be counted as deployable support unless support identity is deliberately rebaselined and fully reverified.
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-22T15:11:25.596668Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `under_minimum_exact_live_structure_bucket`，support 邊界是 `CAUTION|base_caution_regime_or_bias|q00` `1/50 gap=49`。
