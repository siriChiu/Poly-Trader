...
- `data/live_predict_probe.json` generated at `2026-05-26T08:46:56.912894Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.642236`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=D` / `decision_quality_score=0.2420`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-26T08:46:43.254743Z`；verdict `mixed_horizon_false_positive`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-26T08:46:46.102519+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25287`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-26T08:47:03.652417Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-26T08:46:36.532782+00:00`。
- Full sample rows `25149`。
- Recent canonical window `250`: win_rate `81.6%`, dominant regime `chop(51.6%)`, alerts `label_imbalance`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-26T08:47:01.235512+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `7/50`, identity rows before bucket filter `69`, non-current-bucket identity rows `62`, `gap=43`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-26T08:47:04.502828Z`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q15`、exact support `7/50 gap=43`、breaker `release_ready=true` / `21/50`、Top-K `artifact_freshness_status=fresh` / `samples=25287`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
