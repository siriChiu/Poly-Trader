...
- `data/live_predict_probe.json` generated at `2026-05-21T10:10:45.646882Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.569946`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=C` / decision quality score `—`。
...
- Support progress: `support_progress_status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `previous_rows=3` / `delta_vs_previous=0` / `stagnant_run_count=2` / legacy reference is reference-only because support identity does not close current deployment.
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-21T10:10:47.795599Z`；verdict `breaker_clear`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-21T10:10:50.116278+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24781`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-21T10:11:04.399055Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-21T10:10:41.843454+00:00`。
- Full sample rows `24643`。
- Recent canonical window `100`: win_rate `100.0%`, dominant regime `bear(80.0%)`, alerts `constant_target, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-21T10:11:02.112841+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-21T10:11:05.313608Z`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q15`、exact support `3/50 gap=47`、breaker `release_ready=true` / `50/50`、Top-K `artifact_freshness_status=fresh` / `samples=24781`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
