...
- `data/live_predict_probe.json` generated at `2026-05-21T20:13:43.220005Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.557373`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=D` / decision quality score `—`。
...
- Support progress: `support_progress_status=regressed_under_minimum` / `regression_basis=same_identity_same_semantic_signature` / `previous_rows=1` / `delta_vs_previous=0` / `stagnant_run_count=4` / legacy reference is reference-only because support identity does not close current deployment.
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-21T20:13:48.264095Z`；verdict `breaker_clear`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-21T20:13:53.073328+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24825`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-21T20:14:05.520032Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-21T20:13:39.460725+00:00`。
- Full sample rows `24687`。
- Recent canonical window `500`: win_rate `44.0%`, dominant regime `bear(75.4%)`, alerts `regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-21T20:14:02.395131+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-21T20:14:06.453032Z`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q15`、exact support `1/50 gap=49`、breaker `release_ready=true` / `16/50`、Top-K `artifact_freshness_status=fresh` / `samples=24825`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
