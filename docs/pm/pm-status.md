...
- `data/live_predict_probe.json` generated at `2026-05-26T22:13:36.500664Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=CIRCUIT_BREAKER` / `should_trade=false` / confidence `0.500000`；`regime_label=bear` / `regime_gate=BLOCK` / `entry_quality_label=D` / `decision_quality_score=-0.0722`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-26T22:13:20.813885Z`；verdict `canonical_breaker_active`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-26T22:13:24.130246+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25343`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-26T22:13:43.127707Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-26T22:13:11.936086+00:00`。
- Full sample rows `25205`。
- Recent canonical window `100`: win_rate `15.0%`, dominant regime `chop(99.0%)`, alerts `label_imbalance, regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-26T22:13:40.339334+00:00`；scanned current support identity bucket is `BLOCK|bear_bias200_hard_block|q00`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-26T22:13:43.934432Z`。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `100` win_rate `15.0%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
