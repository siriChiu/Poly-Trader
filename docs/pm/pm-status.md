...
- `data/live_predict_probe.json` generated at `2026-05-20T23:01:46.279790Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.568790`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=C` / decision quality score `—`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-20T23:01:48.738715Z`；verdict `breaker_clear`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-20T23:02:23.765287+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=24733`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-20T23:02:35.940913Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-20T23:01:41.886460+00:00`。
- Full sample rows `24595`。
- Recent canonical window `100`: win_rate `82.0%`, dominant regime `bear(100.0%)`, alerts `label_imbalance, regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-20T23:02:33.778207+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-20T23:02:36.889883Z`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q15`、exact support `0/50 gap=50`、breaker `release_ready=true` / `50/50`、Top-K `artifact_freshness_status=fresh` / `samples=24733`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
