...
- `data/live_predict_probe.json` generated at `2026-05-26T10:26:16.359672Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=HOLD` / `should_trade=false` / confidence `0.656760`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=D` / `decision_quality_score=0.2183`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-26T10:25:56.093582Z`；verdict `mixed_horizon_false_positive`。
...
**PM verdict：Top-K remains stale/reference-only research / paper-shadow evidence until refreshed or UI/API labels it stale. Strategy Lab 可顯示 nearest-deployable research rows only with stale/reference-only copy；`deployable_rows=0` means no risk-on live action.**

### Venue readiness

- `data/execution_metadata_smoke.json` generated at `2026-05-26T10:26:25.990276Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-26T10:25:44.619714+00:00`。
- Full sample rows `25153`。
- Recent canonical window `250`: win_rate `81.6%`, dominant regime `chop(53.2%)`, alerts `label_imbalance`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-26T10:26:20.543900+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q15`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `7/50`, identity rows before bucket filter `73`, non-current-bucket identity rows `66`, `gap=43`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-26T10:26:26.917234Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `under_minimum_exact_live_structure_bucket`，support 邊界是 `CAUTION|base_caution_regime_or_bias|q15` `7/50 gap=43`。
2. **Strategy Lab**：看 Top-K / leaderboard 研究候選、OOS ROI、win rate、drawdown、profit factor、worst fold 與 runtime-blocked 原因；目前 Top-K 以 PM wall-clock 判定為 stale/reference-only，須刷新或明確標示 stale 後才可當 customer-facing fresh research；`deployable_rows=0` 時只能作 research / paper-shadow evidence。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `250` win_rate `81.6%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
3. **Top-K freshness lane**：目前 PM wall-clock 已判定 `data/high_conviction_topk_oos_matrix.json` 為 stale/reference-only；下輪必須刷新到 freshness target 內，或讓 `/api/models/leaderboard` / Strategy Lab 明確標示 stale/reference-only。
