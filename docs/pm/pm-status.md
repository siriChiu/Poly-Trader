...
- `data/live_predict_probe.json` generated at `2026-05-24T19:11:25.817266Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=ABSTAIN` / `should_trade=false` / confidence `0.500000`；`regime_label=chop` / `regime_gate=CAUTION` / `entry_quality_label=C` / `decision_quality_score=-0.2443`。
...
- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-24T19:11:18.268191Z`；verdict `breaker_clear`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-24T19:03:27.479312+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25139`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-24T19:11:33.605989Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-24T19:11:06.450255+00:00`。
- Full sample rows `25003`。
- Recent canonical window `100`: win_rate `97.0%`, dominant regime `bear(100.0%)`, alerts `label_imbalance, regime_concentration, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-24T19:11:30.760135+00:00`；scanned current support identity bucket is `CAUTION|base_caution_regime_or_bias|q35`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `0/50`, identity rows before bucket filter `31`, non-current-bucket identity rows `31`, `gap=50`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-24T19:11:34.488774Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `unsupported_exact_live_structure_bucket`，support 邊界是 `CAUTION|base_caution_regime_or_bias|q35` `0/50 gap=50`。
...
本輪維持 **`ORANGE_framework_capture_risk` governance overlay** 與 **`ORANGE_alternative_solution_required`**，不是因為安全 gate 可被推翻，而是避免 PM 被工程 blocker 敘事捕獲。`customer-value delta`：PM status 已承認最新 bucket `CAUTION|base_caution_regime_or_bias|q35`、exact support `0/50 gap=50`、breaker `release_ready=true` / `50/50`、Top-K `artifact_freshness_status=fresh` / `samples=25139`，並保留 Execution Console / Strategy Lab 的 paper-shadow lane；但 no live exposure。
