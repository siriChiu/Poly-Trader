...
- `data/live_predict_probe.json` generated at `2026-05-23T20:09:52.856105Z`；canonical target is `simulated_pyramid_win`。
- Runtime signal: `signal=CIRCUIT_BREAKER` / `should_trade=false` / confidence `0.500000`；`regime_label=bear` / `regime_gate=BLOCK` / `entry_quality_label=D` / decision quality score `—`。
...
- Current-live support: `current_live_structure_bucket=BLOCK|bear_bias200_hard_block|q15`, `support_route_verdict=exact_bucket_missing_exact_lane_proxy_only`, `support_governance_route=exact_live_lane_proxy_available`, rows `0/50`, `gap=50`。
- Support progress: `support_progress_status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `previous_rows=0` / `delta_vs_previous=0` / `stagnant_run_count=2` / legacy reference is reference-only because support identity does not close current deployment.
...
**PM verdict：接受「熔斷仍 active（recent `6/50`，需要 `15/50`，還差 `9` 勝），且 current exact support 仍是 `0/50`、gap `50`，尚未建立同一 support identity 的精準樣本，所以 live buy/add 仍 fail-closed」。不可把 legacy rows、exact-live-lane proxy rows、Top-K OOS pass、或單一 support/governance gate 包裝成 deployable。**

### Circuit breaker

- Latest artifact `data/circuit_breaker_audit.json` generated at `2026-05-23T20:09:40.279931Z`；verdict `canonical_breaker_active`。
- Release context: `release_ready=false`, recent-window wins `6/50`, required wins `15/50`, `additional_recent_window_wins_needed=9`。
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-23T20:09:42.647068+00:00`；`artifact_freshness_status=fresh`, `artifact_deployment_blocking=false`, `samples=25044`, `row_count=24`, `runtime_blocked_candidate_rows=6`。
- Matrix payload: `deployable_rows=0`, `risk_qualified_rows=6`, `support_route=exact_bucket_missing_exact_lane_proxy_only`, `deployment_blocker=circuit_breaker_active`, `current_live_structure_bucket=BLOCK|bear_bias200_hard_block|q15`, bucket rows `0/50`, `gap=50`。
...
- `data/execution_metadata_smoke.json` generated at `2026-05-23T20:09:59.693238Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-23T20:09:34.458665+00:00`。
- Full sample rows `24906`。
- Recent canonical window `250`: win_rate `20.0%`, dominant regime `chop(81.6%)`, alerts `label_imbalance, regime_shift`。
...
- `data/q15_support_fill_feasibility.json` generated at `2026-05-23T20:09:56.657587+00:00`；scanned current support identity bucket is `BLOCK|bear_bias200_hard_block|q15`。
- Verdict: `classification=semantic_window_gap_not_raw_backfill_gap`, current calibration window `200`, current exact bucket rows `0/50`, identity rows before bucket filter `0`, non-current-bucket identity rows `0`, `gap=50`, `time_to_evidence_bucket=semantic_rebaseline_review_required_before_reference_rows_count`, `missing_capability_class=Constraint/Review`, `alternative_solution_required=true`。
- Reference-only evidence: `best_reference_window=all`, `best_reference_exact_bucket_rows=111`, `best_reference_evidence_role=reference_only_calibration_window_mismatch`；reference rows cannot be counted as deployable support unless support identity is deliberately rebaselined and fully reverified.
...
- `data/customer_safe_alternative_proof.json` generated at `2026-05-23T20:10:00.566592Z`。
...
1. **Dashboard**：看 current-live blocker、breaker release context、4H context、decision quality、feature/source blockers；主阻塞是 `circuit_breaker_active`，support 邊界是 `BLOCK|bear_bias200_hard_block|q15` `0/50 gap=50`。
...
2. **Recent tail root-cause lane**：針對 recent canonical pocket（window `250` win_rate `20.0%`）交付一個 no-new-risk / shadow-only falsification artifact；不可把 shadow-only artifact 誤寫成 release patch。
