...
_最後更新：2026-05-19 04:32:38 CST_
...
最新 `data/live_predict_probe.json`（`2026-05-18T20:10:20.543766Z`，本 PM run wall-clock age 約 `22.3m`）顯示：`target_col=simulated_pyramid_win`、`signal=CIRCUIT_BREAKER`、`should_trade=false`、`deployment_blocker=circuit_breaker_active`、`runtime_closure_state=circuit_breaker_active`、`release_ready=false`。熔斷 release math 仍為 `4/50`：最近 50 筆 `current_recent_window_wins=4/50`、`current_recent_window_win_rate=8.0%`，至少需要 `15/50` 與 `>=30%`，因此 `additional_recent_window_wins_needed=11`；`current_streak=21` 仍低於 `<50` 的連敗上限，但 recent-win-rate gate 未通過。
...
- `data/live_predict_probe.json` generated at `2026-05-18T20:10:20.543766Z`；canonical target `simulated_pyramid_win`。
...
- Release condition: `release_ready=false`; recent-window gate is `4/50` wins, requires `15/50`, so `additional_recent_window_wins_needed=11`; `current_streak=21` still passes the `<50` streak ceiling.
...
- Support progress: `status=semantic_rebaseline_under_minimum`, `current_rows=0`, `minimum_support_rows=50`, `gap_to_minimum=50`, `delta_vs_previous=0`, `stagnant_run_count=2`; legacy `173/50@1250` remains reference-only because semantic identity mismatches current bucket support identity.
...
- `data/high_conviction_topk_oos_matrix.json` generated at `2026-05-18T20:10:28.114537+00:00`; artifact reports `artifact_freshness_status=fresh` and PM wall-clock age is about `22.2m`, under `artifact_stale_after_minutes=60.0`.
- Matrix payload: `samples=24571`, `row_count=24`, `deployable_rows=0`, `risk_qualified_rows=6`, `runtime_blocked_candidate_rows=6`.
...
- `data/execution_metadata_smoke.json` generated at `2026-05-18T20:10:39.026043Z`。
...
- `data/recent_drift_report.json` generated at `2026-05-18T20:10:16.880770+00:00`。
- Target: `simulated_pyramid_win`; blocking window last 100 rows.
- Blocking window truth: `win_rate=26.0%`, wins/losses `26/74`, dominant regime `bear=100.0%`.
- Quality metrics: `avg_simulated_pnl=-0.0054`, `avg_simulated_quality=-0.0524`, `avg_drawdown_penalty=0.2181`, weekend share `100.0%`.
- Feature diagnostics flag low-variance / compressed / sparse inputs, including `feat_4h_ma_order`, `feat_body`, `feat_dxy`, `feat_nq_return_1h`, `feat_vix`, `feat_claw`, `feat_claw_intensity`, and `feat_nest_pred`.
...
6. **Recent drift lane**：針對 last-100 win_rate 26%、bear 100%、losses 74/100、weekend concentration / regime shift / compressed sparse features 的 canonical pathology，下一輪給出一個可測的 blocker root-cause patch 或 falsification test。
