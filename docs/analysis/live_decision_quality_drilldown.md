...
- feature_timestamp: **2026-05-14 21:10:16.405718**
- target: `simulated_pyramid_win`
- live path: **chop / CAUTION / D**
- signal: **HOLD** @ confidence **0.3559**
- layers: **0 → 0**
...
- runtime closure summary: **當前即時分桶 CAUTION|base_caution_regime_or_bias|q35 的精準樣本仍未就緒（28/50，路徑=精準樣本未達最小門檻 / 治理=目前即時分桶精準樣本未達最小門檻）；較寬範圍 / 近似樣本 與建議修補方案 目前都只屬僅供治理參考，不可視為部署閉環。 建議修補方案=core_plus_macro_plus_all_4h (非目前即時範圍，僅供治理參考). 阻塞點=當前即時結構分桶 已有精準筆數，但仍低於 部署級最小精準樣本門檻；在 精準樣本補滿前，執行期只能維持 保護欄，不可把這條路徑視為已可部署。 精準路徑與外溢對照：同品質寬範圍 出現 牛市|阻塞 外溢，400 筆 / 勝率 20.2% / 品質 -0.058，明顯劣於 精準即時路徑 勝率 58.9% / 品質 0.222。**
- q35 scaling audit: overall=`bias50_formula_may_be_too_harsh` / redesign=`base_stack_redesign_discriminative_reweight_crosses_floor_but_execution_blocked` / runtime_gap=`0.0713` / mode=`exact_lane_formula_review` / next_patch=`feat_4h_bias50_formula`
...
- final entry_quality: **0.4787** / trade_floor **0.55** / gap **-0.0713**
- base_quality: **0.4583** × weight **0.75**
- structure_quality: **0.5396** × weight **0.25**
- base components: feat_4h_bias50=0.2892 (w=0.4, contrib=0.1157), feat_nose=0.5145 (w=0.18, contrib=0.0926), feat_pulse=0.4285 (w=0.27, contrib=0.1157), feat_ear=0.8957 (w=0.15, contrib=0.1344)
- structure components: feat_4h_bb_pct_b=0.98 (w=0.34, contrib=0.3332), feat_4h_dist_bb_lower=0.3011 (w=0.33, contrib=0.0994), feat_4h_dist_swing_low=0.3245 (w=0.33, contrib=0.1071)

## Gap attribution（哪個 component 真正在卡 floor）

- remaining_gap_to_floor: **0.0713**
- base_group_max_entry_gain: **0.4061** | structure_group_max_entry_gain: **0.1151**
- best_single_component: **feat_4h_bias50**（group=base, Δscore≈0.2377, max_gain≈0.2132）
- single-component floor crossers: feat_4h_bias50 (Δscore≈0.2377), feat_pulse (Δscore≈0.3521)
- bias50 fully relaxed: entry≈**0.6919** / layers≈**2** / required_bias50_cap≈**-0.2345**
...
|---|---:|---:|---:|---:|---:|---:|---|
| chosen `regime_label+regime_gate+entry_quality_label` | 448 | 0.5893 | 0.222 | 0.1493 | 0.4735 | 28 | False |
| exact `regime_label+regime_gate+entry_quality_label` | 448 | 0.5893 | 0.222 | 0.1493 | 0.4735 | 28 | False |
| narrow `regime_label+entry_quality_label` | 448 | 0.5893 | 0.222 | 0.1493 | 0.4735 | 28 | False |
| broad `regime_gate+entry_quality_label` | 501 | 0.5749 | 0.2123 | 0.1528 | 0.4777 | 28 | False |
