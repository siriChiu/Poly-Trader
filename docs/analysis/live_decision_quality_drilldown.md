...
- feature_timestamp: **2026-05-15 17:12:35.506483**
- target: `simulated_pyramid_win`
- live path: **熊市 / 阻塞 / D**
- signal: **HOLD** @ confidence **0.3117**
- layers: **0 → 0**
- allowed_layers_raw_reason: 市場閘門阻塞
- allowed_layers_reason: 精準樣本未達最小門檻
- execution_guardrail_reason: 精準樣本未達最小門檻
- runtime_blocker: None | reason: None
- deployment_blocker: 精準樣本未達最小門檻 | reason: 當前即時結構分桶 `阻塞｜結構品質阻塞｜q00` 的精準支持樣本仍停在 2/50（缺 48），支持路徑=精準樣本未達最小門檻，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 D / 品質分數 0.2252；目前維持不可部署治理。
...
- runtime closure summary: **當前即時分桶 阻塞｜結構品質阻塞｜q00 的精準樣本仍未就緒（2/50，路徑=精準樣本未達最小門檻 / 治理=目前即時分桶精準樣本未達最小門檻）；較寬範圍 / 近似樣本 目前都只屬僅供治理參考，不可視為部署閉環。 阻塞點=當前即時結構分桶 `阻塞｜結構品質阻塞｜q00` 的精準支持樣本仍停在 2/50（缺 48），支持路徑=精準樣本未達最小門檻，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 D / 品質分數 0.2252；目前維持不可部署治理。 精準路徑與外溢對照：同品質寬範圍出現 盤整｜觀察 外溢，56 筆 / 勝率 45.5% / 品質 0.113，明顯劣於 精準即時路徑 勝率 100.0% / 品質 0.670。**
...
- final entry_quality: **0.5333** / trade_floor **0.55** / gap **-0.0167**
- 基礎品質: **0.6967** × 權重 **0.75**
- 結構品質: **0.043** × 權重 **0.25**
- base components: feat_4h_bias50=0.7793 (w=0.4, contrib=0.3117), feat_nose=0.7246 (w=0.18, contrib=0.1304), feat_pulse=0.4259 (w=0.27, contrib=0.115), feat_ear=0.9306 (w=0.15, contrib=0.1396)
- structure components: feat_4h_bb_pct_b=0.0445 (w=0.34, contrib=0.0151), feat_4h_dist_bb_lower=0.0143 (w=0.33, contrib=0.0047), feat_4h_dist_swing_low=0.0703 (w=0.33, contrib=0.0232)

## Gap attribution（哪個 component 真正在卡 floor）

- remaining_gap_to_floor: **0.0167**
- base_group_max_entry_gain: **0.2275** | structure_group_max_entry_gain: **0.2392**
- best_single_component: **feat_4h_bias50**（group=base, Δscore≈0.0557, max_gain≈0.0662）
- single-component floor crossers: feat_4h_bias50 (Δscore≈0.0557), feat_pulse (Δscore≈0.0825), feat_nose (Δscore≈0.1237), feat_4h_bb_pct_b (Δscore≈0.1965)
...
|---|---:|---:|---:|---:|---:|---:|---|
| chosen `entry_quality_label` | 58 | 0.5862 | 0.2496 | 0.2323 | 0.5451 | 2 | False |
