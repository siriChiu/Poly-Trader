...
- feature_timestamp: **2026-05-21 04:10:09.100536**
- target: `simulated_pyramid_win`
- live path: **盤整 / 觀察 / D**
- signal: **觀望** @ confidence **0.4439**
- layers: **0 → 0**
- allowed_layers_raw_reason: 進場品質低於交易門檻
- allowed_layers_reason: 精準樣本尚未建立
- execution_guardrail_reason: 精準樣本尚未建立
- runtime_blocker: None | reason: None
- deployment_blocker: 精準樣本尚未建立 | reason: 當前即時結構分桶 `觀察｜基線觀察（市場狀態 / 偏離）｜q35` 的精準支持樣本仍停在 0/50（缺 50），支持路徑=精準樣本尚未建立，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 D / 品質分數 0.2708；目前維持不可部署治理。
...
- runtime closure summary: **當前即時分桶 觀察｜基線觀察（市場狀態 / 偏離）｜q35 的精準樣本仍未就緒（0/50，路徑=精準樣本尚未建立 / 治理=精準即時代理路徑僅供治理參考）；較寬範圍 / 近似樣本 目前都只屬僅供治理參考，不可視為部署閉環。 阻塞點=當前即時結構分桶 `觀察｜基線觀察（市場狀態 / 偏離）｜q35` 的精準支持樣本仍停在 0/50（缺 50），支持路徑=精準樣本尚未建立，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 D / 品質分數 0.2708；目前維持不可部署治理。 精準路徑與外溢對照：同 gate 寬範圍出現 熊市｜觀察 外溢，200 筆 / 勝率 63.0% / 品質 0.247，明顯劣於 精準即時路徑 勝率 — / 品質 —。**
- q35 scaling audit: overall=bias50_formula_may_be_too_harsh / redesign=base_stack_redesign_discriminative_reweight_crosses_floor_but_執行_blocked / runtime_gap=0.0324 / mode=exact_lane_formula_review / next_patch=feat_4h_bias50_formula
- q35 runtime truth: redesign_entry_quality=0.55 / redesign_layers_after=0 / runtime_layers=0 / blocker=精準樣本尚未建立 / exact_support=0/50 / support_gap=50
...
- final entry_quality: **0.5176** / trade_floor **0.55** / gap **-0.0324**
- 基礎品質: **0.5614** × 權重 **0.75**
- 結構品質: **0.3861** × 權重 **0.25**
- base components: feat_4h_bias50=0.6042 (w=0.4, contrib=0.2417), feat_nose=0.3285 (w=0.18, contrib=0.0591), feat_pulse=0.423 (w=0.27, contrib=0.1142), feat_ear=0.9759 (w=0.15, contrib=0.1464)
- structure components: feat_4h_bb_pct_b=0.6888 (w=0.34, contrib=0.2342), feat_4h_dist_bb_lower=0.2122 (w=0.33, contrib=0.07), feat_4h_dist_swing_low=0.2481 (w=0.33, contrib=0.0819)

## Gap attribution（哪個 component 真正在卡 floor）

- remaining_gap_to_floor: **0.0324**
- base_group_max_entry_gain: **0.3289** | structure_group_max_entry_gain: **0.1535**
- best_single_component: **feat_4h_bias50**（group=base, Δscore≈0.108, max_gain≈0.1187）
- single-component floor crossers: feat_4h_bias50 (Δscore≈0.108), feat_pulse (Δscore≈0.16), feat_nose (Δscore≈0.24), feat_4h_dist_bb_lower (Δscore≈0.3927)
- bias50 fully relaxed: entry≈**0.6363** / layers≈**1** / required_bias50_cap≈**-1.161**
...
|---|---:|---:|---:|---:|---:|---:|---|
| chosen `global` | 200 | 0.63 | 0.2466 | 0.1393 | 0.4652 | 1 | False |
