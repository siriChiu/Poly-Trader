...
- feature_timestamp: **2026-05-15 12:04:37.909407**
- target: `simulated_pyramid_win`
- live path: **chop / CAUTION / D**
- signal: **HOLD** @ confidence **0.3544**
- layers: **0 → 0**
- allowed_layers_raw_reason: `entry_quality_below_trade_floor`
- allowed_layers_reason: `decision_quality_below_trade_floor`
- execution_guardrail_reason: `decision_quality_below_trade_floor`
- runtime_blocker: `None` | reason: `None`
- deployment_blocker: `decision_quality_below_trade_floor` | reason: `當前即時結構分桶 `CAUTION|base_caution_regime_or_bias|q15` 已完成精準樣本閉環（173/50），但頂層即時基準仍停在進場品質=0.4724，低於交易門檻 0.55；目前只能維持明確不可部署治理，不可把支持樣本閉環或元件實驗就緒誤讀成部署閉環。`
...
- runtime closure summary: **當前即時分桶 CAUTION|base_caution_regime_or_bias|q15 已完成精準樣本閉環（173/50），但頂層即時基準仍停在進場品質=0.4724 (D) < 交易門檻 0.55；目前維持明確不可部署治理。 q15 跨門檻結果只代表可做研究型元件實驗，不是執行放行；元件實驗目前被正向辨別力阻塞（同路徑鄰近分桶表現不劣於當前分桶），不可放寬 allowed_layers / execution guardrail。 下一步=先重設 / 重訓 q15 component，使 current bucket 相對 sibling buckets 保留正向 discrimination，再談 allowed_layers / 執行 保護欄 放行。 精準路徑與外溢對照：同品質寬範圍出現 牛市|阻塞 外溢，378 筆 / 勝率 20.2% / 品質 -0.058，明顯劣於 精準即時路徑 勝率 59.1% / 品質 0.224。**
...
- final entry_quality: **0.4724** / trade_floor **0.55** / gap **-0.0776**
- base_quality: **0.5494** × weight **0.75**
- structure_quality: **0.2413** × weight **0.25**
- base components: feat_4h_bias50=0.4952 (w=0.4, contrib=0.1981), feat_nose=0.6516 (w=0.18, contrib=0.1173), feat_pulse=0.3382 (w=0.27, contrib=0.0913), feat_ear=0.9518 (w=0.15, contrib=0.1428)
- structure components: feat_4h_bb_pct_b=0.3866 (w=0.34, contrib=0.1315), feat_4h_dist_bb_lower=0.1144 (w=0.33, contrib=0.0378), feat_4h_dist_swing_low=0.2186 (w=0.33, contrib=0.0721)

## Gap attribution（哪個 component 真正在卡 floor）

- remaining_gap_to_floor: **0.0776**
- base_group_max_entry_gain: **0.3378** | structure_group_max_entry_gain: **0.1897**
- best_single_component: **feat_4h_bias50**（group=base, Δscore≈0.2587, max_gain≈0.1514）
- single-component floor crossers: feat_4h_bias50 (Δscore≈0.2587), feat_pulse (Δscore≈0.3832)
...
| broad `regime_gate+entry_quality_label` | 514 | 0.5856 | 0.224 | 0.1515 | 0.4734 | 173 | False |
