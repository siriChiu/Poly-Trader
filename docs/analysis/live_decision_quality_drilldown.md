...
- feature_timestamp: **2026-05-18 03:01:44.527400**
- target: `simulated_pyramid_win`
- live path: **熊市 / 觀察 / B**
- signal: **HOLD** @ confidence **0.3393**
- layers: **2 → 0**
...
- deployment_blocker: 精準樣本尚未建立 | reason: 當前即時結構分桶 `觀察｜基線觀察（市場狀態 / 偏離）｜q00` 的精準支持樣本仍停在 0/50（缺 50），支持路徑=精準樣本尚未建立，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 D / 品質分數 0.0903；目前維持不可部署治理。
...
- current-bucket root cause: verdict=no_exact_live_lane_筆 / patch=範圍_generation / feature=feat_4h_bb_pct_b / exact_support=0/50 / gap=50 / neighbor=None
- 精準樣本修補: **未啟用** | 支持路徑 **精準樣本尚未建立** | 跨越門檻 **已跨越門檻但精準樣本未就緒**
- runtime closure summary: **當前即時分桶 觀察｜基線觀察（市場狀態 / 偏離）｜q00 的精準樣本仍未就緒（0/50，路徑=精準樣本尚未建立 / 治理=no_support_proxy）；較寬範圍 / 近似樣本 目前都只屬僅供治理參考，不可視為部署閉環。 阻塞點=當前即時結構分桶 `觀察｜基線觀察（市場狀態 / 偏離）｜q00` 的精準支持樣本仍停在 0/50（缺 50），支持路徑=精準樣本尚未建立，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 D / 品質分數 0.0903；目前維持不可部署治理。 精準路徑與外溢對照：同 gate 寬範圍出現 盤整｜觀察 外溢，108 筆 / 勝率 0.0% / 品質 -0.316，明顯劣於 精準即時路徑 勝率 — / 品質 —。**
...
- final entry_quality: **0.6867** / trade_floor **0.55** / gap **0.1367**
- 基礎品質: **0.9092** × 權重 **0.75**
- 結構品質: **0.019** × 權重 **0.25**
- base components: feat_4h_bias50=1.0 (w=0.4, contrib=0.4), feat_nose=0.9167 (w=0.18, contrib=0.165), feat_pulse=0.76 (w=0.27, contrib=0.2052), feat_ear=0.9268 (w=0.15, contrib=0.139)
- structure components: feat_4h_bb_pct_b=0.0424 (w=0.34, contrib=0.0144), feat_4h_dist_bb_lower=0.0137 (w=0.33, contrib=0.0045), feat_4h_dist_swing_low=0.0 (w=0.33, contrib=0.0)

## Gap attribution（哪個 component 真正在卡 floor）

- remaining_gap_to_floor: **0.0**
- base_group_max_entry_gain: **0.068** | structure_group_max_entry_gain: **0.2453**
...
|---|---:|---:|---:|---:|---:|---:|---|
| chosen `regime_label` | 141 | 0.3759 | 0.0676 | 0.1861 | 0.5858 | 0 | False |
| exact `regime_label+regime_gate+entry_quality_label` | 0 | None | None | None | None | 0 | False |
| narrow `regime_label+entry_quality_label` | 6 | 0.3333 | 0.0424 | 0.1441 | 0.1243 | 0 | False |
