...
- feature_timestamp: **2026-05-21 06:19:05.124886**
- target: `simulated_pyramid_win`
- live path: **盤整 / 觀察 / C**
- signal: **觀望** @ confidence **0.5687**
- layers: **1 → 0**
- allowed_layers_raw_reason: 進場品質_C_single_layer
- allowed_layers_reason: 精準樣本尚未建立
- execution_guardrail_reason: 精準樣本尚未建立
- runtime_blocker: None | reason: None
- deployment_blocker: 精準樣本尚未建立 | reason: 當前即時結構分桶 `觀察｜基線觀察（市場狀態 / 偏離）｜q15` 的精準支持樣本仍停在 0/50（缺 50），支持路徑=所有支持路徑仍不足，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 D / 品質分數 0.3140；目前維持不可部署治理。
...
- current-bucket root cause: verdict=structure_scoring_gap_not_boundary / patch=structure_component_scoring / feature=feat_4h_bb_pct_b / exact_support=0/50 / gap=50 / neighbor=觀察｜基線觀察（市場狀態 / 偏離）｜q65
- 精準樣本修補: **未啟用** | 支持路徑 **精準樣本不足且無可部署支持路徑** | 跨越門檻 **已跨越門檻但精準樣本未就緒**
- runtime closure summary: **當前即時分桶 觀察｜基線觀察（市場狀態 / 偏離）｜q15 的精準樣本仍未就緒（0/50，路徑=精準樣本不足且無可部署支持路徑 / 治理=精準即時代理路徑僅供治理參考）；較寬範圍 / 近似樣本 目前都只屬僅供治理參考，不可視為部署閉環。 阻塞點=當前即時結構分桶 `觀察｜基線觀察（市場狀態 / 偏離）｜q15` 的精準支持樣本仍停在 0/50（缺 50），支持路徑=所有支持路徑仍不足，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 D / 品質分數 0.3140；目前維持不可部署治理。 精準路徑與外溢對照：同品質寬範圍出現 熊市｜觀察 外溢，172 筆 / 勝率 66.3% / 品質 0.277，明顯劣於 精準即時路徑 勝率 100.0% / 品質 0.492。**
...
- final entry_quality: **0.5825** / trade_floor **0.55** / gap **0.0325**
- 基礎品質: **0.6671** × 權重 **0.75**
- 結構品質: **0.3288** × 權重 **0.25**
- base components: feat_4h_bias50=0.6668 (w=0.4, contrib=0.2667), feat_nose=0.5727 (w=0.18, contrib=0.1031), feat_pulse=0.555 (w=0.27, contrib=0.1498), feat_ear=0.9831 (w=0.15, contrib=0.1475)
- structure components: feat_4h_bb_pct_b=0.582 (w=0.34, contrib=0.1979), feat_4h_dist_bb_lower=0.1802 (w=0.33, contrib=0.0595), feat_4h_dist_swing_low=0.2166 (w=0.33, contrib=0.0715)

## Gap attribution（哪個 component 真正在卡 floor）

- remaining_gap_to_floor: **0.0**
- base_group_max_entry_gain: **0.2497** | structure_group_max_entry_gain: **0.1677**
...
|---|---:|---:|---:|---:|---:|---:|---|
| chosen `global` | 200 | 0.685 | 0.2898 | 0.1207 | 0.4254 | 133 | False |
| exact `regime_label+regime_gate+entry_quality_label` | 3 | 1.0 | 0.4917 | 0.031 | 0.1528 | 0 | False |
| narrow `regime_label+entry_quality_label` | 3 | 1.0 | 0.4917 | 0.031 | 0.1528 | 0 | False |
| broad `regime_gate+entry_quality_label` | 175 | 0.6686 | 0.281 | 0.1185 | 0.4186 | 117 | False |
