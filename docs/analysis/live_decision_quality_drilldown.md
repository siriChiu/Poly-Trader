...
- feature_timestamp: **2026-05-26 00:13:08.322874**
- target: `simulated_pyramid_win`
- live path: **盤整 / 觀察 / D**
- signal: **觀望** @ confidence **0.6346**
- layers: **0 → 0**
- allowed_layers_raw_reason: 進場品質低於交易門檻
- allowed_layers_reason: 精準樣本未達最小門檻
- execution_guardrail_reason: 精準樣本未達最小門檻
- runtime_blocker: None | reason: None
- deployment_blocker: 精準樣本未達最小門檻 | reason: 當前即時結構分桶 `觀察｜基線觀察（市場狀態 / 偏離）｜q15` 的精準支持樣本仍停在 7/50（缺 43），支持路徑=精準樣本未達最小門檻，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 C / 品質分數 0.4519；目前維持不可部署治理。
...
- runtime closure summary: **當前即時分桶 觀察｜基線觀察（市場狀態 / 偏離）｜q15 的精準樣本仍未就緒（7/50，路徑=精準樣本未達最小門檻 / 治理=目前即時分桶精準樣本未達最小門檻）；較寬範圍 / 近似樣本 目前都只屬僅供治理參考，不可視為部署閉環。 阻塞點=當前即時結構分桶 `觀察｜基線觀察（市場狀態 / 偏離）｜q15` 的精準支持樣本仍停在 7/50（缺 43），支持路徑=精準樣本未達最小門檻，不可把舊範圍的支持閉環誤讀成部署閉環；決策品質仍為 C / 品質分數 0.4519；目前維持不可部署治理。 精準路徑與外溢對照：同品質寬範圍出現 熊市｜觀察 外溢，17 筆 / 勝率 85.7% / 品質 0.395，明顯劣於 精準即時路徑 勝率 90.7% / 品質 0.404。**
...
- final entry_quality: **0.4583** / trade_floor **0.55** / gap **-0.0917**
- 基礎品質: **0.5322** × 權重 **0.75**
- 結構品質: **0.2367** × 權重 **0.25**
- base components: feat_4h_bias50=0.4008 (w=0.4, contrib=0.1603), feat_nose=0.6495 (w=0.18, contrib=0.1169), feat_pulse=0.4086 (w=0.27, contrib=0.1103), feat_ear=0.9642 (w=0.15, contrib=0.1446)
- structure components: feat_4h_bb_pct_b=0.4453 (w=0.34, contrib=0.1514), feat_4h_dist_bb_lower=0.1129 (w=0.33, contrib=0.0372), feat_4h_dist_swing_low=0.1455 (w=0.33, contrib=0.048)

## Gap attribution（哪個 component 真正在卡 floor）

- remaining_gap_to_floor: **0.0917**
- base_group_max_entry_gain: **0.3509** | structure_group_max_entry_gain: **0.1908**
- best_single_component: **feat_4h_bias50**（group=base, Δscore≈0.3057, max_gain≈0.1798）
- single-component floor crossers: feat_4h_bias50 (Δscore≈0.3057), feat_pulse (Δscore≈0.4528)
...
|---|---:|---:|---:|---:|---:|---:|---|
| chosen `regime_label+regime_gate+entry_quality_label` | 43 | 0.907 | 0.4038 | 0.1046 | 0.3627 | 7 | False |
| exact `regime_label+regime_gate+entry_quality_label` | 43 | 0.907 | 0.4038 | 0.1046 | 0.3627 | 7 | False |
| narrow `regime_label+entry_quality_label` | 43 | 0.907 | 0.4038 | 0.1046 | 0.3627 | 7 | False |
| broad `regime_gate+entry_quality_label` | 50 | 0.9 | 0.4026 | 0.0991 | 0.3285 | 7 | False |
