...
- feature_timestamp: **2026-05-18 13:02:30.802828**
...
- support blocker summary: **精準樣本 44/50（缺口 6） 未達目前即時精準樣本門檻；較寬範圍或近似樣本只可作治理參考。 語義重訂後仍未達門檻；舊版 #20260419b 53/50僅能當歷史參考，因進場品質、市場狀態不吻合目前支持語義，不可宣稱同一語義已閉環。**
...
- runtime closure summary: **風控熔斷啟用中：最近 50 筆勝率: 20.00% < 30%；解除條件：連續虧損筆數 < 50 且最近 50 筆勝率 >= 30%；目前最近 50 筆只贏 10/50，至少還差 5 勝。 精準路徑與外溢對照：同 gate 寬範圍出現 盤整｜觀察 外溢，68 筆 / 勝率 0.0% / 品質 -0.316，明顯劣於 精準即時路徑 勝率 22.7% / 品質 -0.107。**
...
- 建議修補特徵: None
- 建議修補說明: 精準樣本 44/50（缺口 6） 未達目前即時精準樣本門檻；較寬範圍或近似樣本只可作治理參考。 語義重訂後仍未達門檻；舊版 #20260419b 53/50僅能當歷史參考，因進場品質、市場狀態不吻合目前支持語義，不可宣稱同一語義已閉環。
- 下一步: 保持禁止部署；先累積或回放同一目前即時結構分桶的精準路徑樣本，不可用較寬範圍或近似樣本放行。 先以目前支持語義累積或回放精準樣本；舊版參考不可作為放行依據。

## Entry-quality component breakdown

- final entry_quality: **0.6124** / trade_floor **0.55** / gap **0.0624**
- 基礎品質: **0.7373** × 權重 **0.75**
- 結構品質: **0.2376** × 權重 **0.25**
- base components: feat_4h_bias50=1.0 (w=0.4, contrib=0.4), feat_nose=0.2647 (w=0.18, contrib=0.0476), feat_pulse=0.5304 (w=0.27, contrib=0.1432), feat_ear=0.9764 (w=0.15, contrib=0.1465)
- structure components: feat_4h_bb_pct_b=0.5338 (w=0.34, contrib=0.1815), feat_4h_dist_bb_lower=0.17 (w=0.33, contrib=0.0561), feat_4h_dist_swing_low=0.0 (w=0.33, contrib=0.0)

## Gap attribution（哪個 component 真正在卡 floor）

- remaining_gap_to_floor: **0.0**
- base_group_max_entry_gain: **0.1971** | structure_group_max_entry_gain: **0.1906**
...
|---|---:|---:|---:|---:|---:|---:|---|
| chosen `regime_label+regime_gate+entry_quality_label` | 44 | 0.2273 | -0.107 | 0.2451 | 0.6466 | 44 | False |
| exact `regime_label+regime_gate+entry_quality_label` | 44 | 0.2273 | -0.107 | 0.2451 | 0.6466 | 44 | False |
| narrow `regime_label+entry_quality_label` | 102 | 0.2549 | -0.0702 | 0.2095 | 0.6339 | 44 | False |
| broad `regime_gate+entry_quality_label` | 48 | 0.2083 | -0.1215 | 0.2503 | 0.6695 | 44 | False |
