...
- legacy_entry_quality: **0.3606** (raw_reason=`regime_gate_block`)
- calibration_runtime_entry_quality: **0.3606** (raw_reason=`regime_gate_block`)
- deployed_runtime_entry_quality: **0.3606** (raw_reason=`regime_gate_block`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **2.626**
- structure_quality: **0.4162**
...
- current bias50 percentile in exact lane: **0.5652**
...
- same_gate_same_quality: rows=**750** | win_rate=**0.532** | bias50_pct=**0.544** | dist={'min': -18.7285, 'p25': -0.0736, 'p50': 2.0406, 'p75': 5.6979, 'p90': 7.1403, 'p95': 8.8165, 'max': 17.0434, 'mean': 2.827}
- same_bucket: rows=**195** | win_rate=**0.6615** | bias50_pct=**0.5897** | dist={'min': -0.7025, 'p25': 1.0207, 'p50': 1.7335, 'p75': 3.9909, 'p90': 4.8559, 'p95': 5.5826, 'max': 6.3969, 'mean': 2.4738}
- bull_all: rows=**10680** | win_rate=**0.5912** | bias50_pct=**0.6603** | dist={'min': -20.6429, 'p25': -1.4147, 'p50': 1.2585, 'p75': 3.4908, 'p90': 5.1664, 'p95': 6.0647, 'max': 17.0434, 'mean': 0.9628}
...
- exact lane band: **warm** (pct=0.5652, Δp90=-2.3236)
- same_gate_same_quality band: **warm** (pct=0.544, Δp90=-4.5143)
- same_bucket band: **warm** (pct=0.5897, Δp90=-2.2299)
- bull_all band: **warm** (pct=0.6603, Δp90=-2.5404)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.544
...
- baseline -> calibration runtime entry_quality: **0.3606 → 0.3606** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.3606 → 0.3606** (Δ=**0.0**)
...
- runtime gap to floor: **0.1894**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.3606**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6606**, layers **0**
- required bias50 cap to cross trade floor: **-0.7565** (current=2.626)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4065** / layers **0** / gap **0.1435**
- required_bias50_cap_after_best_scenario: **0.0085**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.4596** / layers **0** / gap **0.0904** / target_score **0.33**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.4818** / layers **0** / gap **0.0682**
- required_bias50_cap_after_best_scenario: **-0.1265**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.65, 'feat_pulse': 0.0, 'feat_ear': 0.35}** → entry_quality **0.5502** / gap **0.0** / mean_gap **0.004**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8445** / gap **0.0** / mean_gap **-0.0269**
