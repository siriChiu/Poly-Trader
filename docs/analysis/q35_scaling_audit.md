...
- legacy_entry_quality: **0.4323** (raw_reason=`regime_gate_block`)
- calibration_runtime_entry_quality: **0.4323** (raw_reason=`regime_gate_block`)
- deployed_runtime_entry_quality: **0.4323** (raw_reason=`regime_gate_block`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **2.2865**
...
- same_gate_same_quality: rows=**750** | win_rate=**0.532** | bias50_pct=**0.5173** | dist={'min': -18.7285, 'p25': -0.0736, 'p50': 2.0406, 'p75': 5.6979, 'p90': 7.1403, 'p95': 8.8165, 'max': 17.0434, 'mean': 2.827}
...
- bull_all: rows=**10799** | win_rate=**0.5939** | bias50_pct=**0.6091** | dist={'min': -20.6429, 'p25': -1.3878, 'p50': 1.329, 'p75': 3.483, 'p90': 5.1385, 'p95': 6.0419, 'max': 17.0434, 'mean': 0.9861}
...
- exact lane band: **warm** (pct=0.5489, Δp90=-2.6631)
- same_gate_same_quality band: **warm** (pct=0.5173, Δp90=-4.8538)
- same_bucket band: **warm** (pct=0.5744, Δp90=-2.5694)
- bull_all band: **warm** (pct=0.6091, Δp90=-2.852)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.5173
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.022699999999999942** | legacy=**0.022699999999999942** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4323 → 0.4323** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4323 → 0.4323** (Δ=**0.0**)
...
- runtime gap to floor: **0.1177**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4323**, layers **0**
- fully relax bias50 penalty: entry_quality **0.7254**, layers **0**
- required bias50 cap to cross trade floor: **0.325** (current=2.2865)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4808** / layers **0** / gap **0.0692**
- required_bias50_cap_after_best_scenario: **1.133**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5244** / layers **0** / gap **0.0256** / target_score **0.33**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.5088** / layers **0** / gap **0.0412**
- required_bias50_cap_after_best_scenario: **0.3235**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.95, 'feat_pulse': 0.0, 'feat_ear': 0.05}** → entry_quality **0.5553** / gap **0.0** / mean_gap **0.0183**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8325** / gap **0.0** / mean_gap **-0.0269**
