...
- legacy_entry_quality: **0.5651** (raw_reason=`entry_quality_C_single_layer`)
- calibration_runtime_entry_quality: **0.5651** (raw_reason=`entry_quality_C_single_layer`)
- deployed_runtime_entry_quality: **0.5651** (raw_reason=`entry_quality_C_single_layer`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **-0.38**
- structure_quality: **0.4482**
...
- same_gate_same_quality: rows=**1318** | win_rate=**0.8278** | bias50_pct=**0.3376** | dist={'min': -6.6008, 'p25': -0.6553, 'p50': 0.4441, 'p75': 1.458, 'p90': 4.4801, 'p95': 4.7505, 'max': 7.5411, 'mean': 0.7409}
- same_bucket: rows=**1562** | win_rate=**0.7875** | bias50_pct=**0.0781** | dist={'min': -5.5588, 'p25': 0.4479, 'p50': 0.9529, 'p75': 1.2659, 'p90': 1.533, 'p95': 1.7352, 'max': 4.4265, 'mean': 0.7255}
- bull_all: rows=**4579** | win_rate=**0.7759** | bias50_pct=**0.18** | dist={'min': -6.9187, 'p25': 0.073, 'p50': 1.0869, 'p75': 3.0699, 'p90': 4.0978, 'p95': 4.6734, 'max': 8.0151, 'mean': 1.3662}
...
- exact lane band: **core_normal** (pct=0.1235, Δp90=-1.6016)
- same_gate_same_quality band: **core_normal** (pct=0.3376, Δp90=-4.8601)
- same_bucket band: **core_normal** (pct=0.0781, Δp90=-1.913)
- bull_all band: **core_normal** (pct=0.18, Δp90=-4.4778)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.3376
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.5559999999999999** | legacy=**0.5559999999999999** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_crosses_floor_but_execution_blocked**
- baseline -> calibration runtime entry_quality: **0.5651 → 0.5651** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.5651 → 0.5651** (Δ=**0.0**)
...
- runtime gap to floor: **-0.0151**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.5651**, layers **1**
- fully relax bias50 penalty: entry_quality **0.6983**, layers **2**
- required bias50 cap to cross trade floor: **-0.38** (current=-0.38)
...
- best scenario: **winner_p75** → entry_quality **0.5738** / layers **1** / gap **0.0**
- required_bias50_cap_after_best_scenario: **-0.38**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5839** / layers **1** / gap **0.0** / target_score **0.6186**
...
- machine_read: entry_quality>=0.55=**True** | allowed_layers>0=**True**
- best scenario: **winner_triplet_p75** → entry_quality **0.6416** / layers **1** / gap **0.0**
- required_bias50_cap_after_best_scenario: **-0.38**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.8, 'feat_pulse': 0.0, 'feat_ear': 0.2}** → entry_quality **0.5513** / gap **0.0** / mean_gap **0.0872**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8599** / gap **0.0** / mean_gap **0.0063**
