...
- legacy_entry_quality: **0.3113** (raw_reason=`regime_gate_block`)
- calibration_runtime_entry_quality: **0.3113** (raw_reason=`regime_gate_block`)
- deployed_runtime_entry_quality: **0.3113** (raw_reason=`regime_gate_block`, effective_reason=`under_minimum_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **2.5203**
...
- current bias50 percentile in exact lane: **0.5397**
...
- same_gate_same_quality: rows=**755** | win_rate=**0.5285** | bias50_pct=**0.5325** | dist={'min': -18.7285, 'p25': -0.0714, 'p50': 2.1066, 'p75': 5.6935, 'p90': 7.1403, 'p95': 8.7957, 'max': 17.0434, 'mean': 2.8276}
- same_bucket: rows=**200** | win_rate=**0.645** | bias50_pct=**0.565** | dist={'min': -0.7025, 'p25': 1.0443, 'p50': 1.7725, 'p75': 3.9633, 'p90': 4.8462, 'p95': 5.5826, 'max': 6.3969, 'mean': 2.4847}
- bull_all: rows=**10840** | win_rate=**0.5935** | bias50_pct=**0.6384** | dist={'min': -20.6429, 'p25': -1.3733, 'p50': 1.3427, 'p75': 3.4791, 'p90': 5.133, 'p95': 6.0396, 'max': 17.0434, 'mean': 0.9914}
...
- exact lane band: **warm** (pct=0.5397, Δp90=-2.3356)
- same_gate_same_quality band: **warm** (pct=0.5325, Δp90=-4.62)
- same_bucket band: **warm** (pct=0.565, Δp90=-2.3259)
- bull_all band: **warm** (pct=0.6384, Δp90=-2.6127)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.5325
...
- baseline -> calibration runtime entry_quality: **0.3113 → 0.3113** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.3113 → 0.3113** (Δ=**0.0**)
...
- runtime gap to floor: **0.2387**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.3113**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6113**, layers **0**
- required bias50 cap to cross trade floor: **-1.5785** (current=2.5203)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.3787** / layers **0** / gap **0.1713**
- required_bias50_cap_after_best_scenario: **-0.455**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.4101** / layers **0** / gap **0.1399** / target_score **0.3292**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.4659** / layers **0** / gap **0.0841**
- required_bias50_cap_after_best_scenario: **-0.3915**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False** | positive_gap=**True** | execution_blocked_after_floor_cross=**False**
- rows / wins / losses: **189 / 120 / 69**
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 1.0, 'feat_pulse': 0.0, 'feat_ear': 0.0}** → entry_quality **0.2546** / gap **0.2954** / mean_gap **0.0303**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8124** / gap **0.0** / mean_gap **-0.0278**
