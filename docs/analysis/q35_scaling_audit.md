...
- legacy_entry_quality: **0.345** (raw_reason=`regime_gate_block`)
- calibration_runtime_entry_quality: **0.345** (raw_reason=`regime_gate_block`)
- deployed_runtime_entry_quality: **0.345** (raw_reason=`regime_gate_block`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **2.7422**
- structure_quality: **0.434**
...
- same_gate_same_quality: rows=**750** | win_rate=**0.532** | bias50_pct=**0.5493** | dist={'min': -18.7285, 'p25': -0.0736, 'p50': 2.0406, 'p75': 5.6979, 'p90': 7.1403, 'p95': 8.8165, 'max': 17.0434, 'mean': 2.827}
...
- bull_all: rows=**10662** | win_rate=**0.5921** | bias50_pct=**0.6716** | dist={'min': -20.6429, 'p25': -1.4208, 'p50': 1.2449, 'p75': 3.4839, 'p90': 5.1701, 'p95': 6.0647, 'max': 17.0434, 'mean': 0.9581}
...
- exact lane band: **warm** (pct=0.5707, Δp90=-2.2074)
- same_gate_same_quality band: **warm** (pct=0.5493, Δp90=-4.3981)
- same_bucket band: **warm** (pct=0.5949, Δp90=-2.1137)
- bull_all band: **warm** (pct=0.6716, Δp90=-2.4279)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.5493
...
- baseline -> calibration runtime entry_quality: **0.345 → 0.345** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.345 → 0.345** (Δ=**0.0**)
...
- runtime gap to floor: **0.205**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.345**, layers **0**
- fully relax bias50 penalty: entry_quality **0.645**, layers **0**
- required bias50 cap to cross trade floor: **-1.0165** (current=2.7422)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.39** / layers **0** / gap **0.16**
- required_bias50_cap_after_best_scenario: **-0.2665**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.444** / layers **0** / gap **0.106** / target_score **0.33**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.4858** / layers **0** / gap **0.0642**
- required_bias50_cap_after_best_scenario: **-0.06**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.6, 'feat_pulse': 0.0, 'feat_ear': 0.4}** → entry_quality **0.5626** / gap **0.0** / mean_gap **0.0016**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8459** / gap **0.0** / mean_gap **-0.0269**
