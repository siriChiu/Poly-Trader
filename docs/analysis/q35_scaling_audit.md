...
- generated_at: **2026-04-24 02:20:23.220632**
...
- legacy_entry_quality: **0.4168** (raw_reason=`regime_gate_block`)
- calibration_runtime_entry_quality: **0.4168** (raw_reason=`regime_gate_block`)
- deployed_runtime_entry_quality: **0.4168** (raw_reason=`regime_gate_block`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **2.7092**
- structure_quality: **0.4289**
...
- current bias50 percentile in exact lane: **0.5707**
...
- same_gate_same_quality: rows=**750** | win_rate=**0.532** | bias50_pct=**0.548** | dist={'min': -18.7285, 'p25': -0.0736, 'p50': 2.0406, 'p75': 5.6979, 'p90': 7.1403, 'p95': 8.8165, 'max': 17.0434, 'mean': 2.827}
- same_bucket: rows=**195** | win_rate=**0.6615** | bias50_pct=**0.5949** | dist={'min': -0.7025, 'p25': 1.0207, 'p50': 1.7335, 'p75': 3.9909, 'p90': 4.8559, 'p95': 5.5826, 'max': 6.3969, 'mean': 2.4738}
- bull_all: rows=**10751** | win_rate=**0.5921** | bias50_pct=**0.664** | dist={'min': -20.6429, 'p25': -1.3996, 'p50': 1.3135, 'p75': 3.4861, 'p90': 5.1548, 'p95': 6.0578, 'max': 17.0434, 'mean': 0.9779}
...
- exact lane band: **warm** (pct=0.5707, Δp90=-2.2404)
- same_gate_same_quality band: **warm** (pct=0.548, Δp90=-4.4311)
- same_bucket band: **warm** (pct=0.5949, Δp90=-2.1467)
- bull_all band: **warm** (pct=0.664, Δp90=-2.4456)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.548
...
- baseline -> calibration runtime entry_quality: **0.4168 → 0.4168** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4168 → 0.4168** (Δ=**0.0**)
...
- runtime gap to floor: **0.1332**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4168**, layers **0**
- fully relax bias50 penalty: entry_quality **0.7168**, layers **0**
- required bias50 cap to cross trade floor: **0.18** (current=2.7092)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4621** / layers **0** / gap **0.0879**
- required_bias50_cap_after_best_scenario: **0.935**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5158** / layers **0** / gap **0.0342** / target_score **0.33**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.5235** / layers **0** / gap **0.0265**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.65, 'feat_pulse': 0.0, 'feat_ear': 0.35}** → entry_quality **0.563** / gap **0.0** / mean_gap **0.004**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8456** / gap **0.0** / mean_gap **-0.0269**
