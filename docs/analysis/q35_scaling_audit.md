...
- legacy_entry_quality: **0.4839** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4839** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4839** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`decision_quality_below_trade_floor; unsupported_exact_live_structure_bucket_blocks_trade; unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **1.0293**
...
- current bias50 percentile in exact lane: **0.35**
...
- same_gate_same_quality: rows=**2371** | win_rate=**0.8026** | bias50_pct=**0.3163** | dist={'min': -1.1321, 'p25': 0.8138, 'p50': 1.6294, 'p75': 3.0648, 'p90': 4.1029, 'p95': 4.7632, 'max': 8.0151, 'mean': 1.9544}
- same_bucket: rows=**1393** | win_rate=**0.8378** | bias50_pct=**0.5025** | dist={'min': -4.1777, 'p25': 0.5372, 'p50': 1.0258, 'p75': 1.4536, 'p90': 1.7595, 'p95': 1.9746, 'max': 5.3808, 'mean': 0.9852}
- bull_all: rows=**3356** | win_rate=**0.8349** | bias50_pct=**0.4032** | dist={'min': -4.1777, 'p25': 0.5521, 'p50': 1.3679, 'p75': 3.0118, 'p90': 4.2724, 'p95': 4.7818, 'max': 8.0151, 'mean': 1.7274}
...
- exact lane band: **core_normal** (pct=0.35, Δp90=-0.8539)
- same_gate_same_quality band: **core_normal** (pct=0.3163, Δp90=-3.0736)
- same_bucket band: **warm** (pct=0.5025, Δp90=-0.7302)
- bull_all band: **core_normal** (pct=0.4032, Δp90=-3.2431)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.3163
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.27413999999999994** | legacy=**0.27413999999999994** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4839 → 0.4839** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4839 → 0.4839** (Δ=**0.0**)
...
- runtime gap to floor: **0.0661**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4839**, layers **0**
- fully relax bias50 penalty: entry_quality **0.7017**, layers **2**
- required bias50 cap to cross trade floor: **-0.072** (current=1.0293)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_p75** → entry_quality **0.5009** / layers **0** / gap **0.0491**
- required_bias50_cap_after_best_scenario: **0.211**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5188** / layers **0** / gap **0.0312** / target_score **0.3904**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.5216** / layers **0** / gap **0.0284**
- required_bias50_cap_after_best_scenario: **0.182**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.5, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 0.5}** → entry_quality **0.5605** / gap **0.0** / mean_gap **0.0706**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8208** / gap **0.0** / mean_gap **-0.0042**
