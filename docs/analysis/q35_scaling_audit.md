...
- generated_at: **2026-05-25 17:29:53.676265**
...
- legacy_entry_quality: **0.4692** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4692** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4692** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`under_minimum_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **0.888**
- structure_quality: **0.3699**

## Exact lane summary

- rows: **983** | win_rate: **0.7416**
- bias50 distribution: {'min': -1.2312, 'p25': 0.6536, 'p50': 1.1074, 'p75': 1.3344, 'p90': 1.6422, 'p95': 1.8121, 'max': 4.4265, 'mean': 1.0256}
- current bias50 percentile in exact lane: **0.3184**
- winner-only bias50 distribution: {'min': -1.1316, 'p25': 0.6047, 'p50': 1.0455, 'p75': 1.323, 'p90': 1.5893, 'p95': 1.7747, 'max': 3.9157, 'mean': 0.9806}

## Broader bull cohorts

- same_gate_same_quality: rows=**3240** | win_rate=**0.7611** | bias50_pct=**0.3488** | dist={'min': -1.9, 'p25': 0.3006, 'p50': 1.2829, 'p75': 3.1597, 'p90': 4.0323, 'p95': 4.5274, 'max': 8.0151, 'mean': 1.6734}
- same_bucket: rows=**1632** | win_rate=**0.7923** | bias50_pct=**0.4828** | dist={'min': -5.5588, 'p25': 0.3292, 'p50': 0.9241, 'p75': 1.2531, 'p90': 1.5225, 'p95': 1.7174, 'max': 4.4265, 'mean': 0.6805}
- bull_all: rows=**4653** | win_rate=**0.778** | bias50_pct=**0.4417** | dist={'min': -6.9187, 'p25': 0.051, 'p50': 1.0717, 'p75': 3.0562, 'p90': 4.0848, 'p95': 4.6616, 'max': 8.0151, 'mean': 1.3391}
...
- exact lane band: **core_normal** (pct=0.3184, Δp90=-0.7542)
- same_gate_same_quality band: **core_normal** (pct=0.3488, Δp90=-3.1443)
- same_bucket band: **core_normal** (pct=0.4828, Δp90=-0.6345)
- bull_all band: **core_normal** (pct=0.4417, Δp90=-3.1968)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.3488
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.3024** | legacy=**0.3024** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4692 → 0.4692** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4692 → 0.4692** (Δ=**0.0**)
...
- runtime gap to floor: **0.0808**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4692**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6785**, layers **1**
- required bias50 cap to cross trade floor: **-0.4585** (current=0.888)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4931** / layers **0** / gap **0.0569**
- required_bias50_cap_after_best_scenario: **-0.0605**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5152** / layers **0** / gap **0.0348** / target_score **0.4556**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.5217** / layers **0** / gap **0.0283**
- required_bias50_cap_after_best_scenario: **0.133**
...
- rows / wins / losses: **983 / 729 / 254**
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.85, 'feat_ear': 0.15}** → entry_quality **0.5629** / gap **0.0** / mean_gap **0.051**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.833** / gap **0.0** / mean_gap **0.0183**
