...
- generated_at: **2026-05-24 12:29:21.452681**
...
- legacy_entry_quality: **0.5069** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.5069** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.5069** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`under_minimum_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **-0.0328**
- structure_quality: **0.4502**
...
- current bias50 percentile in exact lane: **0.0532**
...
- same_gate_same_quality: rows=**3215** | win_rate=**0.7602** | bias50_pct=**0.1381** | dist={'min': -1.9, 'p25': 0.3512, 'p50': 1.2888, 'p75': 3.1735, 'p90': 4.0393, 'p95': 4.5297, 'max': 8.0151, 'mean': 1.6883}
- same_bucket: rows=**1562** | win_rate=**0.7875** | bias50_pct=**0.1338** | dist={'min': -5.5588, 'p25': 0.4479, 'p50': 0.9529, 'p75': 1.2659, 'p90': 1.533, 'p95': 1.7352, 'max': 4.4265, 'mean': 0.7255}
- bull_all: rows=**4579** | win_rate=**0.7759** | bias50_pct=**0.2182** | dist={'min': -6.9187, 'p25': 0.073, 'p50': 1.0869, 'p75': 3.0699, 'p90': 4.0978, 'p95': 4.6734, 'max': 8.0151, 'mean': 1.3662}
...
- exact lane band: **core_normal** (pct=0.0532, Δp90=-1.6835)
- same_gate_same_quality band: **core_normal** (pct=0.1381, Δp90=-4.0721)
- same_bucket band: **core_normal** (pct=0.1338, Δp90=-1.5658)
- bull_all band: **core_normal** (pct=0.2182, Δp90=-4.1306)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.1381
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.48656** | legacy=**0.48656** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.5069 → 0.5069** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.5069 → 0.5069** (Δ=**0.0**)
...
- runtime gap to floor: **0.0431**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.5069**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6609**, layers **1**
- required bias50 cap to cross trade floor: **-0.7515** (current=-0.0328)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.517** / layers **0** / gap **0.033**
- required_bias50_cap_after_best_scenario: **-0.583**
...
- best scenario: **exact_lane_triplet_p75** → entry_quality **0.5828** / layers **1** / gap **0.0**
- required_bias50_cap_after_best_scenario: **-0.033**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.7, 'feat_ear': 0.3}** → entry_quality **0.5533** / gap **0.0** / mean_gap **0.0496**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8491** / gap **0.0** / mean_gap **0.0174**
