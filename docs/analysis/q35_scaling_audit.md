...
- legacy_entry_quality: **0.4838** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4838** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4838** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`decision_quality_below_trade_floor`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **1.1535**
- structure_quality: **0.5313**
...
- current bias50 percentile in exact lane: **0.5232**
...
- same_gate_same_quality: rows=**3082** | win_rate=**0.7914** | bias50_pct=**0.4254** | dist={'min': -1.7439, 'p25': 0.5007, 'p50': 1.3159, 'p75': 3.2171, 'p90': 4.0736, 'p95': 4.5615, 'max': 8.0151, 'mean': 1.7705}
- same_bucket: rows=**1439** | win_rate=**0.8402** | bias50_pct=**0.6484** | dist={'min': -5.5588, 'p25': 0.5106, 'p50': 0.9938, 'p75': 1.2846, 'p90': 1.552, 'p95': 1.7469, 'max': 4.4265, 'mean': 0.8288}
- bull_all: rows=**4283** | win_rate=**0.8158** | bias50_pct=**0.4964** | dist={'min': -6.9187, 'p25': 0.1795, 'p50': 1.1607, 'p75': 3.1532, 'p90': 4.1576, 'p95': 4.7078, 'max': 8.0151, 'mean': 1.5111}
...
- exact lane band: **warm** (pct=0.5232, Δp90=-0.5343)
- same_gate_same_quality band: **core_normal** (pct=0.4254, Δp90=-2.9201)
- same_bucket band: **warm** (pct=0.6484, Δp90=-0.3985)
- bull_all band: **core_normal** (pct=0.4964, Δp90=-3.0041)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.4254
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.2493** | legacy=**0.2493** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4838 → 0.4838** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4838 → 0.4838** (Δ=**0.0**)
...
- runtime gap to floor: **0.0662**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4838**, layers **0**
- fully relax bias50 penalty: entry_quality **0.709**, layers **2**
- required bias50 cap to cross trade floor: **0.05** (current=1.1535)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.496** / layers **0** / gap **0.054**
- required_bias50_cap_after_best_scenario: **0.2535**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5359** / layers **0** / gap **0.0141** / target_score **0.4233**
...
- best scenario: **winner_triplet_p75** → entry_quality **0.5606** / layers **1** / gap **0.0**
- required_bias50_cap_after_best_scenario: **0.6445**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.55, 'feat_nose': 0.0, 'feat_pulse': 0.05, 'feat_ear': 0.4}** → entry_quality **0.5525** / gap **0.0** / mean_gap **0.0378**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8771** / gap **0.0** / mean_gap **0.0101**
