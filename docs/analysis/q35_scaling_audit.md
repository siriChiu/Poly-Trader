...
- legacy_entry_quality: **0.4857** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4857** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4857** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`under_minimum_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **1.2434**
- structure_quality: **0.4111**

## Exact lane summary

- rows: **879** | win_rate: **0.7782**
- bias50 distribution: {'min': -0.4035, 'p25': 0.7955, 'p50': 1.147, 'p75': 1.3629, 'p90': 1.6898, 'p95': 1.8374, 'max': 4.4265, 'mean': 1.1031}
- current bias50 percentile in exact lane: **0.5961**
- winner-only bias50 distribution: {'min': -0.4035, 'p25': 0.6355, 'p50': 1.0729, 'p75': 1.3382, 'p90': 1.6072, 'p95': 1.7763, 'max': 3.9157, 'mean': 1.027}

## Broader bull cohorts

- same_gate_same_quality: rows=**2892** | win_rate=**0.805** | bias50_pct=**0.4243** | dist={'min': -1.7439, 'p25': 0.6981, 'p50': 1.4292, 'p75': 3.2761, 'p90': 4.0983, 'p95': 4.6026, 'max': 8.0151, 'mean': 1.9556}
- same_bucket: rows=**1413** | win_rate=**0.8401** | bias50_pct=**0.7098** | dist={'min': -5.5588, 'p25': 0.506, 'p50': 0.9963, 'p75': 1.2888, 'p90': 1.5575, 'p95': 1.7536, 'max': 4.4265, 'mean': 0.8264}
- bull_all: rows=**4042** | win_rate=**0.829** | bias50_pct=**0.498** | dist={'min': -6.9187, 'p25': 0.359, 'p50': 1.2527, 'p75': 3.2235, 'p90': 4.2186, 'p95': 4.7277, 'max': 8.0151, 'mean': 1.6673}
...
- exact lane band: **warm** (pct=0.5961, Δp90=-0.4464)
- same_gate_same_quality band: **core_normal** (pct=0.4243, Δp90=-2.8549)
- same_bucket band: **warm** (pct=0.7098, Δp90=-0.3141)
- bull_all band: **core_normal** (pct=0.498, Δp90=-2.9752)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.4243
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.23131999999999997** | legacy=**0.23131999999999997** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4857 → 0.4857** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4857 → 0.4857** (Δ=**0.0**)
...
- runtime gap to floor: **0.0643**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4857**, layers **0**
- fully relax bias50 penalty: entry_quality **0.7163**, layers **2**
- required bias50 cap to cross trade floor: **0.172** (current=1.2434)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4914** / layers **0** / gap **0.0586**
- required_bias50_cap_after_best_scenario: **0.267**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5434** / layers **0** / gap **0.0066** / target_score **0.4237**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.5311** / layers **0** / gap **0.0189**
- required_bias50_cap_after_best_scenario: **0.3205**
...
- rows / wins / losses: **879 / 684 / 195**
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.15, 'feat_nose': 0.0, 'feat_pulse': 0.85, 'feat_ear': 0.0}** → entry_quality **0.5553** / gap **0.0** / mean_gap **0.0427**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8388** / gap **0.0** / mean_gap **0.0093**
