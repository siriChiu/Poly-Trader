...
- generated_at: **2026-04-20 16:32:04.212628**
...
- legacy_entry_quality: **0.4249** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4249** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4249** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`decision_quality_below_trade_floor; unsupported_exact_live_structure_bucket_blocks_trade; unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **1.1379**
- structure_quality: **0.3869**
...
- current bias50 percentile in exact lane: **0.4534**
...
- same_gate_same_quality: rows=**2381** | win_rate=**0.8034** | bias50_pct=**0.3549** | dist={'min': -1.1321, 'p25': 0.8196, 'p50': 1.6294, 'p75': 3.0619, 'p90': 4.1006, 'p95': 4.7595, 'max': 8.0151, 'mean': 1.9528}
- same_bucket: rows=**1393** | win_rate=**0.8378** | bias50_pct=**0.6016** | dist={'min': -4.1777, 'p25': 0.5372, 'p50': 1.0258, 'p75': 1.4536, 'p90': 1.7595, 'p95': 1.9746, 'max': 5.3808, 'mean': 0.9852}
- bull_all: rows=**3366** | win_rate=**0.8354** | bias50_pct=**0.4447** | dist={'min': -4.1777, 'p25': 0.5543, 'p50': 1.3719, 'p75': 3.0074, 'p90': 4.2699, 'p95': 4.7818, 'max': 8.0151, 'mean': 1.727}
...
- exact lane band: **core_normal** (pct=0.4534, Δp90=-0.7453)
- same_gate_same_quality band: **core_normal** (pct=0.3549, Δp90=-2.9627)
- same_bucket band: **warm** (pct=0.6016, Δp90=-0.6216)
- bull_all band: **core_normal** (pct=0.4447, Δp90=-3.132)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.3549
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.25242** | legacy=**0.25242** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4249 → 0.4249** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4249 → 0.4249** (Δ=**0.0**)
...
- runtime gap to floor: **0.1251**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4249**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6492**, layers **1**
- required bias50 cap to cross trade floor: **-0.947** (current=1.1379)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_p75** → entry_quality **0.4402** / layers **0** / gap **0.1098**
- required_bias50_cap_after_best_scenario: **-0.692**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.4663** / layers **0** / gap **0.0837** / target_score **0.3904**
...
- required_bias50_cap_after_best_scenario: **0.2305**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.5, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 0.5}** → entry_quality **0.5642** / gap **0.0** / mean_gap **0.0706**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8423** / gap **0.0** / mean_gap **-0.0042**
