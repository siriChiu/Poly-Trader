...
- legacy_entry_quality: **0.4977** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4977** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.6636** (raw_reason=`entry_quality_C_single_layer`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **True**
- feat_4h_bias50: **1.844**
- structure_quality: **0.4559**
...
- current bias50 percentile in exact lane: **0.0014**
...
- same_gate_same_quality: rows=**9497** | win_rate=**0.6137** | bias50_pct=**0.0016** | dist={'min': -1.1489, 'p25': 2.604, 'p50': 2.7938, 'p75': 3.3017, 'p90': 3.354, 'p95': 3.38, 'max': 4.9835, 'mean': 2.8941}
- same_bucket: rows=**4195** | win_rate=**0.6133** | bias50_pct=**0.0014** | dist={'min': 1.6545, 'p25': 3.2675, 'p50': 3.3114, 'p75': 3.3474, 'p90': 3.3848, 'p95': 3.4559, 'max': 4.9835, 'mean': 3.2802}
- bull_all: rows=**10065** | win_rate=**0.5956** | bias50_pct=**0.0062** | dist={'min': -1.1554, 'p25': 2.6033, 'p50': 2.807, 'p75': 3.3107, 'p90': 3.3745, 'p95': 3.5115, 'max': 9.36, 'mean': 2.9618}
...
- exact lane band: **core_normal** (pct=0.0014, Δp90=-1.5406)
- same_gate_same_quality band: **core_normal** (pct=0.0016, Δp90=-1.51)
- same_bucket band: **core_normal** (pct=0.0014, Δp90=-1.5408)
- bull_all band: **core_normal** (pct=0.0062, Δp90=-1.5305)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.0016
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.11119999999999997** | legacy=**0.11119999999999997** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_crosses_trade_floor**
- baseline -> calibration runtime entry_quality: **0.4977 → 0.4977** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4977 → 0.6636** (Δ=**0.1659**)
...
- runtime gap to floor: **-0.1136**
- next patch target: **verify_runtime_guardrails**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4977**, layers **0**
- fully relax bias50 penalty: entry_quality **0.7644**, layers **2**
- required bias50 cap to cross trade floor: **0.9725** (current=1.844)
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5335** / layers **0** / gap **0.0165** / target_score **0.2304**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 1.0, 'feat_pulse': 0.0, 'feat_ear': 0.0}** → entry_quality **0.6636** / gap **0.0** / mean_gap **0.0095**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8498** / gap **0.0** / mean_gap **-0.004**
