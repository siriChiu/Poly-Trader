...
- legacy_entry_quality: **0.4756** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4756** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4756** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **0.4939**
...
- current bias50 percentile in exact lane: **0.1372**
...
- same_gate_same_quality: rows=**3082** | win_rate=**0.7914** | bias50_pct=**0.2492** | dist={'min': -1.7439, 'p25': 0.5007, 'p50': 1.3159, 'p75': 3.2171, 'p90': 4.0736, 'p95': 4.5615, 'max': 8.0151, 'mean': 1.7705}
- same_bucket: rows=**1439** | win_rate=**0.8402** | bias50_pct=**0.2411** | dist={'min': -5.5588, 'p25': 0.5106, 'p50': 0.9938, 'p75': 1.2846, 'p90': 1.552, 'p95': 1.7469, 'max': 4.4265, 'mean': 0.8288}
- bull_all: rows=**4283** | win_rate=**0.8158** | bias50_pct=**0.3131** | dist={'min': -6.9187, 'p25': 0.1795, 'p50': 1.1607, 'p75': 3.1532, 'p90': 4.1576, 'p95': 4.7078, 'max': 8.0151, 'mean': 1.5111}
...
- exact lane band: **core_normal** (pct=0.1372, Δp90=-1.1939)
- same_gate_same_quality band: **core_normal** (pct=0.2492, Δp90=-3.5797)
- same_bucket band: **core_normal** (pct=0.2411, Δp90=-1.0581)
- bull_all band: **core_normal** (pct=0.3131, Δp90=-3.6637)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.2492
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.38122** | legacy=**0.38122** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4756 → 0.4756** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4756 → 0.4756** (Δ=**0.0**)
...
- runtime gap to floor: **0.0744**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4756**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6612**, layers **1**
- required bias50 cap to cross trade floor: **-0.746** (current=0.4939)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4933** / layers **0** / gap **0.0567**
- required_bias50_cap_after_best_scenario: **-0.451**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.4882** / layers **0** / gap **0.0618** / target_score **0.4233**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.5416** / layers **0** / gap **0.0084**
- required_bias50_cap_after_best_scenario: **0.354**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.6, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 0.4}** → entry_quality **0.5535** / gap **0.0** / mean_gap **0.0389**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8156** / gap **0.0** / mean_gap **0.0101**
