...
- generated_at: **2026-05-14 21:10:16.405718**
...
- legacy_entry_quality: **0.4787** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4787** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4787** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`under_minimum_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **0.9539**
- structure_quality: **0.5396**
...
- current bias50 percentile in exact lane: **0.3299**
...
- same_gate_same_quality: rows=**3080** | win_rate=**0.7912** | bias50_pct=**0.349** | dist={'min': -1.7439, 'p25': 0.4666, 'p50': 1.3184, 'p75': 3.2171, 'p90': 4.0736, 'p95': 4.5615, 'max': 8.0151, 'mean': 1.7559}
- same_bucket: rows=**1416** | win_rate=**0.8404** | bias50_pct=**0.4767** | dist={'min': -5.5588, 'p25': 0.506, 'p50': 0.9965, 'p75': 1.2879, 'p90': 1.5575, 'p95': 1.7536, 'max': 4.4265, 'mean': 0.8263}
- bull_all: rows=**4281** | win_rate=**0.8157** | bias50_pct=**0.4235** | dist={'min': -6.9187, 'p25': 0.1661, 'p50': 1.162, 'p75': 3.1532, 'p90': 4.1576, 'p95': 4.7078, 'max': 8.0151, 'mean': 1.4999}
...
- exact lane band: **core_normal** (pct=0.3299, Δp90=-0.7359)
- same_gate_same_quality band: **core_normal** (pct=0.349, Δp90=-3.1197)
- same_bucket band: **core_normal** (pct=0.4767, Δp90=-0.6036)
- bull_all band: **core_normal** (pct=0.4235, Δp90=-3.2037)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.349
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.28922** | legacy=**0.28922** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4787 → 0.4787** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4787 → 0.4787** (Δ=**0.0**)
...
- runtime gap to floor: **0.0713**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4787**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6919**, layers **2**
- required bias50 cap to cross trade floor: **-0.2345** (current=0.9539)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4926** / layers **0** / gap **0.0574**
- required_bias50_cap_after_best_scenario: **-0.0025**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5193** / layers **0** / gap **0.0307** / target_score **0.4245**
...
- best scenario: **winner_triplet_p75** → entry_quality **0.5554** / layers **1** / gap **0.0**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.55, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 0.45}** → entry_quality **0.5565** / gap **0.0** / mean_gap **0.0367**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8067** / gap **0.0** / mean_gap **0.009**
