...
- generated_at: **2026-05-21 01:12:32.756307**
...
- legacy_entry_quality: **0.5978** (raw_reason=`entry_quality_C_single_layer`)
- calibration_runtime_entry_quality: **0.5978** (raw_reason=`entry_quality_C_single_layer`)
- deployed_runtime_entry_quality: **0.5978** (raw_reason=`entry_quality_C_single_layer`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **-0.6171**
- structure_quality: **0.4327**
...
- same_gate_same_quality: rows=**1161** | win_rate=**0.8932** | bias50_pct=**0.2024** | dist={'min': -6.6008, 'p25': -0.3648, 'p50': 0.6366, 'p75': 2.4842, 'p90': 4.6126, 'p95': 4.8007, 'max': 7.5411, 'mean': 1.0016}
...
- bull_all: rows=**4342** | win_rate=**0.8047** | bias50_pct=**0.108** | dist={'min': -6.9187, 'p25': 0.1774, 'p50': 1.1508, 'p75': 3.1304, 'p90': 4.1466, 'p95': 4.7009, 'max': 8.0151, 'mean': 1.4988}
...
- exact lane band: **core_normal** (pct=0.0157, Δp90=-1.8739)
- same_gate_same_quality band: **core_normal** (pct=0.2024, Δp90=-5.2297)
- same_bucket band: **core_normal** (pct=0.0237, Δp90=-2.1681)
- bull_all band: **core_normal** (pct=0.108, Δp90=-4.7637)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.2024
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.6034200000000001** | legacy=**0.6034200000000001** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_crosses_floor_but_execution_blocked**
- baseline -> calibration runtime entry_quality: **0.5978 → 0.5978** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.5978 → 0.5978** (Δ=**0.0**)
...
- runtime gap to floor: **-0.0478**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.5978**, layers **1**
- fully relax bias50 penalty: entry_quality **0.7168**, layers **2**
- required bias50 cap to cross trade floor: **-0.617** (current=-0.6171)
...
- best scenario: **exact_lane_p75** → entry_quality **0.6133** / layers **1** / gap **0.0**
- required_bias50_cap_after_best_scenario: **-0.617**
...
- machine_read: entry_quality>=0.55=**True** | allowed_layers>0=**True**
- best scenario: **exact_lane_triplet_p75** → entry_quality **0.6503** / layers **1** / gap **0.0**
- required_bias50_cap_after_best_scenario: **-0.617**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 1.0, 'feat_ear': 0.0}** → entry_quality **0.6699** / gap **0.0** / mean_gap **0.0071**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8354** / gap **0.0** / mean_gap **0.0028**
