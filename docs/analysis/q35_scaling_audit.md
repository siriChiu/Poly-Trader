...
- generated_at: **2026-05-25 18:15:53.783491**
...
- legacy_entry_quality: **0.4587** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4587** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4587** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`under_minimum_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **0.8768**
- structure_quality: **0.3676**

## Exact lane summary

- rows: **984** | win_rate: **0.7419**
- bias50 distribution: {'min': -1.2312, 'p25': 0.6503, 'p50': 1.1074, 'p75': 1.3344, 'p90': 1.6422, 'p95': 1.8121, 'max': 4.4265, 'mean': 1.0241}
- current bias50 percentile in exact lane: **0.312**
- winner-only bias50 distribution: {'min': -1.1316, 'p25': 0.6019, 'p50': 1.0396, 'p75': 1.323, 'p90': 1.5893, 'p95': 1.7747, 'max': 3.9157, 'mean': 0.9787}

## Broader bull cohorts

- same_gate_same_quality: rows=**3241** | win_rate=**0.7612** | bias50_pct=**0.3462** | dist={'min': -1.9, 'p25': 0.2997, 'p50': 1.2827, 'p75': 3.1597, 'p90': 4.0323, 'p95': 4.5274, 'max': 8.0151, 'mean': 1.6727}
- same_bucket: rows=**1636** | win_rate=**0.7928** | bias50_pct=**0.4768** | dist={'min': -5.5588, 'p25': 0.3194, 'p50': 0.9226, 'p75': 1.2527, 'p90': 1.5225, 'p95': 1.7174, 'max': 4.4265, 'mean': 0.6776}
- bull_all: rows=**4658** | win_rate=**0.7782** | bias50_pct=**0.4392** | dist={'min': -6.9187, 'p25': 0.0502, 'p50': 1.0695, 'p75': 3.0515, 'p90': 4.0841, 'p95': 4.6616, 'max': 8.0151, 'mean': 1.3371}
...
- exact lane band: **core_normal** (pct=0.312, Δp90=-0.7654)
- same_gate_same_quality band: **core_normal** (pct=0.3462, Δp90=-3.1555)
- same_bucket band: **core_normal** (pct=0.4768, Δp90=-0.6457)
- bull_all band: **core_normal** (pct=0.4392, Δp90=-3.2073)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.3462
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.30463999999999997** | legacy=**0.30463999999999997** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4587 → 0.4587** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4587 → 0.4587** (Δ=**0.0**)
...
- runtime gap to floor: **0.0913**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4587**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6673**, layers **1**
- required bias50 cap to cross trade floor: **-0.6445** (current=0.8768)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4827** / layers **0** / gap **0.0673**
- required_bias50_cap_after_best_scenario: **-0.2445**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5049** / layers **0** / gap **0.0451** / target_score **0.4586**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.5207** / layers **0** / gap **0.0293**
- required_bias50_cap_after_best_scenario: **0.1135**
...
- rows / wins / losses: **984 / 730 / 254**
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.9, 'feat_ear': 0.1}** → entry_quality **0.5553** / gap **0.0** / mean_gap **0.0526**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8283** / gap **0.0** / mean_gap **0.0183**
