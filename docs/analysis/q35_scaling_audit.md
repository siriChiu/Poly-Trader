...
- legacy_entry_quality: **0.4329** (raw_reason=`regime_gate_block`)
- calibration_runtime_entry_quality: **0.4329** (raw_reason=`regime_gate_block`)
- deployed_runtime_entry_quality: **0.4329** (raw_reason=`regime_gate_block`, effective_reason=`unsupported_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **3.0381**
...
- current bias50 percentile in exact lane: **0.5924**
...
- same_gate_same_quality: rows=**750** | win_rate=**0.532** | bias50_pct=**0.5573** | dist={'min': -18.7285, 'p25': -0.0736, 'p50': 2.0406, 'p75': 5.6979, 'p90': 7.1403, 'p95': 8.8165, 'max': 17.0434, 'mean': 2.827}
- same_bucket: rows=**195** | win_rate=**0.6615** | bias50_pct=**0.6154** | dist={'min': -0.7025, 'p25': 1.0207, 'p50': 1.7335, 'p75': 3.9909, 'p90': 4.8559, 'p95': 5.5826, 'max': 6.3969, 'mean': 2.4738}
- bull_all: rows=**10277** | win_rate=**0.5993** | bias50_pct=**0.7337** | dist={'min': -20.6429, 'p25': -1.5342, 'p50': 1.0415, 'p75': 3.1904, 'p90': 5.2302, 'p95': 6.1251, 'max': 17.0434, 'mean': 0.8337}
...
- exact lane band: **warm** (pct=0.5924, Δp90=-1.9115)
- same_gate_same_quality band: **warm** (pct=0.5573, Δp90=-4.1022)
- same_bucket band: **warm** (pct=0.6154, Δp90=-1.8178)
- bull_all band: **warm** (pct=0.7337, Δp90=-2.1921)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.5573
...
- baseline -> calibration runtime entry_quality: **0.4329 → 0.4329** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4329 → 0.4329** (Δ=**0.0**)
...
- runtime gap to floor: **0.1171**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4329**, layers **0**
- fully relax bias50 penalty: entry_quality **0.7329**, layers **0**
- required bias50 cap to cross trade floor: **0.4485** (current=3.0381)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4796** / layers **0** / gap **0.0704**
- required_bias50_cap_after_best_scenario: **1.2265**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.5319** / layers **0** / gap **0.0181** / target_score **0.33**
...
- best scenario: **winner_triplet_p75** → entry_quality **0.5608** / layers **0** / gap **0.0**
- required_bias50_cap_after_best_scenario: **1.01**
...
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 1.0, 'feat_pulse': 0.0, 'feat_ear': 0.0}** → entry_quality **0.2832** / gap **0.2668** / mean_gap **0.0207**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8234** / gap **0.0** / mean_gap **-0.0269**
