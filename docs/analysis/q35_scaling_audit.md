...
- legacy_entry_quality: **0.6001** (raw_reason=`entry_quality_C_single_layer`)
- calibration_runtime_entry_quality: **0.6** (raw_reason=`entry_quality_C_single_layer`)
- deployed_runtime_entry_quality: **0.6** (raw_reason=`entry_quality_C_single_layer`, effective_reason=`under_minimum_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **-0.5859**
- structure_quality: **0.356**

## Exact lane summary

- rows: **524** | win_rate: **0.9714**
- bias50 distribution: {'min': -5.5588, 'p25': 0.2552, 'p50': 0.6543, 'p75': 1.0183, 'p90': 1.2547, 'p95': 1.4262, 'max': 1.6604, 'mean': 0.5209}
- current bias50 percentile in exact lane: **0.0401**
- winner-only bias50 distribution: {'min': -5.5588, 'p25': 0.2483, 'p50': 0.6366, 'p75': 1.0002, 'p90': 1.2372, 'p95': 1.4412, 'max': 1.6604, 'mean': 0.5254}

## Broader bull cohorts

- same_gate_same_quality: rows=**1177** | win_rate=**0.8946** | bias50_pct=**0.2455** | dist={'min': -6.6008, 'p25': -0.572, 'p50': 0.6131, 'p75': 2.2811, 'p90': 4.6063, 'p95': 4.8005, 'max': 7.5411, 'mean': 0.9641}
- same_bucket: rows=**1492** | win_rate=**0.819** | bias50_pct=**0.0322** | dist={'min': -5.5588, 'p25': 0.5078, 'p50': 0.9861, 'p75': 1.2795, 'p90': 1.5493, 'p95': 1.7445, 'max': 4.4265, 'mean': 0.808}
- bull_all: rows=**4359** | win_rate=**0.8055** | bias50_pct=**0.1241** | dist={'min': -6.9187, 'p25': 0.1702, 'p50': 1.1487, 'p75': 3.1251, 'p90': 4.1448, 'p95': 4.6967, 'max': 8.0151, 'mean': 1.486}
...
- exact lane band: **core_normal** (pct=0.0401, Δp90=-1.8406)
- same_gate_same_quality band: **core_normal** (pct=0.2455, Δp90=-5.1922)
- same_bucket band: **core_normal** (pct=0.0322, Δp90=-2.1352)
- bull_all band: **core_normal** (pct=0.1241, Δp90=-4.7307)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.2455
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.59718** | legacy=**0.59718** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_crosses_floor_but_execution_blocked**
- baseline -> calibration runtime entry_quality: **0.6001 → 0.6** (Δ=**-0.0001**)
- baseline -> deployed runtime entry_quality: **0.6001 → 0.6** (Δ=**-0.0001**)
...
- runtime gap to floor: **-0.05**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.6001**, layers **1**
- fully relax bias50 penalty: entry_quality **0.7209**, layers **2**
- required bias50 cap to cross trade floor: **-0.586** (current=-0.5859)
...
- best scenario: **exact_lane_p75** → entry_quality **0.6165** / layers **1** / gap **0.0**
- required_bias50_cap_after_best_scenario: **-0.586**
...
- machine_read: entry_quality>=0.55=**True** | allowed_layers>0=**True**
- best scenario: **exact_lane_triplet_p75** → entry_quality **0.6311** / layers **1** / gap **0.0**
- required_bias50_cap_after_best_scenario: **-0.586**
...
- rows / wins / losses: **524 / 509 / 15**
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 1.0, 'feat_ear': 0.0}** → entry_quality **0.6366** / gap **0.0** / mean_gap **0.0025**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8341** / gap **0.0** / mean_gap **0.0022**
