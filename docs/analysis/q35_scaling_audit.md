...
- generated_at: **2026-05-02 00:26:49.044015**
...
- legacy_entry_quality: **0.4277** (raw_reason=`entry_quality_below_trade_floor`)
- calibration_runtime_entry_quality: **0.4277** (raw_reason=`entry_quality_below_trade_floor`)
- deployed_runtime_entry_quality: **0.4277** (raw_reason=`entry_quality_below_trade_floor`, effective_reason=`under_minimum_exact_live_structure_bucket`)
- q35_discriminative_redesign_applied: **False**
- feat_4h_bias50: **1.3745**
- structure_quality: **0.4696**

## Exact lane summary

- rows: **873** | win_rate: **0.7766**
- bias50 distribution: {'min': -0.4035, 'p25': 0.7875, 'p50': 1.1449, 'p75': 1.3669, 'p90': 1.6949, 'p95': 1.8374, 'max': 4.4265, 'mean': 1.1027}
- current bias50 percentile in exact lane: **0.7526**
- winner-only bias50 distribution: {'min': -0.4035, 'p25': 0.6316, 'p50': 1.0671, 'p75': 1.3466, 'p90': 1.6072, 'p95': 1.7763, 'max': 3.9157, 'mean': 1.0259}

## Broader bull cohorts

- same_gate_same_quality: rows=**2886** | win_rate=**0.8046** | bias50_pct=**0.4886** | dist={'min': -1.7439, 'p25': 0.6929, 'p50': 1.4299, 'p75': 3.28, 'p90': 4.0983, 'p95': 4.6111, 'max': 8.0151, 'mean': 1.9572}
- same_bucket: rows=**1407** | win_rate=**0.8394** | bias50_pct=**0.8237** | dist={'min': -5.5588, 'p25': 0.5057, 'p50': 0.9937, 'p75': 1.2897, 'p90': 1.5575, 'p95': 1.7562, 'max': 4.4265, 'mean': 0.825}
- bull_all: rows=**4036** | win_rate=**0.8288** | bias50_pct=**0.5503** | dist={'min': -6.9187, 'p25': 0.3567, 'p50': 1.2547, 'p75': 3.2265, 'p90': 4.2246, 'p95': 4.7277, 'max': 8.0151, 'mean': 1.6681}
...
- exact lane band: **elevated_but_within_p90** (pct=0.7526, Δp90=-0.3204)
- same_gate_same_quality band: **core_normal** (pct=0.4886, Δp90=-2.7238)
- same_bucket band: **elevated_but_within_p90** (pct=0.8237, Δp90=-0.183)
- bull_all band: **warm** (pct=0.5503, Δp90=-2.8501)
- reference cohort: **same_gate_same_quality** / label=同 bull gate + 同 quality lane / pct=0.4886
- note: current bias50 已回到 exact lane p90 內；下一步應做 exact-lane 內的保守 bias50 校準 / 公式檢查，而不是再走 broader bull segmentation。
- runtime preview: applied=**False** | score=**0.20509999999999998** | legacy=**0.20509999999999998** | Δ=**0.0** | segment=**None**

## Deployment-grade component experiment

- verdict: **runtime_patch_no_material_improvement**
- baseline -> calibration runtime entry_quality: **0.4277 → 0.4277** (Δ=**0.0**)
- baseline -> deployed runtime entry_quality: **0.4277 → 0.4277** (Δ=**0.0**)
...
- runtime gap to floor: **0.1223**
- next patch target: **feat_4h_bias50_formula**

## Counterfactuals

- gate -> ALLOW only: entry_quality **0.4277**, layers **0**
- fully relax bias50 penalty: entry_quality **0.6661**, layers **1**
- required bias50 cap to cross trade floor: **-0.664** (current=1.3745)
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **exact_lane_p75** → entry_quality **0.4325** / layers **0** / gap **0.1175**
- required_bias50_cap_after_best_scenario: **-0.584**
...
- best scenario: **exact_runtime_p90** → entry_quality **0.4935** / layers **0** / gap **0.0565** / target_score **0.4245**
...
- machine_read: entry_quality>=0.55=**False** | allowed_layers>0=**False**
- best scenario: **winner_triplet_p75** → entry_quality **0.5388** / layers **0** / gap **0.0112**
- required_bias50_cap_after_best_scenario: **0.445**
...
- rows / wins / losses: **873 / 678 / 195**
- best discriminative candidate: weights=**{'feat_4h_bias50': 0.4, 'feat_nose': 0.0, 'feat_pulse': 0.1, 'feat_ear': 0.5}** → entry_quality **0.5529** / gap **0.0** / mean_gap **0.0327**
- best floor candidate: weights=**{'feat_4h_bias50': 0.0, 'feat_nose': 0.0, 'feat_pulse': 0.0, 'feat_ear': 1.0}** → entry_quality **0.8076** / gap **0.0** / mean_gap **0.0098**
