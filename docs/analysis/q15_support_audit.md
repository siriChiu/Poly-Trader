...
- current_live_structure_bucket_rows: **33**
...
- support_progress.reason: current q15 exact support 目前是 33/50，仍低於 minimum；歷史上同 bucket 曾有 199/50（heartbeat 20260423i），但該 artifact 缺少相同 support_identity / bucket_semantic_signature，只能當 legacy reference，不能宣稱為 same-identity regression。
...
- best_single_component_required_score_delta: **0.6923**
- best_single_component_can_cross_floor: **True**
- reason: feat_4h_bias50 在數學上可單點補足 floor gap（需要 score Δ≈0.6923），但 current q15 exact support 尚未達 deployment 門檻，因此不得單靠 component calibration 解除 blocker。
