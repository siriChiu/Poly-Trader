...
- reason: 目前先被 runtime blocker 擋下（Consecutive loss streak: 131 >= 50; Recent 50-sample win rate: 0.00% < 30%），不能把 q15 floor-cross 當成當前 deploy 入口。
...
- current_entry_quality: **0.5597**
- trade_floor: **0.55**
- current_trade_floor_gap: **0.0097**
...
- reason: 目前先被 runtime blocker 擋下（Consecutive loss streak: 131 >= 50; Recent 50-sample win rate: 0.00% < 30%），q15 component experiment 只能保留為背景研究。
