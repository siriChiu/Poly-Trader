...
- support_progress.reason: current live exact support 目前是 0/50，仍低於 minimum；歷史上同 bucket 曾有 190/50（heartbeat 1202），語義證據已回填但不吻合 current support_identity（mismatched=['calibration_window', 'entry_quality_label', 'regime_label'], missing=[]），只能當 legacy reference，不能宣稱為 same-identity regression。
...
- reason: 目前先被 runtime blocker 擋下（Recent 50-sample win rate: 10.00% < 30%），不能把 q15 floor-cross 當成當前 deploy 入口。
...
- current_entry_quality: **0.6208**
- trade_floor: **0.55**
- current_trade_floor_gap: **0.0708**
...
- reason: 目前先被 runtime blocker 擋下（Recent 50-sample win rate: 10.00% < 30%），q15 component experiment 只能保留為背景研究。
