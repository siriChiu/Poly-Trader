...
- current_entry_quality: **0.6194**
- trade_floor: **0.55**
- current_trade_floor_gap: **0.0694**
...
- reason: current live row 目前停在 BLOCK|bear_bias200_hard_block|q00，不在 q15 target lane CAUTION|structure_quality_caution|q15；本 artifact 只能描述非 current-live 的 q15/reference route，不得當成 current-live deployment closure。
- verify_next: 先處理 current-live bucket BLOCK|bear_bias200_hard_block|q00 的 exact-support / runtime blocker；只有 live row 回到 q15 lane 且 exact support deployable 時，q15 component experiment 才可進入 deployment verify。
