# Circuit Breaker Audit（Heartbeat #1519）
_generated_at: 2026-05-26T04:19:44.307184Z_

## 結論
- verdict: **mixed_horizon_false_positive**
- summary: 混合 horizon breaker 會被 240m tail labels 觸發（streak=2，recent50 win_rate=0.0200），但 1440m canonical live horizon 目前 release-ready。
- recommended_patch: 將 circuit breaker 對齊 horizon_minutes=1440 的 canonical live contract。
- top_level_release: ready=True / recent wins=36/50 / need=15 / gap=0

## Mixed scope（現況錯誤口徑）
- triggered: **True** via ['recent_win_rate']
- streak: 2 / threshold 50
- recent 50: win_rate=0.02 wins=1 losses=49
- streak horizons: {'240': 2}

## Aligned scope（1440m canonical live horizon）
- triggered: **False** via []
- release_ready: **True**
- streak: 13 / threshold 50
- recent 50: win_rate=0.72 wins=36 losses=14

## Release condition
- streak < 50
- recent 50 win_rate >= 30%
- aligned_scope_now: streak=13, win_rate=0.72
- additional recent-window wins needed: 0
- tail pathology: losses=14 / wins=36 / loss_share=0.28