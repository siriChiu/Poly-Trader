...
**Architecture:** 用 `ExecutionService + ExchangeAdapter + AccountSyncService` 抽出實盤層；目前 production venue 收斂為 OKX-only，同時保留 paper / live_canary / live 模式切換。
...
## 當前已完成的第一段
- 建立 exchange adapter 骨架；目前 active adapter 收斂為 OKX-only
...
4. 做 OKX canary live 驗證，legacy venue 不再作 canary 路徑
