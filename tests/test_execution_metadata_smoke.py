...
def test_run_metadata_smoke_rejects_unsupported_configured_venue(monkeypatch):
    ...
    assert payload["results"]["binance"]["adapter_supported"] is False
    assert payload["results"]["binance"]["enabled_in_config"] is True
    assert payload["results"]["binance"]["credentials_configured"] is True
    assert payload["results"]["binance"]["proof_state"] == "adapter_unsupported"
    ...
    assert "場館 adapter 尚未接入" in payload["results"]["binance"]["blockers"]
    assert "先接入 binance 交易所 adapter" in payload["results"]["binance"]["operator_next_action"]
