...
def test_build_probe_payload_ignores_stale_q15_support_identity_overlay(monkeypatch):
    ...
    payload = hb_predict_probe._build_probe_payload(
        latest={"timestamp": "2026-05-15 16:02:21.916158", "regime_label": "bear"},
        result={
            "target_col": "simulated_pyramid_win",
            ...
            "entry_quality": 0.5354,
            "entry_quality_label": "D",
            "decision_quality_label": "D",
            "decision_quality_score": -0.071,
            ...
            "deployment_blocker_details": {
                ...
                "exact_live_structure_bucket_rows": 2,
                "minimum_support_rows": 50,
        ...
        current_live_structure_bucket="BLOCK|structure_quality_block|q00",
        current_live_structure_bucket_rows=2,
        q15_support_audit=stale_q15_audit,
        four_h_non_null={},
        lag_non_null={},
    )

    assert payload["current_live_structure_bucket_rows"] == 2
    assert payload["current_live_structure_bucket_gap_to_minimum"] == 48
    assert payload["deployment_blocker_details"]["current_live_structure_bucket_rows"] == 2
    assert payload["deployment_blocker_details"]["exact_live_structure_bucket_rows"] == 2
    assert payload["support_progress"]["current_rows"] == 2
    assert payload["support_progress"]["support_identity"]["entry_quality_label"] == "D"
    assert payload["q15_support_audit_identity_mismatch"]["status"] == "support_identity_mismatch_current_live"
    assert payload["q15_support_audit_identity_mismatch"]["mismatched_fields"] == [
        {"field": "entry_quality_label", "audit_value": "C", "current_value": "D"}
    ]
    assert payload["floor_cross_verdict"] is None
    assert payload["component_experiment_verdict"] is None
    assert "2/50" in payload["deployment_blocker_reason"]
...
def test_build_probe_payload_canonicalizes_zero_row_q15_insufficient_support_mode(monkeypatch):
    ...
    payload = hb_predict_probe._build_probe_payload(
        latest={"timestamp": "2026-05-20 02:02:38.742259", "regime_label": "bear"},
        result={
            "target_col": "simulated_pyramid_win",
            "used_model": "regime_bear_ensemble",
            "model_type": "RegimeAwarePredictor",
            ...
            "entry_quality": 0.7374,
            "entry_quality_label": "B",
            "decision_quality_label": "D",
            "decision_quality_score": 0.0217,
            "decision_quality_horizon_minutes": 1440,
            ...
            "deployment_blocker_details": {
                ...
                "current_live_structure_bucket_rows": 0,
                "exact_live_structure_bucket_rows": 0,
                "minimum_support_rows": 50,
                "current_live_structure_bucket_gap_to_minimum": 50,
...
def test_hb_predict_probe_emits_q35_runtime_and_structure_fields(monkeypatch, capsys, tmp_path):
    ...
    assert payload["api_trade_allowed_actions"] == ["wait", "reduce", "sell", "diagnostics", "mode_toggle"]
    assert "等待 / 觀望" in payload["api_trade_guardrail_context"]
...
def test_hb_predict_probe_preserves_q15_audit_support_progress_for_q35_current_bucket(monkeypatch, capsys, tmp_path):
    session = DummySession()
    out_path = tmp_path / "live_predict_probe.json"
    q15_audit_path = tmp_path / "q15_support_audit.json"
    q35_audit_path = tmp_path / "q35_scaling_audit.json"
        ...
        json.dumps(
            {
                ...
                "current_live": {
                    "feature_timestamp": "2026-05-14 16:02:11.050276",
                ...
                "support_route": {
                    ...
                    "support_progress": {
                        "status": "stalled_under_minimum",
                        "reason": "current live exact support 連續 heartbeat 停在同一數量。",
                        "current_rows": 28,
                        "minimum_support_rows": 50,
                        "gap_to_minimum": 22,
    ...
    q35_audit_path.write_text("{}", encoding="utf-8")
    ...
    monkeypatch.setattr(
        ...
        "load_latest_features",
        lambda _session: {
            "timestamp": "2026-05-14 16:02:11.050276",
            "regime_label": "chop",
            "feat_4h_bias50": 0.9226,
        },
    )
    monkeypatch.setattr(
        hb_predict_probe,
    ...
    assert payload["support_progress"]["status"] == "stalled_under_minimum"
    assert payload["support_progress"]["stagnant_run_count"] == 2
    assert payload["deployment_blocker_details"]["support_progress"]["status"] == "stalled_under_minimum"
    assert payload["q15_support_audit"]["scope_applicability"]["status"] == "current_live_not_q15_lane"
    assert payload["floor_cross_verdict"] == "math_cross_possible_but_illegal_without_exact_support"
    assert payload["component_experiment_verdict"] == "reference_only_current_live_not_q15_and_support_not_ready"
