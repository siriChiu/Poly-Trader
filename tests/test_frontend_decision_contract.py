...
def test_strategy_lab_surfaces_high_conviction_topk_gate_contract():
    ...
    required_snippets = [
        ...
        'OOS 候選已過門檻，但即時部署仍阻塞',
        '離線驗證 / 風控已過 {formatDecimal(highConvictionRiskQualifiedCount, 0)} 筆；可部署 {formatDecimal(highConvictionTopK.deployable_count, 0)} 筆。{highConvictionRuntimeBlockerSummary} · 支持樣本 {highConvictionSupportRowsLabel}。請先解除熔斷 / 執行保護，再把候選視為灰度部署對象。',
        '即時支持脈絡',
