...
def test_execution_console_consumes_runtime_status_and_uses_exchange_like_layout():
    ...
    required_snippets = [
        ...
        'Standalone Live Runner 證據',
        'liveRunnerGateCountsLabel',
        'Fail-closed · 不送單',
        '["ready_control_plane", "resume_available", "shadow_start_available"].includes',
        'profileShadowSummaryLabel',
        'paper_shadow: "影子觀察"',
        '恢復',
        '暫停',
        '停止',
...
def test_execution_console_surfaces_m5_readiness_shadow_ledger_venue_proof_and_canary_answers():
    ...
    required_snippets = [
        ...
        'readinessGateByKey.get("current_lane_actionability_gate")',
        '實戰準備度',
        'Shadow / Reduce-only',
        '模型 gate',
        '當前 lane 可行動 gate',
        'sub-gates：strict exact / shadow evidence / cost-aware edge',
        '即時支持 gate',
        '熔斷 gate',
        'release evidence',
...
def test_runtime_copy_and_execution_surfaces_use_humanized_chinese_operator_copy():
    ...
    required_runtime_copy_snippets = [
        ...
        '.replace(/\\bcurrent-live\\s+([a-z0-9]+)\\s+bucket\\b/gi, "即時 $1 分桶")',
        '.split("live 執行期").join("即時執行期")',
        '.replace(/\\blive 執行期/gi, "即時執行期")',
        '.replace(/\\bbucket\\b/gi, "分桶")',
        '.replace(/\\bcanonical breaker\\b/gi, "正式風控熔斷")',
        ...
        '相對舊版已就緒參考 ${formatSupportProgressDelta(deltaVsLegacy)}（${legacyRows} → ${currentRows}，僅供參考${currentGapSuffix}）',
        '`支持語義 v${version}`',
...
def test_support_regression_visibility_surfaces_show_last_supported_reference():
    ...
    confidence_snippets = [
        ...
        '當前分桶支持',
        '狀態 {supportStatusLabel} · 樣本變化 {supportDeltaLabel}',
        '不得把舊分桶或近似樣本當成部署閉環。',
...
def test_confidence_indicator_distinguishes_capacity_opened_vs_patch_blocked_states():
    ...
    required_snippets = [
        ...
        'const currentBucketRootCauseSupportRowsLabel =',
...
def test_strategy_lab_keeps_decision_quality_summary_surfaces():
    ...
    required_snippets = [
        ...
        'next_retry_at',
        'modelLeaderboardStaleStateLabel',
        '模型排行榜快取過期：{modelLeaderboardStaleStateLabel}',
...
def test_strategy_lab_surfaces_high_conviction_topk_gate_contract():
    ...
    required_snippets = [
        ...
        'deployable_rows?: number | null;',
        'risk_qualified_rows?: number | null;',
        ...
        'formatHighConvictionSupportProgressLabel(row.support_progress_status, row.stalled_support_accumulation, row.stagnant_run_count, row.support_delta_vs_previous)',
        ...
        'const highConvictionRuntimeBlockedCount = firstFiniteNumber(',
        'highConvictionTopK?.runtime_blocked_candidate_rows,',
        'highConvictionTopK?.runtime_blocked_candidates,',
        'const highConvictionRiskQualifiedCount = firstFiniteNumber(',
        ...
        'const highConvictionStatusLabel = highConvictionFreshnessBlocking ? "矩陣過期 / 僅影子觀察"',
        '離線通過，但即時部署阻塞 / 影子觀察',
        'const highConvictionCardTone = highConvictionFreshnessBlocking ? "border-amber-500/30 bg-amber-500/10 text-amber-50"',
        ...
        '熔斷解除${releaseReady === true ? "已滿足" : "未滿足"}：最近 ${formatDecimal(windowSize, 0)} 筆 ${formatDecimal(wins, 0)} 勝 / 需 ${formatDecimal(requiredWins, 0)} 勝${additionalText}${winRateText}${streakText}',
        '熔斷解除條件未啟用',
        'const highConvictionSupportGap = highConvictionSupportContext?.current_live_structure_bucket_gap_to_minimum ?? highConvictionPrimaryRuntimeRow?.current_live_structure_bucket_gap_to_minimum;',
        ...
        '最接近部署候選',
        '離線驗證 / 風控門檻已過',
        '遇到即時部署阻塞（例如熔斷、支持樣本或場館保護）',
        '高信心 OOS Top-K 部署門檻',
        '最接近部署候選優先顯示；離線驗證 / 風控門檻已過但遇到即時部署阻塞（例如熔斷、支持樣本或場館保護）時，仍維持模擬觀察 / 影子驗證 / 僅觀察，不開新倉。',
        'OOS 候選已過門檻，但即時部署仍阻塞',
        '離線驗證 / 風控已過 {formatDecimal(highConvictionRiskQualifiedCount, 0)} 筆；可部署 {formatDecimal(highConvictionDeployableCount, 0)} 筆。{highConvictionRuntimeBlockerSummary} · 支持樣本 {highConvictionSupportRowsLabel} · {highConvictionSupportGateNextAction} · {highConvictionSupportProgressLabel} · {highConvictionCircuitBreakerReleaseLabel}。請先補齊當前分桶支持樣本 / 解除執行保護，再把候選視為灰度部署對象。',
        '即時支持脈絡',
        ...
        '即時分桶 {highConvictionSupportBucketLabel} · 樣本 {highConvictionSupportRowsLabel} · {highConvictionSupportGateNextAction} · 閉環 {highConvictionRuntimeClosureLabel}',
        '矩陣新鮮度 {highConvictionFreshnessLabel} · {highConvictionArtifactAgeLabel}',
        'Top-K 矩陣過期，部署降級為影子觀察',
        '請先重跑 high-conviction Top-K / model leaderboard refresh',
        '矩陣新鮮度',
        'Top-K 分層',
        '離線 ROI',
        '勝率',
        ...
        '即時訊號 {formatHighConvictionRuntimeSignalLabel(row.signal)} · 可用層 {isFiniteNumber(row.allowed_layers) ? formatDecimal(row.allowed_layers, 0) : "—"} · 原因 {row.execution_guardrail_reason ? humanizeRuntimeDetailText(row.execution_guardrail_reason) : "—"}',
        ...
        '離線驗證 / 風控門檻已過 · 即時部署仍阻塞：{row.deployment_blocker ? humanizeCurrentLiveBlockerLabel(row.deployment_blocker) : "支持樣本或熔斷保護"}；不開新倉',
        '未通過門檻',
    ]
    for snippet in required_snippets:
        assert snippet in source
    forbidden_snippets = [
        ...
        'formatDecimal(highConvictionTopK.deployable_count, 0)',
    ]
    for snippet in forbidden_snippets:
        assert snippet not in source
...
def test_execution_status_and_strategy_lab_surface_q15_bucket_root_cause_candidate():
    ...
    for snippet in [
        ...
        'const currentBucketRootCauseIsQ15 = currentBucketRootCauseBucketKey === "q15" || currentBucketRootCauseBucketKey.endsWith("|q15");',
        'const currentBucketRootCauseSupportRowsLabel = runtimeStatusPending',
        '精準路徑樣本 ${currentBucketRootCauseSupportRowsLabel} · 缺口 ${currentBucketRootCauseSupportGapLabel}',
        'const currentBucketRootCauseDrilldownLabel = currentBucketRootCauseIsQ35',
        '交易門檻缺口 ${formatNumber(currentBucketRootCauseTradeFloorGap, 4)} · q35 公式 / 重設仍只屬治理參考',
        '當前分桶根因',
...
def test_venue_readiness_summary_component_surfaces_per_venue_contract():
    ...
    required_snippets = [
        ...
        'metadata 正常只代表觀測層，未完成憑證 / 委託確認 / 成交證據前不可 canary/live',
