...
def test_leaderboard_candidate_cache_hit_live_rebuilds_near_expiry_payload_cache(tmp_path, monkeypatch):
    monkeypatch.setattr(hb_parallel_runner, "PROJECT_ROOT", str(tmp_path))
    data_dir = tmp_path / "data"
    data_dir.mkdir(parents=True, exist_ok=True)
        ...
        json.dumps(
            {
                ...
                "leaderboard_payload_source": "latest_persisted_snapshot",
                "leaderboard_payload_stale": False,
                # Fresh at cache-hit time, but would expire before a 240s heartbeat
                # budget finishes unless we proactively live-rebuild it.
                "leaderboard_payload_cache_age_sec": 700,
                "top_model": {"selected_feature_profile": "core_only"},
                "alignment": alignment,
            }
        ),
...
def test_sync_high_conviction_topk_recomputes_model_gates_before_promotion(tmp_path, monkeypatch):
    ...
    row = payload["rows"][0]
    assert payload["deployable_rows"] == 0
    assert payload["risk_qualified_rows"] == 0
    assert payload["runtime_blocked_candidate_rows"] == 0
    assert row["deployable_verdict"] == "not_deployable"
