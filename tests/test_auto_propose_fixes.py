...
def test_sync_current_state_governance_issues_resolves_stale_q15_issue_when_circuit_breaker_is_active():
    ...
    assert "目前 5/50 勝" in breaker_add[2]
    assert "解除至少需要 15 勝" in breaker_add[2]
    assert "5/15" not in breaker_add[2]
    assert "最近 50" in breaker_add[2]
    assert "recent 50" not in breaker_add[2]
    assert "還差 10 勝" in breaker_add[3]
    assert "目前 5/50 勝" in breaker_add[3]
    assert "解除至少需要 15 勝" in breaker_add[3]
    assert not any(event[0] == "add" and event[1] == "#H_AUTO_CURRENT_BUCKET_SUPPORT" for event in events)
...
def test_sync_current_state_governance_issues_prefers_live_support_route_and_refreshes_breaker_context():
    ...
    assert breaker_issue["title"] == "熔斷解除條件未達（最近 50 筆目前 0/50 勝，解除至少需要 15 勝）"
    assert "即時部署阻塞語義切回熔斷解除條件" in breaker_issue["action"]
    assert "最近 50 筆目前 0/50 勝，解除至少需要 15 勝" in breaker_issue["action"]
