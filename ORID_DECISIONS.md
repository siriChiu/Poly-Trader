...
## 心跳 #1126-productization-nearest-gate ORID

### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32473 / Features=23891 / Labels=65559`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.75%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=16.0%` / `dominant_regime=chop(91.0%)` / `avg_quality=-0.1519` / `avg_pnl=-0.0075` / `alerts=label_imbalance,regime_concentration,regime_shift`。
...
- **If fail**：只要 docs / UI 再次把 `unsupported_exact_live_structure_bucket` 誤寫成 breaker-first、漏掉 q35 current-live bucket rows、把 reference-only patch 誤包裝成可部署 truth，或讓最高 ROI 但高回撤 / 負 worst-fold 的 top-k row 壓過 nearest-deployable 候選，就把 heartbeat 升級回 current-state governance blocker。
