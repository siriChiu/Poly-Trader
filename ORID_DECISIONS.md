...
## 心跳 #20260421-0023 ORID

### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31275 / Features=22693 / Labels=63075`；`simulated_pyramid_win=57.20%`。
...
- recent pathological slice：`window=500` / `win_rate=8.6%` / `dominant_regime=bull(88.0%)` / `avg_quality=-0.1904` / `avg_pnl=-0.0067` / `alerts=label_imbalance,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2745` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
