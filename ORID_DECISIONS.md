...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31542 / Features=22960 / Labels=63570`；`simulated_pyramid_win=57.34%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=83.0%` / `dominant_regime=bull(92.0%)` / `avg_quality=+0.4334` / `avg_pnl=+0.0131` / `alerts=label_imbalance,regime_concentration,regime_shift`。
...
- leaderboard live payload 已補上 stale-cache 正規化：cache / persisted snapshot 載入時會統一回傳 `target_candidates=[]`，不再讓 legacy rescoring 候選回灌 bounded-refresh API。
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3012` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
- **Action**：維持 current-live exact-support truth，並把 current live bucket support 與 reference-only patch 持續顯示清楚；本輪同步把 leaderboard cached payload 的 legacy `target_candidates` 清為 `[]`，避免 bounded-refresh contract 被舊快照回灌；下一步沿 recent pathological slice 與 exact-support accumulation 繼續追根因。
...
- **Verify**：browser `/`、browser `/execution/status`、browser `/lab`、`python scripts/hb_predict_probe.py`、`python scripts/live_decision_quality_drilldown.py`、`python scripts/recent_drift_report.py`、`curl http://127.0.0.1:8000/api/models/leaderboard`、`pytest tests/test_model_leaderboard.py tests/test_strategy_lab.py tests/test_frontend_decision_contract.py -q`。
