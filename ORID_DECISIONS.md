...
_最後更新：2026-04-22 18:35:49 CST_
...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31515 / Features=22933 / Labels=63540`；`simulated_pyramid_win=57.31%`。
...
- latest recent-window diagnostics：`latest_window=250` / `win_rate=82.8%` / `dominant_regime=chop(55.6%)` / `avg_quality=+0.4479` / `avg_pnl=+0.0121` / `alerts=label_imbalance`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2985` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
