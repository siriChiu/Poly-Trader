...
## 心跳 #20260424_1730 ORID

### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32165 / Features=23583 / Labels=64865`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.98%`。
...
- q15 current-live bucket truth：`current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=1/50` / `gap=49` / `support_route_verdict=exact_bucket_present_but_below_minimum`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=79.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=+0.3336` / `avg_pnl=+0.0042` / `alerts=regime_concentration,regime_shift`。
- current blocking pathological pocket：`blocking_window=500` / `win_rate=51.0%` / `dominant_regime=bull(99.6%)` / `avg_quality=+0.0893` / `avg_pnl=-0.0001` / `alerts=regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3633` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
### R｜感受直覺
- 這輪最需要防止的誤讀，是把 `1/50` 的 same-bucket support 或 `bull|CAUTION` 參考 patch 誤讀成已可部署；目前 live blocker 已切到 `under_minimum_exact_live_structure_bucket`。
...
### I｜意義洞察
1. **support truth ≠ deployment closure**：`support=1/50` 且 `support_route_verdict=exact_bucket_present_but_below_minimum` 只代表治理前進，還不能把 reference-only patch 升級成 runtime patch。
