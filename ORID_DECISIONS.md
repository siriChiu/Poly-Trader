...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32570 / Features=23988 / Labels=65754`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.78%`。
...
- latest recent-window diagnostics：`latest_window=1000` / `win_rate=45.4%` / `dominant_regime=bull(68.8%)` / `avg_quality=+0.0930` / `avg_pnl=+0.0002` / `alerts=regime_shift`。
...
- 實戰化 P0：`data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-02T06:12:35.963605+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=25/50` / `gap=25`；最接近部署候選 `model=logistic_regression` / `top_k=top_2pct` / `tier=runtime_blocked_oos_pass` / `bucket_rows=25/50` / `gap=25`，仍被矩陣新鮮度或即時分桶 / 支持 gate 擋下。
