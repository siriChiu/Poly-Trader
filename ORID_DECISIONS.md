...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31465 / Features=22883 / Labels=63449`；`simulated_pyramid_win=57.23%`。
...
- current live bucket truth：`current_live_structure_bucket=CAUTION|structure_quality_caution|q35` / `support=70/50` / `gap=0` / `support_route_verdict=exact_bucket_supported`。
- latest recent-window diagnostics：`latest_window=1000` / `win_rate=38.0%` / `dominant_regime=bull(81.6%)` / `avg_quality=+0.0665` / `avg_pnl=+0.0001` / `alerts=regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2935` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
### I｜意義洞察
1. **support truth ≠ deployment closure**：`support=70/50` 且 `support_route_verdict=exact_bucket_supported` 只代表 same-bucket support / patch 治理真相，不能跳過 runtime verify。
