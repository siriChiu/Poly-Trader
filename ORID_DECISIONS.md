...
## 心跳 #release-final-audit ORID

### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32099 / Features=23516 / Labels=64492`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.98%`。
...
- latest recent-window diagnostics：`latest_window=250` / `win_rate=36.0%` / `dominant_regime=bull(99.6%)` / `avg_quality=-0.0374` / `avg_pnl=-0.0039` / `alerts=regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3567` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
