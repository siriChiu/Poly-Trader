...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31479 / Features=22897 / Labels=63467`；`simulated_pyramid_win=57.25%`。
...
- latest recent-window diagnostics：`latest_window=1000` / `win_rate=38.8%` / `dominant_regime=bull(81.3%)` / `avg_quality=+0.0728` / `avg_pnl=+0.0003` / `alerts=regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2949` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
