...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32464 / Features=23882 / Labels=65547`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.74%`。
- 即時部署阻塞點：`deployment_blocker=circuit_breaker_active` / `streak=0` / `recent_window_wins=11/50` / `additional_recent_window_wins_needed=4`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=11.0%` / `dominant_regime=chop(86.0%)` / `avg_quality=-0.1999` / `avg_pnl=-0.0090` / `alerts=label_imbalance,regime_shift`。
...
### D｜決策行動
- **Owner**：即時執行治理 lane
- **Action**：維持熔斷優先真相，並把 q15 current-live bucket support truth 與 deployment closure 邊界持續顯示清楚；下一步沿 recent pathological slice 與解除條件繼續追根因；`/execution` 操作入口在同步中 / 已阻塞時只對買入 / 加倉與啟用自動模式 fail-closed，減碼保留；直接 API 買入 / 加倉也必須 409 暫停，減倉 / 賣出保留風險降低路徑。
