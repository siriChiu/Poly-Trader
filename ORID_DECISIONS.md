...
_最後更新：2026-04-30 06:19:44 CST_
...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32492 / Features=23910 / Labels=65592`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.73%`。
...
- q15 current-live bucket truth：`current_live_structure_bucket=CAUTION|structure_quality_caution|q15` / `support=0/50` / `gap=50` / `support_route_verdict=insufficient_support_everywhere`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=24.0%` / `dominant_regime=chop(91.0%)` / `avg_quality=-0.0671` / `avg_pnl=-0.0047` / `alerts=regime_concentration,regime_shift`。
...
- **If fail**：只要 docs / UI 再次把 `unsupported_exact_live_structure_bucket` 誤寫成 breaker-first、漏掉 q15 current-live bucket rows，或把 support closure 誤讀成 deployment closure，就把 heartbeat 升級回 current-state governance blocker。
