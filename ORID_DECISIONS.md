...
## 心跳 #20260420-fastfix ORID

### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31177 / Features=22595 / Labels=62886`；`simulated_pyramid_win=57.20%`。
- current-live blocker：`deployment_blocker=circuit_breaker_active` / `streak=177` / `recent_window_wins=0/50` / `additional_recent_window_wins_needed=15`。
- q15 current-live bucket truth：`current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_missing_exact_lane_proxy_only`。
- recent pathological slice：`window=100` / `win_rate=0.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=-0.2273` / `avg_pnl=-0.0089` / `alerts=constant_target,regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2648` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
