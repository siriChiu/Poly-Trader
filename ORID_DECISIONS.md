...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32487 / Features=23905 / Labels=65579`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.74%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=23.0%` / `dominant_regime=chop(96.0%)` / `avg_quality=-0.0825` / `avg_pnl=-0.0054` / `alerts=regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3920` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle；metadata smoke venue rows 已帶 proof_state / blockers / operator_next_action / verify_next。
- 實戰化 P0：`data/high_conviction_topk_oos_matrix.json` 已產出 `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6`；最接近部署候選 `model=logistic_regression` / `top_k=top_2pct` / `oos_roi=0.9324` / `max_drawdown=0.022` / `tier=runtime_blocked_oos_pass` 仍因即時分桶 / 支持 gate fail-closed；`/api/models/leaderboard.high_conviction_topk.support_context` 已改為在 `live_predict_probe.json` 較新時覆蓋 matrix 內 stale support snapshot，最新 truth 為 `BLOCK|structure_quality_block|q00 0/50 gap=50 no_support_proxy`。
...
4. **實戰化不是堆模型，而是可拒單部署治理**：high-conviction top-k OOS ROI gate 把六色帽 / 研究交叉分析轉成產品契約；排序先分離離線驗證 / 模型風控門檻與即時分桶 / 支持 gate，避免最高 ROI 但高回撤 / 負最差分折的列誤導部署決策；API 必須用 fresher live probe 覆蓋 stale top-k support snapshot，否則 Strategy Lab 會把舊 `q35` 支持脈絡誤讀成 current-live truth。
...
- **研究到產品 gate**：walk-forward OOS top-k matrix 已透過 `/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板可視化；operator 現在會先看到最接近部署候選（離線驗證 / 風控已過但只剩即時分桶 / 支持 gate 的 rows），並看到支持狀態、治理路徑與部署阻塞；`support_context` 以較新的 `data/live_predict_probe.json` 覆蓋 matrix 內 stale snapshot，保持 `BLOCK|structure_quality_block|q00 0/50 gap=50 no_support_proxy` current-live truth；即時分桶 / 支持 blockers 未解除前仍維持 fail-closed。
