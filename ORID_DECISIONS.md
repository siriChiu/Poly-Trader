...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32047 / Features=23465 / Labels=64204`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=57.09%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=91.0%` / `dominant_regime=bull(91.0%)` / `avg_quality=+0.5943` / `avg_pnl=+0.0175` / `alerts=label_imbalance,regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3517` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
- **Action**：維持 current-live exact-support truth，並把 q15 current-live bucket support 與 reference-only patch 持續顯示清楚；同時把 runtime closure summary 內殘留的 governance-route raw token 一併 humanize，避免 operator surface 再漏出 `exact_live_bucket_proxy_available` 這類 machine token。下一步沿 recent pathological slice 與 exact-support accumulation 繼續追根因。
- **Artifacts**：`ISSUES.md`、`ROADMAP.md`、`ORID_DECISIONS.md`、`data/live_predict_probe.json`、`data/live_decision_quality_drilldown.json`、`data/recent_drift_report.json`、`web/src/utils/runtimeCopy.ts`。
- **Verify**：browser `/execution`、browser `/execution/status`、browser `/lab`、`python scripts/hb_predict_probe.py`、`python scripts/live_decision_quality_drilldown.py`、`python scripts/recent_drift_report.py`、`pytest tests/test_frontend_decision_contract.py -q`、`cd web && npm run build`。
- **If fail**：只要 docs / UI 再次把 `unsupported_exact_live_structure_bucket` 誤寫成 breaker-first、漏掉 q15 current-live bucket rows，或讓 `exact_live_bucket_proxy_available` 這類 governance-route token 再次直接漏到 operator surface，就把 heartbeat 升級回 current-state governance blocker。
