...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32140 / Features=23558 / Labels=64807`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=57.02%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=+0.4422` / `avg_pnl=+0.0060` / `alerts=constant_target,regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3608` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
