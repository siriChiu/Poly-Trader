...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31405 / Features=22823 / Labels=63304`；`simulated_pyramid_win=57.27%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=chop(100.0%)` / `avg_quality=+0.6727` / `avg_pnl=+0.0200` / `alerts=constant_target,regime_concentration,regime_shift`。
- current blocking pathological pocket：`blocking_window=500` / `win_rate=29.6%` / `dominant_regime=bull(67.0%)` / `avg_quality=+0.0086` / `avg_pnl=-0.0004` / `alerts=regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2875` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
- compact live-pathology 摘要卡已改成 rows-aware：Dashboard / Strategy Lab 會分開顯示 `current bucket support`、`exact lane cohort`，必要時再補 `historical lane bucket`，避免 operator 把 broader cohort rows 誤讀成 current live exact support。
