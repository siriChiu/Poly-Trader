...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31353 / Features=22771 / Labels=63202`；`simulated_pyramid_win=57.21%`。
...
- q15 current-live bucket truth：`current_live_structure_bucket=CAUTION|structure_quality_caution|q15` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_missing_exact_lane_proxy_only`。
- recent pathological slice：`window=500` / `win_rate=19.4%` / `dominant_regime=bull(77.2%)` / `avg_quality=-0.0910` / `avg_pnl=-0.0036` / `alerts=label_imbalance,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2823` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
