...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=33223 / Features=24410 / Labels=66411`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.81%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=48.0%` / `dominant_regime=chop(97.0%)` / `avg_quality=+0.1227` / `avg_pnl=+0.0001` / `alerts=regime_concentration,regime_shift`。
- leaderboard / governance：`leaderboard_count=6` / `selected_feature_profile=core_only` / `support_aware_profile=current_full_no_bull_collapse_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.1m`。
- source / venue blockers：`blocked_sparse_features=8`；top source blockers=`fin_netflow(source_auth_blocked/auth_missing, coverage=0.0%, archive_window=0.0%, forward_archive=ready)` / `claw(source_auth_blocked/auth_missing, coverage=14.7%, archive_window=87.7%, forward_archive=ready)` / `claw_intensity(source_auth_blocked/auth_missing, coverage=14.7%, archive_window=87.7%, forward_archive=ready)` / `nest_pred(source_tls_verify_failed/tls_verify_failed, coverage=16.3%, archive_window=97.4%, forward_archive=ready)`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4044` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle；metadata smoke venue rows 已帶 proof_state / blockers / operator_next_action / verify_next。
...
- 實戰化 P0：`data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-14T17:03:31.230984+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=28/50` / `gap=22`；最接近部署候選 `model=logistic_regression` / `top_k=top_2pct` / `tier=runtime_blocked_oos_pass` / `bucket_rows=28/50` / `gap=22`，仍被矩陣新鮮度或即時分桶 / 支持 / release gate 擋下；Strategy Lab leaderboard tab 已 browser 驗證：高信心 OOS Top-K 顯示 `支持樣本 28 / 50（缺 22）`、`下一步補齊 22 筆當前分桶支持樣本`、`熔斷解除條件未啟用`，且舊 copy `請先解除熔斷 / 執行保護` 未出現。
...
4. **實戰化不是堆模型，而是可拒單部署治理**：high-conviction top-k OOS ROI gate 把六色帽 / 研究交叉分析轉成產品契約；排序先分離離線驗證 / 模型風控門檻與即時分桶 / 支持 gate，避免最高 ROI 但高回撤 / 負最差分折的列誤導部署決策；操作員 copy 必須直接顯示剩餘支持樣本缺口與下一步，而不是只說解除熔斷。
...
- **研究到產品 gate**：walk-forward OOS top-k matrix 已透過 `/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板可視化；operator 現在會先看到最接近部署候選（離線驗證 / 風控已過但只剩矩陣新鮮度 / 即時分桶 / 支持 / release gate 的 rows），並看到矩陣新鮮度、支持狀態、治理路徑、部署阻塞、即時分桶、樣本數、support gap 下一步、recent-window wins、required wins 與 additional wins needed；矩陣過期或即時分桶 / 支持 / release blockers 未解除前仍維持 fail-closed。
...
- **Verify**：browser `/lab` leaderboard tab（`高信心 OOS Top-K`、`支持樣本 28 / 50（缺 22）`、`下一步補齊 22 筆當前分桶支持樣本`、舊 copy 0 次）、`/api/models/leaderboard.high_conviction_topk` smoke（runtime-blocked=6、deployable=0、gap=22）、`python -m pytest tests/test_frontend_decision_contract.py tests/test_topk_walkforward_precision.py -q`、`cd web && npm run build`、`python scripts/heartbeat_harness_check.py --format text`。
