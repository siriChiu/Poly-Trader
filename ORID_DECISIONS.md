...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32081 / Features=23499 / Labels=64291`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=57.04%`。
- current-live blocker：`deployment_blocker=circuit_breaker_active` / `streak=3` / `recent_window_wins=7/50` / `additional_recent_window_wins_needed=8`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=45.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=+0.1441` / `avg_pnl=+0.0014` / `alerts=regime_concentration,regime_shift`。
