...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32503 / Features=23921 / Labels=65608`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.73%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=25.0%` / `dominant_regime=chop(84.0%)` / `avg_quality=-0.0501` / `avg_pnl=-0.0040` / `alerts=regime_shift`。
...
3. **docs overwrite sync + row-level API contract 是護欄，不是主阻塞**：current-state docs 已 overwrite sync 到 `issues.json / live probe / drilldown` 最新 truth；`/execution` 快捷列、直接 `/api/trade`、Execution Status、venue metadata rows 都維持 fail-closed / proof-visible；high-conviction Top-K 的 row-level support/blocker 欄位已補齊，避免 Strategy Lab 只能看摘要層 blocker。
...
- **研究到產品 gate**：walk-forward OOS top-k matrix 已透過 `/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板可視化；operator 現在會先看到最接近部署候選（離線驗證 / 風控已過但只剩即時分桶 / 支持 gate 的 rows），且每列直接帶 `support_route / support_governance_route / deployment_blocker / runtime_closure_state / current_live_structure_bucket`；即時分桶 / 支持 blockers 未解除前仍維持 fail-closed。
