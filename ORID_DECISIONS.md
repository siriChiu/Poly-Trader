...
## 心跳 #20260420-0801 ORID

### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31185 / Features=22603 / Labels=62907`；`simulated_pyramid_win=57.18%`。
- current-live blocker：`deployment_blocker=circuit_breaker_active` / `streak=190` / `recent_window_wins=0/50` / `additional_recent_window_wins_needed=15`。
...
- recent pathological slice：`window=100` / `win_rate=0.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=-0.2363` / `avg_pnl=-0.0095` / `alerts=constant_target,regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2656` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
