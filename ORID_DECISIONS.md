...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31290 / Features=22708 / Labels=63092`；`simulated_pyramid_win=57.22%`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2760` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
