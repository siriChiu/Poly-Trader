...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=33003 / Features=24333 / Labels=66292`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.82%`。
...
- latest recent-window diagnostics：`latest_window=250` / `win_rate=66.4%` / `dominant_regime=chop(94.4%)` / `avg_quality=+0.2948` / `avg_pnl=+0.0045` / `alerts=regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3994` / `archive_window_coverage_pct=0.0`；nest_pred=`quality_flag=source_tls_verify_failed` / `latest_status=tls_verify_failed` / `trust_policy=tls_verify_required_no_insecure_fallback` / `tls_verification=required`，Polymarket Gamma 目前必須先修 TLS trust chain，不可用 insecure fallback；venue proof 仍缺 credential / order ack / fill lifecycle；metadata smoke venue rows 已帶 proof_state / blockers / operator_next_action / verify_next。
- 實戰化 P0：`data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-13T14:02:38.768593+00:00` / `freshness=fresh` / `age_min=12.4` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=37/50` / `gap=13`；最接近部署候選 `model=logistic_regression` / `top_k=top_2pct` / `tier=runtime_blocked_oos_pass` / `bucket_rows=37/50` / `gap=13`，仍被矩陣新鮮度或即時分桶 / 支持 gate 擋下。
...
### R｜感受直覺
- 這輪最需要防止的誤讀，是把 `37/50` 的 same-bucket support 或 `bull|CAUTION` 參考 patch 誤讀成已可部署；目前 live blocker 已切到 `under_minimum_exact_live_structure_bucket`。
