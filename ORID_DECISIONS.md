...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32616 / Features=24012 / Labels=65762`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.78%`。
...
- latest recent-window diagnostics：`latest_window=1000` / `win_rate=45.7%` / `dominant_regime=bull(68.5%)` / `avg_quality=+0.0952` / `avg_pnl=+0.0003` / `alerts=regime_shift`。
...
- current live 已落在 `bull/CAUTION/CAUTION|structure_quality_caution|q15`；如果 UI / docs 沒同步 latest artifacts，operator 很容易把 spillover pocket、舊 bucket，或 `/api/status` 尚未返回的 loading 狀態誤讀成可操作 runtime 真相。
