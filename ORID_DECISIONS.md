...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31350 / Features=22768 / Labels=63182`；`simulated_pyramid_win=57.22%`。
...
- q15 current-live bucket truth：`current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=88/50` / `gap=0` / `support_route_verdict=exact_bucket_supported`。
- recent pathological slice：`window=500` / `win_rate=18.0%` / `dominant_regime=bull(78.6%)` / `avg_quality=-0.1048` / `avg_pnl=-0.0040` / `alerts=label_imbalance,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2820` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
### I｜意義洞察
1. **support accumulation ≠ deployment closure**：`support=88/50` 且 `support_route_verdict=exact_bucket_supported` 只代表治理前進，還不能把 reference patch 升級成 runtime patch。
