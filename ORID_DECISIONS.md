...
_最後更新：2026-04-29 14:21:59 CST_
...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32462 / Features=23880 / Labels=65545`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.74%`。
- 即時部署阻塞點：`deployment_blocker=circuit_breaker_active` / `streak=0` / `recent_window_wins=10/50` / `additional_recent_window_wins_needed=5`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=10.0%` / `dominant_regime=chop(85.0%)` / `avg_quality=-0.2093` / `avg_pnl=-0.0093` / `alerts=label_imbalance,regime_shift`。
