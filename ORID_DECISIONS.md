...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31524 / Features=22942 / Labels=63547`；`simulated_pyramid_win=57.31%`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2994` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
