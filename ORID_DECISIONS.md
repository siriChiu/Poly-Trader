...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32143 / Features=23561 / Labels=64813`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=57.01%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=96.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=+0.4247` / `avg_pnl=+0.0059` / `alerts=label_imbalance,regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3611` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
- 本輪 operator UX / Strategy Lab 穩定性 patch：support gap 文案統一改為 `缺口`，並把 leaderboard 兩條前端 API timeout 提升至 20s，防止 current-state leaderboard payload 被預設 8s timeout 誤判。
