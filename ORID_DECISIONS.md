...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31293 / Features=22711 / Labels=63102`；`simulated_pyramid_win=57.23%`。
...
- recent pathological slice：`window=500` / `win_rate=11.0%` / `dominant_regime=bull(85.6%)` / `avg_quality=-0.1712` / `avg_pnl=-0.0061` / `alerts=label_imbalance,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2763` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
