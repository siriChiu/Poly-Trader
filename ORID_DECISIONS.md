...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31393 / Features=22811 / Labels=63295`；`simulated_pyramid_win=57.28%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=chop(100.0%)` / `avg_quality=+0.6681` / `avg_pnl=+0.0200` / `alerts=constant_target,regime_concentration,regime_shift`。
- current blocking pathological pocket：`blocking_window=500` / `win_rate=28.8%` / `dominant_regime=bull(67.8%)` / `avg_quality=+0.0014` / `avg_pnl=-0.0005` / `alerts=regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2863` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
