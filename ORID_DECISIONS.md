...
## 心跳 #20260424_1602 ORID

### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32161 / Features=23579 / Labels=64852`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.98%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=79.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=+0.3318` / `avg_pnl=+0.0045` / `alerts=regime_concentration,regime_shift`。
- current blocking pathological pocket：`blocking_window=500` / `win_rate=50.0%` / `dominant_regime=bull(99.6%)` / `avg_quality=+0.0817` / `avg_pnl=-0.0002` / `alerts=regime_concentration,regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3629` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
- 產品化 patch：Dashboard 已把 `current_bucket_root_cause` 直接放進 ConfidenceIndicator；heartbeat runner full-mode candidate lanes 已加入 bounded artifact reuse + timeout guardrail，避免 single-lane silent ablation 卡住完整心跳。
...
- **Action**：維持 current-live exact-support truth，並把 q15 current-live bucket support、reference-only patch、current-bucket root cause 與 cron-safe heartbeat guardrail 持續顯示清楚；下一步沿 recent pathological slice 與 exact-support accumulation 繼續追根因。
