...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=32467 / Features=23885 / Labels=65553`；歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`；`simulated_pyramid_win=56.74%`。
...
- q35 current-live bucket truth：`current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block`。
- latest recent-window diagnostics：`latest_window=100` / `win_rate=14.0%` / `dominant_regime=chop(89.0%)` / `avg_quality=-0.1718` / `avg_pnl=-0.0082` / `alerts=label_imbalance,regime_shift`。
