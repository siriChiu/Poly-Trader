...
### O｜客觀事實
- collect + diagnostics refresh 完成：`Raw=31403 / Features=22821 / Labels=63303`；`simulated_pyramid_win=57.27%`。
...
- latest recent-window diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=chop(100.0%)` / `avg_quality=+0.6720` / `avg_pnl=+0.0200` / `alerts=constant_target,regime_concentration,regime_shift`。
- current blocking pathological pocket：`blocking_window=500` / `win_rate=29.4%` / `dominant_regime=bull(67.2%)` / `avg_quality=+0.0069` / `avg_pnl=-0.0004` / `alerts=regime_shift`。
...
- source / venue blockers：`blocked_sparse_features=8`；fin_netflow=`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2873` / `archive_window_coverage_pct=0.0`；venue proof 仍缺 credential / order ack / fill lifecycle。
...
- Dashboard / Strategy Lab top-level 摘要已先顯示 `current bucket 0/50 · gap 50`，把 q15 same-bucket support shortage 提前到 operator 首屏，不再只留在 deeper diagnostics。
