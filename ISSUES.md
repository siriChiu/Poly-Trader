...
_最後更新：2026-04-29 18:14:56 CST_
...
## 當前主線事實
- **最新 full heartbeat #1123 已完成 collect + diagnostics refresh**
  - `Raw=32467 / Features=23885 / Labels=65553`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=14.0%` / `dominant_regime=chop(89.0%)` / `avg_quality=-0.1718` / `avg_pnl=-0.0082` / `alerts=label_imbalance,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3900` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 14/50，還差 1 勝；當前 q35 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 當前 q35 分桶 support / 治理背景
...
- same-bucket truth：`bucket=CAUTION|base_caution_regime_or_bias|q35` / `support=0/50` / `support_route_verdict=exact_bucket_unsupported_block` / `support_governance_route=exact_live_bucket_proxy_available`
...
- 目前真相：`mode=paper_shadow_only_until_oos_and_support_deployable` / `validation=walk_forward_oos_topk_matrix` / `top_k_grid=1%,2%,5%,10%` / `output_artifact=data/high_conviction_topk_oos_matrix.json`
...
- 下一步：依照六色帽會議與量化研究交叉分析，優先建立 walk-forward OOS top-k matrix；只讓通過 ROI、勝率、max drawdown、profit factor、worst fold、minimum trades 與 current-live support 的候選進入 deployment profile。未通過前維持 paper/shadow/hold-only。
- 驗證：
  - data/high_conviction_topk_oos_matrix.json
  - Strategy Lab leaderboard shows top-k OOS ROI/win-rate/DD/trades/deployable verdict
  - pytest tests/test_model_leaderboard.py tests/test_strategy_lab.py tests/test_hb_parallel_runner.py -q

### P1. model stability still needs work (cv=0.6507, cv_std=0.1006, cv_worst=0.5501)
- 目前真相：`cv_accuracy=0.650733752620545` / `cv_std=0.1006289308176101` / `cv_worst=0.550104821802935`
...
- 目前真相：`bucket=CAUTION|base_caution_regime_or_bias|q35` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `governance_route=exact_live_bucket_proxy_available`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3900` / `archive_window_coverage_pct=0.0`
