...
## 當前主線事實
- **最新 fast heartbeat #20260425_issues_debug 已完成 diagnostics refresh（collect skipped）**
  - `Raw=32199 / Features=23617 / Labels=64947`
...
- **canonical current-live blocker 以 latest runtime truth 為主**
  ...
  - `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=122/50` / `gap=0` / `support_route_verdict=exact_bucket_supported`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=500` / `win_rate=51.2%` / `dominant_regime=bull(99.4%)` / `avg_quality=+0.1063` / `avg_pnl=+0.0004` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3666` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`deployment_blocker=decision_quality_below_trade_floor` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=122` / `minimum_support_rows=50` / `gap_to_minimum=0` / `support_route_verdict=exact_bucket_supported`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3666` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_macro_plus_stable_4h` / `support_aware_profile=core_macro_plus_stable_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment`
...
- 目前真相：`window=500` / `interpretation=regime_concentration` / `win_rate=0.512` / `dominant_regime=bull` / `dominant_regime_share=0.994` / `avg_pnl=0.0004`
