...
## 當前主線事實
- **最新 full heartbeat #1122 已完成 collect + diagnostics refresh**
...
- **source / venue blockers 仍開啟**
  ...
  - venue：`live exchange credential / order ack lifecycle / fill lifecycle` 尚未有 runtime-backed proof；`data/execution_metadata_smoke.json` 的 `results[]` 現在直接持久化 per-venue `proof_state / readiness_scope / blockers / operator_next_action / verify_next`，API/UI/Docs 不再需要把 `ok=true` 猜成 readiness。
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 13/50，還差 2 勝；當前 q35 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 當前 q35 分桶 support / 治理背景
...
- same-bucket truth：`bucket=CAUTION|structure_quality_caution|q35` / `support=0/50` / `support_route_verdict=exact_bucket_unsupported_block` / `support_governance_route=exact_live_bucket_proxy_available`
...
- 目前真相：`bucket=CAUTION|structure_quality_caution|q35` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `governance_route=exact_live_bucket_proxy_available`
...
- API/artifact/UI contract：`data/execution_metadata_smoke.json` 的 raw `results[]` 已持久化 `proof_state / readiness_scope / blockers / operator_next_action / verify_next`；`/api/status` 再轉成 `execution_metadata_smoke.venues[]` 給 Dashboard、`/execution/status`、`/execution`、`/lab` 直接顯示每個場館的實單證據缺口，不再只靠 metadata OK/FAIL 猜測 readiness。
...
- 驗證：
  ...
  - browser /lab
  - data/execution_metadata_smoke.json (`results[].proof_state / blockers / operator_next_action / verify_next`)
  - pytest tests/test_execution_metadata_smoke.py tests/test_server_startup.py::test_load_execution_metadata_smoke_summary_reports_freshness tests/test_server_startup.py::test_load_execution_metadata_smoke_summary_marks_disabled_venue_as_metadata_only -q
  - pytest tests/test_frontend_decision_contract.py::test_execution_status_reuses_shared_venue_readiness_component_and_explains_public_only_balances tests/test_hb_parallel_runner.py::test_overwrite_current_state_docs_writes_current_state_markdown -q

### P1. fin_netflow remains source_auth_blocked because COINGLASS_API_KEY is missing
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3899` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=distribution_pathology` / `win_rate=0.13` / `dominant_regime=chop` / `dominant_regime_share=0.88` / `avg_pnl=-0.0085`
