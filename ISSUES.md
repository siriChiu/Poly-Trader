...
_最後更新：2026-05-16 18:15:21 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1278-productization 已完成 collect + diagnostics refresh**
  - `Raw=33383 / Features=24523 / Labels=66635`
...
- **canonical 即時部署阻塞仍是熔斷優先真相**
  ...
  - support progress：`status=stalled_under_minimum` / `reason=—` / `regression_basis=same_identity_same_semantic_signature` / `current_rows=0` / `minimum_rows=50` / `gap_to_minimum=50` / `support_rows_needed=50` / `previous_rows=—` / `delta_vs_previous=—` / `legacy_supported_reference=—` / `stagnant_run_count=2` / `stalled_support_accumulation=True` / `escalate_to_blocker=False`
  - 本輪產品化 patch：`scripts/hb_predict_probe.py` 已 canonicalize 非 q15 current-live `0/50` overlay，不再把 q15 audit 的 `insufficient_support_everywhere` / proxy reference 誤升級為 top-level route；top-level 與 `deployment_blocker_details` 現在一致輸出 `support_route_verdict=exact_bucket_unsupported_block` / `support_governance_route=no_support_proxy` / `support_progress.status=stalled_under_minimum`。
  - 驗證：`pytest tests/test_hb_predict_probe.py -q`（26 passed）、`pytest tests/test_api_feature_history_and_predictor.py -q`（66 passed）、`python scripts/hb_predict_probe.py`、`python scripts/hb_parallel_runner.py --fast --hb 1278-productization`。
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=55.2%` / `dominant_regime=chop(81.2%)` / `avg_quality=+0.2012` / `avg_pnl=+0.0032` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - top source blockers：`fin_netflow(source_auth_blocked/auth_missing, coverage=0.0%, archive_window=0.0%, forward_archive=ready)` / `claw(source_auth_blocked/auth_missing, coverage=14.6%, archive_window=86.0%, forward_archive=ready)` / `claw_intensity(source_auth_blocked/auth_missing, coverage=14.6%, archive_window=86.0%, forward_archive=ready)` / `nest_pred(source_tls_verify_failed/tls_verify_failed, coverage=16.2%, archive_window=95.6%, forward_archive=ready)`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4121` / `archive_window_coverage_pct=0.0`
...
### P0. 連續 57 筆 1440m simulated_pyramid_win=0
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=57` / `all_horizon_losing_streak=19`
- 下一步：檢查 recent canonical labels / regime breakdown / circuit breaker；必要時升級為 distribution-aware drift 調查

### P0. 熔斷解除條件仍是唯一即時部署阻塞點
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=57` / `recent 50 wins=0/50` / `additional_recent_window_wins_needed=15`
...
- support progress：`status=stalled_under_minimum` / `reason=—` / `regression_basis=same_identity_same_semantic_signature` / `current_rows=0` / `minimum_rows=50` / `gap_to_minimum=50` / `support_rows_needed=50` / `previous_rows=—` / `delta_vs_previous=—` / `legacy_supported_reference=—` / `stagnant_run_count=2` / `stalled_support_accumulation=True` / `escalate_to_blocker=False`
- 已鎖 regression：非 q15 current-live `0/50` 不得再輸出 `exact_bucket_missing_proxy_reference_only` 或 `exact_live_bucket_present_but_below_minimum` 這類不一致 top-level truth。
...
### P1. model stability still needs work (cv=0.6549, cv_std=0.0871, cv_worst=0.5678)
- 目前真相：`cv_accuracy=0.6548545678000819` / `cv_std=0.0870544858664482` / `cv_worst=0.5678000819336337`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4121` / `archive_window_coverage_pct=0.0`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-16T10:02:53.006505+00:00` / `freshness=fresh` / `age_min=10.0` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=0/50` / `gap=50` / `release_ready=False` / `recent_window_wins=0/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=15` / `current_recent_window_win_rate=0.000` / `support_progress_status=stalled_under_minimum` / `support_progress_reason=current live exact support 連續 heartbeat 停在同一數量，屬於 support accumulation 停滯。` / `regression_basis=same_identity_same_semantic_signature` / `delta_vs_previous=0` / `previous_rows=0` / `support_rows_needed=50` / `stagnant_run_count=2` / `stalled_support_accumulation=True` / `escalate_to_blocker=False`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
