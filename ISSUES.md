...
_最後更新：2026-04-22 15:20:27 CST_
...
## 當前主線事實
- **最新 fast heartbeat #20260422x 已完成 collect + diagnostics refresh**
  - `Raw=31502 / Features=22920 / Labels=63507`
  - `simulated_pyramid_win=57.29%`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=81.6%` / `dominant_regime=chop(60.4%)` / `avg_quality=+0.4284` / `avg_pnl=+0.0110` / `alerts=label_imbalance`
  - `blocking_window=1000` / `win_rate=39.4%` / `dominant_regime=bull(81.3%)` / `avg_quality=+0.0790` / `avg_pnl=+0.0006` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2972` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=1000` / `win_rate=39.4%` / `dominant_regime=bull(81.3%)` / `avg_quality=+0.0790` / `avg_pnl=+0.0006` / `alerts=regime_shift`
- latest diagnostics：`latest_window=250` / `win_rate=81.6%` / `dominant_regime=chop(60.4%)` / `avg_quality=+0.4284` / `avg_pnl=+0.0110` / `alerts=label_imbalance`
- 病態切片：`alerts=regime_shift` / `tail_streak=—` / `top_shift=feat_4h_bias200,feat_vwap_dev,feat_dxy` / `new_compressed=feat_vix`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=1000, alerts=['regime_shift'], win_rate=0.3940, delta_vs_full=-0.2324, dominant_regime=bull(81.30%), interpretation=regime_concentration, avg_pnl=+0.0006, avg_quality=0.0790, avg_dd_penalty=0.2385, spot_long_win_rate=0.1870, feature_diag=variance:7/56, frozen:0, compressed:7, expected_static:0, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=52x1 since 2026-04-21 00:05:39.160858 -> 2026-04-21 08:09:08.315057, adverse_streak=273x0 since 2026-04-17 13:41:12.414408 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.922, delta_vs_prev=-0.528, prev_quality=0.5354, quality_delta_vs_prev=-0.4564, prev_pnl=0.0145, pnl_delta_vs_prev=-0.0139, top_shift_examples=feat_4h_bias200(4.2494→7.6877,Δσ=0.6294)/feat_vwap_dev(-0.3088→-0.1744,Δσ=0.5978)/feat_dxy(98.4767→98.1328,Δσ=0.332), new_compressed=feat_vix, compressed_examples=feat_body(0.0001/998)/feat_ear(0.0103/996)/feat_tongue(0.0143/997), overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-21 07:34:47.737341:1:bull:0.6082/2026-04-21 07:53:24.684760:1:bull:0.7116/2026-04-21 08:09:08.315057:1:bull:0.706, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2972` / `archive_window_coverage_pct=0.0`
