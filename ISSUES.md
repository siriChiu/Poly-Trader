...
## 當前主線事實
- **最新 fast heartbeat #1142 已完成 collect + diagnostics refresh**
  - `Raw=32494 / Features=23912 / Labels=65594`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=24.0%` / `dominant_regime=chop(90.0%)` / `avg_quality=-0.0650` / `avg_pnl=-0.0046` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3927` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3927` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.24` / `dominant_regime=chop` / `dominant_regime_share=0.9` / `avg_pnl=-0.0046`
