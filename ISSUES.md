...
_最後更新：2026-05-15 05:10:45 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1233 已完成 collect + diagnostics refresh**
  - `Raw=33235 / Features=24422 / Labels=66434`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=50.0%` / `dominant_regime=chop(97.0%)` / `avg_quality=+0.1531` / `avg_pnl=+0.0021` / `alerts=regime_concentration,regime_shift`
  - `blocking_window=250` / `win_rate=60.4%` / `dominant_regime=chop(94.4%)` / `avg_quality=+0.2342` / `avg_pnl=+0.0030` / `alerts=regime_concentration,regime_shift`
...
- latest matrix：`generated_at=2026-05-14T21:02:14.064832+00:00` / `freshness=fresh` / `age_min=8.5` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24309` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35` / `current_live_structure_bucket_rows=28/50` / `current_live_structure_bucket_gap_to_minimum=22`
...
### P1. model stability still needs work (cv=0.6344, cv_std=0.0648, cv_worst=0.5695)
- 目前真相：`cv_accuracy=0.6343621399176955` / `cv_std=0.06481481481481483` / `cv_worst=0.5695473251028806`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4052` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`bucket=CAUTION|base_caution_regime_or_bias|q35` / `support=28/50` / `gap=22` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_discriminative_reweight_crosses_floor_but_execution_blocked` / `runtime_gap_to_floor=0.0713`
- q35 scaling audit 已指出目前不是單點 bias50 closure： `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_discriminative_reweight_crosses_floor_but_execution_blocked` / `runtime_gap_to_floor=0.0713` / `redesign_entry_quality=0.5565` / `redesign_allowed_layers=0` / `positive_discriminative_gap=True` / `execution_blocked_after_floor_cross=True`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-14T21:02:14.064832+00:00` / `freshness=fresh` / `age_min=8.5` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=28/50` / `gap=22`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
