...
_最後更新：2026-05-15 06:15:53 CST_
...
## 當前主線事實
- **最新 full heartbeat #1234 已完成 collect + diagnostics refresh**
  - `Raw=33237 / Features=24424 / Labels=66438`
...
- **canonical current-live blocker 已切到 current-live exact-support truth**
  ...
  - support progress：`status=stalled_under_minimum` / `regression_basis=same_identity_same_semantic_signature` / `legacy_supported_reference=—` / `stagnant_run_count=5` / `stalled_support_accumulation=True` / `escalate_to_blocker=True`
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=50.0%` / `dominant_regime=chop(97.0%)` / `avg_quality=+0.1558` / `avg_pnl=+0.0022` / `alerts=regime_concentration,regime_shift`
  - `blocking_window=250` / `win_rate=60.4%` / `dominant_regime=chop(94.8%)` / `avg_quality=+0.2345` / `avg_pnl=+0.0030` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - top source blockers：`fin_netflow(source_auth_blocked/auth_missing, coverage=0.0%, archive_window=0.0%, forward_archive=ready)` / `claw(source_auth_blocked/auth_missing, coverage=14.7%, archive_window=87.5%, forward_archive=ready)` / `claw_intensity(source_auth_blocked/auth_missing, coverage=14.7%, archive_window=87.5%, forward_archive=ready)` / `nest_pred(source_tls_verify_failed/tls_verify_failed, coverage=16.2%, archive_window=97.2%, forward_archive=ready)`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4053` / `archive_window_coverage_pct=0.0`
...
- **startup feature continuity false-deferred 已關閉**
  - 根因：startup lane 用 `BTCUSDT` 查 recent feature continuity，但既有 feature rows 部分以 `BTC/USDT` 保存，exact-symbol 比對把同一市場誤判成 `missing_before=234 / remaining_missing=234 / repair_deferred=True`。
  - Patch 後 `BTCUSDT` 與 `BTC/USDT` 視為同一市場；runtime probe 顯示 `missing_before=0 / remaining_missing=0 / repair_deferred=False`，避免 API readiness surfaces 繼續顯示假 deferred blocker。
...
- latest matrix：`generated_at=2026-05-14T22:02:13.861409+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24311` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35` / `current_live_structure_bucket_rows=28/50` / `current_live_structure_bucket_gap_to_minimum=22`
...
### P1. model stability still needs work (cv=0.6335, cv_std=0.0596, cv_worst=0.5738)
- 目前真相：`cv_accuracy=0.6334841628959276` / `cv_std=0.05964623611682435` / `cv_worst=0.5738379267791033`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4053` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`bucket=CAUTION|base_caution_regime_or_bias|q35` / `support=28/50` / `gap=22` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_discriminative_reweight_crosses_floor_but_execution_blocked` / `runtime_gap_to_floor=0.0322`
- q35 scaling audit 已指出目前不是單點 bias50 closure： `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_discriminative_reweight_crosses_floor_but_execution_blocked` / `runtime_gap_to_floor=0.0322` / `redesign_entry_quality=0.552` / `redesign_allowed_layers=0` / `positive_discriminative_gap=True` / `execution_blocked_after_floor_cross=True`
