...
_最後更新：2026-04-21 22:34:12 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31405 / Features=22823 / Labels=63304`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=100.0%` / `dominant_regime=chop(100.0%)` / `avg_quality=+0.6727` / `avg_pnl=+0.0200` / `alerts=constant_target,regime_concentration,regime_shift`
  - `blocking_window=500` / `win_rate=29.6%` / `dominant_regime=bull(67.0%)` / `avg_quality=+0.0086` / `avg_pnl=-0.0004` / `alerts=regime_shift`
- **leaderboard / governance 仍維持 dual-role contract**
  - `leaderboard_count=6` / `selected_feature_profile=core_plus_macro` / `support_aware_profile=core_plus_macro` / `governance_contract=dual_role_governance_active` / `current_closure=dual_role_split_but_aligned`
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2875` / `archive_window_coverage_pct=0.0`
...
- **Dashboard / Strategy Lab compact live-lane card 已拆開「current bucket support」與「exact-lane cohort」語義**
  - compact card 現在會分開顯示 `current bucket ...`、`exact lane cohort ...`、以及必要時的 `historical lane bucket ...`，避免把 broader cohort rows 誤讀成 current bucket exact support
...
- 目前真相：`deployment_blocker=under_minimum_exact_live_structure_bucket` / `bucket=CAUTION|structure_quality_caution|q35` / `support=12/50` / `gap=38` / `runtime_closure_state=patch_inactive_or_blocked`
...
- 目前真相：`window=500` / `win_rate=29.6%` / `dominant_regime=bull(67.0%)` / `avg_quality=+0.0086` / `avg_pnl=-0.0004` / `alerts=regime_shift`
- latest diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=chop(100.0%)` / `avg_quality=+0.6727` / `avg_pnl=+0.0200` / `alerts=constant_target,regime_concentration,regime_shift`
...
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['regime_shift'], win_rate=0.2960, delta_vs_full=-0.3305, dominant_regime=bull(67.00%), interpretation=regime_concentration, avg_pnl=-0.0004, avg_quality=0.0086, avg_dd_penalty=0.2603, spot_long_win_rate=0.1600, feature_diag=variance:8/56, frozen:0, compressed:8, expected_static:2, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=142x1 since 2026-04-19 10:13:18.192660 -> 2026-04-20 15:07:08.841646, adverse_streak=191x0 since 2026-04-18 14:33:06.990329 -> 2026-04-19 01:10:17.732530, prev_win_rate=0.55, delta_vs_prev=-0.254, prev_quality=0.1875, quality_delta_vs_prev=-0.1789, prev_pnl=0.0017, pnl_delta_vs_prev=-0.0021, top_shift_examples=feat_4h_bias20(1.9407→0.36,Δσ=0.955)/feat_4h_vol_ratio(0.79→0.2716,Δσ=0.9305)/feat_4h_bb_pct_b(0.7002→0.4062,Δσ=0.9147), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.0084/498)/feat_tongue(0.0119/500), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-20 14:05:32.441280:1:chop:0.5817/2026-04-20 14:07:08.841646:1:chop:0.5867/2026-04-20 15:07:08.841646:1:chop:0.5512, adverse_examples=2026-04-19 00:40:09.929353:0:bull:-0.2753/2026-04-19 01:01:04.227452:0:bull:-0.3173/2026-04-19 01:10:17.732530:0:bull:-0.2383
- 驗證：
...
- 目前真相：`reproduced=True` / `heartbeat=fast` / `elapsed_seconds=108.2` / `timed_out_before_completion=True` / `completed_lanes_before_timeout=hb_collect, regime_ic, full_ic, recent_drift_report … (+8)` / `skipped_lanes=feature_group_ablation, bull_4h_pocket_ablation, hb_leaderboard_candidate_probe`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2875` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_plus_macro` / `support_aware_profile=core_plus_macro` / `governance_contract=dual_role_governance_active` / `current_closure=dual_role_split_but_aligned`
