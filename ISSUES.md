...
_最後更新：2026-04-30 15:08:37 CST_
...
## 當前主線事實
- **最新 full heartbeat #1152 已完成 collect + diagnostics refresh**
  - `Raw=32509 / Features=23927 / Labels=65619`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3942` / `archive_window_coverage_pct=0.0`
...
- **語義重訂 support-progress 操作員文案已收斂**
  - shared runtime copy 現在會把 `semantic_rebaseline_under_minimum` 的樣本變化顯示為相對舊版已就緒參考（例：`53 → 4，僅供參考`），不再只顯示上一輪 `+2` 而讓 operator 誤判 same-identity support recovery。
...
### P1. model stability still needs work (cv=0.6526, cv_std=0.1154, cv_worst=0.5373)
- 目前真相：`cv_accuracy=0.6526381909547738` / `cv_std=0.11536850921273029` / `cv_worst=0.5372696817420436`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3942` / `archive_window_coverage_pct=0.0`
