...
_最後更新：2026-04-30 12:16:40 CST_
...
## 當前主線事實
- **最新 full heartbeat #1148 已完成 collect + diagnostics refresh**
  - `Raw=32503 / Features=23921 / Labels=65608`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=25.0%` / `dominant_regime=chop(84.0%)` / `avg_quality=-0.0501` / `avg_pnl=-0.0040` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3936` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=under_minimum_exact_live_structure_bucket`；當前 q15 分桶支持樣本=10/50，缺口=40；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；`/execution/status` 與 `/execution` 會先顯示即時部署阻塞點，再顯示 當前 q15 分桶 support / 治理背景
- **heartbeat current-state docs overwrite sync 已自動化**
  - `scripts/hb_parallel_runner.py` 現在會在 `auto_propose_fixes.py` 後自動覆寫 `ISSUES.md / ROADMAP.md / ORID_DECISIONS.md`
  - 目的：避免 markdown docs 落後 `issues.json / data/live_predict_probe.json / data/live_decision_quality_drilldown.json`，讓 cron 心跳真正完成 docs overwrite 閉環
- **本輪產品化 patch：high-conviction Top-K API row-level blocker contract 已補齊**
  - `/api/models/leaderboard.high_conviction_topk.best_rows[] / nearest_deployable_rows[]` 現在會把 live support overlay 下放到每列：`support_route / support_governance_route / deployment_blocker / runtime_closure_state / current_live_structure_bucket`
  - runtime proof：nearest row `logistic_regression top_2pct` 仍是 `runtime_blocked_oos_pass`，且 row-level blocker 明確為 `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket`；Strategy Lab 不再只能依摘要層推論失敗原因

---

## Open Issues

### P0. current live bucket CAUTION|structure_quality_caution|q15 exact support remains under minimum and remains the deployment blocker (10/50)
- 目前真相：`deployment_blocker=under_minimum_exact_live_structure_bucket` / `bucket=CAUTION|structure_quality_caution|q15` / `support=10/50` / `gap=40` / `runtime_closure_state=patch_inactive_or_blocked`
...
- latest matrix：`generated_at=2026-04-30T04:05:06.947654+00:00` / `samples=23877` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket`
...
- 本輪 patch：`/api/models/leaderboard.high_conviction_topk.best_rows[] / nearest_deployable_rows[]` 已補齊 row-level support/blocker 欄位（`support_route / support_governance_route / deployment_blocker / runtime_closure_state / current_live_structure_bucket`），並在 live probe 較新時用 live support overlay 下放到每列；Strategy Lab 可直接顯示每個 high-conviction 候選為何仍 fail-closed。
- 下一步：保留 high-conviction top-k 的 risk-first/nearest-deployable 排序與 row-level blocker contract；即時分桶 / 支持阻塞未解除前仍維持模擬觀察 / 影子驗證 / 僅觀察，不得把 OOS pass row 升級成 deployable。
- 驗證：
  ...
  - `/api/models/leaderboard.high_conviction_topk.nearest_deployable_rows[0]` runtime proof：`support_route=exact_bucket_present_but_below_minimum` / `support_governance_route=exact_live_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `runtime_closure_state=patch_inactive_or_blocked` / `current_live_structure_bucket=CAUTION|structure_quality_caution|q15`
...
### P1. model stability still needs work (cv=0.6426, cv_std=0.1014, cv_worst=0.5413)
- 目前真相：`cv_accuracy=0.6426476749057394` / `cv_std=0.10138248847926268` / `cv_worst=0.5412651864264767`
...
- 目前真相：`bucket=CAUTION|structure_quality_caution|q15` / `support=10/50` / `gap=40` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `governance_route=exact_live_bucket_present_but_below_minimum`
- support progress：`status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `legacy_supported_reference=53/50@20260419b`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3936` / `archive_window_coverage_pct=0.0`
...
### P1. q15 exact support under minimum after semantic rebaseline while breaker is clear (10/50)
- 目前真相：`bucket=CAUTION|structure_quality_caution|q15` / `support=10/50` / `gap=40` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `governance_route=exact_live_bucket_present_but_below_minimum` / `breaker_context=breaker_clear`
- support progress：`status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `legacy_supported_reference=53/50@20260419b`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.25` / `dominant_regime=chop` / `dominant_regime_share=0.84` / `avg_pnl=-0.004`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示即時支持脈絡；即時分桶 / 支持阻塞未解除前仍 fail-closed。
   - 本輪補齊 API row-level blocker contract：`best_rows[] / nearest_deployable_rows[]` 每列都帶 `support_route / support_governance_route / deployment_blocker / runtime_closure_state / current_live_structure_bucket`，避免 UI 只能靠摘要層推論候選被拒原因。
