...
_最後更新：2026-05-18 09:15:59 CST_
...
## 當前主線事實
- **最新 full heartbeat #1322-productization 已完成 collect + diagnostics refresh**
  - `Raw=33529 / Features=24632 / Labels=66850`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=23.0%` / `dominant_regime=bear(100.0%)` / `avg_quality=-0.0806` / `avg_pnl=-0.0061` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4192` / `archive_window_coverage_pct=0.0`
...
- **Strategy Lab 高信心 Top-K summary counts 已兼容 API / raw artifact aliases**
  - 高信心面板現在同時接受 `deployable_count`/`deployable_rows`、`risk_qualified_count`/`risk_qualified_rows`、`runtime_blocked_candidate_count`/`runtime_blocked_candidate_rows`/`runtime_blocked_candidates`；若 `/api/models/leaderboard` 或 current-state artifact 傳入 row-style 欄位，UI 仍會顯示 `risk_qualified=6 / runtime_blocked=6 / deployable=0`，避免把 runtime-blocked OOS-pass 候選誤報成 0 而弱化 fail-closed 語義。
...
### P1. model stability still needs work (cv=0.5583, cv_std=0.0041, cv_worst=0.5542)
- 目前真相：`cv_accuracy=0.5583197389885808` / `cv_std=0.004078303425774865` / `cv_worst=0.5542414355628059`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4192` / `archive_window_coverage_pct=0.0`
