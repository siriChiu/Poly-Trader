...
## 當前主線事實
- **本輪 heartbeat 已把 dev-runtime active-backend failover 從 Dashboard-only 修到 shared API/WS layer**
  - `web/src/hooks/useApi.ts` 現在會先對 `8000/8001` 跑 `/health` 短超時 prewarm，持久化可用的 active backend base，再讓 `/api/status`、chart 與其他 GET/HEAD requests 走同一條 active-base fallback。
  - `web/src/pages/Dashboard.tsx` 的 `/ws/live` 首次連線與 retry 前會先 prewarm active backend，再依健康 lane 重排 WebSocket candidates。
...
4. **監控 shared active-backend prewarm/failover regression，避免 `/`、`/execution/status`、`/lab` 再回到長時間 `同步中 / UNKNOWN` 假陰性**
