...
_最後更新：2026-05-14 18:17:32 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1218 已完成 collect + diagnostics refresh**
  - `Raw=33204 / Features=24391 / Labels=66370`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=50.0%` / `dominant_regime=chop(100.0%)` / `avg_quality=+0.1413` / `avg_pnl=-0.0003` / `alerts=regime_concentration,regime_shift`
- **leaderboard / governance 仍維持 dual-role contract**
  ...
  - cron-safe reuse guardrail 已補強：`hb_parallel_runner.py` 現在把 `leaderboard_payload_cache_age_sec + 240s heartbeat budget >= 900s stale threshold` 視為即將過期，會先 live-rebuild 再沿用 cache，避免 summary 結束時把近過期 leaderboard payload 當新鮮 truth。
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - top source blockers：`fin_netflow(source_auth_blocked/auth_missing, coverage=0.0%, archive_window=0.0%, forward_archive=ready)` / `claw(source_auth_blocked/auth_missing, coverage=14.7%, archive_window=87.9%, forward_archive=ready)` / `claw_intensity(source_auth_blocked/auth_missing, coverage=14.7%, archive_window=87.9%, forward_archive=ready)` / `nest_pred(source_tls_verify_failed/tls_verify_failed, coverage=16.3%, archive_window=97.7%, forward_archive=ready)`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4031` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 12/50，還差 3 勝；當前 q15 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 當前 q15 分桶 support / 治理背景；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端英文枚舉與混合式治理文案泄漏到 Dashboard / Strategy Lab / Execution Status
...
- latest matrix：`generated_at=2026-05-14T10:02:27.210475+00:00` / `freshness=fresh` / `age_min=11.3` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24277` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=CAUTION|structure_quality_caution|q15` / `current_live_structure_bucket_rows=20/50` / `current_live_structure_bucket_gap_to_minimum=30`
...
### P1. model stability still needs work (cv=0.6143, cv_std=0.0583, cv_worst=0.5560)
- 目前真相：`cv_accuracy=0.6142915980230643` / `cv_std=0.058278418451400305` / `cv_worst=0.556013179571664`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4031` / `archive_window_coverage_pct=0.0`
...
- 產品化修補：fast heartbeat cache reuse 已加入 near-expiry guardrail；若 leaderboard payload 距 900s stale threshold 剩不到本輪 240s cron budget，runner 會用 `allow_rebuild=True` live refresh 後才回報 cache hit。
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.5` / `dominant_regime=chop` / `dominant_regime_share=1.0` / `avg_pnl=-0.0003`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-14T10:02:27.210475+00:00` / `freshness=fresh` / `age_min=11.3` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=20/50` / `gap=30` / `release_ready=False` / `recent_window_wins=12/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=3` / `current_recent_window_win_rate=0.240`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
