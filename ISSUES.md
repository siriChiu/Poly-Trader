...
*最後更新：2026-04-14 04:47 UTC — Heartbeat #715 把 `#H_AUTO_LIVE_DQ_PATHOLOGY` 從 **runtime 只會承認 toxic exact ALLOW lane** 再往前推到 **gate 本身開始切掉一段已驗證為 0-win 的 overextended ALLOW pocket**。本輪 patch：`model/predictor.py` 與 `backtesting/strategy_lab.py` 同步加入 4H overextended allow-lane veto（`feat_4h_bb_pct_b >= 1.0 && feat_4h_dist_bb_lower >= 10 && feat_4h_dist_swing_low >= 11 -> structure_overextended_block`），把 bull `ALLOW+D` 內一段歷史 **19 rows / 0 wins** 的高延伸口袋直接降成 BLOCK；`tests/test_api_feature_history_and_predictor.py` 新增 regression，並用 `python scripts/hb_parallel_runner.py --fast --hb 715` + `python scripts/hb_predict_probe.py` 重驗證。結果：exact `bull+ALLOW+D` live lane 從 **24 rows / wr=0.2917 / q=-0.005 / true_negative_share=0.7083** 收斂到 **14 rows / wr=0.5000 / q=+0.2412 / true_negative_share=0.5000**，`decision_quality_exact_live_lane_toxicity_applied` 已解除；broader bull-D 最差 pocket 仍是 `bull|BLOCK` **116 rows / 0 wins**，且其中 `structure_overextended_block:10`、`structure_quality_block:106`。驗證：`python -m pytest tests/test_api_feature_history_and_predictor.py tests/test_auto_propose_fixes.py tests/test_hb_parallel_runner.py tests/test_strategy_lab.py -q` → **59 passed**；`python scripts/hb_predict_probe.py` → `execution_guardrail_reason="decision_quality_below_trade_floor"`、exact lane 不再被標成 toxic。結論修正：exact live ALLOW lane 的「明顯錯 bucket」已被切掉，但 broader `regime_gate+entry_quality_label` 與 `regime_label+entry_quality_label` lane 仍被 neutral/bull spillover 與 recent pathology 拉成 D；下一輪要直接收斂 calibration scope 是否需要再吃 structure bucket，而不只是 regime/gate/quality label。*
...
- **P1#1 — `#H_AUTO_LIVE_DQ_PATHOLOGY` 已從「runtime 只會承認 toxic exact lane」推進到「gate 直接切掉一段 0-win overextended ALLOW pocket」**：Heartbeat #715 讓 `model/predictor.py` / `backtesting/strategy_lab.py` 同步加入 `structure_overextended_block`。結果是 exact `bull+ALLOW+D` lane **24 → 14 rows**，`wr 0.2917 → 0.5000`，`q -0.005 → +0.2412`，`true_negative_share 0.7083 → 0.5000`，live probe 不再輸出 `decision_quality_exact_live_lane_toxicity_applied=true`。目前 machine-readable 主證據改寫成兩段：**(a)** 被切掉的 overextended pocket 已正式落到 `bull|BLOCK`，`final_reason_counts` 新增 `structure_overextended_block:10`；**(b)** broader bull-only D lane 仍以 `bull|BLOCK` **116 rows / 0 wins** 為主，且 chosen `regime_gate+entry_quality_label` scope 仍有 neutral spillover + recent pathology。下一輪 root-cause 問題因此收斂成：**是否要把 structure bucket 直接納入 calibration scope / scope fallback，而不只是 regime/gate/quality label。**
