...
_最後更新：2026-05-18 18:19:23 CST_
...
## 當前主線事實
- **最新 full heartbeat #1331-productization 已完成 collect + diagnostics refresh**
  - `Raw=33560 / Features=24657 / Labels=66901`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.59%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=34` / `recent_window_wins=6/50` / `additional_recent_window_wins_needed=9`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=23.0%` / `dominant_regime=bear(100.0%)` / `avg_quality=-0.0800` / `avg_pnl=-0.0067` / `alerts=regime_concentration,regime_shift`
...
### P0. 連續 34 筆 1440m simulated_pyramid_win=0
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=34` / `all_horizon_losing_streak=2`
- 下一步：檢查 recent canonical labels / regime breakdown / circuit breaker；必要時升級為 distribution-aware drift 調查

### P0. 熔斷解除條件仍是唯一即時部署阻塞點
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=34` / `recent 50 wins=6/50` / `additional_recent_window_wins_needed=9`
...
### P1. Train-CV gap = 17.8pp (66.6% vs 48.8%)
- 下一步：更正則化: 增加 reg_alpha/reg_lambda; 減少 max_depth; 或減少特徵數

### P1. model stability still needs work (cv=0.4878, cv_std=0.0367, cv_worst=0.4511)
- 目前真相：`cv_accuracy=0.4877750611246944` / `cv_std=0.03667481662591687` / `cv_worst=0.4511002444987775`
...
- 目前真相：`generated_at=2026-05-18T10:03:33.683748Z` / `venues_checked=2` / `ok_count=1` / `runtime_ready_count=0` / `runtime_ready=false` / `readiness_state=blocked_until_runtime_lifecycle_proof` / `runtime_ready_blockers=fill lifecycle 尚未驗證|live exchange credential 尚未驗證|order ack lifecycle 尚未驗證|元資料契約尚未通過|場館 adapter 尚未接入`；`okx=adapter_supported=true,enabled_in_config=true,credentials_configured=false,proof_state=public_metadata_only,runtime_ready=false,blockers=live exchange credential 尚未驗證|order ack lifecycle 尚未驗證|fill lifecycle 尚未驗證` / `binance=adapter_supported=false,enabled_in_config=false,credentials_configured=false,proof_state=adapter_unsupported,runtime_ready=false,blockers=場館 adapter 尚未接入|元資料契約尚未通過|場館設定停用`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4208` / `archive_window_coverage_pct=0.0`
