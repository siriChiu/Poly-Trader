...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31541 / Features=22959 / Labels=63568`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=81.0%` / `dominant_regime=bull(92.0%)` / `avg_quality=+0.4136` / `avg_pnl=+0.0123` / `alerts=label_imbalance,regime_concentration,regime_shift`
...
- **/api/models/leaderboard background refresh 已收斂成 product-safe bounded shortlist**
  - `refresh_model_scope=production_refresh_shortlist` / `background_refresh_surface=2 deployment × 2 feature profiles` / runtime evidence：`GET /api/models/leaderboard` 200 in `~0.89s`
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3011` / `archive_window_coverage_pct=0.0`
...
- latest diagnostics：`latest_window=100` / `win_rate=81.0%` / `dominant_regime=bull(92.0%)` / `avg_quality=+0.4136` / `avg_pnl=+0.0123` / `alerts=label_imbalance,regime_concentration,regime_shift`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3011` / `archive_window_coverage_pct=0.0`
...
- background refresh：`refresh_model_scope=production_refresh_shortlist` / `background_refresh_surface=2 deployment × 2 feature profiles` / `api_runtime≈0.89s`
- 下一步：Keep /api/models/leaderboard and Strategy Lab aligned on latest bounded walk-forward plus the recent-two-year backtest policy; do not regress to placeholder-only, ambiguous backtest windows, or the old full candidate fan-out in background refresh.
- 驗證：
  - browser /lab
  - curl http://127.0.0.1:<active-backend>/api/models/leaderboard
  - pytest tests/test_model_leaderboard.py tests/test_strategy_lab.py tests/test_hb_model_leaderboard_api_probe.py -q

### P1. q35 lane still needs formula review / base-stack redesign before deploy
- 目前真相：`bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.1238`
