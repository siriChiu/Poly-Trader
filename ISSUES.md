...
_最後更新：2026-05-15 13:17:21 CST_
...
## 當前主線事實
- **最新 full heartbeat #1241 已完成 collect + diagnostics refresh**
  - `Raw=33255 / Features=24442 / Labels=66476`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=60.4%` / `dominant_regime=chop(90.8%)` / `avg_quality=+0.2357` / `avg_pnl=+0.0032` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4065` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件，且倉位腿路由 copy 已 operator-safe**
  - `即時部署阻塞點=under_minimum_exact_live_structure_bucket`；當前 q15 分桶支持樣本=4/50，缺口=46；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；`/execution/status` 與 `/execution` 會先顯示即時部署阻塞點，再顯示 當前 q15 分桶 support / 治理背景；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端英文枚舉與混合式治理文案泄漏到 Dashboard / Strategy Lab / Execution Status；本輪把 live sleeve routing 的啟用 / 待命倉位腿 chip 與 tooltip 全部通過 `humanizeRuntimeDetailText()`，避免 `primary sleeves`、`runtime gate` 類後端文案直接露給 operator。
...
- latest matrix：`generated_at=2026-05-15T05:02:47.580117+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24331` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=4/50` / `current_live_structure_bucket_gap_to_minimum=46`
...
### P1. model stability still needs work (cv=0.6580, cv_std=0.0826, cv_worst=0.5754)
- 目前真相：`cv_accuracy=0.6580353473078504` / `cv_std=0.08261405672009864` / `cv_worst=0.5754212905877517`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4065` / `archive_window_coverage_pct=0.0`
