...
## 當前主線事實
- **最新 fast heartbeat #1555 已完成 collect + diagnostics refresh**
...
### P0. 連續 167 筆 1440m simulated_pyramid_win=0
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=167` / `all_horizon_losing_streak=12`
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=167` / `recent 50 wins=0/50` / `additional_recent_window_wins_needed=15`
...
- 目前真相：`generated_at=2026-05-28T04:03:17.662413Z` / `venues_checked=2` / `ok_count=1` / `runtime_ready_count=0` / `runtime_ready=false` / `readiness_state=blocked_until_runtime_lifecycle_proof` / `runtime_ready_blockers=fill lifecycle 尚未驗證|live exchange credential 尚未驗證|order ack lifecycle 尚未驗證|元資料契約尚未通過|場館 adapter 尚未接入`；`okx=adapter_supported=true,enabled_in_config=true,credentials_configured=false,proof_state=public_metadata_only,runtime_ready=false,blockers=live exchange credential 尚未驗證|order ack lifecycle 尚未驗證|fill lifecycle 尚未驗證` / `binance=adapter_supported=false,enabled_in_config=false,credentials_configured=false,proof_state=adapter_unsupported,runtime_ready=false,blockers=場館 adapter 尚未接入|元資料契約尚未通過|場館設定停用`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4926` / `archive_window_coverage_pct=0.0`
