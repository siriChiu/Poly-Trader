...
_最後更新：2026-05-24 02:20:25 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1466 已完成 collect + diagnostics refresh**
  - `Raw=34219 / Features=25191 / Labels=67914`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.37%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=0` / `recent_window_wins=6/50` / `additional_recent_window_wins_needed=9`
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=0` / `recent 50 wins=6/50` / `additional_recent_window_wins_needed=9`
...
- 目前真相：`generated_at=2026-05-23T18:20:21.621597Z` / `venues_checked=2` / `ok_count=1` / `runtime_ready_count=0` / `runtime_ready=false` / `readiness_state=blocked_until_runtime_lifecycle_proof` / `runtime_ready_blockers=fill lifecycle 尚未驗證|live exchange credential 尚未驗證|order ack lifecycle 尚未驗證|元資料契約尚未通過|場館 adapter 尚未接入`；`okx=adapter_supported=true,enabled_in_config=true,credentials_configured=false,proof_state=public_metadata_only,runtime_ready=false,blockers=live exchange credential 尚未驗證|order ack lifecycle 尚未驗證|fill lifecycle 尚未驗證` / `binance=adapter_supported=false,enabled_in_config=false,credentials_configured=false,proof_state=adapter_unsupported,runtime_ready=false,blockers=場館 adapter 尚未接入|元資料契約尚未通過|場館設定停用`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4627` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=distribution_pathology` / `win_rate=0.06` / `dominant_regime=chop` / `dominant_regime_share=0.68` / `avg_pnl=-0.0125`
