...
_最後更新：2026-04-23 08:35:16 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31828 / Features=23246 / Labels=63844`
...
- **canonical current-live blocker 已切到 current-live exact-support truth**
  ...
  - `current_live_structure_bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=13/50` / `gap=37` / `support_route_verdict=exact_bucket_present_but_below_minimum`
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=100.0%` / `dominant_regime=bull(91.0%)` / `avg_quality=+0.6707` / `avg_pnl=+0.0236` / `alerts=constant_target,regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3298` / `archive_window_coverage_pct=0.0`
...
### P0. current live bucket BLOCK|bull_high_bias200_overheat_block|q35 exact support remains under minimum and remains the deployment blocker (13/50)
- 目前真相：`deployment_blocker=under_minimum_exact_live_structure_bucket` / `bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=13/50` / `gap=37` / `runtime_closure_state=patch_inactive_or_blocked`
...
- latest diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=bull(91.0%)` / `avg_quality=+0.6707` / `avg_pnl=+0.0236` / `alerts=constant_target,regime_concentration,regime_shift`
...
- 目前真相：`bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=13/50` / `gap=37` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `governance_route=exact_live_bucket_present_but_below_minimum`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3298` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=13/50` / `gap=37` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.1615`
- q35 scaling audit 已指出目前不是單點 bias50 closure：`overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.1615`
