...
_最後更新：2026-04-21 00:08:17 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31270 / Features=22688 / Labels=63072`
...
- **recent canonical window 仍是 distribution pathology**
  - `window=500` / `win_rate=8.0%` / `dominant_regime=bull(88.6%)` / `avg_quality=-0.1948` / `avg_pnl=-0.0068` / `alerts=label_imbalance,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2740` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=500` / `win_rate=8.0%` / `dominant_regime=bull(88.6%)` / `avg_quality=-0.1948` / `avg_pnl=-0.0068`
- 病態切片：`alerts=label_imbalance,regime_shift` / `tail_streak=34` / `low_variance=9` / `low_distinct=10` / `null_heavy=10`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['label_imbalance', 'regime_shift'], win_rate=0.0800, delta_vs_full=-0.5448, dominant_regime=bull(88.60%), interpretation=distribution_pathology, avg_pnl=-0.0068, avg_quality=-0.1948, avg_dd_penalty=0.3219, spot_long_win_rate=0.0000, feature_diag=variance:9/56, frozen:0, compressed:9, expected_static:3, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=34x1 since 2026-04-19 10:13:18.192660 -> 2026-04-19 16:56:47.963662, adverse_streak=229x0 since 2026-04-17 14:37:30.923630 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.766, delta_vs_prev=-0.686, prev_quality=0.3499, quality_delta_vs_prev=-0.5447, prev_pnl=0.0058, pnl_delta_vs_prev=-0.0126, top_shift_examples=feat_eye(0.9349→-1.5872,Δσ=0.6582)/feat_local_top_score(0.4127→0.2872,Δσ=0.5585)/feat_local_bottom_score(0.3061→0.4224,Δσ=0.5092), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.0062/498)/feat_tongue(0.0092/500), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_nq_return_24h[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-19 16:51:05.759431:1:chop:0.4936/2026-04-19 16:55:34.338056:1:chop:0.5417/2026-04-19 16:56:47.963662:1:chop:0.5123, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2740` / `archive_window_coverage_pct=0.0`
