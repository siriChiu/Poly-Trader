...
_最後更新：2026-04-24 05:16:38 CST_
...
## 當前主線事實
- **最新 fast heartbeat #release-final-audit 已完成 collect + diagnostics refresh**
  - `Raw=32099 / Features=23516 / Labels=64492`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=36.0%` / `dominant_regime=bull(99.6%)` / `avg_quality=-0.0374` / `avg_pnl=-0.0039` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3567` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=250` / `win_rate=36.0%` / `dominant_regime=bull(99.6%)` / `avg_quality=-0.0374` / `avg_pnl=-0.0039` / `alerts=regime_concentration,regime_shift`
- 病態切片：`alerts=regime_concentration,regime_shift` / `tail_streak=—` / `top_shift=feat_4h_dist_bb_lower,feat_4h_vol_ratio,feat_4h_macd_hist` / `new_compressed=None`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=250, alerts=['regime_concentration', 'regime_shift'], win_rate=0.3600, delta_vs_full=-0.2654, dominant_regime=bull(99.60%), interpretation=regime_concentration, avg_pnl=-0.0039, avg_quality=-0.0374, avg_dd_penalty=0.3470, spot_long_win_rate=0.0000, feature_diag=variance:14/56, frozen:0, compressed:14, expected_static:1, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=30x1 since 2026-04-22 21:30:24.993009 -> 2026-04-22 22:05:28.693184, adverse_streak=44x0 since 2026-04-22 09:16:37.688418 -> 2026-04-22 15:02:10.145365, prev_win_rate=0.804, delta_vs_prev=-0.444, prev_quality=0.4527, quality_delta_vs_prev=-0.4901, prev_pnl=0.013, pnl_delta_vs_prev=-0.0169, top_shift_examples=feat_4h_dist_bb_lower(2.8541→6.807,Δσ=1.2414)/feat_4h_vol_ratio(1.0647→1.8214,Δσ=1.2387)/feat_4h_macd_hist(-370.512→423.9976,Δσ=1.1885), compressed_examples=feat_body(0.0/249)/feat_ear(0.0053/248)/feat_tongue(0.008/249), expected_static_examples=feat_4h_ma_order[discrete_regime_feature], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-22 22:02:50.128125:1:bull:0.3115/2026-04-22 22:04:10.997313:1:bull:0.3132/2026-04-22 22:05:28.693184:1:bull:0.3064, adverse_examples=2026-04-22 15:00:35.749241:0:bull:-0.2526/2026-04-22 15:01:51.448437:0:bull:-0.2408/2026-04-22 15:02:10.145365:0:bull:-0.2378
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3567` / `archive_window_coverage_pct=0.0`
