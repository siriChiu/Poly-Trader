...
## 當前主線事實
- **最新 fast heartbeat #1280-productization-ui 已完成 collect + diagnostics refresh**
  - `Raw=33389 / Features=24529 / Labels=66646`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.71%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=64` / `recent_window_wins=0/50` / `additional_recent_window_wins_needed=15`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=52.4%` / `dominant_regime=chop(80.0%)` / `avg_quality=+0.1754` / `avg_pnl=+0.0026` / `alerts=regime_shift`
...
### P0. 連續 64 筆 1440m simulated_pyramid_win=0
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=64` / `all_horizon_losing_streak=23`
- 下一步：檢查 recent canonical labels / regime breakdown / circuit breaker；必要時升級為 distribution-aware drift 調查

### P0. 熔斷解除條件仍是唯一即時部署阻塞點
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=64` / `recent 50 wins=0/50` / `additional_recent_window_wins_needed=15`
...
- latest matrix：`generated_at=2026-05-16T12:03:38.044744+00:00` / `freshness=fresh` / `age_min=11.7` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24417` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=5/50` / `current_live_structure_bucket_gap_to_minimum=45` / `support_progress_status=stalled_under_minimum` / `support_progress_reason=current live exact support 連續 heartbeat 停在同一數量，屬於 support accumulation 停滯。` / `regression_basis=same_identity_same_semantic_signature` / `delta_vs_previous=0` / `previous_rows=5` / `support_rows_needed=45` / `stagnant_run_count=2` / `stalled_support_accumulation=True` / `escalate_to_blocker=False`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4125` / `archive_window_coverage_pct=0.0`
