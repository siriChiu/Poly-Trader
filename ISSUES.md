...
## 當前主線事實
- **最新 fast heartbeat #20260422s 已完成 collect + diagnostics refresh**
  - `Raw=31479 / Features=22897 / Labels=63467`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=1000` / `win_rate=38.8%` / `dominant_regime=bull(81.3%)` / `avg_quality=+0.0728` / `avg_pnl=+0.0003` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2949` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=1000` / `win_rate=38.8%` / `dominant_regime=bull(81.3%)` / `avg_quality=+0.0728` / `avg_pnl=+0.0003` / `alerts=regime_shift`
- 病態切片：`alerts=regime_shift` / `tail_streak=—` / `top_shift=feat_4h_bias200,feat_vwap_dev,feat_dxy` / `new_compressed=feat_vix`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=1000, alerts=['regime_shift'], win_rate=0.3880, delta_vs_full=-0.2382, dominant_regime=bull(81.30%), interpretation=regime_concentration, avg_pnl=+0.0003, avg_quality=0.0728, avg_dd_penalty=0.2372, spot_long_win_rate=0.1740, feature_diag=variance:7/56, frozen:0, compressed:7, expected_static:0, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=39x1 since 2026-04-21 00:05:39.160858 -> 2026-04-21 04:44:20.642922, adverse_streak=273x0 since 2026-04-17 13:41:12.414408 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.915, delta_vs_prev=-0.527, prev_quality=0.529, quality_delta_vs_prev=-0.4562, prev_pnl=0.0143, pnl_delta_vs_prev=-0.014, top_shift_examples=feat_4h_bias200(4.2075→7.666,Δσ=0.6331)/feat_vwap_dev(-0.3094→-0.1761,Δσ=0.593)/feat_dxy(98.4847→98.1322,Δσ=0.3402), new_compressed=feat_vix, compressed_examples=feat_body(0.0001/998)/feat_ear(0.0103/996)/feat_tongue(0.0143/997), overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-21 04:42:29.066587:1:bull:0.6462/2026-04-21 04:44:14.803275:1:bull:0.6516/2026-04-21 04:44:20.642922:1:bull:0.6539, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2949` / `archive_window_coverage_pct=0.0`
