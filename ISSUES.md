...
> **最後更新：2026-04-02 02:34 GMT+8**
> **🔄 心跳 #44：IC 全量重算（n=2160），aura IC=-0.012 降至閾值邊緣；train.py 注釋更新**
...
| ID | 問題 | 影響 | 狀態 |
|----|------|------|------|
| #H33 | 🟡 模型 CV=40.0%（數據不足，需持續收集） | 乾淨數據 2,216 筆，不足以提升 CV | 🟡 P1 — 持續收集（5min 排程中） |
...
| ID | 問題 | 建議 | 狀態 |
|----|------|------|------|
| #H50 | 🟡 feat_aura IC=-0.012（接近無效閾值 <0.05） | 觀察中；若 2 次心跳後仍 <0.05 則替換為鏈上資金流向特徵 | 🟡 P2 — 觀察 |
...
| feat_aura | -0.012 | ✅ 是 | +0.012 ⚠️ 接近無效 |
| feat_mind | +0.036 | ❌ 否 | +0.036 |

> ⚠️ **注意**：全量 IC 比 n=500 IC 低，反映數據集中存在更多雜訊。所有特徵 IC 絕對值均 <0.2，屬弱信號。仍全部方向一致（除 feat_aura 已近 0）。
...
| 優先 | 行動 | Issue |
|------|------|-------|
| P1 | **持續累積乾淨數據**：現有 2,216 筆（5min 排程，每天新增 ~288 筆） | #H33 |
| P1 | **Polymarket 數據補全**：ear_zscore 目前用 price momentum 替代 | #H31 |
| P2 | **監控 feat_aura IC**：若持續 <0.05，替換為鏈上 exchange netflow | #H50 |
