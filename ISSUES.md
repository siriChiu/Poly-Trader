...
> **最後更新：2026-04-02 10:50 GMT+8**
> **🔄 心跳 #86：重訓模型（新增一批數據）CV=50.77%；ic_signs 確認與訓練腳本一致；raw=11079 feats=11066 BTC=$66,674**
...
| #H97 | 🟡 IC 不穩定：Ear/Body/Mind IC 低，Pulse 穩定正向 (+0.153) | 考慮 rolling IC 追蹤機制；ic_signs 確保由訓練腳本生成，不可手動改 | 🟡 P1 |
| #H98 | 🟡 心跳 IC 計算（N=2000 最新）與訓練 IC（全量）不一致 | 心跳 IC 分析改用全量或最新 N=10K；不可用心跳計算結果覆蓋 ic_signs.json | 🟡 P1（新增） |
...
## 📊 當前系統健康 (2026-04-02 10:50 GMT+8)

### 數據管線
| 項目 | 數值 | 狀態 |
|------|------|------|
| Raw data | 11,079 筆 | ✅ 每 5 分鐘增長 |
| Features | 11,066 筆 | ✅ |
| Labels | 22,055 筆 | ✅ |
| BTC 當前 | $66,674 | ✅ |
| FNG | 12.0（極度恐慌）| ⚠️ 市場悲觀 |
| Funding Rate | -1.755e-05（負/偏空）| ⚠️ |
| OI ROC | -0.0175（縮小）| ⚠️ |

### 感官 IC（訓練集全量 ~10,740 筆，h=4）
| 感官 | 特徵 | IC | 符號 | 狀態 |
|------|------|----|------|------|
| Eye（視覺）| feat_eye_dist | +0.039 | pos | ⚠️ 弱但有效 |
| Ear（聽覺）| feat_ear_zscore | -0.019 | neg | ⚠️ 弱 |
| Nose（嗅覺）| feat_nose_sigmoid | +0.027 | pos | ⚠️ 弱 |
| Tongue（味覺）| feat_tongue_pct | +0.043 | pos | ✅ |
| Body（觸覺）| feat_body_roc | -0.021 | neg | ⚠️ 弱 |
| Pulse（脈動）| feat_pulse | +0.153 | pos | ✅ 最強 |
| Aura（磁場）| feat_aura | +0.049 | pos | ✅ |
| Mind（認知）| feat_mind | -0.016 | neg | ⚠️ 弱 |

### 模型性能
| 指標 | 值 | 評估 |
|------|----|----|
| Train Accuracy | **46.1%** | ✅ 欠擬合（無過擬）|
| TimeSeries CV | **50.77% ± 6.27%** | 🟡 穩定但不足 |
...
| P1 | **心跳 IC 分析改用全量數據**：避免 N=2000 短視窗算出不同符號誤導 | #H98 |
