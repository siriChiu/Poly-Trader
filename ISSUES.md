...
_最後更新：2026-05-16 10:26:43 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1268 已完成 collect + diagnostics refresh**
  - `Raw=33350 / Features=24501 / Labels=66594`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.77%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=37` / `recent_window_wins=13/50` / `additional_recent_window_wins_needed=2`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=500` / `win_rate=66.8%` / `dominant_regime=chop(75.4%)` / `avg_quality=+0.3117` / `avg_pnl=+0.0066` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4105` / `archive_window_coverage_pct=0.0`
...
### P0. 連續 37 筆 1440m simulated_pyramid_win=0
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=37` / `all_horizon_losing_streak=0`
- 下一步：檢查 recent canonical labels / regime breakdown / circuit breaker；必要時升級為 distribution-aware drift 調查

### P0. 熔斷解除條件仍是唯一即時部署阻塞點
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=37` / `recent 50 wins=13/50` / `additional_recent_window_wins_needed=2`
...
- latest matrix：`generated_at=2026-05-16T02:25:17.526768+00:00` / `freshness=fresh` / `age_min=1.4` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24391` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=10/50` / `current_live_structure_bucket_gap_to_minimum=40`
...
### P1. model stability still needs work (cv=0.6308, cv_std=0.0736, cv_worst=0.5572)
- 目前真相：`cv_accuracy=0.6307913079130791` / `cv_std=0.07359573595735958` / `cv_worst=0.5571955719557196`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4105` / `archive_window_coverage_pct=0.0`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-16T02:25:17.526768+00:00` / `freshness=fresh` / `age_min=1.4` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=10/50` / `gap=40` / `release_ready=False` / `recent_window_wins=13/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=2` / `current_recent_window_win_rate=0.260`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
