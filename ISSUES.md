...
> **最後更新：2026-04-02 02:59 GMT+8**
> **🔄 心跳 #47：feat_aura v4 替換（fund_x_roc IC=-0.012→funding_zscore_288 IC=-0.052），模型重訓**
> **✅ 上輪修復：feat_aura fund_x_roc IC=-0.012, p=0.57 確認統計無效，本輪替換**
...
| ID | 問題 | 影響 | 狀態 |
|----|------|------|------|
| #H33 | 🟡 模型 CV=38.9%（數據不足，需持續收集） | 乾淨數據 2,221 筆，不足以提升 CV | 🟡 P1 — 持續收集（5min 排程中） |
...
| ID | 問題 | 建議 | 狀態 |
|----|------|------|------|
| #H54 | 🟡 feat_mind IC=+0.036, p=0.093（邊界，統計不顯著） | 若 2 輪後仍 <0.05，考慮替換 | 🟡 觀察中（第2輪） |
...
|------|---------|-----------------|--------|------|
| feat_eye_dist | -0.104 | ✅ 是 | +0.104 | ✅ |
| feat_ear_zscore | -0.091 | ✅ 是 | +0.091 | ✅ |
| feat_nose_sigmoid | -0.094 | ✅ 是 | +0.094 | ✅ |
| feat_tongue_pct | -0.058 | ✅ 是 | +0.058 | ✅ |
| feat_body_roc | -0.071 | ✅ 是 | +0.071 | ✅ |
| feat_pulse | -0.052 | ✅ 是 | +0.052 | ✅ |
| feat_aura (v4: funding_zscore_288) | -0.052 | ✅ 是 | +0.052 | ✅ 新替換 |
| feat_mind | +0.036 | ❌ 否 | +0.036 | ⚠️ 邊界（觀察中）|
...
| 優先 | 行動 | Issue |
|------|------|-------|
| P1 | **持續累積乾淨數據**：現有 2,221 筆（5min 排程，每天新增 ~288 筆） | #H33 |
| P2 | **監控 feat_mind IC=+0.036**：2 輪觀察中，p=0.093 仍不顯著，若下輪仍不達標考慮替換 | #H54 |
| P2 | **驗證 feat_aura (funding_zscore_288) IC**：本輪替換，下輪確認 IC 維持 >0.05 | 新增 |
