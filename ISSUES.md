# ISSUES.md — 問題追蹤

*最後更新：2026-04-07 17:00 UTC（心跳 #579）*

## 📊 系統健康狀態（#579）

| 項目 | 數值 | vs #578 |
|------|------|---------|
| Raw | **9,899** | ⬆️ +14 🟢 |
| Features | **9,858** | ⬆️ +14 🟢 |
| Labels | **18,052** | ➡️ 持平 |
| sell_win (DB all) | **40.37%** | ➡️ 持平 |
| sell_win (regime join) | **49.24%** | ➡️ 持平（n=8770）|
| 全域 IC (22 擴） | **5/22** | ➡️ 持平 |
| TW-IC (22 擴） | **13/22** | ➡️ 持平（歷史新高維持）|
| DW N=100 | **7/8**🟢 | ➡️ 持平 |
| DW N=200 | **7/8**🟢 | ➡️ 持平 |
| DW N=400 | **3/8** | ➡️ 持平 |
| DW N=600 | **0/8** 🔴 | ➡️ 持續死區 |
| DW N=1000 | **4/8** | ➡️ 持平 |
| Global model | Train=63.92%, CV=51.39%, gap=12.53pp | ➡️ 持平 |
| Regime IC | Bear **4/8**, Bull **0/8**🔴, Chop **0/8**🔴 | ➡️ 持平 |
| BTC (live) | **$68,713** | ⬆️ +$57（vs #578 $68,656）|
| FNG | **11**（持續極度恐懼）| ➡️ 持平 |
| FR | **0.00003955** | ⬆️ +6.0%（vs #578 0.00003731，回升）|
| LSR | **1.2614** | ⬆️ +91bps（vs #578 1.2523，持續上升！）|
| OI | **91,047** | ⬇️ -37（vs #578 91,084）|
| 測試 | **5/6** ⚠️ | 🔻 TypeScript FAIL |
| 平行心跳 | **4/5 PASS** 🟢（**28.8s**）| ⚠️ tests 模組 TS 失敗 |

## 系統狀態：🟢 數據持續增長 / BTC回升 / LSR持續上升 / FR回升 / 測試5/6（TS FAIL）

數據管線持續增長：Raw +14, Features +14.
平行心跳 4/5 PASS（28.8s），tests 模組因 TypeScript 編譯錯誤失敗。
LSR 持續上升至 1.2614（+91bps）：多頭長倉比例持續上升。
FR 回升至 0.00003955（+6.0%）：資金費率回升。
BTC 回升至 $68,713（+$57 vs #578）。
**TypeScript FAIL**: `src/pages/StrategyLab.tsx` 多處 JSX 語法錯誤（`>` 符號未轉義）→ 5/6 測試（非 6/6）。

- **BTC $68,713**: ⬆️ +$57 vs #578 $68,656，回升
- **⬆️ FR 0.00003955（+6.0%）**: 資金費率回升
- **⬆️ LSR 1.2614（+91bps）**: 長倉比例持續上升，多頭信心增強
- **⬇️ OI 91,047（-37）**: 持倉量微降
- **⚠️ 4/5 PASS**: 平行心跳，tests 模組 TypeScript FAIL
- **raw +14, features +14**: 🟢 數據管線持續增長
- **測試 5/6 ⚠️**: TypeScript 編譯錯誤（StrategyLab.tsx）
- **DW 持續**: N=100&200 7/8🟢，N=400 3/8，N=600 持續 0/8 死區
- **訓練**: 73 features, 9106 samples, CV=51.39%（持平）
- **Regime**: Bear 4/8（持續）, Bull 0/8🔴（200+輪持續！）, Chop 0/8🔴（200+輪持續！）

## 全域 IC — 5/22（持平）

**22 擴（全域 Spearman, 含4H/5TI特徵）**: 5/22 passing。
PASS: VIX +0.0714 ✅, RSI14 +0.0542 ✅, MACDHist +0.0505 ✅, BB%B +0.0575 ✅, Nose +0.0500 ✅（擦邊）

**TW-IC（13/22 過, tau=200, 維持新高）**: Nose+0.0587, Pulse-0.0871, AURA+0.0799, Mind+0.0750, VIX+0.0876, RSI14+0.0746, MACD+0.0554, ATR%-0.1280, VWAP+0.1293, BB%B+0.0826, 4h_bias50+0.0715, 4h_rsi14+0.0622, 4h_dist_swing_low+0.0620
FAIL: Eye, Ear, Tongue, Body, DXY (core 8 中有 5 個失效); 4h_bias20, 4h_macd_hist, 4h_bb_pct_b, 4h_ma_order 失敗

## Dynamic Window — N=100&200 7/8（持平）

| Window | 通過 | 狀態 |
|--------|------|------|
| N=100 | **7/8**🟢 | eye+0.089, nose+0.177, tongue-0.115, body+0.129, pulse-0.081, aura+0.277, mind+0.230 |
| N=200 | **7/8**🟢 | eye+0.050, nose+0.114, tongue-0.082, body+0.062, pulse-0.131, aura+0.162, mind+0.167 |
| N=400 | **3/8** | Pulse, Aura, Mind |
| N=600 | **0/8** 🔴 | 持續死區 |
| N=1000 | 4/8 | Eye, Pulse, Aura, Mind |
| N=2000 | 2/8 | Pulse, Aura |
| N=5000 | 0/8 | 全滅 |

## Regime IC — Bear 4/8, Bull 0/8🔴, Chop 0/8🔴（持平）

| Regime | 通過 | 信號 |
|--------|------|------|
| Bear | **4/8** | Ear+0.0785✅, Nose+0.0727✅, Body+0.0682✅, Aura+0.0544✅ |
| Bull | **0/8**🔴 | 全部 <0.05（200+輪持續！）|
| Chop | **0/8**🔴 | 全部 <0.05（200+輪持續！）|

**sell_win by Regime**: Bear=0.4855, Bull=0.5090, Chop=0.4829, Overall=0.4924（n=8770）

## 市場數據亮點

- **BTC $68,713**: ⬆️ +$57（+0.08%）vs #578 $68,656，回升
- **⬆️ FR 0.00003955**: +6.0%回升，資金費率改善
- **⬆️ LSR 1.2614**: 長倉比例持續上升（+91bps），多頭信心持續增強
- **⬇️ OI 91,047（-37）**: 持倉量微降
- **FNG 11**（持續極度恐慌）

## ⚠️ 新問題：TypeScript 編譯錯誤

`src/pages/StrategyLab.tsx` 存在多處 JSX 語法錯誤：
- 第156行：`Unexpected token. Did you mean '{'}' or '&gt;`?
- 第185行：同類錯誤
- 第207行：同類錯誤
- `>` 符號在 JSX 中未正確轉義為 `{'>'}` 或 `&gt;`

這導致測試套件 5/6 而非 6/6。**P1 優先級修復**。

## P0

| ID | 問題 | 狀態 |
|----|------|------|
| #P537 | sell_win≥50% 全域 | 🔴 **持續 40.37%（全量Labels），regime子集49.24%** |
| #RAW_STALE | Raw 數據增長 | 🟢 **+14！持續增長中！** |
| #P442 | Chop 0/8 | 🔴 持續 0/8（200+輪）|
| #BULL_BLIND | Bull 0/8 | 🔴 Bull 0/8 持續（200+輪持續！）|
| #IC_FREEZE | 全域/TW-IC/模型凍結 | 🟢 **穩定：全域 5/22，TW-IC 13/22（維持新高）** |
| #NO_DATA | 新感官零數據（Claw/Fang/Fin）| 🔴 Collector 未收集 |
| #DW_DEAD | DW N=600/5000 死區 | 🔴 持續 0/8 |
| #FEATURE_MISMATCH | 4H 特徵部分有效 | 🟢 **3/6 通過 TW-IC（bias50+0.0715, rsi14+0.0622, dist_swing_low+0.0620）** |

## 測試狀態

| 測試 | 結果 |
|------|------|
| 檔案結構 | ✅ PASS |
| 語法檢查 | ✅ PASS (9979 文件) |
| 模組導入 | ✅ PASS (8/8) |
| 感官引擎真實數據 | ✅ PASS |
| 前端 TypeScript | ❌ FAIL (StrategyLab.tsx JSX 語法錯誤) |
| 數據品質 | ✅ PASS |

## 模型訓練

- Global: Train=63.92%, CV=51.39%±3.66%, gap=12.53pp（持平），**73 features**, 9106 samples
- Positive ratio: 30.45%
- Regime models: bear/bull/chop 各 98 features saved
- ✅ **train.py 成功** — 訓練完整（3 regimes saved）

> Heartbeat #579: full_ic 5/22全域, **13/22 TW-IC 維持新高**（持平！）, regime Bear 4/8, Bull 0/8 Chop 0/8, DW N=100&200 **7/8持平**, N=400 3/8, N=600持續0/8死區, 測試**5/6⚠️**(TS FAIL), 平行心跳**4/5 PASS**🟢(28.8s), sell_win **40.37%**（持平）, T63.92%/CV51.39%(持平), BTC **$68,713**(+$57回升), FR ⬆️ **0.00003955**(+6.0%回升), LSR **1.2614**(+91bps持續上升), OI 91,047(-37), **數據管線持續增長：Raw+14, Features+14**！
