...
_最後更新：2026-04-22 21:24:52 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31542 / Features=22960 / Labels=63570`
  - `simulated_pyramid_win=57.34%`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=83.0%` / `dominant_regime=bull(92.0%)` / `avg_quality=+0.4334` / `avg_pnl=+0.0131` / `alerts=label_imbalance,regime_concentration,regime_shift`
...
- **/api/models/leaderboard live payload 已鎖定 bounded-refresh contract**
  - cached / persisted snapshot 也會正規化成 `target_candidates=[]`，避免舊快照把 expensive target rescoring 候選帶回 live API
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3012` / `archive_window_coverage_pct=0.0`
...
- latest diagnostics：`latest_window=100` / `win_rate=83.0%` / `dominant_regime=bull(92.0%)` / `avg_quality=+0.4334` / `avg_pnl=+0.0131` / `alerts=label_imbalance,regime_concentration,regime_shift`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3012` / `archive_window_coverage_pct=0.0`
...
- live payload contract：`/api/models/leaderboard` 現在會把 cache / persisted snapshot 正規化為 `target_candidates=[]`，避免 bounded-refresh API 因舊快照殘留 expensive target rescoring 候選
...
- 目前真相：`bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.1252`
- q35 scaling audit 已指出目前不是單點 bias50 closure：`overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.1252`
