...
> **最後更新：2026-04-02 10:38 GMT+8**
> **🔄 心跳 #85：重算 ic_signs（Pulse/Eye IC 符號反轉）；重訓模型 CV=50.71%；raw=11078 feats=11065 BTC=$66,907**
...
| #H97 | 🟡 IC 不穩定：Pulse 從 +0.152→-0.021，多個感官符號漂移 | 考慮 rolling IC 追蹤機制 + 自動重算觸發 | 🟡 P1 |
...
| ID | 問題 | 解決方案 | 日期 |
|----|------|----------|------|
| **#H96** | ic_signs.json stale（Pulse +0.152→-0.021，Eye 符號反轉）| N=4975 重算 + 重訓模型 | 04-02 10:38 |
...
## 📊 當前系統健康 (2026-04-02 10:38 GMT+8)

### 數據管線
| 項目 | 數值 | 狀態 |
|------|------|------|
| Raw data | 11,078 筆 | ✅ 每 5 分鐘增長 |
| Features | 11,065 筆 | ✅ |
| Labels | 22,053 筆 | ✅ |
| BTC 當前 | $66,907 | ✅ |
| FNG | 12.0（極度恐慌）| ⚠️ |
| Funding Rate | -1.74e-05（負/偏空）| ⚠️ |
| OI ROC | -0.0175（縮小）| ⚠️ |

### 感官 IC（N=4975，h=4，最新數據）
| 感官 | 特徵 | IC | 符號 | 狀態 |
|------|------|----|------|------|
| Eye（視覺）| feat_eye_dist | -0.040 | neg | ⚠️ 弱 |
| Ear（聽覺）| feat_ear_zscore | -0.060 | neg | ✅ 弱 |
| Nose（嗅覺）| feat_nose_sigmoid | -0.048 | neg | ⚠️ 弱 |
| Tongue（味覺）| feat_tongue_pct | +0.056 | pos | ✅ |
| Body（觸覺）| feat_body_roc | -0.017 | neg | ⚠️ 弱 |
| Pulse（脈動）| feat_pulse | -0.021 | neg | ⚠️ 符號漂移 |
| Aura（磁場）| feat_aura | -0.068 | neg | ✅ 弱 |
| Mind（認知）| feat_mind | -0.092 | neg | ✅ |

### 模型性能
| 指標 | 值 | 評估 |
|------|----|----|
| Train Accuracy | **45.98%** | ✅ 欠擬合（無過擬）|
| TimeSeries CV | **50.71% ± 6.26%** | 🟡 穩定但不足 |
