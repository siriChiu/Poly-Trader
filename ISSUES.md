...
| ID | 問題 | 建議 | 狀態 |
|----|------|------|------|
| #H122 | 🔴 7/8 感官 IC 低於 0.05（全量） | **已確認**：僅 Ear IC=-0.052 達標。Bear regime 有 4/8 但 Bull 0/8、Chop 0/8 | 🔴 未突破 |
| #H125 | 🔴 全量 IC 仍低於 0.05 | 衰減模式穩定：N↑→IC↓。N=1000 最佳(5/8)但 N↑→0/8 | 🟡 改善中 |
| #H130 | 🔴 模型過擬合：gap ~20pp | 全局 gap=20.8pp。depth=3 正規化已大幅改善（vs 過擬 33.7pp） | 🟡 全局有改善 |
...
| #H141 | 🔴 **Regime 分配錯誤**：hb105 顯示 8770 筆全 neutral | hb105_explore_features.py 的 regime 分類錯誤（全 neutral）。regime_aware_ic.py 正確：bear=2897, bull=2897, chop=2984 | 🔴 新發現 |
...
| BTC 當前 | $66,826-$66,869 | ⬇️ 平穩 |
| FNG | 9（極度恐慌）| ⚠️ 持續極端 |
| LSR | 1.74（多頭偏斜）| — |
| OI | 90,245 | — |
| Taker Buy/Sell | 1.03 | — |
| Funding Rate | 0.000036 | — |

### 感官 IC 掃描（最新：regime_aware_ic.py, N=8778）
| 窗口/Regime | 達標感官數 | 具備 IC |
|------|-----------|---------|
| 全量 (8778) | **1/8** | Ear(-0.052) |
| Bear (2897) | **3/8** ⬆️ | Ear(-0.075), Nose(-0.075), Aura(-0.053) |
| Bull (2897) | **0/8** ❌ | (none) |
| Chop (2984) | **1/8** | Aura(-0.056) |

**替代特徵發現**（hb105_explore_features.py）：
- eye_dist: IC=+0.0503 ✅ 達標！
- mean_rev_20h: IC=-0.0558 ✅ 達標！
- price_ret_12h: IC=-0.0516 ✅ 達標！
- price_ret_24h: IC=-0.0511 ✅ 達標！
- rsi_14_norm: IC=-0.0510 ✅ 達標！
...
| P0 | **整合已發現的高 IC 特徵**：eye_dist(+0.050), mean_rev_20h(-0.056), rsi_14_norm(-0.051), price_ret 系列 | #H141 |
...
- **#141**: 心跳 — 替代特徵發現：eye_dist(+0.050), mean_rev_20h(-0.056), rsi_14_norm(-0.051) 等 5 個特徵 IC＞0.05
- **#141**: Regime 分配確認：regime_aware_ic.py 正確分類（bear/2897, bull/2897, chop/2984）
