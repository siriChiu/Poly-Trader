...
_最後更新：2026-04-30 01:08:55 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1132-productization-timeout-fix 已完成 collect + diagnostics refresh**
  - `Raw=32483 / Features=23901 / Labels=65573`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=21.0%` / `dominant_regime=chop(96.0%)` / `avg_quality=-0.1025` / `avg_pnl=-0.0060` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3916` / `archive_window_coverage_pct=0.0`
...
- 下一步：把 high-conviction top-k 從 ROI-only 觀測列升級為風控 / OOS / 部署門檻優先排序；nearest-deployable 候選優先顯示，但 current-live/support blockers 未解除前仍 paper/shadow/hold-only。
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3916` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.21` / `dominant_regime=chop` / `dominant_regime_share=0.96` / `avg_pnl=-0.006`
