...
## 當前主線事實
- **最新 fast heartbeat #20260425_084505 已完成 collect + diagnostics refresh**
  - `Raw=32210 / Features=23628 / Labels=64987`
...
- **canonical current-live blocker 仍是 breaker-first truth**
  ...
  - `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=93/50` / `gap=0` / `support_route_verdict=exact_bucket_supported`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=27.0%` / `dominant_regime=bull(99.0%)` / `avg_quality=+0.0006` / `avg_pnl=-0.0030` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3677` / `archive_window_coverage_pct=0.0`
...
- same-bucket truth：`bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=93/50` / `support_route_verdict=exact_bucket_supported` / `support_governance_route=exact_live_bucket_supported`
- support progress：`status=exact_supported` / `regression_basis=current_identity` / `legacy_supported_reference=121/50@20260424a`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3677` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_only` / `support_aware_profile=core_plus_macro_plus_all_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.27` / `dominant_regime=bull` / `dominant_regime_share=0.99` / `avg_pnl=-0.003`
