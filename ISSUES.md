...
## 當前主線事實
- **最新 fast heartbeat #20260420g 已完成 collect + diagnostics refresh**
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=145` / `recent 50 wins=0/50` / `additional_recent_window_wins_needed=15`
...
- 目前真相：`window=100` / `win_rate=0.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=-0.1930` / `avg_pnl=-0.0059`
- 病態切片：`alerts=constant_target,regime_concentration,regime_shift` / `tail_streak=100` / `low_variance=30` / `low_distinct=14` / `null_heavy=10`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2635` / `archive_window_coverage_pct=0.0`
