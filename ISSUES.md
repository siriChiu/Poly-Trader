...
_最後更新：2026-04-21 07:51:07 CST_
...
## 當前主線事實
- **最新 fast heartbeat #20260421-0746 已完成 collect + diagnostics refresh**
  - `Raw=31320 / Features=22738 / Labels=63146`
...
- **recent canonical window 仍是 distribution pathology**
  - `window=500` / `win_rate=14.2%` / `dominant_regime=bull(82.4%)` / `avg_quality=-0.1414` / `avg_pnl=-0.0052` / `alerts=label_imbalance,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2790` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=500` / `win_rate=14.2%` / `dominant_regime=bull(82.4%)` / `avg_quality=-0.1414` / `avg_pnl=-0.0052`
- 病態切片：`alerts=label_imbalance,regime_shift` / `tail_streak=65` / `low_variance=8` / `low_distinct=10` / `null_heavy=10`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['label_imbalance', 'regime_shift'], win_rate=0.1420, delta_vs_full=-0.4833, dominant_regime=bull(82.40%), interpretation=distribution_pathology, avg_pnl=-0.0052, avg_quality=-0.1414, avg_dd_penalty=0.3111, spot_long_win_rate=0.0400, feature_diag=variance:8/56, frozen:0, compressed:8, expected_static:2, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=65x1 since 2026-04-19 10:13:18.192660 -> 2026-04-20 00:47:15.429720, adverse_streak=198x0 since 2026-04-17 17:30:42.295048 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.704, delta_vs_prev=-0.562, prev_quality=0.3057, quality_delta_vs_prev=-0.4471, prev_pnl=0.0048, pnl_delta_vs_prev=-0.01, top_shift_examples=feat_eye(1.003→-2.1549,Δσ=0.8221)/feat_local_top_score(0.427→0.2604,Δσ=0.7416)/feat_local_bottom_score(0.2946→0.4512,Δσ=0.6855), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.0074/498)/feat_vix(0.0102/35), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-20 00:29:16.400867:1:chop:0.6972/2026-04-20 00:30:32.473772:1:chop:0.713/2026-04-20 00:47:15.429720:1:chop:0.7068, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2790` / `archive_window_coverage_pct=0.0`
