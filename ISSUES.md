...
_最後更新：2026-05-17 01:09:13 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1289-productization 已完成 collect + diagnostics refresh**
  - `Raw=33413 / Features=24547 / Labels=66676`
...
- **canonical 即時部署阻塞仍是熔斷優先真相**
  ...
  - support progress：`status=stalled_under_minimum` / `reason=current live exact support 連續 heartbeat 停在同一數量，屬於 support accumulation 停滯。` / `regression_basis=same_identity_same_semantic_signature` / `current_rows=5` / `minimum_rows=50` / `gap_to_minimum=45` / `support_rows_needed=45` / `previous_rows=5` / `delta_vs_previous=0` / `legacy_supported_reference=—` / `stagnant_run_count=3` / `stalled_support_accumulation=True` / `escalate_to_blocker=True`
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=46.4%` / `dominant_regime=chop(74.0%)` / `avg_quality=+0.1258` / `avg_pnl=+0.0014` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4138` / `archive_window_coverage_pct=0.0`
...
- **Execution Console / `/api/trade` 已 fail-closed（同步中 + 阻塞 + 直接 API）**
  ...
  - 本輪 operator-copy 補強：`/execution` 的同步中 / 阻塞中買入拒絕訊息已明確列出「減碼 / 賣出風險降低路徑 / 模式切換 / 查看阻塞原因仍可使用」，避免 operator 把 fail-closed 誤讀成所有風險降低操作都被鎖住。
...
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=79` / `all_horizon_losing_streak=0`
...
- latest matrix：`generated_at=2026-05-16T17:03:18.678598+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24433` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=5/50` / `current_live_structure_bucket_gap_to_minimum=45` / `support_progress_status=stalled_under_minimum` / `support_progress_reason=current live exact support 連續 heartbeat 停在同一數量，屬於 support accumulation 停滯。` / `regression_basis=same_identity_same_semantic_signature` / `delta_vs_previous=0` / `previous_rows=5` / `support_rows_needed=45` / `stagnant_run_count=3` / `stalled_support_accumulation=True` / `escalate_to_blocker=True`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4138` / `archive_window_coverage_pct=0.0`
