...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31334 / Features=22752 / Labels=63160`
...
- **recent canonical window 仍是 distribution pathology**
  - `window=500` / `win_rate=15.2%` / `dominant_regime=bull(81.4%)` / `avg_quality=-0.1317` / `avg_pnl=-0.0049` / `alerts=label_imbalance,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2804` / `archive_window_coverage_pct=0.0`
...
- **Dashboard blocker-first headline 已去除重複 icon / 文案**
  - `AdviceCard.tsx` 現在改為單一 headline icon，blocked/syncing headline 不再出現 `🚫 🚫` / `⏳ ⏳` 類重複 copy
  - 驗證：`pytest tests/test_frontend_decision_contract.py -q`、`npm run build`、browser `/`
...
- 目前真相：`window=500` / `win_rate=15.2%` / `dominant_regime=bull(81.4%)` / `avg_quality=-0.1317` / `avg_pnl=-0.0049`
- 病態切片：`alerts=label_imbalance,regime_shift` / `tail_streak=70` / `low_variance=8` / `low_distinct=10` / `null_heavy=10`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['label_imbalance', 'regime_shift'], win_rate=0.1520, delta_vs_full=-0.4734, dominant_regime=bull(81.40%), interpretation=distribution_pathology, avg_pnl=-0.0049, avg_quality=-0.1317, avg_dd_penalty=0.3081, spot_long_win_rate=0.0500, feature_diag=variance:8/56, frozen:0, compressed:8, expected_static:2, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=70x1 since 2026-04-19 10:13:18.192660 -> 2026-04-20 01:55:21.974179, adverse_streak=193x0 since 2026-04-17 17:39:03.533581 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.694, delta_vs_prev=-0.542, prev_quality=0.2987, quality_delta_vs_prev=-0.4304, prev_pnl=0.0046, pnl_delta_vs_prev=-0.0095, top_shift_examples=feat_eye(0.9702→-2.1418,Δσ=0.8102)/feat_local_top_score(0.4251→0.2611,Δσ=0.7296)/feat_local_bottom_score(0.2968→0.4514,Δσ=0.6767), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.0074/498)/feat_vix(0.01/34), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-20 01:13:46.162533:1:chop:0.7303/2026-04-20 01:38:21.691630:1:chop:0.6606/2026-04-20 01:55:21.974179:1:chop:0.6519, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2804` / `archive_window_coverage_pct=0.0`
