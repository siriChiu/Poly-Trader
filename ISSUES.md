...
## 當前主線事實
- **最新 full heartbeat #1118 已完成 collect + diagnostics refresh**
  - `Raw=32462 / Features=23880 / Labels=65545`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=10.0%` / `dominant_regime=chop(85.0%)` / `avg_quality=-0.2093` / `avg_pnl=-0.0093` / `alerts=label_imbalance,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3895` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 10/50，還差 5 勝；當前 q15 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 當前 q15 分桶 support / 治理背景
...
### P1. model stability still needs work (cv=0.5281, cv_std=0.0224, cv_worst=0.5057)
- 目前真相：`cv_accuracy=0.5280922431865829` / `cv_std=0.022431865828092257` / `cv_worst=0.5056603773584906`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3895` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=distribution_pathology` / `win_rate=0.1` / `dominant_regime=chop` / `dominant_regime_share=0.85` / `avg_pnl=-0.0093`
