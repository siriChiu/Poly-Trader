...
_最後更新：2026-04-30 18:07:20 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1156 已完成 collect + diagnostics refresh**
  - `Raw=32512 / Features=23930 / Labels=65630`
...
- **canonical current-live blocker 已切到 current-live exact-support truth**
  ...
  - support progress：`status=semantic_rebaseline_under_minimum` / `regression_basis=legacy_or_different_semantic_signature` / `legacy_supported_reference=53/50@20260419b` / `stagnant_run_count=3` / `stalled_support_accumulation=False` / `escalate_to_blocker=True`
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=27.0%` / `dominant_regime=chop(80.0%)` / `avg_quality=-0.0316` / `avg_pnl=-0.0033` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3945` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=under_minimum_exact_live_structure_bucket`；當前 q15 分桶支持樣本=4/50，缺口=46；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；`/execution/status` 與 `/execution` 會先顯示即時部署阻塞點，再顯示 當前 q15 分桶 support / 治理背景
...
- latest matrix：`generated_at=2026-04-30T05:54:13.297971+00:00` / `samples=23880` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=CAUTION|structure_quality_caution|q15` / `current_live_structure_bucket_rows=4/50` / `current_live_structure_bucket_gap_to_minimum=46`
...
### P1. model stability still needs work (cv=0.6470, cv_std=0.1122, cv_worst=0.5348)
- 目前真相：`cv_accuracy=0.6469849246231156` / `cv_std=0.1122278056951424` / `cv_worst=0.5347571189279732`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3945` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.27` / `dominant_regime=chop` / `dominant_regime_share=0.8` / `avg_pnl=-0.0033`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=4/50` / `gap=46`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示即時支持脈絡；即時分桶 / 支持阻塞未解除前仍 fail-closed。
