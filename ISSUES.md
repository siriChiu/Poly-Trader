...
_最後更新：2026-05-14 11:06:52 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1208 已完成 collect + diagnostics refresh**
  - `Raw=33172 / Features=24371 / Labels=66342`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.80%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=6` / `recent_window_wins=13/50` / `additional_recent_window_wins_needed=2`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=53.0%` / `dominant_regime=chop(100.0%)` / `avg_quality=+0.1700` / `avg_pnl=+0.0002` / `alerts=regime_concentration,regime_shift`
  - `blocking_window=250` / `win_rate=62.4%` / `dominant_regime=chop(94.4%)` / `avg_quality=+0.2590` / `avg_pnl=+0.0036` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4018` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 13/50，還差 2 勝；當前 q00 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 當前 q00 分桶 support / 治理背景；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端英文枚舉與混合式治理文案泄漏到 Dashboard / Strategy Lab / Execution Status
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=6` / `recent 50 wins=13/50` / `additional_recent_window_wins_needed=2`
...
- latest matrix：`generated_at=2026-05-14T03:03:17.080566+00:00` / `freshness=fresh` / `age_min=3.6` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24270` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=22/50` / `current_live_structure_bucket_gap_to_minimum=28`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4018` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=250` / `interpretation=regime_concentration` / `win_rate=0.624` / `dominant_regime=chop` / `dominant_regime_share=0.944` / `avg_pnl=0.0036`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-14T03:03:17.080566+00:00` / `freshness=fresh` / `age_min=3.6` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=22/50` / `gap=28` / `release_ready=False` / `recent_window_wins=13/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=2` / `current_recent_window_win_rate=0.260`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
