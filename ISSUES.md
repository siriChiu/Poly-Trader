...
## 當前主線事實
- **最新 fast heartbeat #20260423c 已完成 collect + diagnostics refresh**
  - `Raw=31581 / Features=22999 / Labels=63640`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=100.0%` / `dominant_regime=bull(87.0%)` / `avg_quality=+0.5970` / `avg_pnl=+0.0194` / `alerts=constant_target,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3051` / `archive_window_coverage_pct=0.0`
...
- latest diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=bull(87.0%)` / `avg_quality=+0.5970` / `avg_pnl=+0.0194` / `alerts=constant_target,regime_shift`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3051` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.0884`
