...
_最後更新：2026-04-25 13:35:37 CST_
...
## 當前主線事實
- **最新 full heartbeat #20260425_1324 已完成 collect + diagnostics refresh**
  - `Raw=32234 / Features=23652 / Labels=65028`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.90%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=3` / `recent_window_wins=2/50` / `additional_recent_window_wins_needed=13`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=25.0%` / `dominant_regime=bull(99.0%)` / `avg_quality=-0.0150` / `avg_pnl=-0.0034` / `alerts=regime_concentration,regime_shift`
- **leaderboard / governance 已收斂為 single-role alignment**
  - `leaderboard_count=6` / `selected_feature_profile=core_macro_plus_stable_4h` / `support_aware_profile=core_macro_plus_stable_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.8m`
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3701` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=3` / `recent 50 wins=2/50` / `additional_recent_window_wins_needed=13`
...
### P1. model stability still needs work (cv=0.5573, cv_std=0.0146, cv_worst=0.5426)
- 目前真相：`cv_accuracy=0.5572761985574883` / `cv_std=0.014637250742469243` / `cv_worst=0.5426389478150191`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3701` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_macro_plus_stable_4h` / `support_aware_profile=core_macro_plus_stable_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.8m`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.25` / `dominant_regime=bull` / `dominant_regime_share=0.99` / `avg_pnl=-0.0034`
