...
_最後更新：2026-04-30 03:03:50 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1136 已完成 collect + diagnostics refresh**
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 None 筆目前 None/None，還差 — 勝；當前 q00 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 當前 q00 分桶 support / 治理背景
...
- 驗證：
  ...
  - /api/models/leaderboard.high_conviction_topk.support_context 會在 `live_predict_probe.json` 較新時覆蓋 stale matrix support_context，維持 `BLOCK|structure_quality_block|q00 0/50 gap=50 no_support_proxy` 的 current-live truth
...
- 目前真相：`bucket=BLOCK|structure_quality_block|q00` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `governance_route=no_support_proxy`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3920` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.23` / `dominant_regime=chop` / `dominant_regime_share=0.96` / `avg_pnl=-0.0054`
