...
_最後更新：2026-05-15 18:24:50 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1247 已完成 collect + diagnostics refresh**
  - `Raw=33269 / Features=24456 / Labels=66506`
...
- latest matrix：`generated_at=2026-05-15T10:02:32.713983+00:00` / `freshness=fresh` / `age_min=22.3` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24346` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_supported` / `deployment_blocker=decision_quality_below_trade_floor` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=173/50` / `current_live_structure_bucket_gap_to_minimum=0`
...
### P1. model stability still needs work (cv=0.6657, cv_std=0.0965, cv_worst=0.5692)
- 目前真相：`cv_accuracy=0.6657084188911704` / `cv_std=0.09650924024640656` / `cv_worst=0.5691991786447639`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4074` / `archive_window_coverage_pct=0.0`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-15T10:02:32.713983+00:00` / `freshness=fresh` / `age_min=22.3` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=173/50` / `gap=0`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
