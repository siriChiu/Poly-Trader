...
*最後更新：2026-04-14 06:08 UTC — Heartbeat #717 進一步修正 live decision-quality calibration fallback：`model/predictor.py` 不再因為單純 `label_imbalance` 就把 **正向的 same-regime same-quality pocket** 一律拒絕，也不再在 chosen scope 幾乎沒有 live `structure_bucket` 支持時保留過度樂觀的 broader-scope expectation。新 contract 現在新增兩條約束：**(1)** calibration lane 會把 `regime_label+entry_quality_label` 納入正式 fallback 候選，避免 `bull+C` live path 被 `neutral|ALLOW` 假 proxy 直接壓回 global；**(2)** 當 chosen lane 對當前 `structure_bucket` 支持不足時，會用可取得的 same-bucket metrics 把 `expected_win/pnl/quality/dd/tuw` 向下夾回保守值，而不是只打 guardrail 不改 expectation。驗證：`PYTHONPATH=. pytest tests/test_api_feature_history_and_predictor.py -q` → **24 passed**；`PYTHONPATH=. pytest tests/test_hb_parallel_runner.py tests/test_auto_propose_fixes.py -q` → **19 passed**；`python scripts/hb_predict_probe.py`（patch 後、breaker 觸發前）顯示 `decision_quality_calibration_scope=regime_label+entry_quality_label`、`decision_quality_sample_size=46`，且 expectation 已被 bucket-support clamp 到 `win=0.50 / pnl=-0.0019 / quality=0.0996 / dd=0.2676 / tuw=0.4808`，不再沿用先前 global 的樂觀 proxy。當前最新 live row 已被 circuit breaker 攔下，因此 fast heartbeat probe 回到 `used_model=circuit_breaker` / `decision_quality_scope=None`；這代表本輪前進是 **calibration fallback contract 更誠實**，不是放寬 live 交易。下一輪需繼續處理 circuit-breaker lane 與 q35 bucket 歷史口袋不足，而不是再讓 wider q65/q85 spillover 代表 current live structure。*
...
- **P1#1 — `#H_AUTO_LIVE_DQ_PATHOLOGY` 已從「scope-level structure mismatch 可 machine-check」推進到「calibration fallback contract 會先避開假 proxy、再用 same-bucket metrics 保守夾值」**：Heartbeat #715 先把 exact `bull+ALLOW+D` toxic ALLOW pocket 從 gate 本身切掉；Heartbeat #716 再把 `structure_bucket` 正式接進 live decision-quality contract；Heartbeat #717 進一步修掉兩個 fallback 語義漏洞。現在 `model/predictor.py` / `hb_predict_probe.py` 不只會暴露 current live `structure_bucket`、各 scope 的 `recent500_structure_bucket_counts` 與 `current_live_structure_bucket_rows/share/metrics`，還會在 `regime_gate+entry_quality_label` 是 **負向 constant-target proxy** 時改走 `regime_label+entry_quality_label`，並在 chosen scope 幾乎沒有 live bucket 支持時，用可取得的 same-bucket metrics 把 expectation 向下夾回保守值。Patch 後的 direct probe（breaker 觸發前）已從舊的 `global` proxy 變成 **`decision_quality_calibration_scope=regime_label+entry_quality_label` / sample_size=46**，且 expectation 被 q35 bucket support clamp 到 **`win=0.50 / pnl=-0.0019 / quality=0.0996`**；不再讓 `neutral|ALLOW` 或 wider global bucket 代表 current `bull + q35` live 結構。剩餘 root cause 因此更精準：**q35 live structure 的歷史口袋仍然不足，而且最新 row 已直接落入 circuit breaker path；下一輪要決定是補 q35 history 還是把 breaker lane 納入同一條 calibration / probe contract。**
