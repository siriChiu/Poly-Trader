...
*最後更新：2026-04-14 03:35 UTC — Heartbeat #713 把 `#H_AUTO_LIVE_DQ_PATHOLOGY` 從 **spillover pocket machine-check** 再往前推到 **exact live lane toxicity surfacing**。本輪 patch：`scripts/auto_propose_fixes.py::summarize_live_predict_probe()` 現在會直接摘要 `exact_live_lane=(rows/wr/q/dd/tuw/recent500_dom/targets/true_negative_rows/final_gate)`，並在 exact lane 本身仍是 `ALLOW`、但 `canonical_true_negative_share` 偏高時標記 `exact_lane_status=toxic_allow_lane`；`auto_propose_fixes.py` 的 `#H_AUTO_LIVE_DQ_PATHOLOGY` wording 也同步升級成同時要求檢查 **exact live lane + calibration scope + narrowed pathology lane**。驗證：`python -m pytest tests/test_auto_propose_fixes.py tests/test_hb_parallel_runner.py -q` → **19 passed**；`python scripts/auto_propose_fixes.py` → live probe 摘要已明確輸出 **`exact_live_lane=(rows=24, wr=0.2917, q=-0.005, ... targets=loss:17/win:7, true_negative_rows=17@0.7083, final_gate=ALLOW:24), exact_lane_status=toxic_allow_lane`**。結論修正：broader `bull|BLOCK` pocket 仍是 canonical 真負樣本（106/106 loss），但 **exact `bull+ALLOW+D` lane 自己也不是健康 pocket**；下一輪不能只追 spillover `bull|BLOCK` provenance，還必須直接 drill-down 為何 `ALLOW` exact lane 仍有 **17/24 loss**。*
