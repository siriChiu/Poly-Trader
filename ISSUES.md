...
_最後更新：2026-04-30 04:03:21 CST_
...
## 當前主線事實
- **最新 full heartbeat #1137 已完成 collect + diagnostics refresh**
  - `Raw=32488 / Features=23906 / Labels=65583`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=23.0%` / `dominant_regime=chop(94.0%)` / `avg_quality=-0.0812` / `avg_pnl=-0.0053` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3921` / `archive_window_coverage_pct=0.0`
...
### P1. model stability still needs work (cv=0.6438, cv_std=0.0985, cv_worst=0.5453)
- 目前真相：`cv_accuracy=0.6437552388935457` / `cv_std=0.09849119865884326` / `cv_worst=0.5452640402347024`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3921` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.23` / `dominant_regime=chop` / `dominant_regime_share=0.94` / `avg_pnl=-0.0053`
