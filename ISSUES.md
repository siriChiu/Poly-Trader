...
## 當前主線事實
- **最新 full heartbeat #20260425_070309 已完成 collect + diagnostics refresh**
  - `Raw=32204 / Features=23622 / Labels=64973`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.93%`
...
- **leaderboard / governance 已收斂為 single-role alignment**
  ...
  - `background_refresh_candidate_surface=bounded_selected_plus_core_only`，artifact-driven ablation recommendation 不再擠掉 `core_only` 的兩候選 cron-safe fallback
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=41` / `recent 50 wins=4/50` / `additional_recent_window_wins_needed=11`
- same-bucket truth：`bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=101/50` / `support_route_verdict=exact_bucket_supported` / `support_governance_route=exact_live_bucket_supported`
- support progress：`status=exact_supported` / `regression_basis=current_identity` / `legacy_supported_reference=121/50@20260424a`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3671` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_macro_plus_stable_4h` / `support_aware_profile=core_plus_macro_plus_all_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment`
- 下一步：Keep /api/models/leaderboard and Strategy Lab aligned on latest bounded walk-forward plus the recent-two-year backtest policy; background refresh must stay bounded to training-selected profile + `core_only`, and must not regress to placeholder-only, ambiguous backtest windows, or artifact-driven candidate explosions.
- 驗證：
  - browser /lab
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.31` / `dominant_regime=bull` / `dominant_regime_share=0.99` / `avg_pnl=-0.0021`
