...
## 當前主線事實
- **最新 fast heartbeat #20260420-2042 已完成 collect + diagnostics refresh**
  - `Raw=31260 / Features=22678 / Labels=63042`
...
- **recent canonical window 仍是 distribution pathology**
  - `window=500` / `win_rate=4.6%` / `dominant_regime=bull(90.0%)` / `avg_quality=-0.2196` / `avg_pnl=-0.0076` / `alerts=label_imbalance,regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2731` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=500` / `win_rate=4.6%` / `dominant_regime=bull(90.0%)` / `avg_quality=-0.2196` / `avg_pnl=-0.0076`
- 病態切片：`alerts=label_imbalance,regime_concentration,regime_shift` / `tail_streak=17` / `low_variance=9` / `low_distinct=10` / `null_heavy=10`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['label_imbalance', 'regime_concentration', 'regime_shift'], win_rate=0.0460, delta_vs_full=-0.5785, dominant_regime=bull(90.00%), interpretation=distribution_pathology, avg_pnl=-0.0076, avg_quality=-0.2196, avg_dd_penalty=0.3217, spot_long_win_rate=0.0000, feature_diag=variance:9/56, frozen:0, compressed:9, expected_static:3, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=17x1 since 2026-04-19 10:13:18.192660 -> 2026-04-19 13:36:32.320348, adverse_streak=246x0 since 2026-04-17 14:13:05.506716 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.8, delta_vs_prev=-0.754, prev_quality=0.3749, quality_delta_vs_prev=-0.5945, prev_pnl=0.0065, pnl_delta_vs_prev=-0.0141, top_shift_examples=feat_eye(0.843→-1.6639,Δσ=0.6544)/feat_local_top_score(0.3984→0.292,Δσ=0.4732)/feat_vwap_dev(-0.2535→-0.1495,Δσ=0.4611), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0/500)/feat_ear(0.006/498)/feat_tongue(0.0089/500), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_nq_return_24h[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-19 12:51:32.658060:1:chop:0.4534/2026-04-19 13:21:43.939574:1:bull:0.3333/2026-04-19 13:36:32.320348:1:bull:0.3643, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2731` / `archive_window_coverage_pct=0.0`
