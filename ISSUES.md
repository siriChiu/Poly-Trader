...
_最後更新：2026-04-28 12:24:30 CST_
...
## 當前主線事實
- **最新 full heartbeat #1098 已完成 collect + diagnostics refresh**
  - `Raw=32426 / Features=23844 / Labels=65412`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=500` / `win_rate=53.2%` / `dominant_regime=bull(75.6%)` / `avg_quality=+0.1594` / `avg_pnl=+0.0011` / `alerts=regime_shift`
  - `blocking_window=1000` / `win_rate=54.2%` / `dominant_regime=bull(86.0%)` / `avg_quality=+0.1714` / `avg_pnl=+0.0028` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3867` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 13/50，還差 2 勝；支持樣本 / q15 修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 support / q15 治理背景
...
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=37` / `all_horizon_losing_streak=11`
...
### P1. model stability still needs work (cv=0.5372, cv_std=0.0398, cv_worst=0.4975)
- 目前真相：`cv_accuracy=0.5372474747474747` / `cv_std=0.039772727272727265` / `cv_worst=0.49747474747474746`
...
- 目前真相：`bucket=CAUTION|base_caution_regime_or_bias|q00` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `governance_route=no_support_proxy`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3867` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=1000` / `interpretation=regime_concentration` / `win_rate=0.542` / `dominant_regime=bull` / `dominant_regime_share=0.86` / `avg_pnl=0.0028`
...
## Current Priority
1. **維持熔斷優先真相，同時保留 current live bucket support rows 可 machine-read**
