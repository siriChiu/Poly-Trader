...
_最後更新：2026-05-18 08:36:53 CST_
...
## 當前主線事實
- **最新 full heartbeat #1321-productization 已完成 collect + diagnostics refresh**
  - `Raw=33527 / Features=24630 / Labels=66844`
...
- **canonical current-live blocker 已切到 current-live exact-support truth**
  ...
  - support progress：`status=semantic_rebaseline_under_minimum` / `reason=current exact support 0/50 below minimum (gap=50); legacy 190/50@1202 remains reference-only` / `regression_basis=legacy_or_different_semantic_signature` / `current_rows=0` / `minimum_rows=50` / `gap_to_minimum=50` / `support_rows_needed=50` / `previous_rows=—` / `delta_vs_previous=—` / `legacy_supported_reference=190/50@1202` / `stagnant_run_count=0` / `stalled_support_accumulation=False` / `escalate_to_blocker=True`
  - current-bucket root cause 已產品化到 probe / drilldown：`current_bucket_root_cause.verdict=same_lane_neighbor_bucket_dominates` / `candidate_patch_type=structure_component_scoring` / `candidate_patch_feature=feat_4h_bb_pct_b` / `dominant_neighbor_bucket=CAUTION|structure_quality_caution|q15`；這是 4H 結構 component 根因與下一輪 counterfactual 方向，不是部署放行或 same-bucket support closure。
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=23.0%` / `dominant_regime=bear(100.0%)` / `avg_quality=-0.0866` / `avg_pnl=-0.0062` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - top source blockers：`fin_netflow(source_auth_blocked/auth_missing, coverage=0.0%, archive_window=0.0%, forward_archive=ready, next=configure [REDACTED] source credentials)` / `claw(source_auth_blocked/auth_missing, coverage=14.5%, archive_window=84.6%, forward_archive=ready, next=configure [REDACTED] source credentials)` / `claw_intensity(source_auth_blocked/auth_missing, coverage=14.5%, archive_window=84.6%, forward_archive=ready, next=configure [REDACTED] source credentials)` / `nest_pred(source_tls_verify_failed/tls_verify_failed, coverage=16.1%, archive_window=94.0%, forward_archive=ready)`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4191` / `archive_window_coverage_pct=0.0`
...
- **Live DQ drilldown operator-facing markdown 已 enum-safe，且帶 current-bucket root-cause**
  ...
  - `scripts/hb_predict_probe.py` 與 `scripts/live_decision_quality_drilldown.py` 會把 `data/q15_bucket_root_cause.json` 投影到 `q15_bucket_root_cause` / `current_bucket_root_cause`，目前根因為 `same_lane_neighbor_bucket_dominates`，候選 feature 是 `feat_4h_bb_pct_b`，但 exact support 仍是 0/50。
...
- same-bucket truth：`support_route_verdict=exact_bucket_unsupported_block` / `support_governance_route=no_support_proxy` / `recommended_patch=—` / `recommended_patch_status=—` / `reference_scope=—`
- root-cause surface：probe / drilldown 目前直接輸出 `current_bucket_root_cause` 與 `q15_bucket_root_cause`；`verdict=same_lane_neighbor_bucket_dominates` / `candidate_patch_type=structure_component_scoring` / `candidate_patch_feature=feat_4h_bb_pct_b` / `dominant_neighbor_bucket=CAUTION|structure_quality_caution|q15` / `support_gap_to_minimum=50`。
- support progress：`status=semantic_rebaseline_under_minimum` / `reason=current exact support 0/50 below minimum (gap=50); legacy 190/50@1202 remains reference-only` / `regression_basis=legacy_or_different_semantic_signature` / `current_rows=0` / `minimum_rows=50` / `gap_to_minimum=50` / `support_rows_needed=50` / `previous_rows=—` / `delta_vs_previous=—` / `legacy_supported_reference=190/50@1202` / `stagnant_run_count=0` / `stalled_support_accumulation=False` / `escalate_to_blocker=True`
...
### P1. model stability still needs work (cv=0.5498, cv_std=0.0077, cv_worst=0.5420)
- 目前真相：`cv_accuracy=0.5497553017944534` / `cv_std=0.007748776508972266` / `cv_worst=0.5420065252854812`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4191` / `archive_window_coverage_pct=0.0`
...
## Current Priority
1. **維持 current-live exact-support blocker truth，同時保留 q00 current-live bucket support rows 與 root-cause surface 可 machine-read**
2. **持續沿 recent canonical pathological slice / `feat_4h_bb_pct_b` current-bucket root cause 追根因，不要 generic 化 blocker**
