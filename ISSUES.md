...
_最後更新：2026-04-22 18:41:17 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31515 / Features=22933 / Labels=63540`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=82.8%` / `dominant_regime=chop(55.6%)` / `avg_quality=+0.4479` / `avg_pnl=+0.0121` / `alerts=label_imbalance`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2985` / `archive_window_coverage_pct=0.0`
...
- **`/health` runtime_build 現在可 machine-read source-aware freshness**
  - `runtime_build` 新增 `head_sync_basis / latest_runtime_source_path / latest_runtime_source_modified_at`
  - stale lane 不再只靠 git commit time 判定；若 runtime source 已更新但 backend 尚未重啟，lane 仍會維持 `head_sync_status=stale_head_commit`
...
- latest diagnostics：`latest_window=250` / `win_rate=82.8%` / `dominant_regime=chop(55.6%)` / `avg_quality=+0.4479` / `avg_pnl=+0.0121` / `alerts=label_imbalance`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2985` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.1191`
- q35 scaling audit 已指出目前不是單點 bias50 closure：`overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.1191`
