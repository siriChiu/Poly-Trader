...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31564 / Features=22982 / Labels=63629`
  - `simulated_pyramid_win=57.38%`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=100.0%` / `dominant_regime=bull(88.0%)` / `avg_quality=+0.5877` / `avg_pnl=+0.0187` / `alerts=constant_target,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3034` / `archive_window_coverage_pct=0.0`
...
- **operator-facing blocker copy 已進一步產品化成中文 humanized route / closure labels**
  - `Dashboard / ExecutionConsole / ExecutionStatus / StrategyLab / LivePathologySummaryCard / SignalBanner / ConfidenceIndicator` 現在會把 `support_route / support_governance_route / runtime_closure_state / q15 floor-cross / q15 component experiment / q35 verdict` token 轉成可讀中文，而不是直接洩漏 raw machine token
  - 驗證：`pytest tests/test_frontend_decision_contract.py -q`、`cd web && npm run build`、browser `/` `/execution` `/execution/status` `/lab` 均確認不再出現 `exact_bucket_unsupported_block / no_support_proxy / patch_inactive_or_blocked` raw token
...
- latest diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=bull(88.0%)` / `avg_quality=+0.5877` / `avg_pnl=+0.0187` / `alerts=constant_target,regime_shift`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3034` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`bucket=BLOCK|bull_high_bias200_overheat_block|q35` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_unsupported_block` / `overall_verdict=bias50_formula_may_be_too_harsh` / `redesign_verdict=base_stack_redesign_candidate_grid_empty` / `remaining_gap_to_floor=0.1477`
