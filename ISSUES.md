...
## 當前主線事實
- **最新 fast heartbeat #20260422al 已完成 collect + diagnostics refresh**
  - `Raw=31558 / Features=22976 / Labels=63608`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=94.0%` / `dominant_regime=bull(92.0%)` / `avg_quality=+0.5258` / `avg_pnl=+0.0163` / `alerts=label_imbalance,regime_concentration,regime_shift`
...
- **Strategy Lab saved strategy detail 不再把 2 年回測視窗裁成交易視窗**
  - `_select_strategy_chart_payload()` 與 `_backfill_strategy_backtest_range()` 現在保留完整 backtest window；`/api/strategies/{name}` 的 `chart_context / available / effective` 已重新對齊最近兩年固定視窗，不再把首筆~末筆 trade 誤當成可用歷史範圍
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3028` / `archive_window_coverage_pct=0.0`
...
- latest diagnostics：`latest_window=100` / `win_rate=94.0%` / `dominant_regime=bull(92.0%)` / `avg_quality=+0.5258` / `avg_pnl=+0.0163` / `alerts=label_imbalance,regime_concentration,regime_shift`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3028` / `archive_window_coverage_pct=0.0`
