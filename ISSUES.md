...
_最後更新：2026-04-22 14:20:42 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31493 / Features=22911 / Labels=63495`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=1000` / `win_rate=39.4%` / `dominant_regime=bull(81.3%)` / `avg_quality=+0.0779` / `avg_pnl=+0.0005` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2963` / `archive_window_coverage_pct=0.0`
...
- **metadata freshness / external monitor detail 已在三個 operator-facing surfaces 共用，且前端 production build 已恢復綠燈**
  - `ExecutionMetadataFreshnessDetail` 已掛到 `/`、`/execution/status`、`/lab`；browser 可見 `external monitor host scheduler observed-ticking · lane user_crontab`
  - 驗證：`pytest tests/test_frontend_decision_contract.py -q`、`cd web && npm run build`
...
- same-bucket truth：`support_route_verdict=exact_bucket_unsupported_block` / `support_governance_route=no_support_proxy` / `recommended_patch=core_plus_macro_plus_all_4h` / `recommended_patch_status=reference_only_until_exact_support_ready` / `reference_scope=bull|CAUTION`
...
- 目前真相：`window=1000` / `win_rate=39.4%` / `dominant_regime=bull(81.3%)` / `avg_quality=+0.0779` / `avg_pnl=+0.0005` / `alerts=regime_shift`
- 病態切片：`alerts=regime_shift` / `tail_streak=—` / `top_shift=feat_4h_bias200,feat_vwap_dev,feat_dxy` / `new_compressed=feat_vix`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=1000, alerts=['regime_shift'], win_rate=0.3940, delta_vs_full=-0.2323, dominant_regime=bull(81.30%), interpretation=regime_concentration, avg_pnl=+0.0005, avg_quality=0.0779, avg_dd_penalty=0.2375, spot_long_win_rate=0.1820, feature_diag=variance:7/56, frozen:0, compressed:7, expected_static:0, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=47x1 since 2026-04-21 00:05:39.160858 -> 2026-04-21 06:51:38.316218, adverse_streak=273x0 since 2026-04-17 13:41:12.414408 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.917, delta_vs_prev=-0.523, prev_quality=0.5317, quality_delta_vs_prev=-0.4538, prev_pnl=0.0144, pnl_delta_vs_prev=-0.0139, top_shift_examples=feat_4h_bias200(4.2345→7.6767,Δσ=0.6301)/feat_vwap_dev(-0.309→-0.1751,Δσ=0.5957)/feat_dxy(98.4799→98.1323,Δσ=0.3354), new_compressed=feat_vix, compressed_examples=feat_body(0.0001/998)/feat_ear(0.0102/996)/feat_tongue(0.0143/997), overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-21 06:07:46.966913:1:bull:0.6396/2026-04-21 06:24:29.614954:1:bull:0.6402/2026-04-21 06:51:38.316218:1:bull:0.6211, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2963` / `archive_window_coverage_pct=0.0`
