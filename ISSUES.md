...
*最後更新：2026-04-14 06:48 UTC — Heartbeat #718 針對 live bull path 的 **假 ALLOW 語義** 做了 source-level 收斂：`model/predictor.py::_compute_live_regime_gate_debug()` 與 `backtesting/strategy_lab.py::_compute_regime_gate()` 現在會把 **base ALLOW 但 `structure_quality < 0.65`** 的 borderline 4H 結構直接降為 `CAUTION`，不再把 `ALLOW|q35` 這種只有 **2 rows / 5000-window** 支持的稀薄 bucket 包裝成可放心部署的 ALLOW lane。驗證：`python -m pytest tests/test_api_feature_history_and_predictor.py tests/test_strategy_lab.py -q` → **45 passed**；`python scripts/hb_parallel_runner.py --fast --hb 718` → **Raw 21390 / Features 12819 / Labels 42692**, live probe 由上一輪 `bull/ALLOW/D` 收斂成 **`bull/CAUTION/D`**，exact live lane 變成 `rows=17, wr=0.2353, q=-0.0626, true_negative_share=0.7647, final_gate=CAUTION:17`，chosen-scope recent pathology 也不再誤掛在 broader CAUTION lane 上（`recent_pathology=False`），但 narrowed `regime_label+entry_quality_label` bull pocket 仍是 **147 rows / wr=0.0748 / q=-0.2098 / BLOCK-dominant**，且 current live `CAUTION|structure_quality_caution|q35` 仍只有 **2 rows** 支持，所以 execution 仍被 `unsupported_live_structure_bucket_blocks_trade` 擋下。這輪前進是：**把弱結構 bull lane 從假 ALLOW 改成明確 CAUTION，讓 live / Strategy Lab / auto-propose 的語義一致**；下一輪 gate 改成處理 q35 同桶歷史稀缺與 narrowed bull pathology，而不是再容忍 borderline 結構被標成 ALLOW。*
...
- **P1#1 — `#H_AUTO_LIVE_DQ_PATHOLOGY`**：Heartbeat #718 把 live bull path 再往 source-level 推進一步：**borderline 4H 結構 (`structure_quality < 0.65`) 不再保留 ALLOW，而是直接降為 CAUTION**。這修掉了 `bull/ALLOW/q35` 的假語義，讓 `model/predictor.py` / `backtesting/strategy_lab.py` / `hb_predict_probe.py` 不再把只有 **2 rows** 歷史支持的弱結構 bucket 包裝成 ALLOW。驗證後 live probe 已從 `bull/ALLOW/D` 收斂成 **`bull/CAUTION/D`**，exact live lane 也變成 `rows=17 / wr=0.2353 / q=-0.0626 / true_negative_share=0.7647 / final_gate=CAUTION:17`。**剩餘 root cause 更清楚**：不是 exact live lane 還在假 ALLOW，而是 narrowed `regime_label+entry_quality_label` bull pocket 仍有 **147 rows / wr=0.0748 / q=-0.2098 / BLOCK-dominant**，而且 current live `CAUTION|structure_quality_caution|q35` 仍只有 **2 rows** 同桶支持。下一輪 gate 因此收斂成兩件事：**(a)** 決定 q35 同桶歷史要補 archive / 合併 bucket / 還是維持 hard runtime block；**(b)** 繼續對 bull narrowed pocket 的 4H collapse root cause 做 source-level drill-down，而不是再修 ALLOW/CAUTION 語義。 
