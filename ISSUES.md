...
## 當前主線事實
- **最新 full heartbeat #20260425_015620 已完成 collect + diagnostics refresh**
  - `Raw=32190 / Features=23608 / Labels=64926`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=500` / `win_rate=53.8%` / `dominant_regime=bull(99.4%)` / `avg_quality=+0.1159` / `avg_pnl=+0.0005` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3657` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3657` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=500` / `interpretation=regime_concentration` / `win_rate=0.538` / `dominant_regime=bull` / `dominant_regime_share=0.994` / `avg_pnl=0.0005`
