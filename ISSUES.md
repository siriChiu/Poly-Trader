...
_最後更新：2026-05-16 07:11:29 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1262 已完成 collect + diagnostics refresh**
  - `Raw=33341 / Features=24492 / Labels=66578`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.77%`
- **canonical current-live blocker 已切到 current-live exact-support truth**
  ...
  - `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `support=32/50` / `gap=18` / `support_route_verdict=exact_bucket_present_but_below_minimum`
  - support progress：`status=no_recent_comparable_history` / `regression_basis=no_same_identity_same_semantic_signature_history` / `legacy_supported_reference=—` / `stagnant_run_count=0` / `stalled_support_accumulation=False` / `escalate_to_blocker=False`
- **recent canonical diagnostics 已刷新**
  - `latest_window=500` / `win_rate=67.6%` / `dominant_regime=chop(75.4%)` / `avg_quality=+0.3181` / `avg_pnl=+0.0068` / `alerts=regime_shift`
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=under_minimum_exact_live_structure_bucket`；當前 q00 分桶支持樣本=32/50，缺口=18；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；`/execution/status` 與 `/execution` 會先顯示即時部署阻塞點，再顯示 當前 q00 分桶 support / 治理背景；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端 bucket / route / source / reference raw token 泄漏到 Dashboard / Strategy Lab / Execution Status / live DQ operator markdown
...
- **cron runtime facts extractor 已收斂成 compact current-state proof**
  - `scripts/hb_extract_runtime_facts.py` 會選最新 numbered heartbeat summary（目前 `data/heartbeat_1262_summary.json`），輸出 scalar counts / Top-K row_count / q15 nested support / `/api/trade` guardrail / reference-only patch，不再把整個 `rows[]` 矩陣傾倒進 cron 報告，也不再讀到較舊 alias summary 造成 `Raw/Features/Labels=None`。

---

## Open Issues

### P0. current live bucket BLOCK|structure_quality_block|q00 exact support remains under minimum and remains the deployment blocker (32/50)
- 目前真相：`deployment_blocker=under_minimum_exact_live_structure_bucket` / `bucket=BLOCK|structure_quality_block|q00` / `support=32/50` / `gap=18` / `runtime_closure_state=patch_inactive_or_blocked`
...
- latest matrix：`generated_at=2026-05-15T23:02:13.928617+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24383` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=32/50` / `current_live_structure_bucket_gap_to_minimum=18`
- nearest deployable candidate：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trade_count=58` / `tier=runtime_blocked_oos_pass` / `oos_gate_passed=True` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=BLOCK|structure_quality_block|q00` / `bucket_rows=32/50` / `gap=18`
...
### P1. model stability still needs work (cv=0.6358, cv_std=0.0765, cv_worst=0.5593)
- 目前真相：`cv_accuracy=0.6358227328682806` / `cv_std=0.07652851867049654` / `cv_worst=0.5592942141977841`
...
- 目前真相：`bucket=BLOCK|structure_quality_block|q00` / `support=32/50` / `gap=18` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `governance_route=exact_live_bucket_present_but_below_minimum`
- support progress：`status=no_recent_comparable_history` / `regression_basis=no_same_identity_same_semantic_signature_history` / `legacy_supported_reference=—` / `stagnant_run_count=0` / `stalled_support_accumulation=False` / `escalate_to_blocker=False`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4099` / `archive_window_coverage_pct=0.0`
...
5. **守住 cron 報告 extractor 的 compact proof contract：最新 summary、scalar Top-K、reference-only patch、API guardrail 必須同時可見**
6. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-15T23:02:13.928617+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=32/50` / `gap=18`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
