...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31427 / Features=22845 / Labels=63359`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=84.0%` / `dominant_regime=chop(84.0%)` / `avg_quality=+0.5117` / `avg_pnl=+0.0135` / `alerts=label_imbalance,regime_shift`
  - `blocking_window=500` / `win_rate=31.6%` / `dominant_regime=bull(65.0%)` / `avg_quality=+0.0266` / `avg_pnl=-0.0000` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2897` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=500` / `win_rate=31.6%` / `dominant_regime=bull(65.0%)` / `avg_quality=+0.0266` / `avg_pnl=-0.0000` / `alerts=regime_shift`
- latest diagnostics：`latest_window=100` / `win_rate=84.0%` / `dominant_regime=chop(84.0%)` / `avg_quality=+0.5117` / `avg_pnl=+0.0135` / `alerts=label_imbalance,regime_shift`
...
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['regime_shift'], win_rate=0.3160, delta_vs_full=-0.3103, dominant_regime=bull(65.00%), interpretation=regime_concentration, avg_pnl=-0.0000, avg_quality=0.0266, avg_dd_penalty=0.2498, spot_long_win_rate=0.1600, feature_diag=variance:8/56, frozen:0, compressed:8, expected_static:2, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=16x0 since 2026-04-20 18:07:54.813003 -> 2026-04-20 19:42:49.088202, adverse_streak=191x0 since 2026-04-18 14:33:06.990329 -> 2026-04-19 01:10:17.732530, prev_win_rate=0.498, delta_vs_prev=-0.182, prev_quality=0.1464, quality_delta_vs_prev=-0.1198, prev_pnl=0.0008, pnl_delta_vs_prev=-0.0008, top_shift_examples=feat_4h_bias20(2.029→0.2004,Δσ=1.1052)/feat_4h_bb_pct_b(0.7274→0.4004,Δσ=1.0174)/feat_4h_rsi14(63.5124→54.6215,Δσ=0.8871), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.0092/498)/feat_tongue(0.013/500), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-20 19:20:15.579228:0:bull:-0.26/2026-04-20 19:22:33.498951:0:bull:-0.2402/2026-04-20 19:42:49.088202:0:bull:-0.2233, adverse_examples=2026-04-19 00:40:09.929353:0:bull:-0.2753/2026-04-19 01:01:04.227452:0:bull:-0.3173/2026-04-19 01:10:17.732530:0:bull:-0.2383
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2897` / `archive_window_coverage_pct=0.0`
