...
## 當前主線事實
- **最新 full heartbeat #1265 已完成 collect + diagnostics refresh**
  - `Raw=33346 / Features=24497 / Labels=66589`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=500` / `win_rate=67.2%` / `dominant_regime=chop(75.4%)` / `avg_quality=+0.3143` / `avg_pnl=+0.0067` / `alerts=regime_shift`
...
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=35` / `all_horizon_losing_streak=0`
...
- latest matrix：`generated_at=2026-05-16T01:12:04.374020+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24389` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=10/50` / `current_live_structure_bucket_gap_to_minimum=40`
...
### P1. model stability still needs work (cv=0.6456, cv_std=0.0830, cv_worst=0.5625)
- 目前真相：`cv_accuracy=0.6455514555145552` / `cv_std=0.08302583025830257` / `cv_worst=0.5625256252562526`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4102` / `archive_window_coverage_pct=0.0`
