...
## 當前主線事實
- **最新 fast heartbeat #20260420-docsync 已完成 collect + diagnostics refresh**
  - `Raw=31170 / Features=22587 / Labels=62865`
  - `simulated_pyramid_win=57.23%`
- **canonical current-live blocker 仍是 breaker-first truth**
  - `deployment_blocker=circuit_breaker_active` / `streak=171` / `recent_window_wins=0/50` / `additional_recent_window_wins_needed=15`
...
- **recent canonical window 仍是 distribution pathology**
  - `window=100` / `win_rate=0.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=-0.2202` / `avg_pnl=-0.0083` / `alerts=constant_target,regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2641` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=171` / `recent 50 wins=0/50` / `additional_recent_window_wins_needed=15`
...
- 目前真相：`window=100` / `win_rate=0.0%` / `dominant_regime=bull(100.0%)` / `avg_quality=-0.2202` / `avg_pnl=-0.0083`
- 病態切片：`alerts=constant_target,regime_concentration,regime_shift` / `tail_streak=100` / `low_variance=29` / `low_distinct=14` / `null_heavy=10`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2641` / `archive_window_coverage_pct=0.0`
