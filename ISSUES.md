...
## 當前主線事實
- **最新 fast heartbeat #1188 已完成 collect + diagnostics refresh**
...
- **本輪 q15 runtime guardrail patch 已落地**
  - `model/predictor.py` 會在 current-live q15 audit 同 identity 且 `exact_bucket_supported` 時，以 `data/q15_support_audit.json` 作為 canonical support source，避免 stale DQ exact-scope 2-row 計數重新覆蓋 95/50 support closure；q15 component patch 仍必須通過 current-live bucket / regime / `machine_read_answer` / floor-cross legality 後才可啟用。
  - 最新 runtime：`q15_exact_supported_component_patch_applied=True` / `entry_quality=0.5501` / `allowed_layers_raw=1` / `allowed_layers=0` / `runtime_closure_state=patch_active_but_execution_blocked` / final blocker 仍為 `decision_quality_below_trade_floor`。
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=decision_quality_below_trade_floor`；當前 q15 分桶支持樣本=95/50，缺口=0；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；`/execution/status` 與 `/execution` 會先顯示即時部署阻塞點，再顯示 當前 q15 分桶 support / 治理背景
...
- latest matrix：`generated_at=2026-05-13T12:21:36.841568+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24255` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_supported` / `deployment_blocker=decision_quality_below_trade_floor` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=95/50` / `current_live_structure_bucket_gap_to_minimum=0`
- nearest deployable candidate：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trade_count=58` / `tier=runtime_blocked_oos_pass` / `oos_gate_passed=True` / `verdict=not_deployable` / `support_route=exact_bucket_supported` / `governance=exact_live_bucket_supported` / `bucket=CAUTION|base_caution_regime_or_bias|q15` / `bucket_rows=95/50` / `gap=0`
...
### P1. current live bucket CAUTION|base_caution_regime_or_bias|q15 is exact-supported; q15 component patch is active but final execution remains hold-only
- 目前真相：`deployment_blocker=decision_quality_below_trade_floor` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=95` / `minimum_support_rows=50` / `gap_to_minimum=0` / `support_route_verdict=exact_bucket_supported` / `q15_exact_supported_component_patch_applied=True` / `entry_quality=0.5501` / `allowed_layers_raw=1` / `allowed_layers=0`
- 下一步：把這個狀態視為正常 no-deploy risk posture 而非 release-blocking support failure：保留 allowed_layers_raw / final allowed_layers / trade_floor_gap / support metrics，並繼續要求 final decision-quality guardrail 通過後才重新開放下單。
- 驗證：
  - source venv/bin/activate && PYTHONPATH=. pytest tests/test_predictor_q15_component_patch.py tests/test_hb_predict_probe.py tests/test_q15_support_audit.py -q
  - source venv/bin/activate && PYTHONPATH=. python scripts/hb_predict_probe.py
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3991` / `archive_window_coverage_pct=0.0`
...
## Current Priority
1. **維持 current-live blocker truth（decision_quality_below_trade_floor），同時保留 q15 current-live bucket support rows 與 q15 patch 狀態可 machine-read**
2. **持續沿 recent canonical pathological slice 追根因，不要 generic 化 blocker**
3. **守住 q15 current-live bucket support truth / blocker truth、q15 patch guardrail、leaderboard dual-role governance、venue/source blockers 可見性**
4. **讓 heartbeat 自動 overwrite sync current-state docs，不再把 docs drift 留給人工補寫**
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-13T12:21:36.841568+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=95/50` / `gap=0`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度與即時支持脈絡；矩陣過期或即時分桶 / 支持阻塞未解除前仍 fail-closed。
