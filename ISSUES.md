...
_最後更新：2026-04-21 12:01:16 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31353 / Features=22771 / Labels=63202`
...
- **canonical current-live blocker 已切到 current-live exact-support truth**
  ...
  - `current_live_structure_bucket=CAUTION|structure_quality_caution|q15` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_missing_exact_lane_proxy_only`
- **recent canonical window 仍是 distribution pathology**
  - `window=500` / `win_rate=19.4%` / `dominant_regime=bull(77.2%)` / `avg_quality=-0.0910` / `avg_pnl=-0.0036` / `alerts=label_imbalance,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2823` / `archive_window_coverage_pct=0.0`
...
- same-bucket truth：`support_route_verdict=exact_bucket_missing_exact_lane_proxy_only` / `support_governance_route=exact_live_lane_proxy_available` / `recommended_patch=core_plus_macro_plus_all_4h` / `recommended_patch_status=reference_only_until_exact_support_ready` / `reference_scope=bull|CAUTION`
...
- 目前真相：`window=500` / `win_rate=19.4%` / `dominant_regime=bull(77.2%)` / `avg_quality=-0.0910` / `avg_pnl=-0.0036`
- 病態切片：`alerts=label_imbalance,regime_shift` / `tail_streak=91` / `low_variance=8` / `low_distinct=10` / `null_heavy=10`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['label_imbalance', 'regime_shift'], win_rate=0.1940, delta_vs_full=-0.4317, dominant_regime=bull(77.20%), interpretation=distribution_pathology, avg_pnl=-0.0036, avg_quality=-0.0910, avg_dd_penalty=0.2963, spot_long_win_rate=0.0920, feature_diag=variance:8/56, frozen:0, compressed:8, expected_static:2, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=91x1 since 2026-04-19 10:13:18.192660 -> 2026-04-20 04:37:25.630966, adverse_streak=191x0 since 2026-04-18 14:33:06.990329 -> 2026-04-19 01:10:17.732530, prev_win_rate=0.652, delta_vs_prev=-0.458, prev_quality=0.2684, quality_delta_vs_prev=-0.3594, prev_pnl=0.0039, pnl_delta_vs_prev=-0.0075, top_shift_examples=feat_4h_vol_ratio(0.7208→0.3299,Δσ=0.7013)/feat_eye(0.6892→-2.0009,Δσ=0.7005)/feat_local_top_score(0.4151→0.2668,Δσ=0.66), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.0077/498)/feat_vix(0.0096/33), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-20 04:34:39.688050:1:chop:0.6941/2026-04-20 04:36:46.944854:1:chop:0.6926/2026-04-20 04:37:25.630966:1:chop:0.6919, adverse_examples=2026-04-19 00:40:09.929353:0:bull:-0.2753/2026-04-19 01:01:04.227452:0:bull:-0.3173/2026-04-19 01:10:17.732530:0:bull:-0.2383
- 驗證：
...
- 目前真相：`bucket=CAUTION|structure_quality_caution|q15` / `support=0/50` / `gap=50` / `support_route_verdict=exact_bucket_missing_exact_lane_proxy_only` / `governance_route=exact_live_lane_proxy_available`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2823` / `archive_window_coverage_pct=0.0`
