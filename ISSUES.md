...
_最後更新：2026-05-14 08:17:06 CST_
...
## 當前主線事實
- **最新 full heartbeat #1203 已完成 collect + diagnostics refresh**
  - `Raw=33161 / Features=24360 / Labels=66331`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.80%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=3` / `recent_window_wins=14/50` / `additional_recent_window_wins_needed=1`
  - `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `support=22/50` / `gap=28` / `support_route_verdict=exact_bucket_present_but_below_minimum`
  - support progress：`status=accumulating` / `regression_basis=—` / `legacy_supported_reference=—`
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=56.0%` / `dominant_regime=chop(100.0%)` / `avg_quality=+0.1917` / `avg_pnl=+0.0005` / `alerts=regime_concentration,regime_shift`
  - `blocking_window=250` / `win_rate=63.6%` / `dominant_regime=chop(94.4%)` / `avg_quality=+0.2668` / `avg_pnl=+0.0036` / `alerts=regime_concentration,regime_shift`
- **leaderboard / governance 已收斂為 single-role alignment**
  - `leaderboard_count=6` / `selected_feature_profile=core_only` / `support_aware_profile=current_full` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.1m`
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - top source blockers：`fin_netflow(source_auth_blocked/auth_missing, coverage=0.0%, archive_window=0.0%, forward_archive=ready)` / `claw(source_auth_blocked/auth_missing, coverage=14.7%, archive_window=88.3%, forward_archive=ready)` / `claw_intensity(source_auth_blocked/auth_missing, coverage=14.7%, archive_window=88.3%, forward_archive=ready)` / `nest_pred(source_tls_verify_failed/tls_verify_failed, coverage=16.3%, archive_window=98.2%, forward_archive=ready)`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4010` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 14/50，還差 1 勝；當前 q00 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 當前 q00 分桶 support / 治理背景；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端英文枚舉與混合式治理文案泄漏到 Dashboard / Strategy Lab / Execution Status
- **Strategy Lab high-conviction Top-K 已補齊熔斷 release math overlay**
  - `/api/models/leaderboard.high_conviction_topk.support_context` 與 nearest rows 會從 `data/live_predict_probe.json` 補上 `release_ready / current_recent_window_wins / required_recent_window_wins / additional_recent_window_wins_needed`；即使 Top-K matrix 是 live probe 之後生成，只要 matrix 內嵌的 `source_live_probe_generated_at` 指向同一 probe，也會 hydrate release math，避免 Strategy Lab 只顯示 OOS pass 與 support gap、卻漏掉「14/50、需 15/50、還差 1 勝」的唯一解除條件
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=3` / `recent 50 wins=14/50` / `additional_recent_window_wins_needed=1`
- same-bucket truth：`bucket=BLOCK|structure_quality_block|q00` / `support=22/50` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `support_governance_route=exact_live_bucket_present_but_below_minimum`
- support progress：`status=accumulating` / `regression_basis=—` / `legacy_supported_reference=—`
...
- latest matrix：`generated_at=2026-05-14T00:03:17.992262+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24267` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=22/50` / `current_live_structure_bucket_gap_to_minimum=28` / `release_wins=14/15` / `additional_release_wins_needed=1`
- nearest deployable candidate：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trade_count=58` / `tier=runtime_blocked_oos_pass` / `oos_gate_passed=True` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=BLOCK|structure_quality_block|q00` / `bucket_rows=22/50` / `gap=28`
...
- 驗證：
  ...
  - source venv/bin/activate && python -m pytest tests/test_model_leaderboard.py::test_high_conviction_topk_embedded_live_probe_release_math_survives_matrix_refresh tests/test_frontend_decision_contract.py::test_strategy_lab_surfaces_high_conviction_topk_gate_contract -q
  - PYTHONPATH=/home/kazuha/Poly-Trader python /tmp/hb1203_verify_high_conviction_overlay.py（確認 `support=22/50 gap=28 release_wins=14/15 additional_wins=1`）

### P1. model stability still needs work (cv=0.5258, cv_std=0.0420, cv_worst=0.4837)
- 目前真相：`cv_accuracy=0.5257519571487433` / `cv_std=0.04202719406674907` / `cv_worst=0.48372476308199425`
...
- 目前真相：`bucket=BLOCK|structure_quality_block|q00` / `support=22/50` / `gap=28` / `support_route_verdict=exact_bucket_present_but_below_minimum` / `governance_route=exact_live_bucket_present_but_below_minimum`
- support progress：`status=accumulating` / `regression_basis=—` / `legacy_supported_reference=—`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4010` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_only` / `support_aware_profile=current_full` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.1m`
...
- 目前真相：`window=250` / `interpretation=regime_concentration` / `win_rate=0.636` / `dominant_regime=chop` / `dominant_regime_share=0.944` / `avg_pnl=0.0036`
...
3. **守住 q00 current-live bucket support / reference-only patch、leaderboard single-role governance、venue/source blockers 可見性**
4. **讓 heartbeat 自動 overwrite sync current-state docs，不再把 docs drift 留給人工補寫**
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-14T00:03:17.992262+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=22/50` / `gap=28` / `release_wins=14/15` / `additional_release_wins_needed=1`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、即時支持脈絡與熔斷解除條件；矩陣過期或即時分桶 / 支持 / 熔斷 release math 未解除前仍 fail-closed。
