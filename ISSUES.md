# ISSUES.md — 問題追蹤

*最後更新：2026-04-07 16:00 UTC（心跳 #578）*

## 📊 系統健康狀態（#578）

| 項目 | 數值 | vs #577 |
|------|------|---------|
| Raw | **9,885** | ⬆️ +5 🟢 |
| Features | **9,844** | ⬆️ +5 🟢 |
| Labels | **18,052** | ➡️ 持平 |
| sell_win (DB all) | **40.37%** | ➡️ 持平 |
| sell_win (regime join) | **49.24%** | ➡️ 持平（n=8770）|
| 全域 IC (22 擴） | **5/22** | ➡️ 持平 |
| TW-IC (22 擴） | **13/22** | ➡️ 持平（歷史新高維持）|
| DW N=100 | **7/8**🟢 | ➡️ 持平 |
| DW N=200 | **7/8**🟢 | ➡️ 持平 |
| DW N=400 | **3/8** | ➡️ 持平 |
| DW N=600 | **0/8** | ➡️ 持續死區 |
| DW N=1000 | **4/8** | ➡️ 持平 |
| Global model | Train=63.92%, CV=51.39%, gap=12.53pp | ➡️ 持平 |
| Regime IC | Bear **4/8**, Bull **0/8**🔴, Chop **0/8**🔴 | ➡️ 持平 |
| BTC (live) | **$68,656** | ⬇️ -$11（vs #577 $68,667）|
| FNG | **11**（持續極度恐懼）| ➡️ 持平 |
| FR | **0.00003731** | ⬇️ -1.3%（vs #577 0.00003778，微降）|
| LSR | **1.2523** | ⬆️ +11bps（vs #577 1.2512，微升）|
| OI | **91,084** | ⬆️ +2（vs #577 91,082）|
| 測試 | **6/6** ✅ | ➡️ 持平 |
| 平行心跳 | **5/5 PASS** 🟢（**32.2s**）| ✅ 全面通過 |

## 系統狀態：🟢 全面通過 / 數據持續增長 / BTC微降 / LSR微升 / 測試 6/6 全通過

數據管線持續增長：Raw +5, Features +5.
平行心跳 5/5 全面通過（32.2s），所有子任務成功。
LSR 微升至 1.2523（+11bps）：多頭長倉比例微升。
FR 微降至 0.00003731（-1.3%）：資金費率維持正區間但邊際下降。
BTC 微降至 $68,656（-$11 vs #577）。

- **BTC $68,656**: ⬇️ -$11 vs #577 $68,667，微降
- **⬇️ FR 0.00003731（-1.3%）**: 資金費率微降，維持正區間
- **⬆️ LSR 1.2523（+11bps）**: 長倉比例微升
- **⬆️ OI 91,084（+2）**: 持倉量微升
- **✅ 5/5 PASS**: 平行心跳全面通過（32.2s）
- **raw +5, features +5**: 🟢 數據管線持續增長
- **測試 6/6 ✅**: 全部通過
- **DW 持續**: N=100&200 7/8🟢，N=400 3/8，N=600 持續 0/8 死區
- **訓練**: 73 features, 9106 samples, CV=51.39%（持平）
- **Regime**: Bear 4/8（持續）, Bull 0/8🔴（200+輪持續！）, Chop 0/8🔴（200+輪持續！）

## 全域 IC — 5/22（持平）

**22 擴（全域 Spearman, 含4H/5TI特徵）**: 5/22 passing。
PASS: VIX +0.0714 ✅, RSI14 +0.0542 ✅, MACDHist +0.0505 ✅, BB%B +0.0575 ✅, Nose +0.0500 ✅（擦邊）

**TW-IC（13/22 過, tau=200, 歷史新高維持）**: Nose+0.0587, Pulse-0.0871, AURA+0.0799, Mind+0.0750, VIX+0.0876, RSI14+0.0746, MACD+0.0554, ATR%-0.1280, VWAP+0.1293, BB%B+0.0826, 4h_bias50+0.0715, 4h_rsi14+0.0622, 4h_dist_swing_low+0.0620
FAIL: Eye, Ear, Tongue, Body, DXY (core 8 中有 5 個失效); 4h_bias20, 4h_macd_hist, 4h_bb_pct_b, 4h_ma_order 失敗

## Dynamic Window — N=100&200 7/8（持平）

| Window | 通過 | 狀態 |
|--------|------|------|
| N=100 | **7/8**🟢 | eye+0.089, nose+0.177, tongue-0.115, body+0.129, pulse-0.081, aura+0.277, mind+0.230 |
| N=200 | **7/8**🟢 | eye+0.050, nose+0.114, tongue-0.082, body+0.062, pulse-0.131, aura+0.162, mind+0.167 |
| N=400 | **3/8** | Pulse, Aura, Mind |
| N=600 | **0/8** 🔴 | 持續死區 |
| N=1000 | 4/8 | Eye, Pulse, Aura, Mind |
| N=2000 | 2/8 | Pulse, Aura |
| N=5000 | 0/8 | 全滅 |

## Regime IC — Bear 4/8, Bull 0/8🔴, Chop 0/8🔴（持平）

| Regime | 通過 | 信號 |
|--------|------|------|
| Bear | **4/8** | Ear+0.0785✅, Nose+0.0727✅, Body+0.0682✅, Aura+0.0544✅ |
| Bull | **0/8**🔴 | 全部 <0.05（200+輪持續！）|
| Chop | **0/8**🔴 | 全部 <0.05（200+輪持續！）|

**sell_win by Regime**: Bear=0.4855, Bull=0.5090, Chop=0.4829, Overall=0.4924（n=8770）

## 市場數據亮點

- **BTC $68,656**: ⬇️ -$11（-0.02%）vs #577 $68,667，微降
- **⬇️ FR 0.00003731**: 微降（-1.3%），維持正區間
- **⬆️ LSR 1.2523**: 長倉比例微升（+11bps），多頭信心微增
- **⬆️ OI 91,084（+2）**: 持倉量微升
- **FNG 11**（持續極度恐慌）

## P0

| ID | 問題 | 狀態 |
|----|------|------|
| #P537 | sell_win≥50% 全域 | 🔴 **持續 40.37%（全量Labels），regime子集49.24%** |
| #RAW_STALE | Raw 數據增長 | 🟢 **+5！持續增長中！** |
| #P442 | Chop 0/8 | 🔴 持續 0/8（200+輪）|
| #BULL_BLIND | Bull 0/8 | 🔴 Bull 0/8 持續（200+輪持續！）|
| #IC_FREEZE | 全域/TW-IC/模型凍結 | 🟢 **穩定：全域 5/22，TW-IC 13/22（歷史新高維持）** |
| #NO_DATA | 新感官零數據（Claw/Fang/Fin）| 🔴 Collector 未收集 |
| #DW_DEAD | DW N=600/5000 死區 | 🔴 持續 0/8 |
| #FEATURE_MISMATCH | 4H 特徵部分有效 | 🟢 **3/6 通過 TW-IC（bias50+0.0715, rsi14+0.0622, dist_swing_low+0.0620）** |

## 測試狀態

| 測試 | 結果 |
|------|------|
| 檔案結構 | ✅ PASS |
| 語法檢查 | ✅ PASS (9978 文件) |
| 模組導入 | ✅ PASS (8/8) |
| 感官引擎真實數據 | ✅ PASS |
| 前端 TypeScript | ✅ PASS |
| 數據品質 | ✅ PASS |

## 模型訓練

- Global: Train=63.92%, CV=51.39%±3.66%, gap=12.53pp（持平），**73 features**, 9106 samples
- Positive ratio: 30.45%
- Regime models: bear/bull/chop 各 98 features saved
- ✅ **train.py 成功** — 訓練完整（3 regimes saved）

> Heartbeat #578: full_ic 5/22全域, **13/22 TW-IC 維持歷史新高**（持平！）, regime Bear 4/8, Bull 0/8 Chop 0/8, DW N=100&200 **7/8持平**, N=400 3/8, N=600持續0/8死區, 測試**6/6✅**, 平行心跳**5/5 PASS**🟢(32.2s), sell_win **40.37%**（持平）, T63.92%/CV51.39%(持平), BTC **$68,656**(-$11微降), FR ⬇️ **0.00003731**(-1.3%微降), LSR **1.2523**(+11bps微升), OI 91,084(+2), **數據管線持續增長：Raw+5, Features+5**！
