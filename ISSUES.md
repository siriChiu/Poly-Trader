...
## 當前主線事實
- **最新 fast heartbeat #1187 已完成 collect + diagnostics refresh**
...
- latest matrix：`generated_at=2026-05-13T11:31:20.157557+00:00` / `freshness=fresh` / `age_min=8.3` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24192` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_supported` / `deployment_blocker=decision_quality_below_trade_floor` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=95/50` / `current_live_structure_bucket_gap_to_minimum=0`
...
- 目前真相：`deployment_blocker=decision_quality_below_trade_floor` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=95` / `minimum_support_rows=50` / `gap_to_minimum=0` / `support_route_verdict=exact_bucket_supported`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3989` / `archive_window_coverage_pct=0.0`
