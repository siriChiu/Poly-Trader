...
_最後更新：2026-05-17 18:12:33 CST_
...
## 當前主線事實
- **最新 full heartbeat #1307-productization 已完成 collect + diagnostics refresh**
  - `Raw=33473 / Features=24592 / Labels=66765`
...
- **runtime facts cron 摘要已補上 legacy support reference-only compact proof**
  - `scripts/hb_extract_runtime_facts.py` 目前只輸出舊版 `53/50@20260419b` 的 compact proof：`rows=53 / minimum=50 / reference_only=True / supports_current_identity=False / promotable=False / mismatched=calibration_window,entry_quality_label,regime_label`；不再把完整 `semantic_identity_evidence / row_examples / backfilled_support_identity` dump 到 cron 摘要，避免 operator 把舊 bucket 支持誤讀成 current deployment closure。
  - 驗證：`tests/test_hb_extract_runtime_facts.py` 新增 legacy reference compaction regression；實際 `/tmp/hb1307_runtime_facts_after_patch.json` 已確認 `legacy_supported_reference` compact 且不含 heavyweight semantic rows。
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=4.0%` / `dominant_regime=bear(61.0%)` / `avg_quality=-0.2793` / `avg_pnl=-0.0117` / `alerts=label_imbalance,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4166` / `archive_window_coverage_pct=0.0`
...
- 下一步：先把即時部署阻塞語義切回熔斷解除條件；在熔斷未解除前，不要把 q15/q35 support 或 floor-gap 當成本輪主阻塞。 最近 50 筆需至少 15 勝，當前 4 勝，還差 11 勝；同時連續虧損必須 < 50。
...
- latest matrix：`generated_at=2026-05-17T10:02:50.717598+00:00` / `freshness=fresh` / `age_min=0.3` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24474` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=insufficient_support_everywhere` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=CAUTION|structure_quality_caution|q15` / `current_live_structure_bucket_rows=0/50` / `current_live_structure_bucket_gap_to_minimum=50` / `support_progress_status=semantic_rebaseline_under_minimum` / `support_progress_reason=current exact support 0/50 below minimum (gap=50); semantic mismatch=calibration_window,entry_quality_label,regime_label` / `regression_basis=legacy_or_different_semantic_signature` / `delta_vs_previous=-2` / `previous_rows=2` /
  `support_rows_needed=50` / `stagnant_run_count=0` / `stalled_support_accumulation=False` / `escalate_to_blocker=True`
- nearest deployable candidate：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trade_count=58` / `tier=runtime_blocked_oos_pass` / `oos_gate_passed=True` / `verdict=not_deployable` / `support_route=insufficient_support_everywhere` / `governance=exact_live_lane_proxy_available` / `bucket=CAUTION|structure_quality_caution|q15` / `bucket_rows=0/50` / `gap=50` / `support_progress_status=semantic_rebaseline_under_minimum` / `support_progress_reason=current exact support 0/50 below minimum (gap=50); semantic mismatch=calibration_window,entry_quality_label,regime_label` / `regression_basis=legacy_or_different_semantic_signature` / `delta_vs_previous=-2` / `previous_rows=2` / `support_rows_needed=50` / `stagnant_run_count=0` / `stalled_support_accumulation=False` / `escalate_to_blocker=True`
...
- 目前真相：`live_scope=regime_label+entry_quality_label` / `deployment_blocker=circuit_breaker_active` / `window=100` / `alerts=label_imbalance, regime_shift` / `allowed_layers=0`
- 下一步：把 hb_predict_probe 納入每輪 heartbeat 驗證，對 exact live lane、當前 calibration scope 與 worst narrowed scope 做 root-cause drill-down；優先檢查 exact lane 是否仍是 ALLOW 但 canonical true-negative share 已偏高，並交叉比對 recent same-scope / narrowed-scope 4H shifts、scope selection、與 execution guardrail 是否只是正確地把壞 pocket 擋下。 live_scope=regime_label+entry_quality_label, regime=bear/CAUTION, label=D, sample_size=30, window=30, alerts=['label_imbalance'], expected_win_rate=0.0667, expected_pnl=-0.0106, expected_quality=-0.2404, layers=1→0, top_shi…

### P1. model stability still needs work (cv=0.6661, cv_std=0.1030, cv_worst=0.5631)
- 目前真相：`cv_accuracy=0.6661217817736003` / `cv_std=0.10298324478953819` / `cv_worst=0.5631385369840621`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4166` / `archive_window_coverage_pct=0.0`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-17T10:02:50.717598+00:00` / `freshness=fresh` / `age_min=0.3` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=0/50` / `gap=50` / `release_ready=False` / `recent_window_wins=4/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=11` / `current_recent_window_win_rate=0.080` / `support_progress_status=semantic_rebaseline_under_minimum` / `support_progress_reason=current exact support 0/50 below minimum (gap=50); semantic mismatch=calibration_window,entry_quality_label,regime_label` / `regression_basis=legacy_or_different_semantic_signature` / `delta_vs_previous=-2` / `previous_rows=2` / `support_rows_needed=50` / `stagnant_run_count=0` / `stalled_support_accumulation=False` /
