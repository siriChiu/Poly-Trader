...
_最後更新：2026-05-14 00:12:29 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1194 已完成 collect + diagnostics refresh**
  - `Raw=33010 / Features=24340 / Labels=66301`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=65.6%` / `dominant_regime=chop(94.4%)` / `avg_quality=+0.2871` / `avg_pnl=+0.0042` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3999` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=under_minimum_exact_live_structure_bucket`；當前 q00 分桶支持樣本=22/50，缺口=28；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；`/execution/status` 與 `/execution` 會先顯示即時部署阻塞點，再顯示 當前 q00 分桶 support / 治理背景
...
### P0. current live bucket BLOCK|structure_quality_block|q00 exact support remains under minimum and remains the deployment blocker (22/50)
- 目前真相：`deployment_blocker=under_minimum_exact_live_structure_bucket` / `bucket=BLOCK|structure_quality_block|q00` / `support=22/50` / `gap=28` / `runtime_closure_state=patch_inactive_or_blocked`
...
- latest matrix：`generated_at=2026-05-13T16:03:07.949488+00:00` / `freshness=fresh` / `age_min=9.4` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24259` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=22/50` / `current_live_structure_bucket_gap_to_minimum=28`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3999` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=250` / `interpretation=regime_concentration` / `win_rate=0.656` / `dominant_regime=chop` / `dominant_regime_share=0.944` / `avg_pnl=0.0042`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-13T16:03:07.949488+00:00` / `freshness=fresh` / `age_min=9.4` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=22/50` / `gap=28`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度與即時支持脈絡；矩陣過期或即時分桶 / 支持阻塞未解除前仍 fail-closed。
