...
_最後更新：2026-05-18 16:03:44 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1329-productization 已完成 collect + diagnostics refresh**
  - `Raw=33556 / Features=24653 / Labels=66891`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.60%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=30` / `recent_window_wins=6/50` / `additional_recent_window_wins_needed=9`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=23.0%` / `dominant_regime=bear(100.0%)` / `avg_quality=-0.0760` / `avg_pnl=-0.0065` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4206` / `archive_window_coverage_pct=0.0`
  - venue：`adapter_unsupported / live exchange credential / order ack lifecycle / fill lifecycle` 尚未有 runtime-backed proof；`execution_metadata_smoke.venues[]` 已提供 per-venue `adapter_supported / proof_state / blockers / operator_next_action / verify_next` 給 Dashboard / Execution / Lab 直接顯示 adapter 與證據缺口；operator UI 會先 humanize backend error（例如 `unsupported venue` → `不支援的交易場館`），避免 raw venue error 洩漏到操作員畫面；本輪補強 `VenueReadinessSummary` runtime evidence copy：只有 `runtime_ready=true` 且無 blockers 才顯示「實單證據完成」，避免未來已完成生命週期證據的 venue 仍被誤標成「待補實單證據」
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 6/50，還差 9 勝；當前 q00 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；`/execution/status` 與 `/execution` 會先顯示熔斷解除條件，再顯示 當前 q00 分桶 support / 治理背景；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端 bucket / route / source / reference raw token 泄漏到 Dashboard / Strategy Lab / Execution Status / live DQ operator markdown
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=30` / `recent 50 wins=6/50` / `additional_recent_window_wins_needed=9`
...
### P1. model stability still needs work (cv=0.5318, cv_std=0.0002, cv_worst=0.5316)
- 目前真相：`cv_accuracy=0.5317977986139422` / `cv_std=0.00020383204239710073` / `cv_worst=0.531593966571545`
...
- API/UI contract：`execution_metadata_smoke.venues[]` 已帶 `adapter_supported / proof_state / blockers / operator_next_action / verify_next`，Dashboard、`/execution/status`、`/execution`、`/lab` 可直接顯示 OKX 與 Binance 每個場館的 adapter 與實單證據缺口，不再只靠 metadata OK/FAIL 猜測 readiness；`VenueReadinessSummary` 現在以 `runtime_ready=true` 且無 blockers 作為「實單證據完成」唯一條件，未完成證據時才顯示「待補實單證據」。
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4206` / `archive_window_coverage_pct=0.0`
