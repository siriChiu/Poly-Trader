...
_最後更新：2026-04-21 16:57:49 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31376 / Features=22794 / Labels=63264`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=100.0%` / `dominant_regime=chop(95.0%)` / `avg_quality=+0.6415` / `avg_pnl=+0.0194` / `alerts=constant_target,regime_concentration,regime_shift`
  - `blocking_window=500` / `win_rate=25.6%` / `dominant_regime=bull(71.0%)` / `avg_quality=-0.0277` / `avg_pnl=-0.0013` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2846` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=500` / `win_rate=25.6%` / `dominant_regime=bull(71.0%)` / `avg_quality=-0.0277` / `avg_pnl=-0.0013` / `alerts=regime_shift`
- latest diagnostics：`latest_window=100` / `win_rate=100.0%` / `dominant_regime=chop(95.0%)` / `avg_quality=+0.6415` / `avg_pnl=+0.0194` / `alerts=constant_target,regime_concentration,regime_shift`
- 病態切片：`alerts=regime_shift` / `tail_streak=—` / `top_shift=feat_4h_vol_ratio,feat_4h_bias20,feat_4h_bb_pct_b` / `new_compressed=feat_vwap_dev`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['regime_shift'], win_rate=0.2560, delta_vs_full=-0.3702, dominant_regime=bull(71.00%), interpretation=regime_concentration, avg_pnl=-0.0013, avg_quality=-0.0277, avg_dd_penalty=0.2747, spot_long_win_rate=0.1520, feature_diag=variance:8/56, frozen:0, compressed:8, expected_static:2, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=122x1 since 2026-04-19 10:13:18.192660 -> 2026-04-20 09:40:27.742431, adverse_streak=191x0 since 2026-04-18 14:33:06.990329 -> 2026-04-19 01:10:17.732530, prev_win_rate=0.59, delta_vs_prev=-0.334, prev_quality=0.2176, quality_delta_vs_prev=-0.2453, prev_pnl=0.0023, pnl_delta_vs_prev=-0.0036, top_shift_examples=feat_4h_vol_ratio(0.7581→0.2965,Δσ=0.8285)/feat_4h_bias20(1.8773→0.5157,Δσ=0.8224)/feat_4h_bb_pct_b(0.6768→0.4214,Δσ=0.794), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.008/498)/feat_tongue(0.0115/500), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-20 09:10:05.049423:1:chop:0.7189/2026-04-20 09:12:33.648103:1:chop:0.7059/2026-04-20 09:40:27.742431:1:chop:0.6546, adverse_examples=2026-04-19 00:40:09.929353:0:bull:-0.2753/2026-04-19 01:01:04.227452:0:bull:-0.3173/2026-04-19 01:10:17.732530:0:bull:-0.2383
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2846` / `archive_window_coverage_pct=0.0`
