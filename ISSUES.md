...
_最後更新：2026-04-30 17:03:55 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1155 已完成 collect + diagnostics refresh**
  - `Raw=32511 / Features=23929 / Labels=65627`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=27.0%` / `dominant_regime=chop(79.0%)` / `avg_quality=-0.0318` / `avg_pnl=-0.0033` / `alerts=regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3944` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3944` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.27` / `dominant_regime=chop` / `dominant_regime_share=0.79` / `avg_pnl=-0.0033`
