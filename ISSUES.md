...
_最後更新：2026-04-30 05:17:26 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1139 已完成 collect + diagnostics refresh**
  - `Raw=32490 / Features=23908 / Labels=65588`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=23.0%` / `dominant_regime=chop(92.0%)` / `avg_quality=-0.0780` / `avg_pnl=-0.0051` / `alerts=regime_concentration,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3923` / `archive_window_coverage_pct=0.0`
...
### P1. model stability still needs work (cv=0.6532, cv_std=0.1046, cv_worst=0.5486)
- 目前真相：`cv_accuracy=0.6531852472757753` / `cv_std=0.10456831517183568` / `cv_worst=0.5486169321039397`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3923` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.23` / `dominant_regime=chop` / `dominant_regime_share=0.92` / `avg_pnl=-0.0051`
