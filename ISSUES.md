...
## 當前主線事實
- **最新 full heartbeat #1029 已完成 collect + diagnostics refresh**
  - `Raw=32257 / Features=23675 / Labels=65085`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.87%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=14` / `recent_window_wins=9/50` / `additional_recent_window_wins_needed=6`
  - `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=67/50` / `gap=0` / `support_route_verdict=exact_bucket_supported`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=9.0%` / `dominant_regime=bull(97.0%)` / `avg_quality=-0.1245` / `avg_pnl=-0.0052` / `alerts=label_imbalance,regime_concentration,regime_shift`
- **leaderboard / governance 已收斂為 single-role alignment**
  - `leaderboard_count=6` / `selected_feature_profile=current_full_no_bull_collapse_4h` / `support_aware_profile=current_full_no_bull_collapse_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=true` / `payload_age=7.2h`
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3723` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=14` / `recent 50 wins=9/50` / `additional_recent_window_wins_needed=6`
- same-bucket truth：`bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=67/50` / `support_route_verdict=exact_bucket_supported` / `support_governance_route=exact_live_bucket_supported`
- support progress：`status=exact_supported` / `regression_basis=current_identity` / `legacy_supported_reference=121/50@20260424a`
...
### P1. model stability still needs work (cv=0.6335, cv_std=0.0801, cv_worst=0.5534)
- 目前真相：`cv_accuracy=0.6334745762711864` / `cv_std=0.08008474576271185` / `cv_worst=0.5533898305084746`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3723` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=current_full_no_bull_collapse_4h` / `support_aware_profile=current_full_no_bull_collapse_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=true` / `payload_age=7.2h`
...
- 目前真相：`window=100` / `interpretation=distribution_pathology` / `win_rate=0.09` / `dominant_regime=bull` / `dominant_regime_share=0.97` / `avg_pnl=-0.0052`
