...
## 當前主線事實
- **最新 fast heartbeat #1249 已完成 collect + diagnostics refresh**
  - `Raw=33272 / Features=24459 / Labels=66510`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=500` / `win_rate=66.8%` / `dominant_regime=chop(76.2%)` / `avg_quality=+0.3139` / `avg_pnl=+0.0067` / `alerts=regime_shift`
- **leaderboard / governance 已收斂為 single-role alignment**
  - `leaderboard_count=6` / `selected_feature_profile=core_plus_macro` / `support_aware_profile=core_plus_macro` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.0m`
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4076` / `archive_window_coverage_pct=0.0`
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  ...
  - 本輪補強：`floor_cross_legality.legal_to_relax_runtime_gate=true` 現在只會被文字化為「研究型元件實驗」，不再暗示執行放行；`component_experiment_verdict=exact_supported_component_experiment_blocked_by_discrimination`、`component_experiment_deployment_ready=false`、`component_experiment_positive_discrimination_status=failed_exact_lane_bucket_dominance` 已進入 live probe / API blocker details / runtime closure copy，明確要求先重設或重訓 q15 component，且不得放寬 `allowed_layers` / execution guardrail。
...
- 目前真相：`deployment_blocker=decision_quality_below_trade_floor` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=173` / `minimum_support_rows=50` / `gap_to_minimum=0` / `support_route_verdict=exact_bucket_supported` / `entry_quality=0.5116` / `trade_floor=0.55` / `allowed_layers=0`
- 元件實驗真相：`floor_cross_verdict=legal_component_experiment_after_support_ready` / `legal_to_relax_runtime_gate=true` 只代表研究型元件實驗；`component_experiment_verdict=exact_supported_component_experiment_blocked_by_discrimination` / `component_experiment_deployment_ready=false` / `component_experiment_positive_discrimination_status=failed_exact_lane_bucket_dominance`，因此不可部署。
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4076` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_plus_macro` / `support_aware_profile=core_plus_macro` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.0m`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-15T11:03:17.001252+00:00` / `freshness=fresh` / `age_min=33.0` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=173/50` / `gap=0`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
