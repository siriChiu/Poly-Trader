...
## 當前主線事實
- **最新 fast heartbeat #20260421-0446 已完成 collect + diagnostics refresh**
...
- **Dashboard / ExecutionStatus 初始同步不再洩漏假陰性 execution placeholder**
  - `Dashboard.tsx` 的 `💼 Execution 摘要` 與 `ExecutionStatus.tsx` hero / `可部署` metric 在第一次 `/api/status` 尚未返回前直接顯示 `同步中`
  - 不再先渲染 `Blocked / unknown / automation OFF`；已用 browser 初始 snapshot、`pytest tests/test_frontend_decision_contract.py -q`、`cd web && npm run build` 鎖住
...
- 目前真相：`window=500` / `win_rate=11.6%` / `dominant_regime=bull(85.0%)` / `avg_quality=-0.1660` / `avg_pnl=-0.0060`
- 病態切片：`alerts=label_imbalance,regime_shift` / `tail_streak=52` / `low_variance=9` / `low_distinct=10` / `null_heavy=10`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['label_imbalance', 'regime_shift'], win_rate=0.1160, delta_vs_full=-0.5091, dominant_regime=bull(85.00%), interpretation=distribution_pathology, avg_pnl=-0.0060, avg_quality=-0.1660, avg_dd_penalty=0.3192, spot_long_win_rate=0.0140, feature_diag=variance:9/56, frozen:0, compressed:9, expected_static:3, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=52x1 since 2026-04-19 10:13:18.192660 -> 2026-04-19 21:35:47.730701, adverse_streak=211x0 since 2026-04-17 15:31:17.961817 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.73, delta_vs_prev=-0.614, prev_quality=0.3238, quality_delta_vs_prev=-0.4898, prev_pnl=0.0052, pnl_delta_vs_prev=-0.0112, top_shift_examples=feat_eye(1.0087→-1.9485,Δσ=0.7708)/feat_local_top_score(0.421→0.2689,Δσ=0.6764)/feat_local_bottom_score(0.3002→0.4396,Δσ=0.6103), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.0066/498)/feat_tongue(0.0099/500), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_nq_return_24h[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-19 20:54:30.222392:1:chop:0.6106/2026-04-19 21:17:00.026751:1:chop:0.6211/2026-04-19 21:35:47.730701:1:chop:0.5975, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2773` / `archive_window_coverage_pct=0.0`
