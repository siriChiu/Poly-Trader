...
_最後更新：2026-04-25 14:27:01 CST_
...
## 當前主線事實
- **最新 full heartbeat #20260425_1417 已完成 collect + diagnostics refresh**
  - `Raw=32238 / Features=23656 / Labels=65036`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.89%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=7` / `recent_window_wins=2/50` / `additional_recent_window_wins_needed=13`
  - `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=84/50` / `gap=0` / `support_route_verdict=exact_bucket_supported`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=21.0%` / `dominant_regime=bull(99.0%)` / `avg_quality=-0.0391` / `avg_pnl=-0.0037` / `alerts=regime_concentration,regime_shift`
- **leaderboard / governance 已收斂為 single-role alignment**
  - `leaderboard_count=6` / `selected_feature_profile=core_macro_plus_stable_4h` / `support_aware_profile=current_full_no_bull_collapse_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=true` / `payload_age=19.2m`
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3705` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=7` / `recent 50 wins=2/50` / `additional_recent_window_wins_needed=13`
- same-bucket truth：`bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=84/50` / `support_route_verdict=exact_bucket_supported` / `support_governance_route=exact_live_bucket_supported`
- support progress：`status=exact_supported` / `regression_basis=current_identity` / `legacy_supported_reference=121/50@20260424a`
...
### P1. model stability still needs work (cv=0.6319, cv_std=0.0711, cv_worst=0.5609)
- 目前真相：`cv_accuracy=0.6319473907509546` / `cv_std=0.07106491302503182` / `cv_worst=0.5608824777259228`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3705` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_macro_plus_stable_4h` / `support_aware_profile=current_full_no_bull_collapse_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=true` / `payload_age=19.2m`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.21` / `dominant_regime=bull` / `dominant_regime_share=0.99` / `avg_pnl=-0.0037`
