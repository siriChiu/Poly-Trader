...
_最後更新：2026-04-21 08:07:08 CST_
...
## 當前主線事實
- **最新 fast heartbeat #20260421-0804 已完成 collect + diagnostics refresh**
  - `Raw=31321 / Features=22739 / Labels=63147`
...
- **recent canonical window 仍是 distribution pathology**
  - `window=500` / `win_rate=14.4%` / `dominant_regime=bull(82.2%)` / `avg_quality=-0.1393` / `avg_pnl=-0.0052` / `alerts=label_imbalance,regime_shift`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2791` / `archive_window_coverage_pct=0.0`
...
- **Dashboard blocker copy 已去重並與摘要/詳情分工對齊**
  - `AdviceCard` 現在只保留一次 canonical blocker label；次級提示改成「快捷交易已停用 + 查看詳情去執行狀態」
  - `Execution 摘要` 副標改為明說 Dashboard 仍保留 4 張摘要卡，避免再宣稱 metadata / recovery 已完全移出首頁
...
- 目前真相：`window=500` / `win_rate=14.4%` / `dominant_regime=bull(82.2%)` / `avg_quality=-0.1393` / `avg_pnl=-0.0052`
- 病態切片：`alerts=label_imbalance,regime_shift` / `tail_streak=66` / `low_variance=8` / `low_distinct=10` / `null_heavy=10`
- 下一步：直接對 recent canonical rows 做 feature variance / distinct-count / target-path drill-down；維持 decision-quality guardrails，並檢查 calibration scope 是否仍被病態 slice 稀釋。 recent_window=500, alerts=['label_imbalance', 'regime_shift'], win_rate=0.1440, delta_vs_full=-0.4813, dominant_regime=bull(82.20%), interpretation=distribution_pathology, avg_pnl=-0.0052, avg_quality=-0.1393, avg_dd_penalty=0.3104, spot_long_win_rate=0.0420, feature_diag=variance:8/56, frozen:0, compressed:8, expected_static:2, overlay_only:1, unexpected_frozen:0, distinct:10, null_heavy:10, tail_streak=66x1 since 2026-04-19 10:13:18.192660 -> 2026-04-20 00:54:47.987937, adverse_streak=197x0 since 2026-04-17 17:32:12.305748 -> 2026-04-18 13:43:25.809469, prev_win_rate=0.702, delta_vs_prev=-0.558, prev_quality=0.3042, quality_delta_vs_prev=-0.4435, prev_pnl=0.0048, pnl_delta_vs_prev=-0.01, top_shift_examples=feat_eye(0.996→-2.1533,Δσ=0.8199)/feat_local_top_score(0.4265→0.2604,Δσ=0.7391)/feat_local_bottom_score(0.2953→0.451,Δσ=0.6818), new_compressed=feat_vwap_dev, compressed_examples=feat_body(0.0001/500)/feat_ear(0.0074/498)/feat_vix(0.0101/35), expected_static_examples=feat_dxy[weekend_macro_market_closed]/feat_vix[weekend_macro_market_closed], overlay_only_examples=feat_scales_ssr[research_sparse_source], distinct_examples=feat_4h_dist_swing_high(0/0)/feat_chorus(0/0)/feat_fin_netflow(0/0), null_examples=feat_4h_dist_swing_high(0.0)/feat_chorus(0.0)/feat_fin_netflow(0.0), recent_examples=2026-04-20 00:30:32.473772:1:chop:0.713/2026-04-20 00:47:15.429720:1:chop:0.7068/2026-04-20 00:54:47.987937:1:chop:0.6858, adverse_examples=2026-04-18 13:00:00.000000:0:bull:-0.2342/2026-04-18 13:36:43.970223:0:bull:-0.2119/2026-04-18 13:43:25.809469:0:bull:-0.2137
- 驗證：
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2791` / `archive_window_coverage_pct=0.0`
