...
## 當前主線事實
- **最新 full heartbeat #1275 已完成 collect + diagnostics refresh**
  - `Raw=33368 / Features=24513 / Labels=66619`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=250` / `win_rate=58.8%` / `dominant_regime=chop(81.2%)` / `avg_quality=+0.2322` / `avg_pnl=+0.0040` / `alerts=regime_shift`
...
- **Cron compact runtime facts 已補上 canonical 熔斷操作摘要**
  - `scripts/hb_extract_runtime_facts.py` 現在會在 `circuit_breaker.verdict=canonical_breaker_active` 時輸出 `operator_guardrail_summary`：最近 50 筆目前 2/15 勝、還差 13 勝，維持 fail-closed；cron 報告不再把解除條件只留在巢狀 JSON 或顯示為空值。
...
- 目前真相：`canonical_horizon_minutes=1440` / `losing_streak=48` / `all_horizon_losing_streak=12`
...
- latest matrix：`generated_at=2026-05-16T07:03:28.497580+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24402` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=10/50` / `current_live_structure_bucket_gap_to_minimum=40` / `support_progress_status=stalled_under_minimum` / `support_progress_reason=current live exact support 連續 heartbeat 停在同一數量，屬於 support accumulation 停滯。` / `regression_basis=same_identity_same_semantic_signature` / `delta_vs_previous=0` / `previous_rows=10` / `support_rows_needed=40` / `stagnant_run_count=2` / `stalled_support_accumulation=True` / `escalate_to_blocker=False`
...
### P1. model stability still needs work (cv=0.5840, cv_std=0.0201, cv_worst=0.5639)
- 目前真相：`cv_accuracy=0.584016393442623` / `cv_std=0.020081967213114782` / `cv_worst=0.5639344262295082`
- 下一步：優先比較 support-aware / shrinkage profiles 與 current bucket robustness，避免把治理 blocker 誤當單純 parity 問題。

### P1. TW-IC 21 vs Global IC 18 — 信號強依賴近期資料
- 目前真相：`global_pass=18` / `tw_pass=21` / `total_features=30`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4114` / `archive_window_coverage_pct=0.0`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-16T07:03:28.497580+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=10/50` / `gap=40` / `release_ready=False` / `recent_window_wins=2/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=13` / `current_recent_window_win_rate=0.040` / `support_progress_status=stalled_under_minimum` / `support_progress_reason=current live exact support 連續 heartbeat 停在同一數量，屬於 support accumulation 停滯。` / `regression_basis=same_identity_same_semantic_signature` / `delta_vs_previous=0` / `previous_rows=10` / `support_rows_needed=40` / `stagnant_run_count=2` / `stalled_support_accumulation=True` / `escalate_to_blocker=False`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
