...
_最後更新：2026-04-25 13:09:26 CST_
...
## 當前主線事實
- **最新 full heartbeat #20260425_1259 已完成 collect + diagnostics refresh**
  - `Raw=32232 / Features=23650 / Labels=65023`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `simulated_pyramid_win=56.90%`
- **canonical 即時部署阻塞仍是熔斷優先真相**
  - `deployment_blocker=circuit_breaker_active` / `streak=2` / `recent_window_wins=2/50` / `additional_recent_window_wins_needed=13`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=26.0%` / `dominant_regime=bull(99.0%)` / `avg_quality=-0.0086` / `avg_pnl=-0.0034` / `alerts=regime_concentration,regime_shift`
- **leaderboard / governance 已收斂為 single-role alignment**
  - `leaderboard_count=6` / `selected_feature_profile=core_only` / `support_aware_profile=core_macro_plus_stable_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=true` / `payload_age=1.7h`
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3699` / `archive_window_coverage_pct=0.0`
...
- **Execution Console / `/api/trade` 已 fail-closed（同步中 + 阻塞 + 直接 API）**
  ...
  - 測試契約已同步：`tests/test_strategy_lab.py` 的 buy/service 與 ExecutionRejectError 測試現在明確 stub `current-live blocker=None`，避免在真實熔斷啟用時誤把 P0 fail-closed 行為當成 service-path regression；`tests/test_server_startup.py -k api_trade` 仍鎖住 buy/add-exposure 409 與 reduce/sell 風險降低路徑。
...
- 目前真相：`deployment_blocker=circuit_breaker_active` / `streak=2` / `recent 50 wins=2/50` / `additional_recent_window_wins_needed=13`
...
### P1. model stability still needs work (cv=0.5552, cv_std=0.0197, cv_worst=0.5354)
- 目前真相：`cv_accuracy=0.5551548578701739` / `cv_std=0.019728468392023746` / `cv_worst=0.5354263894781502`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3699` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_only` / `support_aware_profile=core_macro_plus_stable_4h` / `governance_contract=single_role_governance_ok` / `current_closure=single_profile_alignment` / `payload_source=latest_persisted_snapshot` / `payload_stale=true` / `payload_age=1.7h`
...
- 目前真相：`window=100` / `interpretation=regime_concentration` / `win_rate=0.26` / `dominant_regime=bull` / `dominant_regime_share=0.99` / `avg_pnl=-0.0034`
