...
## 當前主線事實
- **最新 full heartbeat #1125 已完成 collect + diagnostics refresh**
...
- latest matrix：`generated_at=2026-04-29T10:15:21.328729+00:00` / `samples=23856` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `support_route=exact_bucket_unsupported_block` / `deployment_blocker=circuit_breaker_active`
- best observed row：`model=xgboost` / `regime=all` / `top_k=top_10pct` / `oos_roi=3.8544` / `win_rate=0.7774` / `profit_factor=8.2654` / `max_drawdown=0.2179` / `worst_fold=-0.0611` / `trade_count=292` / `verdict=not_deployable`
...
### P1. model stability still needs work (cv=0.6520, cv_std=0.1069, cv_worst=0.5451)
- 目前真相：`cv_accuracy=0.6519916142557651` / `cv_std=0.10691823899371072` / `cv_worst=0.5450733752620545`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=3902` / `archive_window_coverage_pct=0.0`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單 deployment candidate**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `rows=24` / `deployable_rows=0`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K Gate panel 已接上 matrix/gate_failures，current-live/support blockers 未解除前仍 fail-closed。
