...
_最後更新：2026-05-22 11:10:47 CST_
...
## 當前主線事實
- **最新 fast heartbeat #1428 已完成 collect + diagnostics refresh**
  - `Raw=34009 / Features=25017 / Labels=67562`
...
- **canonical current-live blocker 已切到 current-live exact-support truth**
  ...
  - support progress：`status=semantic_rebaseline_under_minimum` / `reason=current exact support 25/50 below minimum (gap=25); legacy 173/50@1250 remains reference-only; semantic mismatch=calibration_window,entry_quality_label` / `regression_basis=legacy_or_different_semantic_signature` / `current_rows=25` / `minimum_rows=50` / `gap_to_minimum=25` / `support_rows_needed=25` / `previous_rows=25` / `delta_vs_previous=0` / `legacy_supported_reference=173/50@1250` / `stagnant_run_count=5` / `stalled_support_accumulation=False` / `escalate_to_blocker=True` / `governance_reference_route=exact_live_bucket_present_but_below_minimum` / `exact_live_lane_proxy_rows=8` / `governance_reference_only=True`；active repair：`phase=semantic_evidence_backfill_or_exact_accumulation` / `component_verify_ready=False` / `live_exposure_allowed=False` / `shadow_or_paper_allowed=True` / `current_signal=HOLD` /
...
- 目前真相：`generated_at=2026-05-22T03:10:44.310757Z` / `venues_checked=2` / `ok_count=1` / `runtime_ready_count=0` / `runtime_ready=false` / `readiness_state=blocked_until_runtime_lifecycle_proof` / `runtime_ready_blockers=fill lifecycle 尚未驗證|live exchange credential 尚未驗證|order ack lifecycle 尚未驗證|元資料契約尚未通過|場館 adapter 尚未接入`；`okx=adapter_supported=true,enabled_in_config=true,credentials_configured=false,proof_state=public_metadata_only,runtime_ready=false,blockers=live exchange credential 尚未驗證|order ack lifecycle 尚未驗證|fill lifecycle 尚未驗證` / `binance=adapter_supported=false,enabled_in_config=false,credentials_configured=false,proof_state=adapter_unsupported,runtime_ready=false,blockers=場館 adapter 尚未接入|元資料契約尚未通過|場館設定停用`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4487` / `archive_window_coverage_pct=0.0`
