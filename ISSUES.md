...
## 當前主線事實
- **最新 full heartbeat #1244 已完成 collect + diagnostics refresh**
  - `Raw=33261 / Features=24448 / Labels=66493`
...
- **recent canonical diagnostics 已刷新**
  - `latest_window=100` / `win_rate=69.0%` / `dominant_regime=chop(70.0%)` / `avg_quality=+0.3474` / `avg_pnl=+0.0100` / `alerts=regime_shift`
  - `blocking_window=250` / `win_rate=61.2%` / `dominant_regime=chop(87.6%)` / `avg_quality=+0.2456` / `avg_pnl=+0.0038` / `alerts=regime_shift`
- **leaderboard / governance 仍維持 dual-role contract**
  - `leaderboard_count=6` / `selected_feature_profile=core_macro_plus_stable_4h` / `support_aware_profile=current_full_no_bull_collapse_4h` / `governance_contract=dual_role_governance_active` / `current_closure=global_ranking_vs_support_aware_production_split` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.1m`
- **source / venue blockers 仍開啟**
  - `blocked_sparse_features=8` / `{'archive_required': 3, 'snapshot_only': 4, 'short_window_public_api': 1}`
  - top source blockers：`fin_netflow(source_auth_blocked/auth_missing, coverage=0.0%, archive_window=0.0%, forward_archive=ready)` / `claw(source_auth_blocked/auth_missing, coverage=14.6%, archive_window=87.1%, forward_archive=ready)` / `claw_intensity(source_auth_blocked/auth_missing, coverage=14.6%, archive_window=87.1%, forward_archive=ready)` / `nest_pred(source_tls_verify_failed/tls_verify_failed, coverage=16.2%, archive_window=96.8%, forward_archive=ready)`
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4068` / `archive_window_coverage_pct=0.0`
...
- **Strategy Lab 高信心 OOS 列級訊號 copy 已 operator-safe**
  ...
  - 本輪加固 `web/src/utils/runtimeCopy.ts` / `web/src/pages/StrategyLab.tsx`：`not_deployable` 先於 `deployable` 轉譯，避免 `not_可部署`；`exact support / top-level live / decision quality below / production_profile_unspecified / shrinkage / support-aware` 等 raw 或英中混雜 token 轉為操作員繁中；browser `/lab` 已確認高信心面板顯示「OOS 候選已過門檻，但即時部署仍阻塞」「可部署 0」「部署判定 不可部署」，且不外露 `not_deployable / runtime_blocked_oos_pass / decision quality below`。
...
- latest matrix：`generated_at=2026-05-15T08:03:09.739164+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24339` / `rows=24` / `models=logistic_regression,random_forest,xgboost` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_supported` / `deployment_blocker=decision_quality_below_trade_floor` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=173/50` / `current_live_structure_bucket_gap_to_minimum=0`
...
### P1. model stability still needs work (cv=0.6298, cv_std=0.0616, cv_worst=0.5682)
- 目前真相：`cv_accuracy=0.6298274445357437` / `cv_std=0.06162695152013148` / `cv_worst=0.5682004930156122`
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4068` / `archive_window_coverage_pct=0.0`
...
- 目前真相：`leaderboard_count=6` / `selected_feature_profile=core_macro_plus_stable_4h` / `support_aware_profile=current_full_no_bull_collapse_4h` / `governance_contract=dual_role_governance_active` / `current_closure=global_ranking_vs_support_aware_production_split` / `payload_source=latest_persisted_snapshot` / `payload_stale=false` / `payload_age=0.1m`
...
5. **P0 實戰化：建立 high-conviction top-k OOS ROI gate，把研究 winner 轉成可拒單部署候選**
   - `data/high_conviction_topk_oos_matrix.json` 已產出 `generated_at=2026-05-15T08:03:09.739164+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `bucket_rows=173/50` / `gap=0`；`/api/models/leaderboard` 與 Strategy Lab 高信心 OOS Top-K 部署門檻面板已改為最接近部署候選優先，並以操作員繁中 copy 顯示矩陣新鮮度、breaker release math 與即時支持脈絡；矩陣過期或即時分桶 / 支持 / release 條件未解除前仍 fail-closed。
