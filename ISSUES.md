...
_最後更新：2026-04-21 22:06:55 CST_
...
- **最新 fast heartbeat #fast 已完成 collect + diagnostics refresh**
  - `Raw=31403 / Features=22821 / Labels=63303`
...
- **source / venue blockers 仍開啟**
  ...
  - fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2873` / `archive_window_coverage_pct=0.0`
...
- **Dashboard / Strategy Lab top-level live blocker 摘要已保留 q15 current bucket rows + gap**
  - `/` 與 `/lab` 現在都先顯示 `current bucket 0/50 · gap 50`，再顯示 support / governance route 與 venue blockers，避免 operator 只看到 blocker 名稱卻看不到 same-bucket support shortage
...
- 目前真相：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=2873` / `archive_window_coverage_pct=0.0`
