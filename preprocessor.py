...
def compute_features_from_raw(df: pd.DataFrame) -> Optional[Dict]:
    ...
    # 1. 眼 (Eye) - 直接使用 eye_dist（已由 eye_binance 計算為距離比例）
    if "eye_dist" in df.columns and pd.notna(latest.get("eye_dist")):
        features["feat_eye_dist"] = latest["eye_dist"]

    # 2. 耳 (Ear) -  Polymarket 機率 Z-score 標準化（使用 ear_prob）
    if "ear_prob" in df.columns and not df["ear_prob"].isna().all():
        prob_series = df["ear_prob"].dropna()
