...
**Leaderboard feature-profile candidate contract（Heartbeat #726）**：`backtesting/model_leaderboard.py` 與 `/api/models/leaderboard` 不得再默默用單一 dense `current_full` 特徵集評分所有模型。leaderboard 現在必須至少把 **`core_only`、support-aware `core_plus_macro`、`current_full`** 納入正式候選比較，並把 `selected_feature_profile / selected_feature_profile_source / feature_profiles_evaluated / feature_profile_support_cohort / feature_profile_support_rows / feature_profile_exact_live_bucket_rows` 一起序列化到 ranking payload。這樣 heartbeat、API 與後續部署面板才能 machine-read：目前排行榜選中的到底是 global shrinkage winner 還是 bull blocker 下的 support-aware fallback，而不是只看到 deployment lane 卻不知道用的是哪一組特徵語義。
