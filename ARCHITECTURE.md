...
**Execution metadata auto-refresh governance contract（Heartbeat 2026-04-16 15:05 UTC，更新於 15:34 UTC）**：stale metadata smoke 不可只停留在 badge 告警。`server/routes/api.py::api_status()` 現在必須在 `freshness in {stale, unavailable}` 時自動嘗試重跑 read-only `run_metadata_smoke()`，並以單一治理 payload 序列化 `governance={status, operator_action, operator_message, refresh_command, escalation_message, auto_refresh, background_monitor}`；其中 `auto_refresh` 至少要帶 `status / reason / attempted_at / completed_at / next_retry_at / cooldown_seconds / error`，並套用 5 分鐘 cooldown，避免 `/api/status` 輪詢把 venue metadata lane 打爆。**Heartbeat 2026-04-16 15:34 UTC 再加一條 process-internal background governance contract**：`server/main.py` 啟動 FastAPI 後，必須額外啟動常駐背景監看器，每 60 秒執行一次 `run_execution_metadata_smoke_background_governance()`；它會把 `background_monitor={status, reason, checked_at, freshness_status, governance_status, error, interval_seconds}` 寫回 runtime status 與 governance payload，讓 operator 區分「靠 `/api/status` 被動 refresh」與「API process 內主動背景監看」。`web/src/pages/Dashboard.tsx` 必須直接渲染這兩份治理訊息，讓 operator 一眼分辨：目前是健康、等待自動 refresh、背景監看是否仍在跑、還是 artifact lane 需要升級成 blocker。注意：這條 contract 只治理 metadata smoke freshness，不代表 live/canary order-level readiness 已完成；若 API process 掛掉，仍需 process 外 scheduler / pager / cron 做下一層升級。
