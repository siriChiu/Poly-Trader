...
5. high-conviction Top-K OOS gate 的 `/api/models/leaderboard.high_conviction_topk.support_context` 必須以較新的 `data/live_predict_probe.json` current-live support truth 覆蓋 stale matrix snapshot；`hb_leaderboard_candidate_probe.py` 的 support-progress 摘要若 bucket/route/rows/minimum 與 live probe 相符，必須直接重用 live-probe `support_progress`，不得自行重算成另一套 stalled/regressed run count；離線 / 風控已過但 live support 未過的 rows 只能標 `runtime_blocked_oos_pass`，不得升級為 deployable；compact rows 必須保留 row-level `signal / allowed_layers / execution_guardrail_reason / live truth source / support_progress_status / stalled_support_accumulation / stagnant_run_count / support_delta_vs_previous / support_rows_needed`，讓 Strategy Lab 同時顯示 OOS 成績、即時不可部署原因與 same-bucket 支持累積是否停滯。
