...
**Heartbeat full-mode candidate-eval time-budget contract（Heartbeat 2026-04-24）**：`scripts/hb_parallel_runner.py` 的 full mode 不得讓 `feature_group_ablation.py`、`bull_4h_pocket_ablation.py` 或 `hb_leaderboard_candidate_probe.py` 任一 silent lane 吃完整輪 10 分鐘預算。當 candidate artifact 的 `source_meta` 與 current canonical label signature 相符或只在 bounded label drift 內時，full mode 也可沿用該 fresh artifact；若必須重跑，這些 candidate lanes 必須套用 explicit full-mode timeout，timeout 後保留 artifact-backed governance 摘要並讓 heartbeat 繼續完成 docs / summary / verification。
