...
**Auto-propose TW-history recency contract（Heartbeat 2026-04-24 20:55 CST）**：`scripts/auto_propose_fixes.py::load_recent_tw_history()` 必須把 cron timestamp labels（`heartbeat_YYYYMMDD_HHMM[_SS]_summary.json`）視為 stable numbered runs，且排序優先於 legacy numeric IDs 與 `fast` alias。`📊 TW 歷史` 不得再把本輪 `#20260424_2054` 接到 stale `#1024/#1023`，否則 TW drift issue 會使用錯誤 lineage；regression test 必須同時覆蓋 legacy numbered-vs-fast alias 與 timestamped cron-vs-legacy numeric 的排序。
...
**Current-bucket root-cause surface contract（Heartbeat 2026-04-24）**：`/predict/confidence`、`/api/status.execution.live_runtime_truth` 與前端 surfaces 必須共同保留 `current_bucket_root_cause`（fallback `q15_bucket_root_cause`），且 Dashboard `ConfidenceIndicator`、Strategy Lab、Execution Status 都要直接顯示 `verdict / candidate_patch_feature / candidate_patch_type / near_boundary_rows / gap_to_q35_boundary / verify_next`。首頁不得再退回 generic q15/q35 摘要；若 artifact 缺失，UI 必須明確提示需重跑 `hb_predict_probe.py` / q15 root-cause lane，而不是把 root cause 隱藏成空白。**同輪 q35 copy hygiene 補充**：若 current bucket 本身已是 q35（例如 `BLOCK|bull_high_bias200_overheat_block|q35`），operator-facing root-cause 卡不得再顯示「近邊界樣本 / 距 q35 還差」這種 q15 boundary copy；必須改顯示 q35 audit 的 `runtime_remaining_gap_to_floor / remaining_gap_to_floor` 與 `q35 公式 / 重設仍只屬治理參考`，讓 Dashboard、`/execution/status`、`/lab` 的下一步聚焦 trade-floor / formula review，而不是誤導到 q15 邊界修補。
...
**Heartbeat 2026-04-25 Execution Console shortcut guardrail**：`web/src/pages/ExecutionConsole.tsx` 的自然語句快捷列也必須遵守同一個 current-live blocker contract。當 `/api/status` 初次同步中、`deployment_blocker` / `live_ready=false` 存在時，買入、減碼與啟用自動模式快捷操作必須顯示 `買入暫停 / 減碼暫停 / 自動模式暫停` 並保持 disabled；僅保留 `查看阻塞原因` 與 `重新整理`，自然語句 fallback 也必須在送出 `/api/trade` 或 `/api/automation/toggle(enabled=true)` 前 fail-closed。
**Heartbeat 2026-04-20 dev-runtime backend failover contract**：當本地開發同時存在 Vite (`:5173`) 與多條後端 lane（例如 `:8000` reload lane、`:8001` stable lane）時，`web/src/hooks/useApi.ts` 不得再把 API / chart / WebSocket 綁死在單一 port。它現在必須先用 `/health` 對 `8000/8001` 做短超時 prewarm，持久化目前可用的 active backend base，再讓 GET/HEAD requests 與 chart requests 共用同一條 active-base + timeout-aware fallback 路徑；`CandlestickChart.tsx` 不得繞過這條 `fetchApiResponse()`；`Dashboard.tsx` 的 `/ws/live` 也必須在首次連線與每次 retry 前先 prewarm active backend，再用 `buildWsCandidateUrls()` 依健康 lane 重新排序 candidate，避免 fresh session 先卡在 reload lane 的 `/api/status` timeout 或假性 `UNKNOWN`。**本輪再補 runtime-build freshness 約束**：`/health` 不得只回 `200 ok`，還必須 machine-read `runtime_build={process_started_at, git_head_commit, git_head_committed_at, head_sync_status, head_sync_basis, latest_runtime_source_path, latest_runtime_source_modified_at}`；其中 `head_sync_status` 不可再只看 git commit time，必須優先比較 `latest_runtime_source_modified_at`，而且這個 source 集合只允許 runtime code/config（例如 `.py` 與根層 config files），不得把 `model/ic_signs.json` 這類 generated artifacts 算進 freshness anchor。這樣 working tree 已更新但 backend 尚未重啟時，stable lane 仍會被標成 `stale_head_commit`；反過來說，單純 heartbeat artifact 更新也不會誤把健康 lane 判成 stale。當兩條 lane 都健康時，`useApi.ts` prewarm 必須優先 `head_sync_status=current_head_commit`，並能從 `head_sync_basis + latest_runtime_source_*` 直接看出 freshness 是依據 commit 還是最新 runtime source；避免未重啟的 stable lane 繼續覆蓋 fresh current-live blocker truth。驗證基準是：browser `poly_trader.active_api_base` 會收斂到 current-head lane，且 `curl :8000/health` / `curl :8001/health` 都能直接揭露 backend code freshness 與其判定依據。
...
**Heartbeat full-mode candidate-eval time-budget contract（Heartbeat 2026-04-24 / 2026-04-25）**：`scripts/hb_parallel_runner.py` 的 full mode 不得讓 `feature_group_ablation.py`、`bull_4h_pocket_ablation.py` 或 `hb_leaderboard_candidate_probe.py` 任一 silent lane 吃完整輪 10 分鐘預算。當 candidate artifact 的 `source_meta` 與 current canonical label signature 相符或只在 bounded label drift 內時，full mode 也可沿用該 fresh artifact；若需要刷新，heartbeat 只允許走 bounded lane：`feature_group_ablation.py --bounded-refresh` 重新評估治理關鍵 profile，`bull_4h_pocket_ablation.py --refresh-live-context` 更新 current-live support truth 並把 live-specific proxy cohorts 標成 reference-only。完整 full rebuild 必須改由人工/獨立長任務觸發，不得阻塞 cron heartbeat；若 bounded lane 仍 timeout，summary/docs 必須保留 artifact-backed governance 摘要與 non-authoritative 狀態。
...
- **Heartbeat 2026-04-24 operator-copy hygiene contract**：Execution Console 與 Strategy Lab 不得再把 control-plane / strategy-routing raw tokens（例如 `run`、`execution run`、`sleeve`、`profile`、`equal_split_active_sleeves`）直接露給 operator；support / patch 摘要也不得顯示英文 `gap`，必須用 `缺口` 表示 minimum-support 差距。`web/src/utils/runtimeCopy.ts` 是這批 run / sleeve / status token 的中文化入口，`ExecutionConsole.tsx`、`ExecutionStatus.tsx`、`Dashboard.tsx`、`StrategyLab.tsx` 與 `LivePathologySummaryCard.tsx` 必須使用中文化 support copy，避免 operator 把內部 routing token 或半英文治理欄位誤讀成產品語義。這條 contract 由 `tests/test_frontend_decision_contract.py` / `tests/test_execution_surface_contract.py`、browser `/` `/execution` `/execution/status` `/lab` DOM 檢查與 `npm run build` 鎖住。
...
**Heartbeat 2026-04-25 `/api/trade` current-live blocker fail-closed contract**：前端 disabled 不是安全邊界。`server/routes/api.py::api_trade()` 在處理 `buy` / add-exposure 前，必須讀取 `/predict/confidence` → `live_runtime_truth`；若存在 `deployment_blocker`、`CIRCUIT_BREAKER`、或 no-deploy `runtime_closure_state`，必須回 HTTP 409 `detail.code=current_live_deployment_blocker`，並在 context 內保留 `allowed_sides=[reduce,sell]`、`release_condition`、`current_live_structure_bucket` 與 support truth。`reduce/sell` 風險降低路徑不得依賴 predictor availability，必須在 blocker active 時仍可送入 normal execution guardrail。
...
**Support-regression visibility contract（Heartbeat 2026-04-23）**：`/execution`、`/execution/status`、`/lab` 不得只顯示 `support_rows_text`。當 current-live exact support 從已達門檻回落時，operator-facing surfaces 還必須同步顯示 `support_progress.status` 與 `support_progress.delta_vs_previous`，並共用中文 humanizer（例如 `regressed_under_minimum -> 精準樣本從已就緒回落到未達門檻`）。原因是 `0/50` 只告訴 operator「目前不足」，卻無法分辨這是持續累積、長期停滯，還是 **199 → 0** 的 support regression；若沒有這層 visibility，current-live blocker 會被誤讀成一般 support shortage，而不是需要優先調查的 runtime/current-bucket regression。
...
**Execution breaker-first visibility contract（Heartbeat 2026-04-19q）**：`web/src/pages/ExecutionConsole.tsx` 與 `web/src/pages/ExecutionStatus.tsx` 的首屏 blocked summary / metric card / 主 blocker 區塊，必須先顯示 `execution.live_runtime_truth.deployment_blocker / deployment_blocker_reason`，再顯示 `execution_surface_contract.live_ready_blockers`。`live_ready_blockers` 的角色是 **venue readiness secondary visibility**，不得再覆蓋 canonical current-live blocker；否則 operator 會把 `live exchange credential 尚未驗證` 誤讀成比 `circuit_breaker_active` / `decision_quality_below_trade_floor` / `unsupported_exact_live_structure_bucket` 更高優先級的 runtime 真相。
**Execution diagnostics posture contract（Heartbeat 2026-04-21）**：`web/src/pages/ExecutionStatus.tsx` 不得再讓 `fresh / healthy / metadata smoke` 與 `blocked` 形成同級訊號。頁首必須先顯示單一 `overall execution posture = syncing | blocked | ready`，並在說明文中明確寫出：`fresh / healthy` 只代表**觀測層正常**，不代表可部署。若 `execution_reconciliation.status=healthy` 但 `lifecycle_audit.stage=no_runtime_order` 或 `artifact_coverage=not_applicable`，頁面摘要必須降級成 `limited evidence`，避免把「尚無 runtime order 可核對」誤讀成完整實單驗證。若帳戶尚未配置私有憑證，頂層卡片與帳戶快照 badge 也必須顯示 `metadata-only snapshot` / `metadata-only`，而不是用模糊的 `review` 或讓 operator 誤以為 private balance 已可見。這條 contract 由 `tests/test_frontend_decision_contract.py`、`npm run build` 與 browser `/execution/status` 鎖住。
**Execution support-alignment visibility contract（Heartbeat 2026-04-21）**：`web/src/pages/ExecutionConsole.tsx` 與 `web/src/pages/ExecutionStatus.tsx` 的 support 區塊不得再只顯示單一 `support_rows_text` badge。operator-facing surfaces 現在必須同時顯示：
- `support=current bucket rows/minimum`
- `runtime/calibration = runtime_exact_support_rows / calibration_exact_lane_rows`
- alignment summary（例如 `runtime exact support 與 calibration exact lane 已對齊（97 筆）`）

目的不是增加文字，而是避免 current bucket 已 closure、但 calibration lane 仍落後或 split-brain 時，被單一 `97/50` badge 藏掉。這條 contract 由 `tests/test_frontend_decision_contract.py`、`npm run build`、browser `/execution` 與 browser `/execution/status` 鎖住。
