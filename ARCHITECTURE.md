...
- **Heartbeat 2026-04-21 same-origin strategy workspace contract**：在本地 dev/runtime 同時存在多條 backend lane（例如 `8000/8001`）時，`web/src/pages/StrategyLab.tsx` 對 `/api/strategies/leaderboard`、`/api/strategies/{name}`、`/api/strategy_data_range` 的讀取必須先走目前開啟頁面的 same-origin shell / Vite proxy，再 fallback 到 `fetchApi()` 的 direct backend failover。目的不是繞過 failover，而是避免 `/lab` workspace 因 `poly_trader.active_api_base` 指到較舊 lane 而讀到 stale strategy detail / data-range，造成排行榜、工作區與兩年預設視窗 split-brain。這條 contract 由 `tests/test_strategy_lab_uniqueness_contract.py`、`tests/test_frontend_decision_contract.py` 與 browser `/lab`（強制 `poly_trader.active_api_base=http://127.0.0.1:8001`）共同鎖住。
