...
- **Heartbeat 2026-04-24 operator-copy hygiene contract**：Execution Console 與 Strategy Lab 不得再把 control-plane / strategy-routing raw tokens（例如 `run`、`execution run`、`sleeve`、`profile`、`equal_split_active_sleeves`）直接露給 operator。`web/src/utils/runtimeCopy.ts` 是這批 run / sleeve / status token 的中文化入口，`ExecutionConsole.tsx` 與 `StrategyLab.tsx` 必須用 humanizer 渲染 run action、profile id、sleeve filter、sleeve badge 與 sleeve labels，避免 operator 把內部 routing token 誤讀成產品語義。這條 contract 由 `tests/test_frontend_decision_contract.py` / `tests/test_execution_surface_contract.py` 與 `npm run build` 鎖住。
