...
3. **ROI / 回撤 / 高信心 precision 優先於 CV accuracy**
   CV 只作診斷；實戰排序應優先看 OOS ROI、win rate、profit factor、max drawdown、worst fold、time underwater、top-k precision。

4. **Fail-closed，不硬交易**
   若 current-live support、circuit breaker、conformal uncertainty、venue proof 或 execution guardrail 任一關鍵條件不通過，只允許 paper/shadow/hold/reduce-only。
...
- Execution Console 保持 kill switch、reduce-only、pause strategy、reconcile position、order preview。
...
- 顯示 ROI、win rate、profit factor、max drawdown、worst fold、trade count、time underwater、fee/slippage sensitivity。
...
目前若 `CIRCUIT_BREAKER`、`allowed_layers=0`、`support_route_deployable=false` 或 venue proof 不足，只允許 research / backtest / paper / shadow。
...
- [ ] 將 deployment profile 分為 research / paper / shadow / canary / deployable / risk_locked。
