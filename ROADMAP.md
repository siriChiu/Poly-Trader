...
## 已完成
- **fast heartbeat #1329-productization 已完成 collect + diagnostics refresh**
...
- **Venue readiness runtime-evidence copy 已 fail-closed / fail-open 都不誤導**
  - `VenueReadinessSummary` 現在用同一個 `runtimeEvidenceSummary`：只有 `runtime_ready=true` 且無 blockers 才顯示「實單證據完成 · 委託確認 / 成交生命週期已驗證」；OKX/Binance 仍未具備實單生命週期證據時維持「待補實單證據」與 per-venue blockers。
...
- venue blockers：`adapter_unsupported / live exchange credential / order ack lifecycle / fill lifecycle` 仍未驗證；API/UI 已把 per-venue adapter_supported、proof state 與下一步驗證欄位掛到 metadata smoke venue rows，且 operator-facing venue error copy 會 humanize backend error，不直接顯示 raw `unsupported venue` 字串；`VenueReadinessSummary` 只有在 `runtime_ready=true` 且無 blockers 時才顯示「實單證據完成」，未完成生命週期證據時維持「待補實單證據」
