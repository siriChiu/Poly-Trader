...
## 已完成
- **full heartbeat #1307-productization 已完成 collect + diagnostics refresh**
...
- **runtime facts cron 摘要已產品化 q15 legacy support compact proof**
  - `legacy_supported_reference` 現在只輸出 `heartbeat / rows / minimum / reference_only / supports_current_identity / promotable_to_same_identity_history / mismatched_fields`，不再 dump nested semantic evidence；舊版 `53/50@20260419b` 在 cron report 中會明確標成 reference-only，不能被誤讀成 current support closure。
  - 驗證：`PYTHONPATH=/home/kazuha/Poly-Trader /home/kazuha/Poly-Trader/venv/bin/python -m pytest tests/test_hb_extract_runtime_facts.py -q`；`scripts/hb_extract_runtime_facts.py` runtime output 不含 `semantic_identity_evidence / row_examples / backfilled_support_identity`。
...
- `scripts/hb_extract_runtime_facts.py` 的 cron 摘要只可輸出 compact legacy reference proof；舊版 `53/50@20260419b` 必須帶 `reference_only=True / supports_current_identity=False / promotable=False / mismatched_fields`，且不得 dump `semantic_identity_evidence / row_examples / backfilled_support_identity` 這類會讓 operator 誤讀或污染摘要的 heavyweight evidence。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-17T10:02:50.717598+00:00` / `freshness=fresh` / `age_min=0.3` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24474` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=insufficient_support_everywhere` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=CAUTION|structure_quality_caution|q15` / `current_live_structure_bucket_rows=0/50` / `current_live_structure_bucket_gap_to_minimum=50` / `release_ready=False` / `recent_window_wins=4/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=11` / `current_recent_window_win_rate=0.080` / `support_progress_status=semantic_rebaseline_under_minimum` /
...
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=insufficient_support_everywhere` / `governance=exact_live_lane_proxy_available` / `bucket=CAUTION|structure_quality_caution|q15` / `bucket_rows=0/50` / `gap=50` / `release_ready=False` / `recent_window_wins=4/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=11` / `current_recent_window_win_rate=0.080` / `support_progress_status=semantic_rebaseline_under_minimum` / `support_progress_reason=current exact support 0/50 below minimum (gap=50); semantic mismatch=calibration_window,entry_quality_label,regime_label` / `regression_basis=legacy_or_different_semantic_signature` /
  `delta_vs_previous=-2` / `previous_rows=2` / `support_rows_needed=50` / `stagnant_run_count=0` / `stalled_support_accumulation=False` / `escalate_to_blocker=True`；若只剩即時分桶 / 支持 / release gate，仍模擬觀察 / 影子驗證 / 僅觀察。
**成功標準**
