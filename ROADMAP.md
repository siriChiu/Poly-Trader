...
## 已完成
- **fast heartbeat #1511 已完成 collect + diagnostics refresh**
...
- **Execution Console 高信心 Top-K 影子觀察入口已產品化**
  - `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `deployable_rows=0` / `paper_shadow=true` / `risk_on_order_enabled=false` / `support=2/50` / `gap=48`；selective sleeve 在可部署仍為 0 時只允許 `paper_shadow`，不會送單或加倉。
- **高低震盪 / 擁塞不是停工：區間影子觀察 + 減風險劇本已產品化**
  - `support=2/50` / `gap=48` / `paper_shadow=true` / `risk_on_order_enabled=false` / `order_submission_enabled=false` / `reduce_risk_allowed=true`；只允許影子觀察、減碼 / 取消掛單與證據收集，進攻買入 / 加倉與啟用自動模式仍等即時部署門檻。
...
- current live q15 truth 維持：**2/50 + exact_bucket_present_but_below_minimum + —**
