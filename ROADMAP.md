...
## 已完成
- **fast heartbeat #1278-productization 已完成 collect + diagnostics refresh**
...
- **本輪 probe support-route canonicalization 已完成**
  - 非 q15 current-live bucket `BLOCK|structure_quality_block|q00` 在 `0/50` exact support 時，probe / drilldown / docs top-level truth 固定為 `exact_bucket_unsupported_block` + `no_support_proxy` + `stalled_under_minimum`，不再被 q15 audit overlay 或 proxy reference 語義覆蓋。
  - 驗證：`tests/test_hb_predict_probe.py` 26 passed、`tests/test_api_feature_history_and_predictor.py` 66 passed、fast heartbeat runtime probe 已輸出一致欄位。
...
- regression lock：`tests/test_hb_predict_probe.py::test_build_probe_payload_canonicalizes_zero_row_non_q15_support_overlay` 覆蓋 q15 overlay 與 current q00 `0/50` 衝突時的 canonical fallback。
**成功標準**
...
## 成功標準
- 即時部署阻塞點清楚且唯一：**熔斷解除條件**
- current live bucket support truth 維持：**0/50 + exact_bucket_unsupported_block + no_support_proxy**
