...
- [~] canonical target 升級：從單一 `simulated_pyramid_win` 擴展到 **win + pnl_quality + drawdown_penalty + time_underwater** 的複合交易品質評分  
  ↳ Heartbeat #642：leaderboard scoring / API payload 已開始直接使用並輸出 `avg_decision_quality_score + avg_expected_*` canonical quality contract；Heartbeat #643 把 Strategy Lab 模型排行榜前端摘要切到這組欄位；Heartbeat #644 再把 Strategy Lab **策略排行榜主表 + `/api/strategies/{name}` 詳情** 切到同語義並改成依 DQ 排序；Heartbeat #645 再把 **active strategy summary + `/api/strategies/run` payload** 補上 `decision_contract`（target label / sort semantics / horizon）並直接顯示 DQ / 預期勝率 / DD-UW。下一步是把 saved strategy comparison 文案也切到同一組 canonical quality semantics，而不是停在 ROI / 勝率 / PF
