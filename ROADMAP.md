...
- **Strategy Lab workspace 與 app shell lane 已重新對齊**
  - `/api/strategies/leaderboard`、`/api/strategies/{name}`、`/api/strategy_data_range` 現在同樣以 same-origin shell 優先；即使 `poly_trader.active_api_base` 指到較舊 backend lane，`/lab` 仍先跟目前開啟的 app shell 同步，避免兩年區間 / 策略明細 split-brain
