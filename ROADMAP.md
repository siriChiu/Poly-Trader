...
## 已完成
- **full heartbeat #20260425_1136 已完成 collect + diagnostics refresh**
...
- **Execution Console / `/api/trade` 操作入口已 fail-closed（同步中 + 阻塞 + 直接 API）**
  - `/api/status` 初次同步前或部署阻塞存在時，前端買入 / 減碼 / 啟用自動模式快捷操作都顯示暫停；後端 `POST /api/trade` 也會在 `buy` / 加倉前檢查 current-live blocker，阻塞時回 409 `current_live_deployment_blocker`，但保留 `reduce/sell` 風險降低路徑
...
**成功標準**
- `/`、`/execution`、`/execution/status`、`/lab`、probe、drilldown、docs 都把熔斷解除條件視為唯一即時部署阻塞點；`/execution` 在 `/api/status` 初次同步前不得開放買入 / 減碼 / 啟用自動模式；直接呼叫 `POST /api/trade` 的 buy/add-exposure 也必須依 current-live blocker fail-closed，且只保留 `reduce/sell` 風險降低路徑。
...
1. **維持熔斷優先真相 + q15 current-live bucket visibility across API / UI / docs**
   - 驗證：browser `/`、browser `/execution`（含初次同步時買入 / 減碼 / 自動模式暫停）、browser `/execution/status`、browser `/lab`、`python scripts/hb_predict_probe.py`、`python scripts/live_decision_quality_drilldown.py`、`pytest tests/test_server_startup.py -k api_trade -q`
   - 升級 blocker：若熔斷解除條件被 support / floor-gap / venue 話題覆蓋、q15 current-live bucket rows 再次從 top-level surfaces 消失，或 `POST /api/trade` 在 blocker active 時仍允許 buy/add-exposure
...
- `/api/trade` direct API 不能繞過 current-live blocker：buy/add-exposure 在 `circuit_breaker_active` 等 no-deploy 狀態必須 409，`reduce/sell` 仍可用
