...
## 已完成
- **full heartbeat #1275 已完成 collect + diagnostics refresh**
...
- **Cron compact runtime facts 已補上 canonical 熔斷操作摘要**
  - `scripts/hb_extract_runtime_facts.py` 會把 canonical breaker active 的 release math 投影成操作員摘要：最近 50 筆目前 2/15 勝、還差 13 勝，維持 fail-closed；排程回報可直接讀到解除條件，不必解析巢狀 artifact。
...
- `hb_extract_runtime_facts.py` compact payload 必須保留 `operator_guardrail_summary`，讓 cron / heartbeat 摘要直接顯示 canonical 熔斷解除差距。
