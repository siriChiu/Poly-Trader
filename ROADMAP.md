...
- [~] Dynamic Window distribution-aware 版：窗口評估必須顯示 label distribution / regime distribution / constant-target guardrail，不再只看固定 N 導致近期窗口誤判  
  ↳ Heartbeat #661：`scripts/recent_drift_report.py` 已把 canonical 1440m recent-window `label balance + dominant regime + constant-target` 做成 machine-readable artifact，並接入 fast heartbeat summary / auto-propose。Heartbeat #662 再把這個 guardrail 真正接進 `scripts/dynamic_window_train.py`：當 window 出現 `constant_target` 或 `regime_concentration` 時，會被標成 `distribution_guardrail=skip_for_recommendation`，並把 `raw_best_n` 與 `recommended_best_n` 分開落地到 `data/dw_result.json`。Heartbeat #663 再把這個 policy 接進 `model/predictor.py` 的 live decision-quality calibration：predictor 會消費 `recommended_best_n`，並對外暴露 `decision_quality_calibration_window / decision_quality_guardrail_applied / decision_quality_guardrail_reason`。**Heartbeat #664 再把同一份 policy 接進 `model/train.py` 的 training-side TW weighting：當 recent drift report 指向 `constant_target / regime_concentration` polluted slice 時，會對 recent tail 做 damping，並把 `tw_guardrail` metadata 寫進 `model/ic_signs.json`；Heartbeat #666 再把 guardrail 向下推到 calibration bucket：若最窄 `regime_gate+entry_quality_label` lane 本身是 label-imbalanced polluted slice，就拒用該 bucket 並回退到更廣的 calibration scope；Heartbeat #667 再把 guardrail 推到 live execution sizing：`model/predictor.py` 會把 `decision_quality_label` + `decision_quality_guardrail_applied` 轉成 `allowed_layers_raw -> allowed_layers` 的 runtime 層數上限，並由 `hb_predict_probe.py` 暴露 `execution_guardrail_*`；Heartbeat #668 再把 drift artifact 升級成 `quality_metrics + drift_interpretation`，把 recent constant-target 視窗分成 `supported_extreme_trend` 與 `distribution_pathology`，避免治理層把真實極端單向行情誤寫成 label corruption。** 下一步是直接比較 retrain 前後 `tw_guardrail + execution_guardrail` 對 `last_metrics.json / ic_signs.json / hb_predict_probe.py` 的聯動影響，並補 recent feature variance / distinct-count diagnostics，驗證剩下的確是 signal 弱而不是 weighting / calibration 假語義。
