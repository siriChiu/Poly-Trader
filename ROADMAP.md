...
- **operator-facing current-bucket patch copy 已 productize**
  - `web/src/utils/runtimeCopy.ts::humanizePatchTargetLabel()` 現在會把非 `feat_*` patch target 對齊到 q15/q35 action mappings
  - browser `/execution/status`、browser `/lab` 已確認 `候選 patch` 顯示為 `先解除風控熔斷，再重跑 q35 分段校準審核`，不再殘留 `release ... then rerun ...` 混合英文字串
