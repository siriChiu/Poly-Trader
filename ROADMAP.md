...
## 已完成
- **fast heartbeat #1262 已完成 collect + diagnostics refresh**
...
- **cron runtime facts extractor 已產品化為 compact proof**
  - `scripts/hb_extract_runtime_facts.py` 現在選最新 heartbeat summary（目前 `data/heartbeat_1262_summary.json`），輸出 scalar `Raw/Features/Labels`、Top-K `rows=24`、reference-only patch、q15 current-live support、`/api/trade` fail-closed proof；避免 cron 報告讀到舊 alias summary 或傾倒完整 Top-K rows。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-15T23:02:13.928617+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24383` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=32/50` / `current_live_structure_bucket_gap_to_minimum=18`。
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=BLOCK|structure_quality_block|q00` / `bucket_rows=32/50` / `gap=18`；若只剩即時分桶 / 支持 / release gate，仍模擬觀察 / 影子驗證 / 僅觀察。
**成功標準**
...
5. **守住 cron runtime facts compact proof contract**
   - 驗證：`python scripts/hb_extract_runtime_facts.py`、`python -m pytest tests/test_hb_extract_runtime_facts.py -q`
   - 升級 blocker：若 cron 報告再次輸出 `Raw/Features/Labels=None`、把 `rows[]` 完整矩陣倒出、漏掉 reference-only patch，或漏掉 `/api/trade` 409 guardrail proof

---

## 成功標準
- current-live blocker 清楚且唯一：**under_minimum_exact_live_structure_bucket**
- current live bucket support truth 維持：**32/50 + exact_bucket_present_but_below_minimum + reference_only_non_current_live_scope**
...
- cron runtime facts 保持 compact：**最新 summary + scalar counts/topk + reference-only patch + API guardrail proof**
