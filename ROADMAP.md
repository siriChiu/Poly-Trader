...
- **compact live-pathology summary 已改成 rows-aware 說明**
  - Dashboard / Strategy Lab 現在會分開顯示 `current bucket support`、`exact lane cohort`，必要時再補 `historical lane bucket`，避免同一張摘要卡把 broader cohort rows 誤包裝成 current live exact support
