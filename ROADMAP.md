...
_最後更新：2026-04-14 16:54 UTC — Heartbeat #736（已把 bull q35 under-minimum exact bucket 的 blocker-state contract 校正完成；現在主問題只剩 support 不足與 proxy cohort 邊界，而不是語義漂移）_
...
- fast heartbeat 持續刷新：
  ...
  - `issues.json`
  - numbered summary：`data/heartbeat_736_summary.json`

### 本輪新完成：under-minimum exact-bucket blocker-state alignment
- `scripts/bull_4h_pocket_ablation.py` 已升級：
  - 當 `exact_live_bucket_rows > 0` 且 `< minimum_support_rows` 時，`blocker_state` 不再誤回報 `exact_missing`；
  - 若 exact-bucket proxy 已達 minimum，`preferred_support_cohort` 仍可指向 proxy，但 blocker 仍維持 under-minimum；
  - 這讓 `blocker_state / exact_bucket_root_cause / support_governance_route` 三條語義終於一致。
- regression tests 已補：
  - `tests/test_bull_4h_pocket_ablation.py`
  - `tests/test_hb_leaderboard_candidate_probe.py`
- `ARCHITECTURE.md` 已同步 #736 contract。
...
- live predictor：
  ...
  - narrowed bull+D pathology：**157 rows / win_rate 0.1338 / quality -0.1522**
...
- bull pocket artifact（當前 live bucket）
  ...
  - exact rows：**10**
  - exact bucket gap to minimum：**40**
  - exact-bucket proxy rows：**53**
  - exact-lane proxy rows：**316**
...
### 目標 A：把 bull live blocker 從「語義修補」推進到「support 根因 + proxy cohort 邊界」
本輪已確認：
- q35 live bucket 仍是 **under-minimum support（10 / 50）**；
- blocker-state 漂移已修完，現在剩下的是**真實 support 缺口**與**proxy cohort 合法性**。

下一步主目標是：
- **直接追 q35 exact bucket 10 → 50 的 support 累積與 cohort contract**；
- 不再把 blocker-state 對齊本身當主要進展。

### 目標 B：把 exact / proxy / broader same-bucket 證據切得更窄、更可部署
目前 evidence 一致指向：
- exact live lane：**27 rows / win_rate 0.5185 / quality 0.2177**
- current exact bucket：**10 rows / 仍不足 minimum support**
...
下一步主目標：
- 判定 proxy 53 rows 是否過寬；
- 判定 broader same-bucket 是否仍受 cross-regime spillover 汙染；
- 若 proxy 顯著偏離 exact 10 rows，縮窄 support-aware cohort。
...
要做：
- 比對 **exact 10 rows、exact live lane 27 rows、proxy 53 rows、broader same-bucket 12 rows**；
- 檢查 proxy cohort 是否混入太多非 exact 結構；
...
- heartbeat summary / probe / docs 必須能 machine-read：
  ...
  - `allowed_layers`
- 下一輪不應再出現「exact rows 已出現 = blocker 已解」的語義漂移
...
原因：
> 現在真正的瓶頸已收斂成 **q35 exact bucket support 不足 + proxy cohort 是否過寬 + bull-only D pocket pathology**；不是模型容量問題，也不是 blocker-state 語義問題。
...
接下來幾輪工作的成功標準：
1. next run 必須留下至少一個與 **q35 under-minimum exact bucket / proxy cohort 邊界** 直接相關的 patch / run / verify。
...
- **Next focus:**
  1. 直接推進 `CAUTION|structure_quality_caution|q35` exact bucket 10 → 50 的 support 根因；
  2. 釐清 exact 10 / exact lane 27 / proxy 53 / broader same-bucket 12 之間，到底是 proxy cohort 過寬，還是 bull D pocket 本身不適合部署；
  3. 繼續把 `fin_netflow` 當外部 source blocker 管理。

- **Success gate:**
  1. next run 必須留下 q35 under-minimum exact bucket / proxy cohort 邊界的真 patch / artifact / verify，不能只重報目前摘要；
  2. `support_blocker_state`、`support_governance_route`、`exact_bucket_root_cause`、`allowed_layers` 對 blocker 的敘述零漂移；
  3. 若 exact rows 仍 < 50，所有路徑同輪同步維持 blocker 結論。

- **Fallback if fail:**
  - 若 q35 exact rows 持續卡住，維持 `0 layers`；
  - 若仍無法判定 proxy 邊界，下一輪至少要留下更窄的 exact 10 vs proxy 53 feature / gate-path 對照 artifact；
