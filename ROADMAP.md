...
## 已完成
- **full heartbeat #1263 已完成 collect + diagnostics refresh**
...
- **cron runtime facts 已納入熔斷 release math 壓縮證據**
  - `hb_extract_runtime_facts.py` 會輸出 compact `circuit_breaker`：混合 horizon false positive、canonical `1440m` aligned release-ready、mixed scope 還差 6 勝、aligned scope 還差 0 勝；報告層可直接說明這不是部署放行，仍以 q00 current-live exact support `10/50` 阻塞為準。
...
- cron/runtime 摘要若提到熔斷，必須同時顯示 canonical 1440m release math 與「不得繞過目前即時精準支持阻塞」的操作員摘要。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-16T00:03:31.279634+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24386` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=10/50` / `current_live_structure_bucket_gap_to_minimum=40`。
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=BLOCK|structure_quality_block|q00` / `bucket_rows=10/50` / `gap=40`；若只剩即時分桶 / 支持 / release gate，仍模擬觀察 / 影子驗證 / 僅觀察。
**成功標準**
...
4. **守住 cron runtime facts 壓縮報告不再漏掉 breaker false-positive / canonical release math**
   - 驗證：`PYTHONPATH=. python scripts/hb_extract_runtime_facts.py`、`python -m pytest tests/test_hb_extract_runtime_facts.py -q`
   - 升級 blocker：若報告只顯示 mixed-horizon breaker 或 release-ready，卻沒有 q00 exact support `10/50` / 不可繞過支持阻塞語義
