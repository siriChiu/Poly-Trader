...
- **本輪 UI productization patch 已落地**
  - q15 current-bucket / recent drift 的 operator-facing copy 已移除 `q15 floor-cross`、`NW Envelope`、`NW Env`、`volume_exhaustion`，改顯示 `Q15 跨門檻合法性`、`4H NW 包絡位置`、`NW包絡`、`量能衰竭`
  - 驗證：frontend contract tests、production build、browser DOM counts on `/` and `/lab`
...
1. **維持 current-live exact-support blocker + q15 current-live bucket visibility across API / UI / docs**
   - 驗證：browser `/`、browser `/execution/status`、browser `/lab`、DOM counts for `q15 floor-cross / NW Envelope / volume_exhaustion`、`python scripts/hb_predict_probe.py`、`python scripts/live_decision_quality_drilldown.py`
