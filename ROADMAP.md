...
- **leaderboard governance live truth overlay 已落地到 API / runtime**
  - `/api/models/leaderboard` 會用較新的 `data/live_predict_probe.json` 覆蓋 governance live bucket 欄位，修正 Strategy Lab governance 卡片吃到 stale `q00 / 0/50` 的 split-brain
  - `8000/8001` 後端 lane 已重新對齊到 `q15 / 31/50 / exact_bucket_present_but_below_minimum`
...
- current live q15 truth 維持：**31/50 + exact_bucket_present_but_below_minimum + reference_only_until_exact_support_ready**
