...
## 已完成
- **fast heartbeat #20260422g 已完成 collect + diagnostics refresh**
...
- **execution observability surfaces 已 productize 為 limited-evidence aware**
  - Dashboard / Execution Console / Execution Status / Strategy Lab 共用 `runtimeCopy` helper，把 `healthy + no_runtime_order/not_applicable` 降級成 `limited evidence`
  - 目的：避免 operator 把「尚無 runtime order 的乾淨對帳」誤讀成完整 live execution closure；已用 pytest/build/browser 驗證
