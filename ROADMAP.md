...
## 已完成
- **fast heartbeat #20260423l 已完成 collect + diagnostics refresh**
...
- **operator-facing support progress visibility 已完成**
  - `/execution`、`/execution/status`、`/lab` 已補上 `支持狀態 / 樣本變化`，讓 exact-support regression / stagnation 不再被 `0/50` 單一數字吞掉
  ...
  - 驗證：`cd web && npm run build` → pass；browser `/execution`、`/execution/status`、`/lab` 顯示 support-progress 文案且 `0 JS errors`
