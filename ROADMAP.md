...
## 已完成
- **fast heartbeat #1190 已完成 collect + diagnostics refresh**
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=under_minimum_exact_live_structure_bucket`；當前 q00 分桶支持樣本=37/50，缺口=13；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；操作員執行介面先看即時部署阻塞點，再看 當前 q00 分桶 support / 背景治理
...
- nest_pred：`quality_flag=source_tls_verify_failed` / `latest_status=tls_verify_failed` / `source=polymarket_gamma` / `trust_policy=tls_verify_required_no_insecure_fallback`；這是 TLS trust gate，不是單純 history gap，production 不允許關閉 TLS 驗證。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-13T14:02:38.768593+00:00` / `freshness=fresh` / `age_min=12.4` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24257` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q00` / `current_live_structure_bucket_rows=37/50` / `current_live_structure_bucket_gap_to_minimum=13`。
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=CAUTION|base_caution_regime_or_bias|q00` / `bucket_rows=37/50` / `gap=13`；若只剩即時分桶 / 支持 gate，仍模擬觀察 / 影子驗證 / 僅觀察。
**成功標準**
...
3. **守住 q00 current-live bucket support / reference-only patch、leaderboard governance、venue/source blockers 與 docs automation 閉環**
   - 驗證：browser `/lab`、`curl http://127.0.0.1:<active-backend>/api/models/leaderboard`（依 `/health` 選 8000/8001 健康 lane，不要硬綁單一 port）、`data/q15_support_audit.json`、`data/execution_metadata_smoke.json`、`data/heartbeat_1190_summary.json source_blockers.nest_pred`、下輪 heartbeat docs sync status
   - 升級 blocker：若 patch 被誤升級成 deployable truth、排行榜 drift 成 placeholder-only、venue/source blocker 消失、Nest TLS 被 insecure fallback 掩蓋、或 docs 再次落後 latest artifacts
...
- current live bucket support truth 維持：**37/50 + exact_bucket_present_but_below_minimum + reference_only_non_current_live_scope**
