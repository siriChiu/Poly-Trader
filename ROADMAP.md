...
## 已完成
- **fast heartbeat #1434 已完成 collect + diagnostics refresh**
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 1/50，還差 14 勝；當前 q15 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；操作員執行介面先看熔斷解除條件，再看 當前 q15 分桶 support / 背景治理；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端 bucket / route / source / reference raw token 泄漏到 Dashboard / Strategy Lab / Execution Status / live DQ operator markdown
...
- **Execution Console 高信心 Top-K 影子觀察入口已產品化**
  - `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `deployable_rows=0` / `paper_shadow=true` / `risk_on_order_enabled=false` / `support=66/50` / `gap=0`；selective sleeve 在可部署仍為 0 時只允許 `paper_shadow`，不會送單或加倉。
- **高低震盪 / 擁塞不是停工：區間影子觀察 + 減風險劇本已產品化**
  - `support=66/50` / `gap=0` / `paper_shadow=true` / `risk_on_order_enabled=false` / `order_submission_enabled=false` / `reduce_risk_allowed=true`；只允許影子觀察、減碼 / 取消掛單與證據收集，進攻買入 / 加倉與啟用自動模式仍等即時部署門檻。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-22T09:21:17.440432+00:00` / `freshness=fresh` / `age_min=0.3` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24884` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_supported` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `current_live_structure_bucket_rows=66/50` / `current_live_structure_bucket_gap_to_minimum=0` / `release_ready=False` / `recent_window_wins=1/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=14` / `current_recent_window_win_rate=0.020` / `support_progress_status=exact_supported` / `support_progress_reason=current q15 exact bucket 已達 minimum support，
  可轉向 exact-supported deployment verify。` / `regression_basis=current_identity` / `delta_vs_previous=0` / `previous_rows=66` / `support_rows_needed=0` / `stagnant_run_count=2` / `stalled_support_accumulation=False` / `escalate_to_blocker=False`。
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=exact_bucket_supported` / `governance=exact_live_bucket_supported` / `bucket=CAUTION|base_caution_regime_or_bias|q15` / `bucket_rows=66/50` / `gap=0` / `release_ready=False` / `recent_window_wins=1/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=14` / `current_recent_window_win_rate=0.020` / `support_progress_status=exact_supported` / `support_progress_reason=current q15 exact bucket 已達 minimum support，可轉向 exact-supported deployment verify。` / `regression_basis=current_identity` / `delta_vs_previous=0` / `previous_rows=66` / `support_rows_needed=0` / `stagnant_run_count=2` /
...
## 成功標準
- 即時部署阻塞點清楚且唯一：**熔斷解除條件**
- current live q15 truth 維持：**66/50 + exact_bucket_supported + —**
