...
- [~] confidence-based layer sizing：依 entry quality / confidence 決定只開第一層、允許兩層、或完整 20/30/50 金字塔，避免低品質訊號重倉進場  
  ↳ Heartbeat #637：live predictor 已回傳 `allowed_layers` 並讓 `should_trade` 受 layer allowance 約束；Heartbeat #714 再把 **exact toxic ALLOW lane** 直接接進 runtime sizing：當 `regime_label+regime_gate+entry_quality_label` 的 exact live lane 仍是 `ALLOW`、但 canonical `true_negative_share` 過高時，predictor 會輸出 `decision_quality_exact_live_lane_*` 並把 execution reason 明確標成 `exact_live_lane_toxic_allow_lane_blocks_trade`；**Heartbeat #715 再把這條治理往前推到 gate 本身**：`model/predictor.py` / `backtesting/strategy_lab.py` 新增 4H overextended allow-lane veto（`bb_pct_b>=1.0 && dist_bb_lower>=10 && dist_swing_low>=11 -> structure_overextended_block`），讓 bull `ALLOW+D` 內已驗證 **19 rows / 0 wins** 的高延伸 pocket 直接改判 BLOCK，exact live lane 也因此從 `24 rows / wr=0.2917 / q=-0.005` 收斂到 `14 rows / wr=0.5000 / q=+0.2412`。**Heartbeat #716 再把 structure support 接進 calibration/runtime contract**：predictor 會輸出 `structure_quality / structure_bucket`，scope diagnostics 也會持久化 `recent500_structure_bucket_counts` 與 `current_live_structure_bucket_rows/share/metrics`；當 chosen scope 幾乎沒有當前 live structure bucket 的歷史支持時，runtime 直接觸發 `unsupported_live_structure_bucket_blocks_trade`。**Heartbeat #717 再把 fallback contract 從「只會報 weak support」推到「會先避開負向 gate proxy，再用 same-bucket metrics 保守夾值」**：`regime_label+entry_quality_label` 現在成為正式 calibration fallback 候選，且 structure-support guardrail 會用可取得的 q35 bucket metrics 實際下修 expectation，而不是只留下警告字串。**Heartbeat #718 再把 borderline weak-structure bull lane 從假 ALLOW 收回到 CAUTION**：`structure_quality < 0.65` 時，即使 base gate 仍是 ALLOW，也必須在 live predictor / Strategy Lab 一起落到 `structure_quality_caution`，避免只有 **2 rows** 歷史支持的 `ALLOW|q35` bucket 再被當成 permissive lane。下一步收斂成兩條：補足/治理 `CAUTION|structure_quality_caution|q35` 同桶歷史口袋，或把 q35 bucket 併入更穩定的 CAUTION taxonomy；同時繼續對 narrowed bull pathology (`147 rows / wr=0.0748 / q=-0.2098`) 做 source-level 修復，而不是再容忍弱結構 lane 掛著 ALLOW 名字。
...
- [x] Strategy Lab 非同步回測進度條 + 全域 top progress 統一顯示
- [x] Strategy Lab / FeatureChart K 線本地快取 + incremental append refresh
- [x] benchmark 後處理平行化（含 blind benchmark process fallback）
