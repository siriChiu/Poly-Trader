...
- **leaderboard live payload 已補上 stale-cache 正規化護欄**
  - `/api/models/leaderboard` 現在會把 cache / persisted snapshot 的 legacy `target_candidates` 清成 `[]`，確保 bounded-refresh contract 不會被舊快照回灌
