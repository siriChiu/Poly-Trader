...
## 已完成
- **fast heartbeat #1173 已完成 collect + diagnostics refresh**
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=under_minimum_exact_live_structure_bucket`；當前 q35 分桶支持樣本=23/50，缺口=27；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；操作員執行介面先看即時部署阻塞點，再看 當前 q35 分桶 support / 背景治理
...
- **high-conviction Top-K matrix refresh 已納入 heartbeat candidate-refresh lane**
  - `scripts/hb_parallel_runner.py --fast --fast-refresh-candidates` 與 full candidate refresh 會顯式重跑 `scripts/topk_walkforward_precision.py`；summary `serial_results.topk_walkforward_precision` 會保留 success/rc/stdout preview/artifact mtime，避免 deployment gate 靠人工刷新。
...
- 最新 matrix artifact 已由 heartbeat candidate-refresh lane 自動產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-02T04:22:35.165182+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=23953` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35` / `current_live_structure_bucket_rows=23/50` / `current_live_structure_bucket_gap_to_minimum=27`。
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=CAUTION|base_caution_regime_or_bias|q35` / `bucket_rows=23/50` / `gap=27`；若只剩即時分桶 / 支持 gate，仍模擬觀察 / 影子驗證 / 僅觀察。
**成功標準**
...
4. **建立 high-conviction top-k OOS ROI gate，讓 Strategy Lab winner 先經研究→模擬觀察→影子驗證→小流量分級**
   - 驗證：`python scripts/hb_parallel_runner.py --fast --fast-refresh-candidates --hb 1173`、`data/high_conviction_topk_oos_matrix.json`、`/api/models/leaderboard.high_conviction_topk`、Strategy Lab 高信心 OOS Top-K 部署門檻面板、`python -m pytest tests/test_model_leaderboard.py tests/test_frontend_decision_contract.py -k high_conviction -q`
...
- current live bucket support truth 維持：**23/50 + exact_bucket_present_but_below_minimum + —**
