...
- **Dashboard AdviceCard blocker-first 文案已產品化**
  - blocked / syncing 時主摘要改為 current live blocker + 訊號背景；方向建議只保留在次級 `訊號分析仍為...`
  - browser `/` 已驗證 blocked card 不再把 `綜合建議：...買入` 當成首頁主文案
