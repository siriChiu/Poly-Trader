...
## 已完成
- **full heartbeat #1236 已完成 collect + diagnostics refresh**
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=decision_quality_below_trade_floor`；當前 q35 分桶支持樣本=2/50，缺口=48；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；操作員執行介面先看即時部署阻塞點，再看 當前 q35 分桶 support / 背景治理；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端英文枚舉與混合式治理文案泄漏到 Dashboard / Strategy Lab / Execution Status
...
- **FeatureChart 覆蓋率 / 稀疏來源阻塞 copy 已 productized**
  - Dashboard 特徵圖現在把 `quality_flag / reasons / backfill_blocker / raw_snapshot / archive_window` 轉成繁中 operator copy；隱藏特徵摘要直接顯示覆蓋率、唯一值、快照歸檔、最新分鐘、最近歸檔窗與中文阻塞原因，避免 raw coverage/source telemetry 被誤讀成產品狀態。
...
5. **守住 Dashboard FeatureChart 覆蓋率 copy 產品化**
   ...
   - 升級 blocker：若特徵圖再次顯示 raw `coverage/archive/last/latest/distinct/fallback` token，或稀疏來源阻塞未顯示中文阻塞原因 / 快照歸檔 / 最近歸檔窗脈絡
