...
- **metadata freshness / external monitor detail 已 productize 到 operator-facing surfaces，且 build 已恢復可部署**
  - `ExecutionMetadataFreshnessDetail` 已共用於 Dashboard、`/execution/status`、`/lab`，避免 Strategy Lab 再把 metadata freshness 簡化成只剩 timestamp
  - 驗證：`pytest tests/test_frontend_decision_contract.py -q` 通過、`cd web && npm run build` 通過、browser `/` `/execution/status` `/lab` 可見 `external monitor host scheduler observed-ticking · lane user_crontab`
