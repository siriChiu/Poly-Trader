...
## 已完成
- **fast heartbeat #20260421-1224 已完成 collect + diagnostics refresh**
...
- **Execution support 對齊資訊已進入 operator surfaces**
  - `/execution` 與 `/execution/status` 現在會同時顯示 `support=current bucket rows/minimum` + `runtime/calibration` 計數 + alignment summary
  - 這條 lane 的目的，是讓 operator 不必再從單一 support badge 反推 runtime / calibration 是否 split-brain
...
**成功標準**
- `/`、`/execution`、`/execution/status`、`/lab`、probe、drilldown、docs 都把 `exact_live_lane_toxic_sub_bucket_current_bucket` 視為唯一 current-live deployment blocker。
...
**目前真相**
- `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q15` / `support=97/50` / `gap=0` / `support_route_verdict=exact_bucket_supported`
- `recommended_patch=None` / `status=None` / `reference_scope=—`
**成功標準**
- probe / drilldown / `/api/status` / `/execution/status` / `/lab` / docs 全都承認 q15 current-live bucket exact support 已達 minimum rows；deployment blocker 仍以 `exact_live_lane_toxic_sub_bucket_current_bucket` 為準，不可把 support closure 誤讀成 deployment closure；recommended patch 若存在也只能作治理 / 訓練參考。
...
## 下一輪 gate
1. **維持 latest runtime blocker（exact_live_lane_toxic_sub_bucket_current_bucket）+ q15 current-live bucket visibility across API / UI / docs**
...
- current live q15 truth 維持：**97/50 + exact_bucket_supported + None**
