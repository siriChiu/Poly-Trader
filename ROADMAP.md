...
## 已完成
- **full heartbeat #1234 已完成 collect + diagnostics refresh**
...
- **startup feature continuity symbol-alias false-positive 已修復**
  - `feature_engine/preprocessor.py` 現在把 `BTCUSDT` 與 `BTC/USDT` 視為同一市場做 feature continuity / save 去重；startup probe 從 `missing_before=234 / remaining_missing=234 / repair_deferred=True` 變成 `0 / 0 / False`。
  - `P1_feature_continuity_startup_deferred` 已在 `issues.json` 標記 resolved；保留回歸測試，避免 API readiness surfaces 再被假 deferred blocker 污染。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-14T22:02:13.861409+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24311` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q35` / `current_live_structure_bucket_rows=28/50` / `current_live_structure_bucket_gap_to_minimum=22`。
...
5. **守住 startup feature continuity symbol-alias contract**
   - 驗證：`python /tmp/hb1234_feature_continuity_probe_quick.py` 或等價 startup probe 應維持 `BTCUSDT missing_before=0 / remaining_missing=0 / repair_deferred=False`；`python -m pytest tests/test_preprocessor_missing_feature_backfill.py -q`
   - 升級 blocker：若 `BTCUSDT` / `BTC/USDT` 再次被拆成不同市場並讓 `/health` 或 `/api/status` 顯示假 deferred feature continuity
