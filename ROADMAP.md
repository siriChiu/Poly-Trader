...
## 已完成
- **fast heartbeat #20260424_2111 已完成 collect + diagnostics refresh**
...
- **本輪 operator-facing q35 current-bucket 根因 copy 已修正**
  - Dashboard、`/execution/status`、`/lab` 已在 q35 current bucket 顯示 `交易門檻缺口 0.2387 · q35 公式 / 重設仍只屬治理參考`
  - 目的：避免 current row 已在 q35 時仍顯示「近邊界樣本 / 距 q35 還差」，把 operator 導錯到 q15 邊界修補
...
- operator-facing root-cause copy 必須依 current bucket 語義切換：q35 current bucket 顯示 trade-floor gap；只有 q15/boundary replay 才顯示 `近邊界樣本 / 距 q35 還差`。
