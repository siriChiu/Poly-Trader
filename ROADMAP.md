...
## 已完成
- **full heartbeat #1115 已完成 collect + diagnostics refresh**
  ...
  - canonical tail root cause 已產品化到 artifact/API/UI：`losses=100/100`、`tp_miss=100/100`、`high_underwater=86/100`、`dominant_loss_regime=chop`、top 4H shifts=`feat_4h_bias200/feat_4h_bias50/feat_4h_bb_pct_b`。
...
**目前真相**
- `latest_window=100` / `win_rate=0.0%` / `dominant_regime=chop(75.0%)` / `avg_quality=-0.2794` / `avg_pnl=-0.0110` / `alerts=constant_target,regime_shift`
- canonical tail root cause：`rows=100` / `losses=100` / `tp_miss=100` / `high_underwater=86` / `avg_time_underwater=77.97%` / `dominant_loss_regime=chop` / `top_4h_shift=feat_4h_bias200,feat_4h_bias50,feat_4h_bb_pct_b`
**成功標準**
- drift / probe / docs 能同時指出 latest recent-window diagnostics、current blocker pocket、canonical tail loss-path root cause，而不是退回 generic leaderboard / venue 摘要。
...
2. **持續鑽 recent canonical pathological slice，而不是 generic 化 root cause；tail loss-path / regime loss breakdown / 4H shift 必須仍在 API + UI 顯示**
   - 驗證：`python scripts/recent_drift_report.py`、`python scripts/hb_predict_probe.py`、`/api/status.recent_canonical_drift.canonical_tail_root_cause`、RecentCanonicalDriftCard
   - 升級 blocker：若 drift artifact、API 或 UI 再失去 tail root-cause / loss-path / top 4H shift 證據
