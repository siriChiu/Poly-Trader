...
## 已完成
- **fast heartbeat #20260424u 已完成 collect + diagnostics refresh**
...
- **本輪 operator UX / Strategy Lab 穩定性 patch**
  - support gap 的 operator-facing 文案統一中文化成 `缺口`，涵蓋 Dashboard、Bot 營運、執行狀態、Strategy Lab 與 LivePathologySummaryCard
  - `/api/strategies/leaderboard`、`/api/models/leaderboard` 前端 request timeout 提升到 20s，避免排行榜 current-state payload 在本地 failover / 初始化時被 8s 預設 timeout 誤判
