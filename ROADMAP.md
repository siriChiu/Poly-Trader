...
## 已完成
- **fast heartbeat #1289-productization 已完成 collect + diagnostics refresh**
...
- **Execution Console / `/api/trade` 操作入口已 fail-closed（同步中 + 阻塞 + 直接 API）**
  ...
  - `/execution` 同步中 / 阻塞中提示文案已與 API guardrail 對齊：買入 / 加倉暫停，但「減碼 / 賣出風險降低路徑 / 模式切換 / 查看阻塞原因」仍明確可見；回歸：`tests/test_frontend_decision_contract.py::test_execution_console_blocks_only_add_exposure_while_syncing_or_blocked_and_keeps_derisk_actions_available`。
