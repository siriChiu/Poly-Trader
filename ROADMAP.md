...
## 已完成
- **full heartbeat #1242 已完成 collect + diagnostics refresh**
...
- **Live Decision-Quality Drilldown 語義重訂 guardrail 已產品化**
  - drilldown JSON/MD 會把 current q15 exact support `4/50`、legacy `53/50@20260419b`、`legacy_mismatched=calibration_window,regime_label` 與「舊版參考不可作為同語義閉環」一起輸出，避免 operator 把 legacy support closure、broader/proxy rows 或 semantic backfill 誤讀成部署放行。
  - 回歸測試已覆蓋：legacy reference 不得生成 exact-support/deployment-closure copy，且 operator next action 必須要求同一目前支持語義累積或回放精準樣本。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-15T06:02:52.854327+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24334` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=under_minimum_exact_live_structure_bucket` / `current_live_structure_bucket=CAUTION|structure_quality_caution|q15` / `current_live_structure_bucket_rows=4/50` / `current_live_structure_bucket_gap_to_minimum=46`。
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=CAUTION|structure_quality_caution|q15` / `bucket_rows=4/50` / `gap=46`；若只剩即時分桶 / 支持 / release gate，仍模擬觀察 / 影子驗證 / 僅觀察。
**成功標準**
...
3. **守住 q15 current-live bucket support truth / semantic rebaseline / blocker truth、leaderboard governance、venue/source blockers 與 docs automation 閉環**
   - 驗證：browser `/lab`、`curl http://127.0.0.1:<active-backend>/api/models/leaderboard`（依 `/health` 選 8000/8001 健康 lane，不要硬綁單一 port）、`data/q15_support_audit.json`、`data/live_decision_quality_drilldown.json`、`docs/analysis/live_decision_quality_drilldown.md`、`data/execution_metadata_smoke.json`、下輪 heartbeat docs sync status
   - 升級 blocker：若 support closure 被誤讀成 deployment closure、legacy `53/50@20260419b` 被誤讀成 current support identity closure、排行榜 drift 成 placeholder-only、venue/source blocker 消失、或 docs 再次落後 latest artifacts
