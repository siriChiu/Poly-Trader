...
## 已完成
- **fast heartbeat #20260422b 已完成 collect + diagnostics refresh**
...
- **/api/status live runtime truth 已補上 top-level recommended patch structured fields**
  - `execution.live_runtime_truth` 現在直接輸出 `recommended_patch / recommended_patch_profile / recommended_patch_status / recommended_patch_reference_scope / recommended_patch_reference_source / recommended_patch_reason`
  - `/execution/status` 與 `/lab` 已可直接消費同一份 reference-only patch machine-read truth，不再只靠 `runtime_closure_summary` 長文反推
