...
_最後更新：2026-05-16 11:30:47 CST_
...
## 已完成
- **fast heartbeat #1270 已完成 collect + diagnostics refresh**
  - `Raw=33354 / Features=24505 / Labels=66599`
  - 歷史覆蓋確認：`2y_backfill_ok=True` / `raw_start=2024-04-13T22:00:00+00:00` / `features_start=2024-04-14T07:00:00+00:00` / `labels_start=2024-04-14T07:00:00+00:00`
  - `deployment_blocker=circuit_breaker_active` / `streak=40` / `recent_window_wins=10/50` / `additional_recent_window_wins_needed=5`
  - `latest_window=500` / `win_rate=66.2%` / `dominant_regime=chop(75.4%)` / `avg_quality=+0.3078` / `avg_pnl=+0.0065` / `alerts=regime_shift`
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 10/50，還差 5 勝；當前 q00 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；操作員執行介面先看熔斷解除條件，再看 當前 q00 分桶 support / 背景治理；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端 bucket / route / source / reference raw token 泄漏到 Dashboard / Strategy Lab / Execution Status / live DQ operator markdown
  - 本輪產品化 patch：`model/runtime_closure.py::build_circuit_breaker_release_surface()` 變成 release math 共用出口，`/api/status` live runtime truth、`hb_predict_probe.py`、`hb_parallel_runner.py` 的 live predictor 摘要都直接帶 top-level `release_condition / release_ready / current_recent_window_wins / required_recent_window_wins / additional_recent_window_wins_needed / current_recent_window_win_rate`，避免 UI / docs / probe 讀巢狀 blocker details 時出現 release math split-brain
...
- fin_netflow：`quality_flag=source_auth_blocked` / `latest_status=auth_missing` / `forward_archive_rows=4108` / `archive_window_coverage_pct=0.0`
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-16T03:02:55.578148+00:00` / `freshness=fresh` / `age_min=27.9` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24393` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=8/50` / `current_live_structure_bucket_gap_to_minimum=42` / `release_ready=False` / `recent_window_wins=10/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=5` / `current_recent_window_win_rate=0.200`。
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=BLOCK|structure_quality_block|q00` / `bucket_rows=8/50` / `gap=42` / `release_ready=False` / `recent_window_wins=10/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=5` / `current_recent_window_win_rate=0.200`；若只剩即時分桶 / 支持 / release gate，仍模擬觀察 / 影子驗證 / 僅觀察。
**成功標準**
