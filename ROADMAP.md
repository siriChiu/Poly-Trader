...
## 已完成
- **fast heartbeat #1164 已完成 collect + diagnostics refresh**
...
- **Execution Status / Bot 營運 已顯示即時部署阻塞條件**
  - `即時部署阻塞點=under_minimum_exact_live_structure_bucket`；當前 q35 分桶支持樣本=18/50，缺口=32；目前不是熔斷解除數學，候選修補不可取代同分桶最低樣本門檻；操作員執行介面先看即時部署阻塞點，再看 當前 q35 分桶 support / 背景治理
...
- current live bucket support truth 維持：**18/50 + exact_bucket_present_but_below_minimum + —**
