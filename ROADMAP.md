...
## 已完成
- **full heartbeat #1199 已完成 collect + diagnostics refresh + runtime verification**
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 14/50，還差 1 勝；當前 q00 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；操作員執行介面先看熔斷解除條件，再看 當前 q00 分桶 support / 背景治理
- **API startup readiness 已恢復非阻塞**
  - `server/main.py` startup 會以 `max_backfill_rows=0` 延後重型 feature continuity backfill，並在 `/health` / `/api/status` 顯示 `feature_continuity.status=deferred`、`repair_deferred=true`、`remaining_missing=217`；API readiness 不再卡在同步重算 217 個缺失 feature windows。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-13T20:02:28.123369+00:00` / `freshness=fresh` / `age_min=0.2` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24263` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=CAUTION|base_caution_regime_or_bias|q00` / `current_live_structure_bucket_rows=37/50` / `current_live_structure_bucket_gap_to_minimum=13`。
...
### 目標 F：API startup readiness 不得被 feature continuity backfill 阻塞
**目前真相**
- `feature_continuity.status=deferred` / `missing_before=217` / `remaining_missing=217` / `max_backfill_rows=0`；缺口保留在 `/health` 與 `/api/status`，但 startup 不再同步重算 217 個缺失 feature windows。
**成功標準**
- API startup 可快速回應 `/health`；feature continuity 缺口若存在必須以 `deferred` + `remaining_missing` 呈現，不得隱藏成 clean。
- 下一步 maintenance/backfill lane 必須關閉 217-row 缺口或把增量 backfill 改成 bounded job，且不影響 `/api/trade` fail-closed 與 `/api/status` runtime truth。
...
5. **守住 API startup 非阻塞 feature continuity repair contract**
   - 驗證：`curl http://127.0.0.1:<active-backend>/health`、`/api/status.feature_continuity`、`python -m pytest tests/test_preprocessor_missing_feature_backfill.py tests/test_server_startup.py -k 'raw_continuity or feature_backfill' -q`
   - 升級 blocker：若 startup 再卡在 feature backfill、或缺失 feature rows 被誤顯示成 clean
