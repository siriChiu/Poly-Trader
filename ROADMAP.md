...
## 已完成
- **full heartbeat #1321-productization 已完成 collect + diagnostics refresh**
...
- **Live DQ drilldown operator-facing markdown 已 enum-safe，且 current-bucket root-cause 已接到 probe / drilldown**
  ...
  - `data/live_predict_probe.json` 與 `data/live_decision_quality_drilldown.json` 現在都有 `current_bucket_root_cause` / `q15_bucket_root_cause`：目前 `verdict=same_lane_neighbor_bucket_dominates`、`candidate_patch_feature=feat_4h_bb_pct_b`、`support=0/50`、`gap=50`。
...
**成功標準**
- `/`、`/execution`、`/execution/status`、`/lab`、probe、drilldown、docs 都把 `unsupported_exact_live_structure_bucket` 視為唯一 current-live deployment blocker，且不再誤回退成 breaker-first 舊敘事；`current_bucket_root_cause` / `q15_bucket_root_cause` 必須同時可 machine-read，避免 q15 component artifact 跟 q00 live bucket truth split-brain；`/execution` 在 `/api/status` 初次同步前也不得開放買入 / 啟用自動模式，阻塞期間只暫停買入 / 加倉與啟用自動模式，減碼 / 賣出風險降低路徑仍可用；直接呼叫 `POST /api/trade` 的買入 / 加倉也必須依即時部署阻塞點以 409 暫停，且只保留減倉 / 賣出風險降低路徑。
...
**成功標準**
- probe / drilldown / `/api/status` / `/execution/status` / `/lab` / docs 全都承認 q00 current-live bucket exact support 未達 minimum rows，recommended patch 只能作治理 / 訓練參考；probe / drilldown 必須直接保留 `same_lane_neighbor_bucket_dominates` 與 `feat_4h_bb_pct_b` root-cause 線索，但不可把 q15 neighbor support 誤當成 q00 deployment closure。
...
## 下一輪 gate
1. **維持 current-live exact-support blocker + q00 current-live bucket root-cause visibility across API / UI / docs**
   - 驗證：browser `/`、browser `/execution`（含初次同步時買入 / 啟用自動模式暫停、減碼可用）、browser `/execution/status`、browser `/lab`、`python scripts/hb_predict_probe.py`、`python scripts/live_decision_quality_drilldown.py`、`python -m pytest tests/test_hb_predict_probe.py tests/test_live_decision_quality_drilldown.py -q`、`python -m pytest tests/test_server_startup.py -k api_trade -q`
   - 升級 blocker：若 current-live blocker 被 breaker 舊敘事 / venue 話題覆蓋、q00 current-live bucket rows 再次從 top-level surfaces 消失，或 `current_bucket_root_cause` / `q15_bucket_root_cause` 再次 split-brain
...
- current live bucket support truth 維持：**0/50 + exact_bucket_unsupported_block + current_bucket_root_cause/q15_bucket_root_cause 可 machine-read**
