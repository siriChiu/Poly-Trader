...
## 已完成
- **fast heartbeat #1271 已完成 collect + diagnostics refresh**
...
- **Execution Status / Bot 營運 已顯示熔斷解除條件**
  - `最近 50 筆目前 8/50，還差 7 勝；當前 q00 分桶支持樣本 / 候選修補不可取代熔斷解除條件`；操作員執行介面先看熔斷解除條件，再看 當前 q00 分桶 support / 背景治理；`runtime_closure_summary` 已由 `model/runtime_closure.py` 共用中文化，避免後端 bucket / route / source / reference raw token 泄漏到 Dashboard / Strategy Lab / Execution Status / live DQ operator markdown
...
- **高低震盪 / 盤整阻塞 playbook 已產品化到 API + Execution Console**
  - 新增 `execution/range_chop_playbook.py` 並在 `/api/status`、`execution_surface_contract`、`/api/execution/overview` 與 Execution Console 顯示 `range_chop_playbook`；pullback / rebound / selective sleeve 在 bear/chop/BLOCK + q00 support 低於門檻時進 `shadow_reduce_only`，只允許影子觀察、累積支持證據、取消過期掛單與減碼降風險，買入 / 加倉仍等即時部署門檻。
  - 前端 copy 已把 `current-live gate`、`paper_shadow`、`runtime evidence` 改成人可讀繁中；`web` build 與 87 個 targeted tests 通過。
...
- 最新 matrix artifact 已產出：`artifact=data/high_conviction_topk_oos_matrix.json` / `generated_at=2026-05-16T04:03:30.455430+00:00` / `freshness=fresh` / `age_min=0.3` / `stale_after_min=60` / `deployment_blocking=False` / `samples=24396` / `rows=24` / `deployable_rows=0` / `risk_qualified_rows=6` / `runtime_blocked_candidates=6` / `support_route=exact_bucket_present_but_below_minimum` / `deployment_blocker=circuit_breaker_active` / `current_live_structure_bucket=BLOCK|structure_quality_block|q00` / `current_live_structure_bucket_rows=2/50` / `current_live_structure_bucket_gap_to_minimum=48` / `release_ready=False` / `recent_window_wins=8/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=7` / `current_recent_window_win_rate=0.160`。
- 最接近部署候選優先：`model=logistic_regression` / `regime=all` / `top_k=top_2pct` / `oos_roi=0.9324` / `win_rate=0.8621` / `profit_factor=19.8864` / `max_drawdown=0.022` / `worst_fold=0.2068` / `trades=58` / `tier=runtime_blocked_oos_pass` / `verdict=not_deployable` / `support_route=exact_bucket_present_but_below_minimum` / `governance=exact_live_bucket_present_but_below_minimum` / `bucket=BLOCK|structure_quality_block|q00` / `bucket_rows=2/50` / `gap=48` / `release_ready=False` / `recent_window_wins=8/50` / `required_recent_window_wins=15` / `additional_recent_window_wins_needed=7` / `current_recent_window_win_rate=0.160`；若只剩即時分桶 / 支持 / release gate，仍模擬觀察 / 影子驗證 / 僅觀察。
**成功標準**
...
4. **高低震盪 / 盤整 playbook browser 驗證**
   - 驗證：browser `/execution`、`/api/status.range_chop_playbook`、`/api/execution/overview.range_chop_playbook`；確認只顯示影子觀察 / 減風險，且買入 / 加倉仍需即時部署門檻。
   - 升級 blocker：若 playbook copy 再洩漏 raw token，或任何 range/chop 候選被 UI 顯示成可送單 / 可加倉。
...
- 高低震盪 / 盤整候選只能先進 **影子觀察 / 減風險先行**；買入 / 加倉仍需即時部署門檻與場館證據鏈，UI/API 不得把 range/chop 候選標成可送單。
