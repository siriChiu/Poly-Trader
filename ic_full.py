#!/usr/bin/env python3
"""Complete IC Analysis including Regime-Aware using feature regime labels"""
import sqlite3
import numpy as np
...
label_map = {}
for row in labels:
    label_map[row[0]] = (row[1], row[2], row[3])

matched_feats = [f for f in features if f[feat_cols.index('timestamp')] in label_map]
n = len(matched_feats)

# Use feature's regime_label (not the label's, which is all 'neutral')
...
regime_arr = [f[regime_feat_col] for f in matched_feats]
...
print()
print('=== Regime-Aware IC (using feature regime) ===')
rp = {}
for reg in ['bear', 'bull', 'chop']:
    ...
    for col in sense_cols:
        ...
        if v.sum() > 100 and vals[v].std() > 1e-10:
            ...
            if abs(ic) >= 0.05:
                rp[reg][0] += 1
                rp[reg][1].append(f"{col}({ic:+.4f})")
            print(f'    {col:>15s}: IC={ic:+.4f} [{s}]')

print()
for r, (c, ns) in rp.items():
    print(f'  {r.capitalize()}: {c}/8 => {ns}')
...
if metrics:
    for m in metrics:
        print(f'Model: Train={m[0]:.4f} CV={m[1]:.4f} std={m[2]:.4f} nf={m[3]} | {m[4]}')
db2.close()
