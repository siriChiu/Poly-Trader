...
def main():
    ...
    for feat_col, name in zip(feature_cols, sense_names):
        ...
        if abs(ic) < 0.05:
            print("  ?嚙踝蕭 IC < 0.05 ??Sense ineffective, consider replacement")
        elif abs(ic) < 0.03:
            print("  ?嚙踝蕭 IC < 0.03 ??Sense needs optimization")
        if std_val < 1e-6:
            print("  ?嚙踝蕭? Std ??0 ??No variation, white noise")
        # Check accuracy vs target 90%
        if accuracy >= 90:
            print("  ??Accuracy >= 90% (meets target)")
        else:
            print("  ?嚙踝蕭 Accuracy < 90% ??Below target")
    ...
    for i in range(len(senses)):
        for j in range(i+1, len(senses)):
            ...
            if abs(ics[si] - ics[sj]) < tol:
                print(f"?嚙踝蕭? {si} and {sj} have nearly identical IC: {ics[si]:.6f} vs {ics[sj]:.6f} (possible leakage)")
    ...
    if ineffective:
        print(f"?嚙踝蕭 Ineffective senses (IC < 0.05): {', '.join(ineffective)} -> consider replacement")
    if needs_opt:
        print(f"?嚙踝蕭 Senses needing optimization (IC < 0.03): {', '.join(needs_opt)}")
    if no_variation:
        print(f"?嚙踝蕭? Senses with no variation (std ??0): {', '.join(no_variation)} -> white noise")
    if low_acc:
        print(f"?嚙踝蕭 Senses with accuracy < 90%: {', '.join(low_acc)} -> below target")
