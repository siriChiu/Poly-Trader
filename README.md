...
- **Support-aware runtime blocker**：current live q15 lane 會 machine-read 顯示 exact-support 與 proxy route；目前 exact support 仍是 `0 / 50`、`support_route_verdict=exact_bucket_missing_exact_lane_proxy_only`，canonical blocker 仍先由 circuit breaker 接管；當 broader spillover patch 只能作治理參考時，UI / API 也必須明確分開顯示 live spillover 與 `reference_patch_scope/reference_source`，不能把 reference patch 誤當 current live pocket。
...
> 目前仍建議先用 **paper / live_canary**，不要直接把系統視為正式大資金實盤產品。

目前 Dashboard 已可直接看到：
- venue / execution mode
- 帳戶餘額 / 倉位 / 掛單數
- kill switch / daily loss halt / failure halt
- 最近拒單 / 最近失敗 / 最近委託
