...
- **Support-aware runtime blocker**：current live q15 lane 會 machine-read 顯示 exact-support 與 proxy route；目前 exact support 仍是 `0 / 50`、`support_route_verdict=exact_bucket_missing_proxy_reference_only`，且 canonical blocker 先由 circuit breaker 接管，不再把 proxy / neighbor 假裝成 deployable support
