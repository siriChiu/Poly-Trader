...
> **多感官量化交易系統** — 模擬人類感官，用 AI 解讀加密貨幣市場
...
Poly-Trader 是一套以**多感官**為核心的加密貨幣量化交易系統。

傳統的交易機器人只看一個指標（例如 RSI 或 MACD），就像一個人只用眼睛看世界——能看到東西，但看不全。人類做交易決策時，會同時運用視覺（看 K 線）、聽覺（聽市場消息）、嗅覺（聞到不對勁）、味覺（感受情緒）、觸覺（感受壓力）等多種感官，綜合判斷後才下決定。

**Poly-Trader 的核心理念：模擬人類多感官，把加密貨幣市場行為映射到 8 個感官維度，每個感官獨立運作、獨立驗證，再由 AI 綜合判斷。**

### 為什麼是「多感官」？

| 問題 | 單一指標的困境 | 多感官的解法 |
|------|---------------|-------------|
| 指標失靈 | RSI 在趨勢市中一直超買，錯過大行情 | 8 個感官同時運作，不會全部同時失靈 |
| 過擬合 | 單一模型容易記住歷史雜訊 | 多個獨立數據源降低過擬合風險 |
| 黑箱決策 | 不知道模型為什麼下單 | 每個感官的貢獻可視化、可解釋 |
| 無法改進 | 不知道哪個環節有問題 | IC 驗證每個感官，無效的直接汰換 |

---

## 🧠 8 感官系統

每個感官代表一種解讀市場的方式，經過 **IC（Information Coefficient）實證**，只有預測力達標的才會被採用。

| 感官 | 圖示 | 解讀什麼 | 數據來源 | 特徵計算 |
|------|------|----------|----------|----------|
...
除了 8 個感官之外，系統還收集 Binance 衍生品數據：
...
- 8 個頂點代表 8 個感官
- 中間區域越大 = 多數感官偏多（看漲）
- 點擊任何一個感官 → 下方顯示該感官的歷史走勢
- Hover 可看到感官的 IC 值和數據來源
