...
- **Support-aware runtime blocker**：current live q15 lane 會 machine-read 顯示 exact-support 與 proxy route；目前 exact support 已進到 `1 / 50`、`support_route_verdict=exact_bucket_present_but_below_minimum`，canonical blocker 仍先由 circuit breaker 接管；當 broader spillover patch 只能作治理參考時，UI / API 也必須明確分開顯示 live spillover 與 `reference_patch_scope/reference_source`，不能把 reference patch 誤當 current live pocket。
